...
bool CServerBrowser::OpenGameInfoDialog(uint64 ulSteamIDFriend, const char *pszConnectCode) {
    ...
    if (m_hInternetDlg.Get()) {
        ...
        if (steamapicontext->SteamFriends()->GetFriendGamePlayed(ulSteamIDFriend, &friendGameInfo)) {
            ...
            CDialogGameInfo *pDialogGameInfo = m_hInternetDlg->OpenGameInfoDialog(friendGameInfo.m_unGameIP,
                                                                                  friendGameInfo.m_usGamePort,
                                                                                  usConnPort, pszConnectCode);
...
//-----------------------------------------------------------------------------
const char *
CServerBrowser::GetMapFriendlyNameAndGameType(const char *pszMapName, char *szFriendlyMapName, int cchFriendlyName) {
