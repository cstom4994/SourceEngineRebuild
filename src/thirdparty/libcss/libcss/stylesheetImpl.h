...
#include "thirdparty/libcss/libcss/parse/parse.h"
#include "thirdparty/libcss/libcss/parse/mq.h"
...
static inline css_error css__stylesheet_style_appendOPV(css_style* style,
    int opcode, uint8_t flags, uint16_t value)
{
    return css__stylesheet_style_append(style,
        buildOPV((opcode_t)opcode, flags, value));
}
...
{
    return css__stylesheet_style_append(style,
        buildOPV((opcode_t)opcode, FLAG_INHERIT, 0));
}
