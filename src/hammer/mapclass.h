...
public:
    ...
    virtual void
    OnPaste(CMapClass *pCopy, CMapWorld *pSourceWorld, CMapWorld *pDestWorld, const CMapObjectList &OriginalList,
            CMapObjectList &NewList);
    ...
    virtual bool
    IsClutter(void) { return false; }            // Whether this object should be hidden when the user hides helpers.
    virtual bool IsCulledByCordon(const Vector &vecMins,
                                  const Vector &vecMaxs);    // Whether this object is hidden based on its own intersection with the cordon, independent of its parent's intersection.
...
protected:
    ...
    friend void
    FixHiddenObject(MapError *pError);    // So that the Check for Problems dialog can fix visgroups problems.
};
