...
public:
    ...
    void *QueryInterface(const char *pInterfaceName) override {
        if (!V_strcmp(pInterfaceName, VDMELEMENTFRAMEWORK_VERSION))return this;
        return NULL;
    }
...
public:
    ...
    void *QueryInterface(const char *pInterfaceName) override {
        if (!V_strcmp(pInterfaceName, DMSERIALIZERS_INTERFACE_VERSION))return this;
        return NULL;
    }
...
void CHammer::OnFileOpen(void) {
    ...
    CFileDialog dlg(TRUE, NULL, NULL, OFN_LONGNAMES | OFN_HIDEREADONLY | OFN_NOCHANGEDIR,
                    "Valve Map Files (*.vmf)|*.vmf|Valve Map Files Autosave (*.vmf_autosave)|*.vmf_autosave|Worldcraft RMFs (*.rmf)|*.rmf|Worldcraft Maps (*.map)|*.map||");
...
CDocument *CHammer::OpenDocumentFile(LPCTSTR lpszFileName) {
    ...
    if (((CMapDoc *) pDoc)->IsAutosave()) {
        ...
        sprintf(szRenameMessage,
                ...
                lpszFileName, (LPCTSTR) newMapPath);
...
bool CHammer::VerifyAutosaveDirectory(char *szAutosaveDirectory) const {
    ...
    if (hDir == INVALID_HANDLE_VALUE) {
        ...
        if (!bDirResult) {
            AfxMessageBox(
                    "The current autosave directory does not exist and could not be created.  \nThe autosave feature will be disabled until a new directory is entered.",
                    MB_OK);
            return false;
        }
    } else {
        ...
        if (hTestFile == INVALID_HANDLE_VALUE) {
            if (GetLastError() == ERROR_ACCESS_DENIED) {
                AfxMessageBox(
                        "The autosave directory is marked as read only.  Please remove the read only attribute or select a new directory in Tools->Options->General.\nThe autosave feature will be disabled.",
                        MB_OK);
                return false;
            } else {
                AfxMessageBox(
                        "There is a problem with the autosave directory.  Please select a new directory in Tools->Options->General.\nThe autosave feature will be disabled.",
                        MB_OK);
                return false;
            }
    ...
    if (!strLastGoodSave.IsEmpty()) {
        ...
        if (!((CMapDoc *) pCurrentDoc)->IsAutosave() && Q_stristr(pCurrentDoc->GetPathName(), szAutoSaveDir)) {
            ...
            sprintf(szRenameMessage,
                    ...
                    szLastSaveCopy, (LPCTSTR) newMapPath);
