...
bool GetRequiredMaterial(const char *pName, IMaterial *&pMaterial) {
    ...
    if (pMaterial) {
        ...
        Q_snprintf(str, sizeof(str),
                   "Missing material '%s'. Go to Tools | Options | Game Configurations and verify that your game directory is correct.",
                   pName);
...
// 				DIB_RGB_COLORS, SRCCOPY);
        ...
        if (g_bShowStatistics) {
            ...
            int nLen = sprintf(szText, "FPS=%3.2f Pos=[%.f %.f %.f]", m_fFrameRate, ViewPoint[0], ViewPoint[1],
                               ViewPoint[2]);
