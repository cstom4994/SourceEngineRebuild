...
void CMapOverlay::Basis_BuildAxes(void) {
    ...
    m_Basis.m_vecAxes[OVERLAY_BASIS_V] = m_Basis.m_vecAxes[OVERLAY_BASIS_NORMAL].Cross(
...
void CMapOverlay::ClipFace_Clip(ClipFace_t *pClipFace, cplane_t *pClipPlane, float flEpsilon,
    ...
    *ppFront = *ppBack = NULL;
    ...
    for (iPoint = 0; iPoint < pClipFace->m_nPointCount; iPoint++) {
        ...
        Vector vecPoint = pClipFace->m_aPoints[iPoint] +
                          (pClipFace->m_aPoints[(iPoint + 1) % pClipFace->m_nPointCount] -
                           pClipFace->m_aPoints[iPoint]) * fraction;
        for (int iTexCoord = 0; iTexCoord < NUM_CLIPFACE_TEXCOORDS; iTexCoord++) {
            Vector2D vecTexCoord = pClipFace->m_aTexCoords[iTexCoord][iPoint] +
                                   (pClipFace->m_aTexCoords[iTexCoord][(iPoint + 1) % pClipFace->m_nPointCount] -
                                    pClipFace->m_aTexCoords[iTexCoord][iPoint]) * fraction;
...
void CMapOverlay::ClipFace_ClipBarycentric(ClipFace_t *pClipFace, cplane_t *pClipPlane, float flEpsilon,
    ...
    for (iPoint = 0; iPoint < pClipFace->m_nPointCount; iPoint++) {
        ...
        Vector vecPoint = pClipFace->m_aPoints[iPoint] +
                          (pClipFace->m_aPoints[(iPoint + 1) % pClipFace->m_nPointCount] -
                           pClipFace->m_aPoints[iPoint]) * fraction;
        Vector vecDispPointUV = pClipFace->m_aDispPointUVs[iPoint] +
                                (pClipFace->m_aDispPointUVs[(iPoint + 1) % pClipFace->m_nPointCount] -
                                 pClipFace->m_aDispPointUVs[iPoint]) * fraction;
...
void CMapOverlay::DoTransform(const VMatrix &matrix) {
    ...
    if (!tmpMatrix.IsIdentity()) {
        ...
        bool bIsUnit = (fequal(fScaleU, 1.0f, 0.0001) && fequal(fScaleV, 1.0f, 0.0001) &&
