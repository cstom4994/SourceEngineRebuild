            ...
            if (hitData.m_CornerCount != 0) {
                for (int i = 0; i < hitData.m_CornerCount; i++) {
                    ...
                    for (int j = 0; j < count; j++) {
                        ...
                        //
                        int orient;
                        CMapDisp *pAdjDisp = GetImage(pFilter, pDisp, adjHgt, adjWid, hitData.m_ndxCorners[i], j,
                                                      orient);
                        if (pAdjDisp) {
        ...
        for (int ndxWid = 0; ndxWid < filterWid; ndxWid++) {
            ...
            if (hitData.m_CornerCount != 0) {
                for (int i = 0; i < hitData.m_CornerCount; i++) {
                    int count = GetCornerImageCount(pDisp, hitData.m_ndxCorners[i]);
                    for (int j = 0; j < count; j++) {
                        if (GetImageFlatSubdivValues(pFilter, pDisp, adjHgt, adjWid, hitData.m_ndxCorners[i], j,
                                                     vImg)) {
