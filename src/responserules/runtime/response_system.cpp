...
ConVar rr_debugresponseconcept("rr_debugresponseconcept", "", FCVAR_NONE,
...
float CResponseSystem::RecursiveScoreSubcriteriaAgainstRule(const CriteriaSet &set, Criteria *parent, bool &exclude,
                                                            bool verbose /*=false*/ ) {
    float score = 0.0f;
...
float CResponseSystem::ScoreCriteriaAgainstRuleCriteria(const CriteriaSet &set, int icriterion, bool &exclude,
                                                        bool verbose /*=false*/ ) {
    ...
    if (verbose) {
        DevMsg("  criterion '%25s':'%15s' ", m_Criteria.GetElementName(icriterion),
               CriteriaSet::SymbolToStr(c->nameSym));
    }
...
float
CResponseSystem::ScoreCriteriaAgainstRule(const CriteriaSet &set, ResponseRulePartition::tRuleDict &dict, int irule,
                                          bool verbose /*=false*/ ) {
    ...
    if (rule->m_nForceWeight >
        0) {    // this means override the cumulative weight of criteria and just force the rule's total score,
...
int CResponseSystem::SelectWeightedResponseFromResponseGroup(ResponseGroup *g, IResponseFilter *pFilter) {
    ...
    if (slot == -1) {
        for (int i = 0; i < c; i++) {
            ...
            if (!totalweight ||
                IEngineEmulator::Get()->GetRandomStream()->RandomFloat(0, totalweight) < r->weight.GetFloat()) {
                slot = i;
            }
...
bool CResponseSystem::GetBestResponse(ResponseSearchResult &searchResult, Rule *rule, bool verbose /*=false*/,
    ...
    if (g->IsSequential()) {
        ...
        do {
            ...
            if (!pFilter ||
                pFilter->IsValidResponse(g->group[responseIndex].GetType(), g->group[responseIndex].value)) {
                bFoundValid = true;
                break;
            }
...
//-----------------------------------------------------------------------------
ResponseRulePartition::tIndex
CResponseSystem::FindBestMatchingRule(const CriteriaSet &set, bool verbose, float &scoreOfBestMatchingRule) {
...
bool CResponseSystem::FindBestResponse(const CriteriaSet &set, CRR_Response &response, IResponseFilter *pFilter) {
    ...
    ResponseRulePartition::tIndex bestRule = FindBestMatchingRule(set,
                                                                  (iDbgResponse >= 3 &&
                                                                   iDbgResponse < RR_DEBUGRESPONSES_SPECIALCASE),
                                                                  scoreOfBestRule);
    ...
    if (showResult) {
		...
		*/
        ...
        if (valid || showRules) {
            ...
            if (V_strlen(pConceptFilter) > 0 && !rr_debugresponseconcept.GetBool()) {    // filter for only one concept
                ...
                if (m_DebugExcludeList.IsValidIndex(m_DebugExcludeList.Head())) {
                    ...
                    if (!m_DebugExcludeList.IsValidIndex(
                            m_DebugExcludeList.Find(test))) {    // if not found in exclude list, then print
                        response.Describe(&set);
                    }
                } else {
                    // describe everything
    ...
    for (int i = 0; i < (int) m_Responses.Count(); i++) {
        ...
        for (int j = 0; j < group.group.Count(); j++) {
            ...
            if (response.type != RESPONSE_RESPONSE) {
				...
				*/
                pResponses->Element(pResponses->AddToTail()).Init(response.GetType(), response.value, response.params,
                                                                  NULL, NULL, false);
            }
        }
    }
...
void
CResponseSystem::ParseResponse_DefaultDelay(ParserResponse &newResponse, ResponseGroup &group, AI_ResponseParams *rp) {
    rp->flags |= AI_ResponseParams::RG_DELAYAFTERSPEAK;
...
void
CResponseSystem::ParseResponse_SpeakOnce(ParserResponse &newResponse, ResponseGroup &group, AI_ResponseParams *rp) {
...
void
CResponseSystem::ParseResponse_StopOnNonIdle(ParserResponse &newResponse, ResponseGroup &group, AI_ResponseParams *rp) {
...
void
CResponseSystem::ParseResponse_RespeakDelay(ParserResponse &newResponse, ResponseGroup &group, AI_ResponseParams *rp) {
    ParseToken();
...
void
CResponseSystem::ParseResponse_WeaponDelay(ParserResponse &newResponse, ResponseGroup &group, AI_ResponseParams *rp) {
    ParseToken();
...
void
CResponseSystem::ParseResponse_Soundlevel(ParserResponse &newResponse, ResponseGroup &group, AI_ResponseParams *rp) {
    ParseToken();
...
void
CResponseSystem::ParseResponse_DisplayFirst(ParserResponse &newResponse, ResponseGroup &group, AI_ResponseParams *rp) {
    newResponse.first = true;
    group.m_bHasFirst = true;
}

void
CResponseSystem::ParseResponse_DisplayLast(ParserResponse &newResponse, ResponseGroup &group, AI_ResponseParams *rp) {
...
void
CResponseSystem::ParseOneResponse(const char *responseGroupName, ResponseGroup &group, ResponseParams *defaultParams) {
...
void CResponseSystem::ParseResponseGroup_Start(char const *responseGroupName, ResponseGroup &newGroup,
                                               AI_ResponseParams &groupResponseParams) {
    while (1) {
        ParseToken();
        if (!Q_stricmp(token, "}"))
            break;
...
void CResponseSystem::ParseResponseGroup_PreDelay(char const *responseGroupName, ResponseGroup &newGroup,
                                                  AI_ResponseParams &groupResponseParams) {
    ParseToken();
...
void CResponseSystem::ParseResponseGroup_NoDelay(char const *responseGroupName, ResponseGroup &newGroup,
                                                 AI_ResponseParams &groupResponseParams) {
    ParseToken();
...
void CResponseSystem::ParseResponseGroup_DefaultDelay(char const *responseGroupName, ResponseGroup &newGroup,
                                                      AI_ResponseParams &groupResponseParams) {
    groupResponseParams.flags |= AI_ResponseParams::RG_DELAYAFTERSPEAK;
...
void CResponseSystem::ParseResponseGroup_Delay(char const *responseGroupName, ResponseGroup &newGroup,
                                               AI_ResponseParams &groupResponseParams) {
    ParseToken();
    groupResponseParams.flags |= AI_ResponseParams::RG_DELAYAFTERSPEAK;
...
void CResponseSystem::ParseResponseGroup_SpeakOnce(char const *responseGroupName, ResponseGroup &newGroup,
                                                   AI_ResponseParams &groupResponseParams) {
    groupResponseParams.flags |= AI_ResponseParams::RG_SPEAKONCE;
}

void CResponseSystem::ParseResponseGroup_NoScene(char const *responseGroupName, ResponseGroup &newGroup,
                                                 AI_ResponseParams &groupResponseParams) {
    groupResponseParams.flags |= AI_ResponseParams::RG_DONT_USE_SCENE;
}

void CResponseSystem::ParseResponseGroup_StopOnNonIdle(char const *responseGroupName, ResponseGroup &newGroup,
                                                       AI_ResponseParams &groupResponseParams) {
    groupResponseParams.flags |= AI_ResponseParams::RG_STOP_ON_NONIDLE;
}

void CResponseSystem::ParseResponseGroup_Odds(char const *responseGroupName, ResponseGroup &newGroup,
                                              AI_ResponseParams &groupResponseParams) {
    ParseToken();
...
void CResponseSystem::ParseResponseGroup_RespeakDelay(char const *responseGroupName, ResponseGroup &newGroup,
                                                      AI_ResponseParams &groupResponseParams) {
    ParseToken();
...
void CResponseSystem::ParseResponseGroup_WeaponDelay(char const *responseGroupName, ResponseGroup &newGroup,
                                                     AI_ResponseParams &groupResponseParams) {
    ParseToken();
...
void CResponseSystem::ParseResponseGroup_Soundlevel(char const *responseGroupName, ResponseGroup &newGroup,
                                                    AI_ResponseParams &groupResponseParams) {
    ParseToken();
...
void CResponseSystem::ParseResponse(void) {
    ...
    while (1) {
        ...
        if (DispatchParseResponseGroup(token, hash, m_ResponseGroupDispatch, responseGroupName, newGroup,
                                       groupResponseParams)) {
            continue;
        }
        ParseOneResponse(responseGroupName, newGroup);
...
void CResponseSystem::CopyResponsesFrom(Rule *pSrcRule, Rule *pDstRule, CResponseSystem *pCustomSystem) {
    ...
    for (int iResponseGroup = 0; iResponseGroup < nResponseGroupCount; ++iResponseGroup) {
        ...
        if (pSrcResponseGroup) {
            ...
            int iInsertIndex = pCustomSystem->m_Responses.Insert(m_Responses.GetElementName(iSrcResponseGroup),
                                                                 dstResponseGroup);
...
CResponseSystem::CopyRuleFrom(Rule *pSrcRule, ResponseRulePartition::tIndex iRule, CResponseSystem *pCustomSystem) {
    ...
    // Add rule.
    pCustomSystem->m_RulePartitions.GetDictForRule(this, dstRule).Insert(m_RulePartitions.GetElementName(iRule),
                                                                         dstRule);
}
...
void CResponseSystem::DumpDictionary(const char *pszName) {
    ...
    for (ResponseRulePartition::tIndex idx = m_RulePartitions.First();
         m_RulePartitions.IsValid(idx);
         idx = m_RulePartitions.Next(idx)) {
        Msg("	Rule %d/%d: %s\n", m_RulePartitions.BucketFromIdx(idx), m_RulePartitions.PartFromIdx(idx),
            m_RulePartitions.GetElementName(idx));
        ...
        for (int iCriteria = 0; iCriteria < nCriteriaCount; ++iCriteria) {
            ...
            Msg("		Criteria %d: %s %s\n", iCriteria, CriteriaSet::SymbolToStr(pCriteria->nameSym),
                pCriteria->value);
        }
...
bool CResponseSystem::DispatchParseRule(char const *pToken, unsigned int uiHash, ParseRuleDispatchMap_t &rMap,
                                        Rule &newRule) {
    int slot = rMap.Find(uiHash);
    if (slot != rMap.InvalidIndex()) {
...
bool CResponseSystem::DispatchParseResponse(char const *pToken, unsigned int uiHash, ParseResponseDispatchMap_t &rMap,
                                            ParserResponse &newResponse, ResponseGroup &group, AI_ResponseParams *rp) {
    int slot = rMap.Find(uiHash);
    if (slot != rMap.InvalidIndex()) {
...
bool CResponseSystem::DispatchParseResponseGroup(char const *pToken, unsigned int uiHash,
                                                 ParseResponseGroupDispatchMap_t &rMap, char const *responseGroupName,
                                                 ResponseGroup &newGroup, AI_ResponseParams &groupResponseParams) {
    int slot = rMap.Find(uiHash);
    if (slot != rMap.InvalidIndex()) {
...
unsigned int ResponseRulePartition::GetBucketForSpeakerAndConcept(const char *pszSpeaker, const char *pszConcept,
...
const char *Rule::RecursiveGetValueForRuleCriterionByName(CResponseSystem *RESTRICT pSystem,
    ...
    if (pCrit->IsSubCriteriaType()) {
        ...
        for (int i = 0; i < pCrit->subcriteria.Count(); ++i) {
            pRet = RecursiveGetValueForRuleCriterionByName(pSystem, &pSystem->m_Criteria[pCrit->subcriteria[i]],
                                                           pCritNameSym);
...
const Criteria *Rule::RecursiveGetPointerForRuleCriterionByName(CResponseSystem *pSystem, const Criteria *pCrit,
                                                                const CUtlSymbol &pCritNameSym) {
    Assert(pCrit);
    if (!pCrit) return NULL;
    if (pCrit->IsSubCriteriaType()) {
        ...
        for (int i = 0; i < pCrit->subcriteria.Count(); ++i) {
            pRet = RecursiveGetPointerForRuleCriterionByName(pSystem, &pSystem->m_Criteria[pCrit->subcriteria[i]],
                                                             pCritNameSym);
            if (pRet) // if found something, early out
                return pRet;
        }
