...
String ^ Matcher::Token::get() {
    return gcnew
    String(m_pNative->GetToken());
}

String ^ Matcher::RawSource::get() {
    return gcnew
    String(m_pNative->GetRaw());
}
...
^ from )
{
AssignParserResponse( &m_pNative
->group[idx], from->
...
ResponseGroup::ResponseGroup(ResponseRules::ResponseGroup *RESTRICT ptr, int index)
    ...
    m_shadowResponseArray = gcnew
    array<Response ^>(numOfResponses);

    for (int i = 0; i < numOfResponses; ++i) {
        m_shadowResponseArray[i] = gcnew
        Response(&ptr->group[i], i);
    }
}
