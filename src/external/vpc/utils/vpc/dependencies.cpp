...
void CProjectDependencyGraph::GetProjectDependencyTree(projectIndex_t iProject,
    ...
    for (int i = 0; i < m_Projects.Count(); i++) {
        ...
        for (int iOther = 0; iOther < m_Projects.Count(); iOther++) {
            ...
            if (bDownwards)
                bThereIsADependency = pProject->DependsOn(pOther, k_EDependsOnFlagCheckNormalDependencies |
                                                                  k_EDependsOnFlagCheckAdditionalDependencies |
                                                                  k_EDependsOnFlagRecurse |
                                                                  k_EDependsOnFlagTraversePastLibs);
            else
                bThereIsADependency = pOther->DependsOn(pProject, k_EDependsOnFlagCheckNormalDependencies |
                                                                  k_EDependsOnFlagCheckAdditionalDependencies |
                                                                  k_EDependsOnFlagRecurse |
                                                                  k_EDependsOnFlagTraversePastLibs);
