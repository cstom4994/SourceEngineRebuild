...
bool CProjectGenerator_Win32_2010::WriteTools(CProjectConfiguration *pConfig) {
    ...
    m_XMLWriter.PushNode("ItemDefinitionGroup",
                         CFmtStr("Condition=\"'$(Configuration)|$(Platform)'=='%s|%s'\"", pConfig->m_Name.Get(),
                                 pTargetPlatformName));
...
bool CProjectGenerator_Win32_2010::WriteFolderToSecondaryXML(CProjectFolder *pFolder, const char *pParentPath) {
    CUtlString parentPath = CUtlString(
            CFmtStr("%s%s%s", pParentPath, pParentPath[0] ? "\\" : "", pFolder->m_Name.Get()));
...
bool CProjectGenerator_Win32_2010::WriteProperty(const PropertyState_t *pPropertyState, bool bEmitConfiguration,
    ...
    if (bEmitConfiguration) {
        ...
        conditionString = CFmtStr(" Condition=\"'$(Configuration)|$(Platform)'=='%s|%s'\"", pConfigName,
                                  pTargetPlatformName);
