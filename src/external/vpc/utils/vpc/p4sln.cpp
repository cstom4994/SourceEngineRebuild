...
static void
AddAdditionalDependencies(CUtlVector<CDependency_Project *> &projects, CUtlVector<CDependency_Project *> &allProjects) {
    for (int nProject = 0; nProject < projects.Count(); nProject++) {
        ...
        for (int nDependency = 0; nDependency < pCurProject->m_AdditionalProjectDependencies.Count(); nDependency++) {
            ...
            if (nFound == -1) {
                g_pVPC->VPCError(
                        "P4SLN: Project %s lists '%s' in its $AdditionalProjectDependencies, but there is no project by that name.",
...
static void GetProjectsDependingOnFiles(CProjectDependencyGraph &dependencyGraph, CUtlVector<CUtlString> &filenames,
    ...
    for (int iFile = 0; iFile < filenames.Count(); iFile++) {
        ...
        if (!pFile) {
            ...
            if (!V_MakeRelativePath(filenames[iFile].String(), g_pVPC->GetSourcePath(), szRelative,
                                    sizeof(szRelative))) {
