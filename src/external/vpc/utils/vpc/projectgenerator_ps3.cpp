...
bool CProjectGenerator_PS3::WriteFolder(CProjectFolder *pFolder) {
    ...
    for (int iIndex = pFolder->m_Folders.Head();
         iIndex != pFolder->m_Folders.InvalidIndex(); iIndex = pFolder->m_Folders.Next(iIndex)) {
        if (!WriteFolder(pFolder->m_Folders[iIndex]))
            return false;
    }

    for (int iIndex = pFolder->m_Files.Head();
         iIndex != pFolder->m_Files.InvalidIndex(); iIndex = pFolder->m_Files.Next(iIndex)) {
    ...
    for (int i = 0; i < pPreBuildEventTool->m_PropertyStates.m_Properties.Count(); i++) {
        switch (pPreBuildEventTool->m_PropertyStates.m_Properties[i].m_pToolProperty->m_nPropertyId) {
            ...
            case PS3_PREBUILDEVENT_ExcludedFromBuild:
                m_XMLWriter.Write(CFmtStrMax("ExcludedFromBuild=\"%s\"", BoolStringToTrueFalseString(
                        pPreBuildEventTool->m_PropertyStates.m_Properties[i].m_StringValue.Get())));
                break;
        }
    }
    ...
    for (int i = 0; i < pCompilerTool->m_PropertyStates.m_Properties.Count(); i++) {
        ...
        switch (pCompilerTool->m_PropertyStates.m_Properties[i].m_pToolProperty->m_nPropertyId) {
            ...
            case PS3_SNCCOMPILER_CreateUsePrecompiledHeader:
                if (nOrdinalValue == 1) {
                    additionalOptions += CFmtStrMax("--create_pch=&quot;%s&quot; ",
                                                    pCompilerTool->m_PropertyStates.GetProperty(
                                                            PS3_SNCCOMPILER_PrecompiledHeaderFile)->m_StringValue.String());
                } else if (nOrdinalValue == 2) {
                    additionalOptions += "--pch --pch_dir=&quot;$(IntDir)&quot; ";
                } else if (nOrdinalValue == 3) {
                    additionalOptions += CFmtStrMax("--use_pch=&quot;%s&quot; ",
                                                    pCompilerTool->m_PropertyStates.GetProperty(
                                                            PS3_SNCCOMPILER_PrecompiledHeaderFile)->m_StringValue.String());
                }
                break;
    ...
    for (int i = 0; i < pLinkerTool->m_PropertyStates.m_Properties.Count(); i++) {
        ...
        switch (pLinkerTool->m_PropertyStates.m_Properties[i].m_pToolProperty->m_nPropertyId) {
            ...
            case PS3_SNCLINKER_GenerateMapFile:
                if (nOrdinalValue == 1) {
                    additionalOptions += CFmtStrMax("-Map=&quot;%s&quot; ", pLinkerTool->m_PropertyStates.GetProperty(
                            PS3_SNCLINKER_MapFileName)->m_StringValue.String());
                } else if (nOrdinalValue == 2) {
                    additionalOptions += CFmtStrMax("-Map=&quot;%s&quot; -sn-full-map ",
                                                    pLinkerTool->m_PropertyStates.GetProperty(
                                                            PS3_SNCLINKER_MapFileName)->m_StringValue.String());
                }
                break;
            ...
            case PS3_SNCLINKER_LinkLibraryDependencies:
                m_XMLWriter.Write(CFmtStrMax("LinkLibraryDependencies=\"%s\"", BoolStringToTrueFalseString(
                        pLinkerTool->m_PropertyStates.m_Properties[i].m_StringValue.Get())));
                break;
    ...
    for (int i = 0; i < pLinkerTool->m_PropertyStates.m_Properties.Count(); i++) {
        ...
        switch (pLinkerTool->m_PropertyStates.m_Properties[i].m_pToolProperty->m_nPropertyId) {
            ...
            case PS3_GCCLINKER_GenerateMapFile:
                if (nOrdinalValue == 1) {
                    additionalOptions += CFmtStrMax("-Map=&quot;%s&quot; ", pLinkerTool->m_PropertyStates.GetProperty(
                            PS3_GCCLINKER_MapFileName)->m_StringValue.String());
                }
                break;
    ...
    for (int i = 0; i < pConfig->m_PropertyStates.m_Properties.Count(); i++) {
        switch (pConfig->m_PropertyStates.m_Properties[i].m_pToolProperty->m_nPropertyId) {
            ...
            case PS3_GENERAL_BuildLogFile:
                m_XMLWriter.Write(CFmtStrMax("BuildLogFile=\"%s\"",
                                             pConfig->m_PropertyStates.m_Properties[i].m_StringValue.Get()));
                break;
...
bool CProjectGenerator_PS3::WriteToXML() {
    ...
    for (int iIndex = pRootFolder->m_Folders.Head();
         iIndex != pRootFolder->m_Folders.InvalidIndex(); iIndex = pRootFolder->m_Folders.Next(iIndex)) {
        if (!WriteFolder(pRootFolder->m_Folders[iIndex]))
            return false;
    }

    for (int iIndex = pRootFolder->m_Files.Head();
         iIndex != pRootFolder->m_Files.InvalidIndex(); iIndex = pRootFolder->m_Files.Next(iIndex)) {
