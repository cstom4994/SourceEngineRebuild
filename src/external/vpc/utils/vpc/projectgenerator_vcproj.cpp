...
void CVCProjGenerator::ApplyInternalPreprocessorDefinitions() {
    ...
    for (int iIndex = m_FileDictionary.FirstInorder();
         iIndex != m_FileDictionary.InvalidIndex(); iIndex = m_FileDictionary.NextInorder(iIndex)) {
        ...
        for (int i = 0; i < pProjectFile->m_Configs.Count(); i++) {
            ...
            if (pCompilerTool) {
                PropertyState_t *pPropertyState = pCompilerTool->m_PropertyStates.GetProperty(
