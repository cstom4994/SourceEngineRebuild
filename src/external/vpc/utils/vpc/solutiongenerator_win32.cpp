    ...
    virtual void GenerateSolutionFile(const char *pSolutionFilename, CUtlVector<CDependency_Project *> &projects) {
        ...
        if (!g_pVPC->Is2010()) {
            ...
            if (pSolutionItemsFilename[0] != '\0') {
                fprintf(fp,
                        "Project(\"{2150E333-8FDC-42A3-9474-1A3956D46DE8}\") = \"Solution Items\", \"Solution Items\", \"{AAAAAAAA-8B4A-11D0-8D11-90A07D6D6F7D}\"\n");
        ...
        for (int i = 0; i < projects.Count(); i++) {
            ...
            for (int iTestProject = 0; iTestProject < projects.Count(); iTestProject++) {
                ...
                if (pCurProject->DependsOn(pTestProject,
                                           k_EDependsOnFlagCheckNormalDependencies | k_EDependsOnFlagTraversePastLibs |
