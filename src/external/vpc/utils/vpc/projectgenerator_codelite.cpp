...
void
CProjectGenerator_CodeLite::GenerateCodeLiteProject(CBaseProjectDataCollector *pCollector, const char *pOutFilename,
    ...
    Write("<CodeLite_Project Name=\"%s\" InternalType=\"\">\n", pCollector->GetProjectName().String());
    {
...

        {
            ...
            WriteFilesFolder(k_pchSource,
                             "*.c;*.C;*.cc;*.cpp;*.cp;*.cxx;*.c++;*.prg;*.pas;*.dpr;*.asm;*.s;*.bas;*.java;*.cs;*.sc;*.e;*.cob;*.html;*.rc;*.tcl;*.py;*.pl;*.m;*.mm");
...
void CProjectGenerator_CodeLite::WriteFilesFolder(const char *pFolderName, const char *pExtensions) {
    ...
    Write("<VirtualDirectory Name=\"%s\">\n", pFolderName);
    {
        ...
        for (int i = m_pCollector->m_Files.First();
             i != m_pCollector->m_Files.InvalidIndex(); i = m_pCollector->m_Files.Next(i)) {
