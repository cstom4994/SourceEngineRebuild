...
public:
    virtual void GenerateSolutionFile(const char *pSolutionFilename, CUtlVector<CDependency_Project *> &projects) {
        ...
        for (int i = 0; i < projects.Count(); i++) {
            ...
            V_MakeAbsoluteCygwinPath(sDir, sizeof(sDir), sDirTemp);
            fprintf(fp,
                    "\t@$(FIND) %s -name \'*.cpp\' -print0 | $(XARGS) -0 $(ETAGS) --declarations --ignore-indentation --append\n",
                    sDir);
            fprintf(fp,
                    "\t@$(FIND) %s -name \'*.h\' -print0 | $(XARGS) -0 $(ETAGS) --language=c++ --declarations --ignore-indentation --append\n",
                    sDir);
            fprintf(fp,
                    "\t@$(FIND) %s -name \'*.c\' -print0 | $(XARGS) -0 $(ETAGS) --declarations --ignore-indentation --append\n",
                    sDir);
        }
        fprintf(fp, "\n\n");
    ...
    void ResolveAdditionalProjectDependencies(
        ...
        for (int i = 0; i < pCurProject->m_AdditionalProjectDependencies.Count(); i++) {
            ...
            if (j == projects.Count())
                g_pVPC->VPCError(
                        "Project %s lists '%s' in its $AdditionalProjectDependencies, but there is no project by that name in the selected projects.",
