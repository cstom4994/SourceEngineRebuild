...
CBaseProjectDataCollector::CBaseProjectDataCollector(CRelevantPropertyNames *pNames) : m_Files(
...
void CBaseProjectDataCollector::StartConfigurationBlock(const char *pConfigName, bool bFileSpecific) {
    ...
    if (index == -1) {
        CSpecificConfig *pParent = (pFileConfig == &m_BaseConfigData ? NULL : m_BaseConfigData.GetOrCreateConfig(
                sLowerCaseConfigName, NULL));
...
void
CBaseProjectDataCollector::DoStandardVisualStudioReplacements(const char *pStartString, const char *pFullInputFilename,
