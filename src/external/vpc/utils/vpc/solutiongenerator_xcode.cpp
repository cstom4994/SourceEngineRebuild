...
private:
    ...
    void
    EmitBuildSettings(const char *pszProjectName, const char *pszProjectDir, CUtlDict<CFileConfig *, int> *pDictFiles,
                      KeyValues *pConfigKV, KeyValues *pReleaseKV, bool bIsDebug);
...
static CUtlString OutputFileWithDirectoryFromConfig(KeyValues *pConfigKV) {
    ...
    if (V_strstr(szOutputDir, "$") || V_strstr(szOutputFile, "$")) {
        g_pVPC->VPCWarning(
                "$OutputDirectory '%s' or $OutputFile directive '%s' contains what looks like a shell variable reference -- "
...
static bool
FileBuildsWithCustomBuildRule(const CBaseProjectDataCollector *pProjectDataCollector, CFileConfig *pFileConfig) {
    if (!pFileConfig || !pProjectDataCollector)
        return false;
    CSpecificConfig *pFileSpecificData = pFileConfig->GetOrCreateConfig(
            pProjectDataCollector->m_BaseConfigData.m_Configurations[1]->GetConfigName(),
...
static bool
AppearsInSourcesBuildPhase(const CBaseProjectDataCollector *pProjectDataCollector, CFileConfig *pFileConfig) {
    if (!pFileConfig)
        return false;
...
static bool
IsCreatedByCustomBuildStep(const CBaseProjectDataCollector *pProjectDataCollector, CFileConfig *pDynamicFileConfig) {
    for (int i = pProjectDataCollector->m_Files.First();
         i != pProjectDataCollector->m_Files.InvalidIndex(); i = pProjectDataCollector->m_Files.Next(i)) {
        ...
        if (V_stristr(sOutputFiles, V_UnqualifiedFileName(pDynamicFileConfig->GetName())) &&
            ...
            g_pVPC->VPCWarning(
                    "Not adding '%s' to the build sources list in project '%s', it's a dyanmic file and seems to be created by building '%s'\n",
...
void CSolutionGenerator_Xcode::EmitBuildSettings(const char *pszProjectName, const char *pszProjectDir,
    ...
    if (outStrings.Count()) {
        ...
        for (int i = 0; i < outStrings.Count(); i++) {
            ...
            CBaseProjectDataCollector::DoStandardVisualStudioReplacements(outStrings[i], CFmtStr("%s/dummy.txt",
                                                                                                 pszProjectDir).Access(),
                                                                          sExpandedOutString,
        ...
        ++m_nIndent;
        {
            ...
            for (int i = 0; i < localFrameworks.Count(); i++) {
                ...
                rgchFrameworkPath[V_strlen(rgchFrameworkPath) -
                                  V_strlen(V_UnqualifiedFileName(localFrameworks[i]))] = '\0';
        ...
        Write("objects = {\n");
        {
            ...
            ++m_nIndent;
            {
                FOR_EACH_VEC(g_vecPGenerators, iGenerator) {
                    ...
                    for (int i = g_vecPGenerators[iGenerator]->m_Files.First(); i !=
                                                                                g_vecPGenerators[iGenerator]->m_Files.InvalidIndex(); i = g_vecPGenerators[iGenerator]->m_Files.Next(
                            i)) {
                        ...
                        if ((!bIsDynamicFile ||
                            ...
                            for (int i = pFileConfig->m_Configurations.First(); i !=
                                                                                pFileConfig->m_Configurations.InvalidIndex(); i = pFileConfig->m_Configurations.Next(
                                    i)) {
                                sCompilerFlags += pFileConfig->m_Configurations[i]->m_pKV->GetString(
                                        g_pOption_ExtraCompilerFlags);
                            }
                            // File reference OIDs are unique per project per file
                            Write("%024llX /* %s in Sources */ = {isa = PBXBuildFile; fileRef = %024llX /* %s */; ",
                                  makeoid2(g_vecPGenerators[iGenerator]->GetProjectName(), pFileName,
                                           EOIDTypeBuildFile), V_UnqualifiedFileName(pFileName),
                                  makeoid2(g_vecPGenerators[iGenerator]->GetProjectName(), pFileName,
                                           EOIDTypeFileReference), pFileName);
                    ...
                    if (sOutputFile.Length() && (IsStaticLibrary(sOutputFile) || IsDynamicLibrary(sOutputFile))) {
                        for (int iTestProject = 0; iTestProject < projects.Count(); iTestProject++) {
                            ...
                            int dependsOnFlags =
                                    k_EDependsOnFlagTraversePastLibs | k_EDependsOnFlagCheckNormalDependencies |
                            ...
                            ++m_nIndent;
                            {
                                ...
                                ++m_nIndent;
                                {
                                    ...
                                    for (int i = 0; i < outFiles.Count(); i++) {
                                        ...
                                        CBaseProjectDataCollector::DoStandardVisualStudioReplacements(outFiles[i],
                                                                                                      sInputFile,
                                                                                                      sOutputFile.Get(),
                                                                                                      MAX_PATH);
                                ...
                                --m_nIndent;
                                Write(");\n");
                                CBaseProjectDataCollector::DoStandardVisualStudioReplacements(sCustomBuildCommandLine,
                                                                                              sInputFile,
                ...
                FOR_EACH_VEC(g_vecPGenerators, iGenerator) {
                    ...
                    if (sOutputFile.Length()) {
                        ...
                        Write("%024llX /* %s */ = {isa = PBXFileReference; explicitFileType = \"%s\"; includeInIndex = 0; path = \"%s\"; sourceTree = BUILT_PRODUCTS_DIR; };",
                              makeoid2(g_vecPGenerators[iGenerator]->GetProjectName(), sOutputFile,
                                       EOIDTypeFileReference), sOutputFile.String(), rgchFileType,
                              sOutputFile.String());
                    }
                    ...
                    if (sGameOutputFile.Length()) {
                        ...
                        Write("%024llX /* %s */ = {isa = PBXFileReference; explicitFileType = \"%s\"; includeInIndex = 0; path = \"%s\"; sourceTree = \"<absolute>\"; };",
                              makeoid2(g_vecPGenerators[iGenerator]->GetProjectName(), sGameOutputFile,
                                       EOIDTypeFileReference), sGameOutputFile.String(), rgchFileType, rgchFilePath);
                    }
                }
            }
            --m_nIndent;
                    ...
                    ++m_nIndent;
                    {
                        ...
                        ++m_nIndent;
                        {
                            ...
                            for (int i = g_vecPGenerators[iGenerator]->m_Files.First(); i !=
                                                                                        g_vecPGenerators[iGenerator]->m_Files.InvalidIndex(); i = g_vecPGenerators[iGenerator]->m_Files.Next(
                                    i)) {
                                CUtlString sFileName = UsePOSIXSlashes(
                                        g_vecPGenerators[iGenerator]->m_Files[i]->m_Filename.String());
                                ...
                                if (bInclude) {
                                    Write("%024llX /* %s in Frameworks (explicit) */,\n",
                                          makeoid2(g_vecPGenerators[iGenerator]->GetProjectName(),
                                                   ...
                                                   EOIDTypeFileReference), sFileName.String());
                                }
                            }
                            ...
                            if (sOutputFile.Length())
                                Write("%024llX /* %s */,\n",
                                      makeoid2(g_vecPGenerators[iGenerator]->GetProjectName(), sOutputFile.String(),
                                               EOIDTypeFileReference), sOutputFile.String());
                        }
            ...
            FOR_EACH_VEC(projects, iProject) {
                ...
                ++m_nIndent;
                {
                    ...
                    ++m_nIndent;
                    {
                        for (int i = g_vecPGenerators[iProject]->m_Files.First(); i !=
                                                                                  g_vecPGenerators[iProject]->m_Files.InvalidIndex(); i = g_vecPGenerators[iProject]->m_Files.Next(
                                i)) {
                            ...
                            if (AppearsInSourcesBuildPhase(g_vecPGenerators[iProject], pFileConfig)) {
                                Write("%024llX /* %s in Sources */,\n",
                                      makeoid2(g_vecPGenerators[iProject]->GetProjectName(), pFileName,
                                               EOIDTypeBuildFile), V_UnqualifiedFileName(UsePOSIXSlashes(pFileName)));
                            }
                        }
                    }
                    --m_nIndent;
                                ...
                                if (sOutputFile.Length() &&
                                    ...
                                    Write("%024llX /* (lib)%s (dependency) */,\n",
                                          makeoid2(g_vecPGenerators[iTestProject]->GetProjectName(), sOutputFile,
                                                   EOIDTypeBuildFile, iProject), sOutputFile.String());
                                }
                            }
                        }
            ...
            ++m_nIndent;
            {
                FOR_EACH_VEC(g_vecPGenerators, iGenerator) {
                    ...
                    CUtlString sPreBuildCommandLine = g_vecPGenerators[iGenerator]->m_Files[0]->GetOrCreateConfig(
                            g_vecPGenerators[iGenerator]->m_BaseConfigData.m_Configurations[1]->GetConfigName(),
                            g_vecPGenerators[iGenerator]->m_BaseConfigData.m_Configurations[1])->GetOption(
                            g_pOption_PreBuildEventCommandLine);
                    if (sPreBuildCommandLine.Length()) {
                        ...
                        ++m_nIndent;
                        {
                            ...
                            Write("name = \"%s\";\n", CFmtStr("PreBuild Event for %s",
                                                              projects[iGenerator]->m_ProjectName.String()).Access());
                            Write("outputPaths = (\n);\n");
                            Write("runOnlyForDeploymentPostprocessing = 0;\n");
                            Write("shellPath = /bin/bash;\n");

                            CBaseProjectDataCollector::DoStandardVisualStudioReplacements(sPreBuildCommandLine,
                                                                                          CFmtStr("%s/dummy.txt",
                                                                                                  rgchProjectDir).Access(),
                                                                                          sCommand.Get(), cCommand);
                            ...
                            ++m_nIndent;
                            {
                                ...
                                if (pFileSpecificData->GetOption(g_pOption_Description)) {
                                    ...
                                    sDescription = CFmtStr("Custom Build Step for %s",
                                                           pFileConfig->m_Filename.String()).Access();
                    ...
                    if (sDebugGameOutputFile.Length() || sReleaseGameOutputFile.Length()) {
                        ...
                        ++m_nIndent;
                        {
                            ...
                            ++m_nIndent;
                            {
                                ...
                                CBaseProjectDataCollector::DoStandardVisualStudioReplacements(rgchDebugFilePath,
                                                                                              rgchDebugFilePath,
                                                                                              rgchDebugFilePath,
                                ...
                                CBaseProjectDataCollector::DoStandardVisualStudioReplacements(rgchReleaseFilePath,
                                                                                              rgchReleaseFilePath,
                                                                                              rgchReleaseFilePath,
                            ...
                            bool bHasReleasePostBuildCmd = V_strlen(SkipLeadingWhitespace(
                                    pReleaseKV->GetString(g_pOption_PostBuildEventCommandLine, ""))) > 0;
                            bool bHasDebugPostBuildCmd = V_strlen(SkipLeadingWhitespace(
                                    pDebugKV->GetString(g_pOption_PostBuildEventCommandLine, ""))) > 0;
                            strScriptExtra.Format(
                                    ...
                                    "\";\n",
                                    rgchReleaseFilePath,
                                    bHasReleasePostBuildCmd ? UsePOSIXSlashes(
                                            pReleaseKV->GetString(g_pOption_PostBuildEventCommandLine, "true"))
                                                            : "true",
                                    bHasDebugPostBuildCmd ? UsePOSIXSlashes(
                                            pDebugKV->GetString(g_pOption_PostBuildEventCommandLine, "true")) : "true");
                            ...
                            if (pCurProject->DependsOn(pTestProject, dependsOnFlags) ||
                                ...
                                Write("%024llX /* %s */,\n",
                                      makeoid(projects[iProject]->m_ProjectName, EOIDTypeTargetDependency,
                                              (uint16_t) iTestProject), pTestProject->GetName());
                            }
                        }
                    }
                    --m_nIndent;
                        ...
                        FOR_EACH_VEC(projects, iProject) {
                            ...
                            if (!ProjectProducesBinary(((CProjectGenerator_Xcode *) g_vecPGenerators[iProject]))) {
                                int cSubAggregateTargets =
                                        ...
                                        (((CProjectGenerator_Xcode *) g_vecPGenerators[iProject])->m_nShellScriptPhases %
                                         k_nShellScriptPhasesPerAggregateTarget ? 1 : 0);
