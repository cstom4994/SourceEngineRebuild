...
bool
V_StrSubstInPlace(char *pchInOut, int cchInOut, const char *pMatch, const char *pReplaceWith, bool bCaseSensitive) {
    bool bRet = false;
    ...
    for (int i = 0; i < files.Count(); i++) {
        ...
        if (!g_pVPC->m_sUnityCurrent.IsEmpty() && !V_stricmp(pExtension, "cpp") && !bHasSection && !bHasConditional) {
            bool bEmitUnityFiles = (!g_pVPC->m_bInMkSlnPass && (g_pVPC->IsForceGenerate() ||
                                                                !g_pVPC->IsProjectCurrent(g_pVPC->GetOutputFilename(),
                                                                                          false)));
            if (bEmitUnityFiles) {
...
static void
VPC_HandleLibraryExpansion(char const *pDefaultPath, char const *pFileNamePrefix, char const *pSuffix, bool bRemove) {
    if (!pFileNamePrefix) {
        pFileNamePrefix = "";
    }
...
//-----------------------------------------------------------------------------
enum MacroType_t {
    VPC_MACRO_VALUE, VPC_MACRO_EMPTY_STRING
};
...
enum MacroRequiredType_t {
    VPC_MACRO_REQUIRED_NOT_EMPTY, VPC_MACRO_REQUIRED_ALLOW_EMPTY
};
...
void Internal_LoadAddressMacroAuto(bool bPad) {
    ...
    while (1) {
        ...
        if (!V_stricmp(pToken, "}")) {
            ...
            if (dllLength == 0) {
                g_pVPC->VPCSyntaxError(
                        "$LoadAddressMacroAuto no longer supports 0 size dlls. Use $LoadAddressMacroAlias to have two orthogonal projects load in the same space");
            }
...
void VPC_AddCurrentVPCScriptToProjectFolder(bool bDoCRCCheck) {
    ...
    if (bDoCRCCheck && g_pVPC->EvaluateConditionalExpression("$WINDOWS")) {
        ...
        for (int i = 0; i < configurationNames.Count(); i++) {
            ...
            g_pVPC->GetProjectGenerator()->HandleProperty("$Description", CFmtStr("\"Running VPC CRC Check - %s\"",
                                                                                  g_pVPC->GetScript().GetName()));
            g_pVPC->GetProjectGenerator()->HandleProperty("$CommandLine",
...
void VPC_FakeKeyword_SchemaFolder(CBaseProjectDataCollector *pDataCollector) {
    ...
    V_snprintf(szSchemaOutAnchorPath, sizeof(szSchemaOutAnchorPath),
               "generated_code_%s_%s/%s_schema_anchor.cpp", g_pVPC->GetProjectName(), pPlatformName,
