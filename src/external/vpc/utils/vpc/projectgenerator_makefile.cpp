    ...
    void Setup(CUtlDict<CFileConfig *, int> &files, CFileConfig *pBaseConfig) {
        for (int i = files.First(); i != files.InvalidIndex(); i = files.Next(i)) {
            ...
            for (int iSpecific = pFile->m_Configurations.First();
                 iSpecific != pFile->m_Configurations.InvalidIndex(); iSpecific = pFile->m_Configurations.Next(
                    iSpecific)) {
                ...
                if (pPrecompiledHeaderOption && V_stristr(pPrecompiledHeaderOption, "Create")) {
                    ...
                    if (!pUsePCHThroughFile) {
                        g_pVPC->VPCError(
                                "File %s creates a precompiled header in config %s but no UsePCHThroughFile option specified.",
    ...
    void WriteConfigSpecificStuff(CSpecificConfig *pConfig, FILE *fp, CPrecompiledHeaderAccel *pAccel,
        ...
        for (int i = 0; i < OriginalSort.Count(); i++) {
            ...
            if (IsLibraryFile(pFilename)) {
                ...
                if ((!sImportLibraryFile[0] || V_stricmp(sImportLibraryFile, pFilename)) &&
                    // only link this as a library if it isn't our own output!
            ...
            if (pOutputFile && pCustomBuildCommandLine && V_strlen(pCustomBuildCommandLine) > 0) {
                ...
                if (!g_pVPC->IsPosixPCHDisabled() && pPrecompiledHeaderOption && pUsePCHThroughFile) {
                    ...
                    if (V_stristr(pPrecompiledHeaderOption, "Not Using")) {
                        ...
                        // entirely.
                        fprintf(fp, "\n%s : %s %s.gch $(PWD)/%s %s\n",
                                sIncludeFilename, pUsePCHThroughFile, sIncludeFilename, g_pVPC->GetOutputFilename(),
                                sMakeFileDependency);
                        fprintf(fp, "\tcp -f $< %s\n", sIncludeFilename);
                    } else if (V_stristr(pPrecompiledHeaderOption, "Use")) {
                        CFileConfig *pCreator = pAccel->FindFileThatCreatesPrecompiledHeader(pConfig->GetConfigName(),
                                                                                             pUsePCHThroughFile);
