...
static void PerformFileSubstitions(char *line, int linelen) {
    ...
    if (bFindFilePending) {
        ...
        if (V_stristr(token, "$os")) {
            if (!IsValidPathChar(*token))
                fprintf(stderr,
                        "Warning: can't expand %s for crc calculation.  Changes to this file set won't trigger automatic rebuild\n",
                        token);
    ...
    if (bFindFilePatternPending) {
        ...
        if (vecResults.Count()) {
            ...
            if (!IsValidPathChar(*token))
                fprintf(stderr,
                        "Warning: %s couldn't be expanded during crc calculation.  Changes to this file set won't trigger automatic project rebuild\n",
                        token);
        }
    }
...
int VPC_CommandLineCRCChecks(int argc, const char **argv) {
    if (argc < 2) {
        fprintf(stderr,
                "Invalid arguments to " VPCCRCCHECK_EXE_FILENAME ". Format: " VPCCRCCHECK_EXE_FILENAME " [project filename]\n");
        return 1;
    }
    ...
    if (pFirstCRC[0] == '-' && pFirstCRC[1] == 'c' && pFirstCRC[2] == 'r' && pFirstCRC[3] == 'c' &&
        pFirstCRC[4] != '2') {
