...
public:
    ...
    virtual HKeySymbol
    GetSymbolForStringCaseSensitive(HKeySymbol &hCaseInsensitiveSymbol, const char *name, bool bCreate = true);
...
HKeySymbol CKeyValuesSystem::GetSymbolForString(const char *name, bool bCreate) {
    ...
    while (1) {
        ...
        if (item->next == NULL) {
            ...
            *reinterpret_cast< uint32 * >( pString +
                                           numStringBytes ) = 0;    // string null-terminator + 3 alternative spelling bytes
            return (HKeySymbol) item->stringIndex;
        }
...
//-----------------------------------------------------------------------------
HKeySymbol
CKeyValuesSystem::GetSymbolForStringCaseSensitive(HKeySymbol &hCaseInsensitiveSymbol, const char *name, bool bCreate) {
    if (!name) {
        return (-1);
    }
    ...
    while (1) {
        ...
        if (iResult == 0) {
            ...
            if (!bCreate) {
                ...
                *reinterpret_cast< uint32 * >( pString +
                                               numNameStringBytes ) = 0;    // string null-terminator + 3 alternative spelling bytes
