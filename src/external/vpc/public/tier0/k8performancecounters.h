...
typedef union EVENT_MASK(k8_dispatched_fpu_ops)
        {
                ...
                struct
                {
                    uint16
                    AddPipeOps:
                    1; // Add pipe ops excluding junk ops" },
                    uint16
                    MulPipeOps:
                    1; // Multiply pipe ops excluding junk ops" },,
                    uint16
                    StoreOps:
                    1; // Store pipe ops excluding junk ops" },
                    uint16
                    AndPipeOpsJunk:
                    1; // Add pipe junk ops" },,
                    uint16
                    MulPipeOpsJunk:
                    1; // Multiply pipe junk ops" },
                    uint16
                    StoreOpsJunk:
                    1; // Store pipe junk ops" } }
                };
                uint16 flat;
        }
...
typedef union EVENT_MASK(k8_locked_op)
        {
                ...
                struct
                {
                    uint16
                    NumLockInstr :
                    1;  //Number of lock instructions executed
                    uint16
                    NumCyclesInRequestGrant :
                    1;  //Number of cycles spent in the lock request/grant stage
...
typedef union EVENT_MASK(  k8_lock_accesses)
        {
                struct
                {
                    uint16
                    DcacheAccesses:
                    1;       // Number of dcache accesses by lock instructions" },
                    uint16
                    DcacheMisses:
                    1;       // Number of dcache misses by lock instructions" } }
                };
                uint16 flat;
...
typedef union EVENT_MASK(  k8_retired_fpu_instr)
        {
                struct
                {
                    ...
                    uint16
                    CombinedMMX_3DNow:
                    1;     // Combined MMX & 3DNow! instructions" },
                    uint16
                    CombinedPackedSSE_SSE2:
                    1;     // Combined packed SSE and SSE2 instructions" },
                    uint16
                    CombinedScalarSSE_SSE2:
                    1;      // Combined scalar SSE and SSE2 instructions" } }
                };
                uint16 flat;
...
typedef union EVENT_MASK(  k8_retired_fastpath_double_op_instr )
        {
                ...
                struct
                {
                    uint16
                    LowOpPosition0:
                    1;             // With low op in position 0" },
                    uint16
                    LowOpPosition1:
                    1;         // With low op in position 1" },
                    uint16
                    LowOpPosition2:
                    1;    // With low op in position 2" } }
                };
                uint16 flat;
...
typedef union EVENT_MASK(  k8_fpu_exceptions)
        {
                ...
                {
                    uint16
                    x87ReclassMicrofaults:
                    1;     // x87 reclass microfaults" },
                    uint16
                    SSERetypeMicrofaults:
                    1;      // SSE retype microfaults" },
                    uint16
                    SSEReclassMicrofaults:
                    1;     // SSE reclass microfaults" },
                    uint16
                    SSE_x87Microtraps:
                    1;         // SSE and x87 microtraps" } }
                };
                uint16 flat;
...
typedef union EVENT_MASK(  k8_turnaround)
        {
                ...
                struct
                {
                    ...
                    1;       //DIMM turnaround" },
                    uint16
                    ReadToWriteTurnaround:
                    1;      //Read to write turnaround" },
                    uint16
                    WriteToReadTurnaround:
                    1;  //Write to read turnaround" } }
                };
                uint16 flat;
...
typedef union EVENT_MASK(  k8_bypass_counter_saturation)
        {
                ...
                {
                    uint16
                    MEM_HighPriorityBypass:
                    1;       // Memory controller high priority bypass" },
                    uint16
                    MEM_LowPriorityBypass:
                    1;      // Memory controller low priority bypass" },
                    uint16
                    DRAM_InterfaceBypass:
                    1;  // DRAM controller interface bypass" },
                    uint16
                    DRAM_QueueBypass:
                    1;  // DRAM controller queue bypass" } }
                };
                uint16 flat;
...
typedef union EVENT_MASK(  k8_probe_result)
        {
                struct
                {
                    ...
                    1;       // Probe hit" },
                    uint16
                    ProbeHitDirtyWithoutMemoryCancel:
                    1;       // Probe hit dirty without memory cancel" },
                    uint16
                    ProbeHitDirtyWithMemoryCancel:
                    1;       // Probe hit dirty with memory cancel" } }
                    uint16
                    UpstreamDisplayRefreshReads:
                    1;       // Rev D and later
                    uint16
                    UpstreamNonDisplayRefreshReads:
                    1;       // Rev D and later
                    uint16
                    UpstreamWrites:
                    1;       // Rev D and later
