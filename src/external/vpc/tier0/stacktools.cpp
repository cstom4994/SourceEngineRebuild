...
typedef BOOL (WINAPI *PFN_EnumerateLoadedModules64)(IN HANDLE hProcess, IN
                                                    PENUMLOADED_MODULES_CALLBACK64 EnumLoadedModulesCallback, IN
                                                    PVOID UserContext);

typedef DWORD64 (WINAPI *PFN_SymLoadModule64)(IN HANDLE hProcess, IN HANDLE hFile, IN PSTR ImageName, IN
                                              PSTR ModuleName, IN DWORD64 BaseOfDll, IN DWORD SizeOfDll);

typedef BOOL (WINAPI *PFN_SymUnloadModule64)(IN HANDLE hProcess, IN DWORD64 BaseOfDll);

typedef BOOL (WINAPI *PFN_SymFromAddr)(IN HANDLE hProcess, IN DWORD64 Address, OUT PDWORD64 Displacement, IN OUT
                                       PSYMBOL_INFO Symbol);

typedef BOOL (WINAPI *PFN_SymGetLineFromAddr64)(IN HANDLE hProcess, IN DWORD64 qwAddr, OUT PDWORD pdwDisplacement, OUT
...
typedef USHORT (WINAPI *PFN_CaptureStackBackTrace)(IN ULONG FramesToSkip, IN ULONG FramesToCapture, OUT
                                                   PVOID *BackTrace, OUT OPTIONAL PULONG BackTraceHash);
...
BOOL WINAPI SymEnumerateModules64_DummyFn(IN HANDLE hProcess, IN PSYM_ENUMMODULES_CALLBACK64 EnumModulesCallback, IN
                                          PVOID UserContext) {
    return FALSE;
}

BOOL
WINAPI EnumerateLoadedModules64_DummyFn(IN HANDLE hProcess, IN PENUMLOADED_MODULES_CALLBACK64 EnumLoadedModulesCallback,
                                        IN PVOID UserContext) {
    return FALSE;
}

DWORD64 WINAPI SymLoadModule64_DummyFn(IN HANDLE hProcess, IN HANDLE hFile, IN PSTR ImageName, IN PSTR ModuleName, IN
...
BOOL WINAPI SymFromAddr_DummyFn(IN HANDLE hProcess, IN DWORD64 Address, OUT PDWORD64 Displacement, IN OUT
                                PSYMBOL_INFO Symbol) {
    return FALSE;
}

BOOL WINAPI SymGetLineFromAddr64_DummyFn(IN HANDLE hProcess, IN DWORD64 qwAddr, OUT PDWORD pdwDisplacement, OUT
                                         PIMAGEHLP_LINE64 Line64) {
    return FALSE;
}
...
USHORT
WINAPI CaptureStackBackTrace_DummyFn(IN ULONG FramesToSkip, IN ULONG FramesToCapture, OUT PVOID *BackTrace, OUT OPTIONAL
...
public:
...
#endif
    {
        ...
        pThis->m_pSymLoadModule64(pThis->m_hProcess, NULL, (PSTR) ModuleName, (PSTR) ModuleName, ModuleBase,
                                  ModuleSize);
        return TRUE;
    }
    ...
    bool
    GetFileAndLineFromAddress(const void *pAddress, tchar *pFileNameOut, int iMaxFileNameLength, uint32 &iLineNumberOut,
        ...
        if (style & (TSISTYLEFLAG_FULLPATH | TSISTYLEFLAG_SHORTPATH | TSISTYLEFLAG_LINE | TSISTYLEFLAG_LINEANDOFFSET)) {
            ...
            if (this->GetFileAndLineFromAddress(pAddress, szFileName, MAX_PATH, iLine, &iDisplacement)) {
                ...
                if (style & (TSISTYLEFLAG_LINE | TSISTYLEFLAG_LINEANDOFFSET)) {
                    int nBytesWritten = _snprintf(pWrite, iTranslationBufferLength,
                                                  ((style & TSISTYLEFLAG_LINEANDOFFSET) && (iDisplacement != 0))
                                                  ? "(%d) + %d bytes" : "(%d)", iLine, iDisplacement);
                    if (nBytesWritten < 0) {
                        *pWrite = '\0'; // if we can't write all of the line/lineandoffset, don't write any at all
                        return false;
                    }
    ...
    void EnsureReady(void) {
        ...
        if (!m_hDbgHelpDll) {
            ...
            if (m_szPDBSearchPath ==
                NULL) //not a rock solid check, but pretty good compromise between endless failing initialization and general failure due to trying too early
                m_bIsInitialized = false;
        ...
        m_pEnumerateLoadedModules64 = (PFN_EnumerateLoadedModules64) ::GetProcAddress(m_hDbgHelpDll,
...
int EncodeBinaryToString(const void *pToEncode, int iDataLength, char *pEncodeOut, int iEncodeBufferSize) {
    ...
    while (pEncodeRead < pEncodeStop) {
        if (iControl == 0) {
            ...
            *pEncodeWriteLastControlByte |= (((*pEncodeRead) & 0x80) ^ 0x80)
