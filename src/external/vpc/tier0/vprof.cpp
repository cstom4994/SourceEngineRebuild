...
//-------------------------------------
static void
DumpSorted(const tchar *pszHeading, double totalTime, bool (*pfnSort)(const TimeSums_t &, const TimeSums_t &),
           int maxLen = 999999) {
    unsigned i;
    ...
    for (i = 0; i < sortedSums.size() && i < (unsigned) maxLen; i++) {
        ...
        double avgLessChildren = (sortedSums[i].calls) ? sortedSums[i].timeLessChildren / (double) sortedSums[i].calls
                                                       : 0.0;
