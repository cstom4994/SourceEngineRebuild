...
COptionsDialogXbox::COptionsDialogXbox(vgui::Panel *parent, bool bControllerOptions) : BaseClass(parent,
                                                                                                 "OptionsDialog") {
#ifdef _X360
            ...
            switch ((*m_pOptions)[m_iSelection]->eOptionType) {
                ...
                case OPTION_TYPE_CHOICE:
                    m_pFooter->AddNewButtonLabel(
                            ((*m_pOptions)[m_iSelection]->m_Choices.Count() == 2) ? ("#GameUI_Toggle")
                                                                                  : ("#GameUI_Modify"),
                            "#GameUI_Icons_DPAD");
                    break;
...
void COptionsDialogXbox::UpdateValue(OptionData_t *pOption, int iLabel) {
    ...
    switch (pOption->eOptionType) {
        ...
        case OPTION_TYPE_SLIDER: {
            ...
            float fNormalizeHome = fabsf(
                    (pOption->fMinValue - pOption->fSliderHomeValue) / (pOption->fMaxValue - pOption->fMinValue));
            if (pOption->fIncValue < 0.0f)
                fNormalizeHome = 1.0f - fNormalizeHome;
            m_pValueBars[iLabel]->SetHomeValue(fNormalizeHome);

            float fNormalizeValue = fabsf(
                    (pOption->fMinValue - varOption.GetFloat()) / (pOption->fMaxValue - pOption->fMinValue));
            if (pOption->fIncValue < 0.0f)
