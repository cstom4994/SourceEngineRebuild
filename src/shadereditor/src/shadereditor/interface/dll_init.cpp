...
bool ShaderEditorInterface::Init(CreateInterfaceFn appSystemFactory, CGlobalVarsBase *pGlobals, void *pSEditMRender,
    ...
    if (ProcShaderModule) {
        ...
        gProcShaderCTRL = ProcShaderFactory ? ((IVProcShader *) ProcShaderFactory(PROCSHADER_INTERFACE_VERSION, NULL))
                                            : NULL;
    }
...
void ShaderEditorInterface::OverridePaths(const char *pszWorkingDirectory, const char *pszCompilePath,
                                          const char *pszLocalCompilePath,
