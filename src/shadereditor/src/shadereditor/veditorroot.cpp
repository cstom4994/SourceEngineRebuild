...
//	m_pKV_BGPreviewMat = NULL;
    ...
    pMenu_Shader->AddMenuItem("Compile all precached", new KeyValues("onmenushader", "entry", ER_SMENU_ALLCOMPILE),
                              this);
    pMenu_Shader->AddMenuItem("Terminate compilers", new KeyValues("onmenushader", "entry", ER_SMENU_KILLCOMPILER),
                              this);
    pMenu_Shader->AddSeparator();
    pMenu_Shader->AddMenuItem("Inject shader into world", new KeyValues("onmenushader", "entry", ER_SMENU_INJECT),
                              this);
    ...
    pMenu_PostProc->AddMenuItem("Effect settings", new KeyValues("onmenupostprocessing", "entry", ER_PMENU_PCONFIG),
                                this);
    pMenu_PostProc->AddMenuItem("Effect precache", new KeyValues("onmenupostprocessing", "entry", ER_PMENU_PPRECACHE),
                                this);
    pMenu_PostProc->AddMenuItem("Manage rendertargets", new KeyValues("onmenupostprocessing", "entry", ER_PMENU_RTS),
                                this);
...
void CEditorRoot::OnThink(void) {
    ...
    _timer.Start();
    m_pLabel_FrameTime->SetText(VarArgs("fps: %4.1f, ms: %8.7f", 1.0f / max(0.00001, _frametime), _frametime));
...
void CEditorRoot::LoadLayout() {
    ...
    bool bPreview = false;
    if (pKV->LoadFromFile(filesystem, VarArgs("%s/shadereditorui/editor_config.txt", engine->GetGameDirectory()),
                          "MOD")) {
        px = pKV->GetInt("p_x");
        py = pKV->GetInt("p_y");
        psx = pKV->GetInt("p_sx");
