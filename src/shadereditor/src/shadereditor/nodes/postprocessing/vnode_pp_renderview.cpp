...
int CNodePP_RenderView::UpdateInputsValid() {
    ...
    localerror = max(localerror,
                     (int) ((GetJack_In(1)->GetNumBridgesConnected() < 1) ? ERRORLEVEL_UNDEFINED : ERRORLEVEL_NONE));
    localerror = max(localerror,
                     (int) ((m_ViewRenderData->GetName() == NULL ||
                             GetPPCache()->FindVrCallback(m_ViewRenderData->GetName()) < 0) ?
