    ...
    for (int i = LERROR__FIRST; i <= LERROR__LAST; i++) {
        ...
        switch (i) {
            ...
            case LERROR_VMT_PARAMS_MUTABLE: {
                const int TargetEnvID = (i == LERROR_VMT_PARAMS_STATIC) ? HLSLENV_SMART_VMT_STATIC
...
ITexture *GetPingPongRT(int num) {
    if (!_tex_PingPong[num]) {
        _tex_PingPong[num].Init(
                materials->FindTexture(VarArgs("_rt_SEdit_pingpong_%i", num), TEXTURE_GROUP_RENDER_TARGET));
...
void
CleanupString(const char *pSrc, char *pOut, int maxLength, bool AllowSpaces, bool AllowUnderscore, bool AllowNumbers,
              bool AllowNumberInFirstPlace) {
    Assert(maxLength > 1);
    int slotWrite = 0;
