...
public:
    ...
    static void
    RequestUpdate( /*CSmartText *target,*/ int flags = PARSERREQUEST_PS | PARSERREQUEST_VS, wchar_t *pszStream = NULL,
                                           CSmartObjectList *pParentDB = NULL);
};
...
void CSmartText::AddEnvInfoToDB() {
    ...
    if (m_iEnvInfo & NODECUSTOM_ADDENV_LIGHTING_PS) {
        ...
        pO = new CSmartObject(ACOMP_VAR, "PixelShaderLightInfo g_cLightInfo[3]", "g_cLightInfo",
                              "PixelShaderLightInfo");
        AutoCopyStringPtr(IDENT_STATIC_LIVE_CODE, &pO->m_pszSourceFile);
        m_pGlobalDatabase->AddUnit(pO);
                ...
                if (!bOnTyping || cw.x >= 1 && IS_SPACE(hRows[cw.y]->Read()[cw.x - 1])) {
                    ...
                    for (;;) {
                        ...
                        if (!bSkipLine && bracketClose.x < hRows[bracketClose.y]->Read().Count()) {
                            const wchar_t &cB = Read(
                                    &bracketClose); //hRows[ bracketClose.y ]->Read()[ bracketClose.x ];
