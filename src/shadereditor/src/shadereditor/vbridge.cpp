...
#define BRIDGEHULLEXTRUDE 60.0f
    ...
    swap(bounds_min.y, bounds_max.y);
    if (!::ShouldSimpleDrawObject(pNodeView, pNodeView,
                                  Vector4D(bounds_min.x, bounds_min.y, bounds_max.x, bounds_max.y)))
        return;
    ...
    int segments = ((int) RemapValClamped(pNodeView->GetZoomScalar(), 0, VIEWZOOM_OUT_MAX, BRIDGE_DRAW_SEGMENTS_MIN,
                                          BRIDGE_DRAW_SEGMENTS_MAX));
