    ...
    switch (m_iState) {
        ...
        case STD_SKINNING_MODE_POS_NORMAL:
            Q_snprintf(tmp, MAXTARGC,
                       "SkinPositionAndNormal( SKINNING, float4( %s, 1 ), %s,\n\t\t%s, %s,\n\t\t%s, %s );\n",
        ...
        case STD_SKINNING_MODE_POS_NORMAL_TANGENT:
            Q_snprintf(tmp, MAXTARGC,
                       "SkinPositionNormalAndTangentSpace( SKINNING, float4( %s, 1 ), %s, %s,\n\t\t%s, %s,\n\t\t%s, %s, %s, %s );\n",
                       pVar_ObjPos->GetName(), pVar_ObjNormal->GetName(), pVar_ObjTangentS->GetName(),
                       pVar_BoneWeights->GetName(), pVar_BoneIndices->GetName(),
                       GetTargetVar(0)->GetName(), GetTargetVar(1)->GetName(), GetTargetVar(2)->GetName(),
                       GetTargetVar(3)->GetName());
            break;
    }
