    ...
    if (m_bAllowNonAsciiCharacters) {
        ...
        if (count > 0) {
            ...
            for (int i = 0; i < count; ++i) {
                int id = subMenu->AddCheckableMenuItem("Language", UnlocalizeUnicode(langs[i].menuname),
                                                       new KeyValues("DoLanguageChanged", "handle",
                                                                     langs[i].handleValue), this);
                if (langs[i].active) {
        ...
        if (count > 0) {
            ...
            for (int i = 0; i < count; ++i) {
                int id = subMenu->AddCheckableMenuItem("ConversionMode", UnlocalizeUnicode(modes[i].menuname),
                                                       new KeyValues("DoConversionModeChanged", "handle",
                                                                     modes[i].handleValue), this);
                if (modes[i].active) {
                    subMenu->SetMenuItemChecked(id, true);
                }
            }
        ...
        if (count > 0) {
            ...
            for (int i = 0; i < count; ++i) {
                int id = subMenu->AddCheckableMenuItem("SentenceMode", UnlocalizeUnicode(sentencemodes[i].menuname),
                                                       new KeyValues("DoConversionModeChanged", "handle",
                                                                     modes[i].handleValue), this);
                if (modes[i].active) {
                    subMenu->SetMenuItemChecked(id, true);
...
const char *TextEntry::GetDescription() {
    ...
    Q_snprintf(buf, sizeof(buf),
               "%s, bool textHidden, bool editable, bool unicode, bool NumericInputOnly, int maxchars, bool TokenChannels",
