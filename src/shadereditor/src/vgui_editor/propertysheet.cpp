...
//#define __ptab_ANIMDUR 0.5f
    ...
    public:
        ...
        PageTab(PropertySheet *parent, const char *panelName, const char *text, char const *imageName, int maxTabWidth,
                Panel *page, bool showContextButton,
        ...
        void PerformLayout() {
            ...
            if (IsAnimating()) {
                ...
                if (abs(goal - m_flCurSizeX) > 1)
                    m_flCurSizeX = Approach(goal, m_flCurSizeX,
                                            m_pParent->m_dFrametime * 1000.0f * ((float) maxWidth / 200.0f));
                else
                    m_flCurSizeX = goal;
...
void PropertySheet::AddPage(Panel *page, const char *title, char const *imageName /*= NULL*/,
    ...
    PageTab *tab = new PageTab(this, "tab", title, imageName, _tabWidth, page, m_bContextButton && bHasContextMenu,
                               hoverActivatePageTime, m_bCloseableTabs);
...
void PropertySheet::OnThink() {
    ...
    if (ShouldShowArrows()) {
        ...
        if (abs(m_iTabXScroll_Target - m_flTabXScroll_Accum) > 1.0f)
            m_flTabXScroll_Accum +=
                    ((float) m_iTabXScroll_Target - m_flTabXScroll_Accum) * min(1.0f, m_dFrametime * 16.0f);
