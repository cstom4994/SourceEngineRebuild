...
void FileOpenDialog::Init(const char *title, KeyValues *pContextKeyValues) {
    ...
    m_pOpenButton->SetCommand(
            (m_DialogType != FOD_SELECT_DIRECTORY) ? new KeyValues("OnOpen") : new KeyValues("SelectFolder"));
...
void FileOpenDialog::BeginIterate(const char *szDir, const char *pszFilter, ListPanel *pList, int iFlags) {
    ...
    while (curFile[0]) {
        ...
        const bool bIsDirectory = g_pFullFileSystem->FindIsDirectory(
                _handle); //g_pFullFileSystem->IsDirectory( FullPath_Local, pPathID );
