...
ConversionErrorType ImgUtl_ConvertJPEGToTGA(const char *jpegpath, const char *tgaPath, bool bRequirePowerOfTwo) {
    ...
    CUtlBuffer outBuf;
    bool bRetVal = TGAWriter::WriteToBuffer(buf, outBuf, image_width, image_height, IMAGE_FORMAT_RGB888,
                                            IMAGE_FORMAT_RGB888);
    if (bRetVal) {
...
ConversionErrorType ImgUtl_ConvertBMPToTGA(const char *bmpPath, const char *tgaPath) {
    ...
    CUtlBuffer outBuf;
    bool retval = TGAWriter::WriteToBuffer(pBufRGBA, outBuf, nWidth, nHeight, IMAGE_FORMAT_RGBA8888,
                                           IMAGE_FORMAT_RGB888);
    free(pBufRGBA);
...
unsigned char *
ImgUtl_ReadTGAAsRGBA(const char *tgaPath, int &width, int &height, ConversionErrorType &errcode, TGAHeader &tgaHeader) {
        ...
        for (destColumn = 0; destColumn < destWidth; ++destColumn) {
            ...
            while (srcCurrentY < srcEndY) {
                ...
                while (srcCurrentX < srcEndX) {
                    ...
                    unsigned char srcCurrentGreen = srcBuf[(srcCurrentPixelY * srcWidth * 4) + (srcCurrentPixelX * 4) +
                                                           1];
                    unsigned char srcCurrentBlue = srcBuf[(srcCurrentPixelY * srcWidth * 4) + (srcCurrentPixelX * 4) +
                                                          2];
                    unsigned char srcCurrentAlpha = srcBuf[(srcCurrentPixelY * srcWidth * 4) + (srcCurrentPixelX * 4) +
                                                           3];
...
ConversionErrorType
ImgUtl_ConvertToVTFAndDumpVMT(const char *pInPath, const char *pMaterialsSubDir, int nMaxWidth/*=-1*/,
                              int nMaxHeight/*=-1*/) {
#ifndef _XBOX
    ...
    if (convertTGAToVTF && !failed) {
        nErrorCode = ImgUtl_ConvertTGA(tgaPath, nMaxWidth,
                                       nMaxHeight); // resize TGA so that it has power-of-two dimensions with a max size of (nMaxWidth)x(nMaxHeight).
        if (nErrorCode != CE_SUCCESS) {
            failed = true;
        }
...
ConversionErrorType
ImgUtl_LoadBitmapFromBuffer(CUtlBuffer &fileData, Bitmap_t &bitmap, ImageFileFormat eImageFileFormat) {
...
ConversionErrorType
ImgUtl_SaveBitmapToBuffer(CUtlBuffer &fileData, const Bitmap_t &bitmap, ImageFileFormat eImageFileFormat) {
    switch (eImageFileFormat) {
        case kImageFileFormat_JPG:
