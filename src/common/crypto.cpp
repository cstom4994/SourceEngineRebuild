...
#include "../thirdparty/cryptopp860/cryptlib.h"
#include "../thirdparty/cryptopp860/osrng.h"
#include "../thirdparty/cryptopp860/crc.h"
#include "../thirdparty/cryptopp860/modes.h"
#include "../thirdparty/cryptopp860/files.h"
#include "../thirdparty/cryptopp860/hex.h"
#include "../thirdparty/cryptopp860/base64.h"
#include "../thirdparty/cryptopp860/base32.h"
#include "../thirdparty/cryptopp860/words.h"
#include "../thirdparty/cryptopp860/rsa.h"
#include "../thirdparty/cryptopp860/aes.h"
#include "../thirdparty/cryptopp860/hmac.h"
#include "../thirdparty/cryptopp860/zlib.h"
#include "../thirdparty/cryptopp860/gzip.h"
#include "../thirdparty/cryptopp860/pwdbased.h"
...
//-----------------------------------------------------------------------------
bool
CCrypto::RSAGenerateKeys(uint8 *pubPublicKey, uint32 *pcubPublicKey, uint8 *pubPrivateKey, uint32 *pcubPrivateKey) {
...
bool CCrypto::RSADecrypt(const uint8 *pubEncryptedData, uint32 cubEncryptedData,
    ...
    try        // handle any exceptions crypto++ may throw
    {
        ...
        for (uint32 nBlock = 0; nBlock < cBlocks; nBlock++) {
            ...
            DecodingResult decodingResult = rsaDecryptor.Decrypt(rng.GetRNG(), pubEncryptedData, cubToDecrypt,
                                                                 pubPlaintextData);
...
uint32 CCrypto::Base64EncodeMaxOutput(const uint32 cubData, const char *pszLineBreak) {
    ...
    str_size cchRequired = 1 + nGroups * 4 +
                           (pszLineBreak ? Q_strlen(pszLineBreak) * (1 + (nGroups - 1) / k_LineBreakEveryNGroups) : 0);
    return cchRequired;
}
...
bool CCrypto::GenerateSHA1Digest(const uint8 *pubInput, const int cubInput, SHADigest_t *pOutDigest) {
    ...
    bool bSuccess = true;
    try {
        CryptoPP::SHA256().CalculateDigest(*pOutDigest, pubInput, cubInput);
    }
    catch (...) {
        bSuccess = false;
    }
...
bool CCrypto::GenerateSaltedSHA1Digest(const char *pchInput, const Salt_t *pSalt, SHADigest_t *pOutDigest) {
    ...
    bool bSuccess = true;
    try {
        CryptoPP::SHA256().CalculateDigest(*pOutDigest, pubSaltedInput, iInputLen + sizeof(Salt_t));
    }
    catch (...) {
        bSuccess = false;
    }
...
//-----------------------------------------------------------------------------
bool
CCustomHexEncoder::Encode(const uint8 *pubData, const uint32 cubData, char *pchEncodedData, uint32 cchEncodedData) {
...
//-----------------------------------------------------------------------------
bool
CCustomBase32Encoder::Encode(const uint8 *pubData, const uint32 cubData, char *pchEncodedData, uint32 cchEncodedData) {
...
bool CCrypto::BGeneratePBKDF2Hash(const char *pchInput, const Salt_t &Salt, unsigned int rounds,
    ...
    unsigned int iterations = pbkdf.DeriveKey((byte *) &OutPasswordHash.pbkdf2, sizeof(OutPasswordHash.pbkdf2), 0,
                                              (const byte *) pchInput, Q_strlen(pchInput), (const byte *) &Salt,
                                              sizeof(Salt), rounds);
...
//-----------------------------------------------------------------------------
bool
CCrypto::BUpgradeOrWrapPasswordHash(PasswordHash_t &InPasswordHash, EPasswordHashAlg hashTypeIn, const Salt_t &Salt,
                                    PasswordHash_t &OutPasswordHash, EPasswordHashAlg &hashTypeOut) {
    bool bResult = true;;
    ...
    if (hashTypeIn == k_EHashSHA1 || hashTypeIn == k_EHashBigPassword) {
        ...
        if (hashTypeOut == k_EHashPBKDF2_10000) {
            ...
            unsigned int iterations = pbkdf.DeriveKey((byte *) passOut.pbkdf2, sizeof(passOut.pbkdf2), 0, pbHash,
                                                      k_HashLengths[k_EHashSHA1], (const byte *) &Salt, sizeof(Salt),
                                                      10000);
