...
public:
    ...
    virtual void FlagReplayForFlush(CBaseReplay *pReplay,
                                    bool bForceImmediateWrite) = 0;    // Mark the given replay as dirty - flush to disk at the next opportunity (see CBaseReplayHistoryManager::FlushThink())
    ...
    virtual const char *
    GetBaseDirectory() = 0;    // Returns full directory to wherever replays.dmx lives, e.g. c:\program files (x86)\steam\steamapps\someuser\team fortress 2\game\tf\replays\client\ (or server\) - NOTE: includes trailing slash
...
//	virtual CBaseReplay		*GetReplayAtIndex( int nIndex ) = 0;
    ...
    virtual void FlagMovieForFlush(IReplayMovie *pMovie,
                                   bool bForceImmediate) = 0;    // Flag the movie for flush - if pMovie is NULL, mark the index for flush
    virtual void SetMovieRenderer(
            IReplayMovieRenderer *pRenderer) = 0;    // Set to be the panel that renders replay movies, or NULL when nothing is rendering
    virtual bool
    ShouldGameRenderView() = 0;    // Called from V_RenderView() to determine whether the game should render - used during movie rendering
    ...
    virtual void GetUnpaddedScreenshotSize(int &nWidth,
                                           int &nHeight) = 0;    // Get the dimensions for a screenshot if we take one right now, based on replay_screenshotresolution and the current aspect ratio
    virtual void
    DeleteScreenshotsForReplay(CReplay *pReplay) = 0;    // Deletes all screenshots associated with the given replay
    ...
    virtual void RenderMovie(
            RenderMovieParams_t const &params) = 0;    // Renders the given replay - or if params.hReplay is -1, render all unrendered replays
    ...
    virtual void
    OnPlayerClassChanged() = 0;    // Called when the player's class changes - we use this instead of an event for immediacy
    ...
    virtual CReplay *
    GetPlayingReplay() = 0;            // Get the currently playing replay, otherwise NULL if one isn't playing
    virtual CReplay *
    GetReplayForCurrentLife() = 0;    // Gets the current replay (constant from local player spawn until next spawn/disconnect/exit)
