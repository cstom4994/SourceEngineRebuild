...
int CStudioRender::R_StudioRenderModel(IMatRenderContext *pRenderContext, int skin,
                                       ...
                                       IMaterial **ppMaterials, int *pMaterialFlags, int flags, int boneMask, int lod,
...
int CStudioRender::R_StudioRenderFinal(IMatRenderContext *pRenderContext,
                                       int skin, int nBodyPartCount, BodyPartInfo_t *pBodyPartInfo,
...
static matrix3x4_t *
ComputeSkinMatrix(mstudioboneweight_t &boneweights, matrix3x4_t *pPoseToWorld, matrix3x4_t &result) {
...
inline void
CStudioRender::R_ComputeLightAtPoints3(const FourVectors &pos, const FourVectors &normal, FourVectors &color) {
    if (m_pRC->m_Config.fullbright) {
    ...
    for (int i = 0; i < m_pRC->m_NumLocalLights; i++) {
        ...
        switch (wl->m_Type) {
            ...
            case MATERIAL_LIGHT_SPOT: {
                fltx4 dot2 = SubSIMD(Four_Zeros,
                                     delta * wl->m_Direction); // dot position with spot light dir for cone falloff
...
public:
    static void R_PerformLighting(const Vector &forward, float fIllum,
                                  const Vector &pos, const Vector &norm, unsigned int nAlphaMask,
                                  unsigned int *pColor) {
    ...
    static void R_StudioSoftwareProcessMesh(const mstudio_meshvertexdata_t *vertData, matrix3x4_t *pPoseToWorld,
        ...
        for (int j = 0; j < numVertices; ++j) {
            ...
            R_TransformVert(pSrcPos, pSrcNorm, pSrcTangentS, pSkinMat,
                            *(VectorAligned *) &dstVertex.m_vecPosition, dstVertex.m_vecNormal,
...
template
void
CCachedRenderData::ComputeFlexedVertex_StreamOffset<mstudiovertanim_t>(studiohdr_t *pStudioHdr, mstudioflex_t *pflex,
                                                                       mstudiovertanim_t *pvanim, int vertCount,
...
void CStudioRender::ComputeFlexWeights(int nFlexCount, mstudioflex_t *pFlex, MorphWeight_t *pWeights) {
    for (int i = 0; i < nFlexCount; ++i, ++pFlex) {
        ...
        if (pFlex->flexpair != 0) {
            ...
            weight.m_pWeight[MORPH_WEIGHT_STEREO_LAGGED] = RampFlexWeight(*pFlex,
                                                                          m_pFlexDelayedWeights[pFlex->flexpair]);
        } else {
...
int CStudioRender::R_StudioDrawStaticMesh(IMatRenderContext *pRenderContext, mstudiomesh_t *pmesh,
    ...
    if (pColorMeshes && (pGroup->m_ColorMeshID != -1)) {
        ...
        numTrianglesRendered = R_StudioDrawGroupHWSkin(pRenderContext, pGroup, pGroup->m_pMesh,
...
void CStudioRender::SetEyeMaterialVars(IMaterial *pMaterial, mstudioeyeball_t *peyeball,
                                       Vector const &eyeOrigin, const matrix3x4_t &irisTransform,
...
int
CStudioRender::R_StudioDrawEyeball(IMatRenderContext *pRenderContext, mstudiomesh_t *pmesh, studiomeshdata_t *pMeshData,
...
//-----------------------------------------------------------------------------
int
CStudioRender::R_StudioDrawMesh(IMatRenderContext *pRenderContext, mstudiomesh_t *pmesh, studiomeshdata_t *pMeshData,
    ...
    for (int j = 0; j < pMeshData->m_NumGroup; ++j) {
        ...
        if (bShouldHardwareSkin && !m_pRC->m_Config.bDrawNormals && !m_pRC->m_Config.bDrawTangentFrame &&
            ...
            if (!m_pRC->m_Config.bNoSoftware) {
                numTrianglesRendered += R_StudioDrawDynamicMesh(pRenderContext, pmesh, pGroup, lighting,
...
int
CStudioRender::R_StudioDrawPoints(IMatRenderContext *pRenderContext, int skin, void /*IClientEntity*/ *pClientEntity,
                                  IMaterial **ppMaterials, int *pMaterialFlags, int boneMask, int lod,
                                  ColorMeshInfo_t *pColorMeshes) {
...
//	int numMeshes = SortMeshes( pIndices, ppMaterials, pskinref, vforward, r_origin );
    ...
    for (i = 0; i < m_pSubModel->nummeshes; ++i) {
        ...
        switch (pmesh->materialtype) {
            ...
            default:
                numTrianglesRendered += R_StudioDrawMesh(pRenderContext, pmesh, pMeshData, lighting, pMaterial,
                                                         pColorMeshes, lod);
                break;
        }
    }
