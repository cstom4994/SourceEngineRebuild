...
public:
    ...
    void
    DrawModelStaticProp(const DrawModelInfo_t &info, const StudioRenderContext_t &rc, const matrix3x4_t &modelToWorld,
                        int flags = STUDIORENDER_DRAW_ENTIRE_MODEL);
    ...
    void
    AddShadow(IMaterial *pMaterial, void *pProxyData, FlashlightState_t *pFlashlightState, VMatrix *pWorldToTexture,
...
private:
    ...
    IMaterial *R_StudioSetupSkinAndLighting(IMatRenderContext *pRenderContext, int index, IMaterial **ppMaterials,
                                            int materialFlags,
                                            void /*IClientEntity*/ *pClientEntity, ColorMeshInfo_t *pColorMeshes,
    ...
    void SetEyeMaterialVars(IMaterial *pMaterial, mstudioeyeball_t *peyeball,
                            const Vector &eyeOrigin, const matrix3x4_t &irisTransform,
    ...
    int BuildGlintRenderData(GlintRenderData_t *pData, int nMaxGlints,
                             const eyeballstate_t *pstate, const Vector &vright, const Vector &vup,
    ...
    R_StudioComputeLighting(IMaterial *pMaterial, int materialFlags, ColorMeshInfo_t *pColorMeshes);
...
private:
    inline float
    R_WorldLightAngle(const LightDesc_t *wl, const Vector &lnormal, const Vector &snormal, const Vector &delta);
    ...
    int
    SortMeshes(int *pIndices, IMaterial **ppMaterials, short *pskinref, const Vector &vforward, const Vector &r_origin);
    ...
    bool
    TransformToDecalSpace(DecalBuildInfo_t &build, const Vector &pos, mstudioboneweight_t *pboneweight, Vector2D &uv);
...
public:
    ...
    WorldLightAngle(const LightDesc_t *wl, const Vector &lnormal, const Vector &snormal, const Vector &delta) {
