...
IMaterial *CStudioRender::GetModelSpecificDecalMaterial(IMaterial *pDecalMaterial) {
    ...
    if (found) {
        IMaterial *pModelMaterial = g_pMaterialSystem->FindMaterial(pModelMaterialVar->GetStringValue(),
                                                                    TEXTURE_GROUP_DECAL, false);
...
//-----------------------------------------------------------------------------
bool
CStudioRender::DrawMultiBoneDecals(CMeshBuilder &meshBuilder, DecalMaterial_t &decalMaterial, studiohdr_t *pStudioHdr) {
    const thinModelVertices_t *thinVertData = NULL;
