...
//	config.m_VideoMode.m_Width = config.m_VideoMode.m_Height = 0;
    ...
    g_materialWireframeVertexColor = g_pMaterialSystem->FindMaterial("debug/debugwireframevertexcolor",
    ...
    // test: create this from code - you need a vmt to make $nocull 1 happen, can't do it from the render context
    {
        ...
        g_materialWireframeVertexColorNoCull = g_pMaterialSystem->CreateMaterial("debug/wireframenocull",
                                                                                 pVMTKeyValues);
                        ...
                        for (int i = 0; i < HLMV_MAX_MERGED_MODELS; i++) {
                            ...
                            if (iSequence == -1) {
                                ...
                                g_pStudioExtraModel[i]->SetFrame(
                                        g_pStudioModel->GetCycle() * g_pStudioExtraModel[i]->GetMaxFrame());
                            }
                        }
                    }
                }
