...
void CGameLump::ParseGameLump(dheader_t *pHeader) {
    ...
    if (length > 0) {
        ...
        for (int i = 0; i < pGameLumpHeader->lumpCount; ++i) {
            ...
            GameLumpHandle_t lump = g_GameLumps.CreateGameLump(pGameLump[i].id, length, pGameLump[i].flags,
                                                               pGameLump[i].version);
            if (g_bSwapOnLoad) {
    ...
    for (h = g_GameLumps.FirstGameLump(); h != g_GameLumps.InvalidGameLump(); h = g_GameLumps.NextGameLump(h)) {
        ...
        if (g_bSwapOnWrite) {
            g_GameLumps.SwapGameLump(g_GameLumps.GetGameLumpId(h), g_GameLumps.GetGameLumpVersion(h),
                                     (byte *) g_GameLumps.GetGameLump(h), (byte *) g_GameLumps.GetGameLump(h),
                                     lumpsize);
        }
...
static void SwapPhyscollideLump(byte *pDestBase, byte *pSrcBase, unsigned int &count) {
    ...
    CSysModule *pPhysicsModule = g_pFullFileSystem->LoadModule("engine.dll");
    if (pPhysicsModule) {
...
//		ParseEntities ();
    ...
    totalmemory += ArrayUsage("LDR worldlights", numworldlightsLDR, ENTRIES(dworldlightsLDR),
                              ENTRYSIZE(dworldlightsLDR));
    totalmemory += ArrayUsage("HDR worldlights", numworldlightsHDR, ENTRIES(dworldlightsHDR),
    ...
    totalmemory += ArrayUsage("cubemapsamples", g_nCubemapSamples, ENTRIES(g_CubemapSamples),
    ...
    totalmemory += ArrayUsage("LDR ambient table", g_LeafAmbientIndexLDR.Count(), MAX_MAP_LEAFS,
                              sizeof(g_LeafAmbientIndexLDR[0]));
    totalmemory += ArrayUsage("HDR ambient table", g_LeafAmbientIndexHDR.Count(), MAX_MAP_LEAFS,
                              sizeof(g_LeafAmbientIndexHDR[0]));
    ...
    totalmemory += ArrayUsage("occluder vert ind", g_OccluderVertexIndices.Count(), 0,
...
bool
CToolBSPTree::EnumerateLeavesInSphere(Vector const &center, float radius, ISpatialLeafEnumerator *pEnum, int context) {
...
static bool EnumerateLeavesAlongRay_R(int node, Ray_t const &ray,
                                      Vector const &start, Vector const &end, ISpatialLeafEnumerator *pEnum,
                                      int context) {
    float front, back;
...
bool CompressGameLump(dheader_t *pInBSPHeader, dheader_t *pOutBSPHeader, CUtlBuffer &outputBuffer,
    ...
    dgamelumpheader_t *pInGameLumpHeader = (dgamelumpheader_t *) (((byte *) pInBSPHeader) +
