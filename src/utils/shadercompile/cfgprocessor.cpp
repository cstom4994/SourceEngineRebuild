...
public:
    ...
    int Get(char const *szName) const {
        ...
        if (end() != it) return it->second; else return -1;
    };
...
public:
    ...
    char const *Lookup(char const *szString) {
        ...
        if (end() != it) return it->data(); else return NULL;
    }
...
class Define {
public:
    explicit Define(char const *szName, int min, int max, bool bStatic) : m_sName(szName), m_min(min), m_max(max),
                                                                          m_bStatic(bStatic) {}
...
namespace CfgProcessor {
    ...
    ComboHandle Combo_GetNext(uint64 &riCommandNumber, ComboHandle &rhCombo, uint64 iCommandEnd) {
        ...
        if (!rhCombo) {
            ...
            CPCHI_t emptyCPCHI;
            CPCHI_t const &chiFound = ConfigurationProcessing::s_mapComboCommands.GetLessOrEq(iCommandFound,
                                                                                              emptyCPCHI);
        ...
        for (;;) {
            ...
            CPCHI_t emptyCPCHI;
            CPCHI_t const &chiNext = ConfigurationProcessing::s_mapComboCommands.GetLessOrEq(iCommandLookup,
                                                                                             emptyCPCHI);
