...
namespace Threading {
    ...
    enum Mode {
        eSingleThreaded = 0, eMultiThreaded = 1
    };
    ...
    public:
        FORCEINLINE explicit CSwitchableMutex(Mode eMode, MT_MUTEX_TYPE *pMtMutex = NULL) : m_pMtx(pMtMutex), m_pUseMtx(
                eMode ? pMtMutex : NULL) {}
    ...
    public:
        ...
        FORCEINLINE bool
        TryLock() { if (MT_MUTEX_TYPE *pUseMtx = m_pUseMtx) return pUseMtx->TryLock(); else return true; }

        FORCEINLINE bool
        AssertOwnedByCurrentThread() { if (MT_MUTEX_TYPE *pUseMtx = m_pUseMtx) return pUseMtx->AssertOwnedByCurrentThread(); else return true; }

        FORCEINLINE void SetTrace(bool b) { if (MT_MUTEX_TYPE *pUseMtx = m_pUseMtx) pUseMtx->SetTrace(b); }

        FORCEINLINE uint32
        GetOwnerId() { if (MT_MUTEX_TYPE *pUseMtx = m_pUseMtx) return pUseMtx->GetOwnerId(); else return 0; }

        FORCEINLINE int
        GetDepth() { if (MT_MUTEX_TYPE *pUseMtx = m_pUseMtx) return pUseMtx->GetDepth(); else return 0; }
...
static void WriteShaderFiles(const char *pShaderName) {
    ...
    for (int nChain = 0; nChain < NELEMS(pByteCodeArray->m_HashChains); nChain++) {
        for (CStaticCombo *pStatic = pByteCodeArray->m_HashChains[nChain].m_pHead;
            ...
            if (pStatic->m_abPackedCode.GetLength()) {
                ...
                Hdr.m_nCRC32 = CRC32_ProcessSingleBuffer(pStatic->m_abPackedCode.GetData(),
    ...
    //
    CUtlStreamBuffer ShaderFile(szVCSfilename,
...
public:
    using CParallelProcessorBase::Run;
    ...
    if ((!g_bSuppressWarnings && szListing) || !pResponse->Succeeded()) {
        ...
        if (!szListing) {
            sprintf(chUnreportedListing,
                    "(0): error 0000: Compiler failed without error description, latest version of fxc.exe might give a description.");
...
void CDistributeShaderCompileMaster::OnWorkUnitsCompleted(uint64 numWorkUnits) {
    ...
    CfgProcessor::CfgEntryInfo const *pEntry = GetEntryByStaticComboNum(g_numCompletedStaticCombos,
