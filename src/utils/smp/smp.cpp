        ...
        switch (version.dwMinorVersion) {
            ...
            case 10:
                return (strstr(version.szCSDVersion, _TEXT(" A")) == version.szCSDVersion) ||
                       (strstr(version.szCSDVersion, _TEXT(" B")) == version.szCSDVersion) ? OSV_98SE : OSV_98;
            case 90:
                return OSV_ME;
        }
...
void
SetRegistryValue(const char *pKeyName, const char *pValueName, DWORD dwValue, DWORD &dwOldValue, bool &bValueExisted) {
    HKEY hKey = 0;
...
bool InitD3D(HWND hWnd, bool blur) {
    ...
    if (FAILED(g_pd3dDevice->CreateTexture(g_screenWidth, g_screenHeight, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_MANAGED,
                                           &g_pImg))) {
            ...
            if (wparam == ID_DRAW_TIMER) {
                ...
                if (g_timeAtFadeStart == 0.0) {
                    ...
                    SetTimer(hWnd, ID_SKIP_FADE_TIMER, 100 + UINT(FADE_TIME * 1000),
                             NULL); // restart skip fade timer and give it an extra 100ms to allow the fade to draw fully black once
                }
...
#endif
                    g_pFrame = &g_frame;
                    g_pFrame->GetWndClassInfo().m_wc.hIcon = LoadIcon(_Module.GetResourceInstance(),
...
void KillOtherSMPs() {
    ...
    for (int i = 0; i < nProcIds; ++i) {
        ...
        HANDLE hProcess = OpenProcess(PROCESS_TERMINATE | PROCESS_VM_READ | PROCESS_QUERY_INFORMATION, FALSE,
                                      procIds[i]);
        if (!hProcess) {
