...
static SRes Lzma2State_Code(void *pp, Byte *dest, SizeT *destLen, const Byte *src, SizeT *srcLen,
    ...
    SRes res = Lzma2Dec_DecodeToBuf((CLzma2Dec *) pp, dest, destLen, src, srcLen, (ELzmaFinishMode) finishMode,
                                    &status);
    srcWasFinished = srcWasFinished;
    ...
    for (;;) {
        ...
        */
        ...
        for (i = 0; i < p->numCoders; i++) {
            ...
            res = coder->Code(coder->p, destCur, &destLenCur, srcCur, &srcLenCur, srcFinishedCur, finishMode,
                              &encodingWasFinished);
