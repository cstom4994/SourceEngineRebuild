...
bool IsRestrictedFileType(const char *pcFileName) {
    return (V_stristr(pcFileName, ".bat") || V_stristr(pcFileName, ".cmd") || V_stristr(pcFileName, ".com") ||
            V_stristr(pcFileName, ".dll") ||
            V_stristr(pcFileName, ".exe") || V_stristr(pcFileName, ".msi") || V_stristr(pcFileName, ".rar") ||
            V_stristr(pcFileName, ".reg") ||
...
static void
AddFileToPack(CPackedStore &mypack, char const *pSrcName, int nPreloadSize = 0, char const *pDestName = NULL) {
    ...
    ePackedStoreAddResultCode rslt = mypack.AddFile(pDestName, Min(fileSize, nPreloadSize), pData, fileSize,
                                                    s_bMakeMultiChunk);
...
void VPKBuilder::BuildSteamPipeFriendlyFromInputKeys() {
    ...
    for (int idxChunk = 0; idxChunk < nNewChunkCount; ++idxChunk) {
        ...
        for (int idxFile = r.m_iFirstInputFile; idxFile <= r.m_iLastInputFile; ++idxFile) {
            ...
            int nBytesWritten = g_pFullFileSystem->Write((byte *) buf.Base() + f->m_iPreloadSize, nBytesToWrite,
                                                         fChunkWrite);
