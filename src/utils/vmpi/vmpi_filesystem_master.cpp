...
int
CMasterMulticastThread::AddFileRequest(const char *pFilename, const char *pPathID, int clientID, bool *bZeroLength) {
...
DWORD CMasterMulticastThread::MulticastThread() {
    ...
    while (WaitForSingleObject(m_hTermEvent, msToWait) != WAIT_OBJECT_0) {
        ...
        unsigned long optimalTimeTaken = (unsigned long) (transmitRateMgr.GetMicrosecondsPerByte() *
