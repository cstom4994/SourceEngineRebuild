...
void VMPI_WorkerThread(int iThread, void *pUserData) {
    ...
    while (g_iMasterFinishedDistributeWorkCall < g_iCurDSInfo && !g_bVMPIEarlyExit) {
        ...
        WUIndexType const *pBegin = &pWorkerInfo->m_WorkUnitsRunning.Get(0ull), *pEnd =
                pBegin + pWorkerInfo->m_WorkUnitsRunning.PastVisibleIndex();
...
double DistributeWork(
    ...
    if (g_bMPIMaster) {
        ...
        g_pCurDistributorMaster = (eWorkUnitDistributor == k_eWorkUnitDistributor_SDK ? CreateWUDistributor_SDKMaster()
        ...
        g_pCurDistributorWorker = (eWorkUnitDistributor == k_eWorkUnitDistributor_SDK ? CreateWUDistributor_SDKWorker()
