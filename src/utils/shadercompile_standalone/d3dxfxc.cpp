    ...
    namespace Private {
        ...
        public:
            ...
            const char *GetListing() override {
                return static_cast<const char *>(m_pListing ? m_pListing->GetBufferPointer() : nullptr);
            }
        ...
        //
        void FastShaderCompile(const char *pszFilename, std::span<const D3D_SHADER_MACRO> pMacros, const char *pszModel,
                               CmdSink::IResponse **ppResponse, DWORD flags) {
            ...
            if (!FAILED(hr)) {
                hr = D3DCompile(lpcvData, numBytes, pszFilename, pMacros.data(), &s_incDxImpl, "main", pszModel, flags,
                                0, &pShader, &pErrorMessages);
