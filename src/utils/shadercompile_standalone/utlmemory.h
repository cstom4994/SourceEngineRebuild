...
CUtlMemory<T, I>::CUtlMemory(int nGrowSize, int nInitAllocationCount)
        : m_pMemory(nullptr), m_nAllocationCount(nInitAllocationCount), m_nGrowSize(nGrowSize) {
    Assert(nGrowSize >= 0);
...
void CUtlMemory<T, I>::Grow(int num) {
    ...
    if (static_cast<int>( static_cast<I>( nNewAllocationCount )) < nAllocationRequested) {
        if (static_cast<int>( static_cast<I>( nNewAllocationCount )) == 0 &&
            static_cast<int>( static_cast<I>( nNewAllocationCount - 1 )) >= nAllocationRequested)
...
CUtlMemoryAligned<T, nAlignment>::CUtlMemoryAligned(T *pMemory, int numElements) {
    ...
    this->m_nAllocationCount =
            (reinterpret_cast<intptr_t>( pMemory + numElements ) - reinterpret_cast<intptr_t>( this->m_pMemory )) /
            sizeof(T);
}
...
void CUtlMemoryAligned<T, nAlignment>::Grow(int num) {
    ...
    this->m_nAllocationCount = UtlMemory_CalcNewAllocationCount(this->m_nAllocationCount, this->m_nGrowSize,
                                                                nAllocationRequested, sizeof(T));

    if (this->m_pMemory)
        this->m_pMemory = static_cast<T *>( _aligned_realloc(this->m_pMemory, this->m_nAllocationCount * sizeof(T),
                                                             nAlignment));
    else
