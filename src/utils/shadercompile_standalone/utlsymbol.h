...
inline CUtlRBTree<T, I, L>::CUtlRBTree(int growSize, int initSize, const LessFunc_t &lessfunc, void *context)
        : m_LessFunc(lessfunc), m_pLessFuncContext(context), m_Elements(growSize, initSize), m_Root(InvalidIndex()),
          m_NumElements(0), m_FirstFree(InvalidIndex()), m_LastAlloc(m_Elements.InvalidIterator()) {
    ResetDbgInfo();
}

template<class T, class I, typename L>
inline CUtlRBTree<T, I, L>::CUtlRBTree(const LessFunc_t &lessfunc, void *context)
        : m_LessFunc(lessfunc), m_pLessFuncContext(context), m_Elements(0, 0), m_Root(InvalidIndex()), m_NumElements(0),
