...
ConVar bink_mat_queue_mode("bink_mat_queue_mode", "1", 0,
                           "Update bink on mat queue thread if mat_queue_mode is on (if turned off, always update bink on main thread; may cause stalls!)");
ConVar bink_try_load_vmt("bink_try_load_vmt", "0", 0,
                         "Try and load a VMT with the same name as the BIK file to override settings");
ConVar bink_use_preallocated_scratch_texture("bink_use_preallocated_scratch_texture", "1", 0,
                                             "Use a pre-allocated VTF instead of creating a new one and deleting it for every texture update. Gameconsole only.");
...
public:
    ...
    virtual bool HasPreallocatedScratchTexture() const {
        return IsGameConsole() ? bink_use_preallocated_scratch_texture.GetBool() : false;
    }
...
//-----------------------------------------------------------------------------
void
CBIKMaterialYTextureRegenerator::RegenerateTextureBits(ITexture *pTexture, IVTFTexture *pVTFTexture, Rect_t *pRect) {
...
//-----------------------------------------------------------------------------
void
CBIKMaterialCrTextureRegenerator::RegenerateTextureBits(ITexture *pTexture, IVTFTexture *pVTFTexture, Rect_t *pRect) {
    int nWidth = m_nSourceWidth;
    int nHeight = m_nSourceHeight;
...
//-----------------------------------------------------------------------------
void
CBIKMaterialCbTextureRegenerator::RegenerateTextureBits(ITexture *pTexture, IVTFTexture *pVTFTexture, Rect_t *pRect) {
    int nWidth = m_nSourceWidth;
    int nHeight = m_nSourceHeight;
...
void CBIKMaterial::CreateProceduralTextures(const char *pTextureName) {
    ...
    unsigned int nTextureFlags = (TEXTUREFLAGS_CLAMPS | TEXTUREFLAGS_CLAMPT | TEXTUREFLAGS_NOMIP |
    ...
    for (int i = 0; i < m_buffers.TotalFrames; i++) {
        if (m_buffers.Frames[i].YPlane.Allocate) {
            ...
            m_buffers.Frames[i].YPlane.Buffer = MemAlloc_AllocAligned(
                    m_buffers.Frames[i].YPlane.BufferPitch * m_buffers.YABufferHeight, 16);
        }
        if (m_buffers.Frames[i].cRPlane.Allocate) {
            ...
            m_buffers.Frames[i].cRPlane.Buffer = MemAlloc_AllocAligned(
                    m_buffers.Frames[i].cRPlane.BufferPitch * m_buffers.cRcBBufferHeight, 16);
        }
        ...
        if (m_buffers.Frames[i].cBPlane.Allocate) {
            ...
            m_buffers.Frames[i].cBPlane.Buffer = MemAlloc_AllocAligned(
                    m_buffers.Frames[i].cBPlane.BufferPitch * m_buffers.cRcBBufferHeight, 16);
        }
...
public:
    ...
    // Inherited from IBik
    virtual BIKMaterial_t
    CreateMaterial(const char *pMaterialName, const char *pFileName, const char *pPathID, int flags);
...
private:
    static void *RADLINK
    BinkMemAlloc( U32
    bytes ) { return malloc(bytes); };
    static void RADLINK

    BinkMemFree(void PTR4
...
void CBik::DumpPrecachedMovieList() {
    ...
    for (int i = 0; i < m_PrecachedMovies.Count(); ++i) {
        Msg("%d: %s, %d bytes\n", i, m_PrecachedMovies[i].m_BaseName.String(),
            m_PrecachedMovies[i].m_MemoryBuffer.Size());
    }
}
