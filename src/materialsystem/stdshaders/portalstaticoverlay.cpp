...
#include "convar.h"
#include "pp_portalstaticoverlay_vs20.inc"
#include "pp_portalstaticoverlay_ps20.inc"
#include "pp_portalstaticoverlay_ps20b.inc"
#include "cpp_shader_constant_register_map.h"

BEGIN_VS_SHADER( PP_PortalStaticOverlay, 
				...
				SHADER_PARAM(GHOSTOVERLAY, SHADER_PARAM_TYPE_BOOL, "0", "")

				SHADER_PARAM(PORTALTINTENABLED, SHADER_PARAM_TYPE_BOOL, "0", "")
				SHADER_PARAM(PORTALTINTCOLOR, SHADER_PARAM_TYPE_COLOR, "{255 255 255}", "")
				END_SHADER_PARAMS
...
SHADER_INIT
{
	...
	if (!params[PORTALTINTENABLED]->IsDefined())
		params[PORTALTINTENABLED]->SetIntValue(0);
}
...
SHADER_DRAW
{
	...
	bool bPortalTint = bColorWrites && params[PORTALTINTENABLED]->GetIntValue() != 0;
	...
	SHADOW_STATE
	{
		...
		if (params[GHOSTOVERLAY]->GetIntValue() != 0)
		{
		...
		DECLARE_STATIC_VERTEX_SHADER( pp_portalstaticoverlay_vs20 );
		SET_STATIC_VERTEX_SHADER_COMBO( MODEL,  bIsModel );
		SET_STATIC_VERTEX_SHADER( pp_portalstaticoverlay_vs20 );
		...
		if( bColorWrites || bAlphaMaskTexture )
		{
			...
			{
				DECLARE_STATIC_PIXEL_SHADER( pp_portalstaticoverlay_ps20b );
				...
				SET_STATIC_PIXEL_SHADER_COMBO(PORTAL_TINT, bPortalTint);
				SET_STATIC_PIXEL_SHADER( pp_portalstaticoverlay_ps20b );
			}
			else
			{
				DECLARE_STATIC_PIXEL_SHADER( pp_portalstaticoverlay_ps20 );
				SET_STATIC_PIXEL_SHADER_COMBO( HASALPHAMASK, bAlphaMaskTexture );
				SET_STATIC_PIXEL_SHADER_COMBO( HASSTATICTEXTURE, bStaticBlendTexture );
				SET_STATIC_PIXEL_SHADER_COMBO(PORTAL_TINT, bPortalTint);
				SET_STATIC_PIXEL_SHADER( pp_portalstaticoverlay_ps20 );
			}
		}
	}
	DYNAMIC_STATE
	{
		...
		SetPixelShaderConstant(1, PORTALTINTCOLOR);

		DECLARE_DYNAMIC_VERTEX_SHADER( pp_portalstaticoverlay_vs20 );
		SET_DYNAMIC_VERTEX_SHADER_COMBO( SKINNING, pShaderAPI->GetCurrentNumBones() > 0 );
		SET_DYNAMIC_VERTEX_SHADER( pp_portalstaticoverlay_vs20 );
		...
		if( bColorWrites || bAlphaMaskTexture )
		{
			if(g_pHardwareConfig->GetDXSupportLevel() >= 92)
			{
				DECLARE_DYNAMIC_PIXEL_SHADER( pp_portalstaticoverlay_ps20b );
				SET_DYNAMIC_PIXEL_SHADER_COMBO( HDRENABLED, IsHDREnabled() );
				SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo() );
				SET_DYNAMIC_PIXEL_SHADER( pp_portalstaticoverlay_ps20b );
			}
			else
			{
				DECLARE_DYNAMIC_PIXEL_SHADER( pp_portalstaticoverlay_ps20 );
				SET_DYNAMIC_PIXEL_SHADER_COMBO( HDRENABLED, IsHDREnabled() );
				SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo() );
				SET_DYNAMIC_PIXEL_SHADER( pp_portalstaticoverlay_ps20 );
			}
		}
	}
