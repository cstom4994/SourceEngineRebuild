...
#include "pp_portal_refract_vs20.inc"
#include "pp_portal_refract_ps20.inc"
#include "pp_portal_refract_ps20b.inc"
...
void InitParamsPortalRefract( CBaseVSShader *pShader, IMaterialVar** params, const char *pMaterialName, PortalRefractVars_t &info )
{
	...
	SET_PARAM_INT_IF_NOT_DEFINED(info.m_nPortalTintEnabled, 0);
}
...
{
	int nStage = IS_PARAM_DEFINED( info.m_nStage ) ? params[info.m_nStage]->GetIntValue() : 0;
	bool bPortalTint = IS_PARAM_DEFINED(info.m_nPortalTintEnabled) ? params[info.m_nPortalTintEnabled] : 0;
	...
	SHADOW_STATE
	{
		...
		// Vertex Shader
		DECLARE_STATIC_VERTEX_SHADER( pp_portal_refract_vs20 );
		SET_STATIC_VERTEX_SHADER_COMBO( STAGE, nStage );
		SET_STATIC_VERTEX_SHADER(pp_portal_refract_vs20 );

		// Pixel Shader
		if(g_pHardwareConfig->GetDXSupportLevel() >= 92)
		{
			DECLARE_STATIC_PIXEL_SHADER(pp_portal_refract_ps20b );
			SET_STATIC_PIXEL_SHADER_COMBO( STAGE, nStage );
			SET_STATIC_PIXEL_SHADER_COMBO(PORTAL_TINT, bPortalTint);
			SET_STATIC_PIXEL_SHADER(pp_portal_refract_ps20b );
		}
		else
		{
			DECLARE_STATIC_PIXEL_SHADER(pp_portal_refract_ps20 );
			SET_STATIC_PIXEL_SHADER_COMBO( STAGE, nStage );
			SET_STATIC_PIXEL_SHADER_COMBO(PORTAL_TINT, bPortalTint);
			SET_STATIC_PIXEL_SHADER(pp_portal_refract_ps20 );
		}
	...
	DYNAMIC_STATE
	{
		...
		DECLARE_DYNAMIC_VERTEX_SHADER(pp_portal_refract_vs20 );
		SET_DYNAMIC_VERTEX_SHADER(pp_portal_refract_vs20 );
		...
		if(g_pHardwareConfig->GetDXSupportLevel() >= 92)
		{
			DECLARE_DYNAMIC_PIXEL_SHADER(pp_portal_refract_ps20b );
			SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo() );
			SET_DYNAMIC_PIXEL_SHADER(pp_portal_refract_ps20b );
		}
		else
		{
			DECLARE_DYNAMIC_PIXEL_SHADER(pp_portal_refract_ps20 );
			SET_DYNAMIC_PIXEL_SHADER(pp_portal_refract_ps20 );
		}
		...
		pShader->SetPixelShaderConstant(7, info.m_nPortalTintColor);
	}
	pShader->Draw();
}
