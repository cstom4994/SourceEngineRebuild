...
#include "lightshafts_vs20.inc"
#include "lightshafts_ps20b.inc"
					  ...
					  IShaderShadow* pShaderShadow, LightShaftsVars_t &info, VertexCompressionType_t vertexCompression )
{
	...
	SHADOW_STATE
	{
		...
		{
			DECLARE_STATIC_VERTEX_SHADER(lightshafts_vs20);
			SET_STATIC_VERTEX_SHADER(lightshafts_vs20);

			DECLARE_STATIC_PIXEL_SHADER(lightshafts_ps20b);
			SET_STATIC_PIXEL_SHADER_COMBO(FLASHLIGHTDEPTHFILTERMODE, g_pHardwareConfig->GetShadowFilterMode());
			SET_STATIC_PIXEL_SHADER(lightshafts_ps20b);
		}
	...
	DYNAMIC_STATE
	{
		...
		{
			DECLARE_DYNAMIC_VERTEX_SHADER(lightshafts_vs20);
			SET_DYNAMIC_VERTEX_SHADER(lightshafts_vs20);
		}
		...
		UberlightState_t uberlightState;
		float flFlashlightTime = 0.f;
		float flNoiseStrength = 0.8f;
		int nNumPlanes = 64;
		...
		SetFlashLightColorFromState( flashlightState, pShaderAPI, PSREG_FLASHLIGHT_COLOR, false);
		...
		{
			flFlashlightTime = params[info.m_nFlashlightTime]->GetFloatValue();
		}

		if ( (info.m_nNumPlanes != -1) && params[info.m_nNumPlanes]->IsDefined() )
		{
			nNumPlanes = params[info.m_nNumPlanes]->GetIntValue();
		}

		if ( (info.m_nUberlight != -1) && params[info.m_nUberlight]->IsDefined() )
		{
			uberlightState.m_bEnabled = ( params[info.m_nUberlight]->GetIntValue() != 0 );
		}
		...
		{
			flNoiseStrength = params[info.m_nNoiseStrength]->GetFloatValue();
		}
		...
		packedParams[0] = flNoiseStrength;
		packedParams[1] = 64.0f / (float) nNumPlanes;
		packedParams[2] = 0.0f;
		packedParams[3] = 0.0f;
		pShaderAPI->SetPixelShaderConstant( 0, packedParams, 1 );

		// Directions for projective noise
		noiseScroll[0] = fmodf( flFlashlightTime * 0.043f *  0.394, 1.0f);	// UV offset for noise in red
		noiseScroll[1] = fmodf( flFlashlightTime * 0.043f *  0.919, 1.0f);
		noiseScroll[2] = fmodf( flFlashlightTime * 0.039f * -0.781, 1.0f);	// UV offset for noise in green
		noiseScroll[3] = fmodf( flFlashlightTime * 0.039f *  0.625, 1.0f);
		...
		if (uberlightState.m_bEnabled && bSM30)
		{
			if ( (info.m_nUberNearFar != -1) && params[info.m_nUberNearFar]->IsDefined() )
			{
				float v[4];
				params[info.m_nUberNearFar]->GetVecValue( v, 4 );
				uberlightState.m_fNearEdge = v[0];
				uberlightState.m_fFarEdge  = v[1];
				uberlightState.m_fCutOn    = v[2];
				uberlightState.m_fCutOff   = v[3];
			}
			...
			if ( (info.m_nUberHeightWidth != -1) && params[info.m_nUberHeightWidth]->IsDefined() )
			{
				...
				uberlightState.m_fWidth  = v[0];
				uberlightState.m_fWedge  = v[1];
				uberlightState.m_fHeight = v[2];
				uberlightState.m_fHedge  = v[3];
			}

			if ( (info.m_nUberShearRoundness != -1) && params[info.m_nUberShearRoundness]->IsDefined() )
			{
				float v[3];
				params[info.m_nUberShearRoundness]->GetVecValue(v, 3);
				uberlightState.m_fShearx = v[0];
				uberlightState.m_fSheary = v[1];
