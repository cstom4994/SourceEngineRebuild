...
#include "pp_weapon_sheen_pass_vs20.inc"
#include "pp_weapon_sheen_pass_ps20.inc"
#include "pp_weapon_sheen_pass_ps20b.inc"

#ifndef _X360
#include "pp_weapon_sheen_pass_vs30.inc"
#include "pp_weapon_sheen_pass_ps30.inc"
#endif
						  ...
						  IShaderShadow* pShaderShadow, WeaponSheenPassVars_t &info, VertexCompressionType_t vertexCompression )
{
	...
	SHADOW_STATE
	{
		...
		{
			// Vertex Shader
			DECLARE_STATIC_VERTEX_SHADER( pp_weapon_sheen_pass_vs20 );
			SET_STATIC_VERTEX_SHADER_COMBO( BUMPMAP, bBumpMapping ? 1 : 0 );
			SET_STATIC_VERTEX_SHADER( pp_weapon_sheen_pass_vs20 );

			// Pixel Shader
			if( g_pHardwareConfig->SupportsPixelShaders_2_b() )
			{
				DECLARE_STATIC_PIXEL_SHADER( pp_weapon_sheen_pass_ps20b );
				SET_STATIC_PIXEL_SHADER_COMBO( BUMPMAP, bBumpMapping ? 1 : 0 );
				SET_STATIC_PIXEL_SHADER( pp_weapon_sheen_pass_ps20b );
			}
			else
			{
				DECLARE_STATIC_PIXEL_SHADER( pp_weapon_sheen_pass_ps20 );
				SET_STATIC_PIXEL_SHADER_COMBO( BUMPMAP, bBumpMapping ? 1 : 0 );
				SET_STATIC_PIXEL_SHADER( pp_weapon_sheen_pass_ps20 );
			}
		}
#ifndef _X360
		else
		{
			...
			// Vertex Shader
			DECLARE_STATIC_VERTEX_SHADER( pp_weapon_sheen_pass_vs30 );
			SET_STATIC_VERTEX_SHADER_COMBO( BUMPMAP, bBumpMapping ? 1 : 0 );
			SET_STATIC_VERTEX_SHADER( pp_weapon_sheen_pass_vs30 );

			// Pixel Shader
			DECLARE_STATIC_PIXEL_SHADER( pp_weapon_sheen_pass_ps30 );
			SET_STATIC_PIXEL_SHADER_COMBO( BUMPMAP, bBumpMapping ? 1 : 0 );
			SET_STATIC_PIXEL_SHADER( pp_weapon_sheen_pass_ps30 );
		}
#endif
	...
	DYNAMIC_STATE
	{
...
#endif
		{
			...
			DECLARE_DYNAMIC_VERTEX_SHADER( pp_weapon_sheen_pass_vs20 );
			SET_DYNAMIC_VERTEX_SHADER_COMBO( SKINNING, pShaderAPI->GetCurrentNumBones() > 0 );
			SET_DYNAMIC_VERTEX_SHADER_COMBO( COMPRESSED_VERTS, (int)vertexCompression );
			SET_DYNAMIC_VERTEX_SHADER( pp_weapon_sheen_pass_vs20 );

			// Set Pixel Shader Combos
			if( g_pHardwareConfig->SupportsPixelShaders_2_b() )
			{
				DECLARE_DYNAMIC_PIXEL_SHADER( pp_weapon_sheen_pass_ps20b );
				SET_DYNAMIC_PIXEL_SHADER( pp_weapon_sheen_pass_ps20b );
			}
			else
			{
				DECLARE_DYNAMIC_PIXEL_SHADER( pp_weapon_sheen_pass_ps20 );
				SET_DYNAMIC_PIXEL_SHADER( pp_weapon_sheen_pass_ps20 );
			}
		}
#ifndef _X360
		else
		{
			...
			// Set Vertex Shader Combos
			DECLARE_DYNAMIC_VERTEX_SHADER( pp_weapon_sheen_pass_vs30 );
			SET_DYNAMIC_VERTEX_SHADER_COMBO( SKINNING, pShaderAPI->GetCurrentNumBones() > 0 );
			SET_DYNAMIC_VERTEX_SHADER_COMBO( MORPHING, pShaderAPI->IsHWMorphingEnabled() );
			SET_DYNAMIC_VERTEX_SHADER_COMBO( COMPRESSED_VERTS, (int)vertexCompression );
			SET_DYNAMIC_VERTEX_SHADER( pp_weapon_sheen_pass_vs30 );

			// Set Pixel Shader Combos
			DECLARE_DYNAMIC_PIXEL_SHADER( pp_weapon_sheen_pass_ps30 );
			SET_DYNAMIC_PIXEL_SHADER( pp_weapon_sheen_pass_ps30 );
		}
#endif
