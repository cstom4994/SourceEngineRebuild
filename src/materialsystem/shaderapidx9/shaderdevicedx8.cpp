...
bool CShaderDeviceMgrDx8::ComputeCapsFromD3D(HardwareCaps_t *pCaps, int nAdapter) {
    ...
    if (pCaps->m_bSoftwareVertexProcessing) {
        ...
        pCaps->m_NumBooleanVertexShaderConstants = pCaps->m_SupportsPixelShaders_2_0 ? 16
                                                                                     : 0;    // 2.0 parts have 16 bool vs registers
        pCaps->m_NumBooleanPixelShaderConstants = pCaps->m_SupportsPixelShaders_2_0 ? 16
                                                                                    : 0;    // 2.0 parts have 16 bool ps registers
        pCaps->m_NumIntegerVertexShaderConstants = pCaps->m_SupportsPixelShaders_2_0 ? 16
                                                                                     : 0;    // 2.0 parts have 16 bool vs registers
        pCaps->m_NumIntegerPixelShaderConstants = pCaps->m_SupportsPixelShaders_2_0 ? 16
                                                                                    : 0;    // 2.0 parts have 16 bool ps registers
    } else {
    ...
    if (IsOpenGL()) {
        ...
        pCaps->m_MaxNumLights = MIN(pCaps->m_MaxNumLights,
                                    (pCaps->m_bSupportsStaticControlFlow && pCaps->m_SupportsPixelShaders_2_b)
                                    ? MAX_NUM_LIGHTS : (MAX_NUM_LIGHTS - 2));
    }
    ...
    if (IsPC() || !IsX360()) {
        pCaps->m_SupportsSRGB = (
                D3D()->CheckDeviceFormat(nAdapter, DX8_DEVTYPE, D3DFMT_X8R8G8B8, D3DUSAGE_QUERY_SRGBREAD,
        ...
        if (pCaps->m_VendorID == VENDORID_NVIDIA) {
            ...
            if (IsOpenGL()) {
                ...
                if (!((pCaps->m_DeviceID == 0x009d) ||
                      ((pCaps->m_DeviceID >= 0x0391) && (pCaps->m_DeviceID <= 0x0395)))) {
                    pCaps->m_bFogColorAlwaysLinearSpace = true;
                    pCaps->m_bDX10Card = true;
                    pCaps->m_bDX10Blending = true;
                }
    ...
    pCaps->m_bSupportsStreamOffset = ((caps.DevCaps2 & D3DDEVCAPS2_STREAMOFFSET) &&
                                      // Tie these caps together since we want to filter out
    ...
    pCaps->m_SupportsCompressedVertices =
            (pCaps->m_nDXSupportLevel >= 90) && (pCaps->m_CanDoSRGBReadFromRTs) ? VERTEX_COMPRESSION_ON
...
bool CShaderDeviceMgrDx8::ValidateMode(int nAdapter, const ShaderDeviceInfo_t &info) const {
    ...
    ImageFormat backBufferFormat = FindNearestSupportedBackBufferFormat(nAdapter,
                                                                        DX8_DEVTYPE, displayMode.m_Format,
    ...
    for (int i = 0; i < 256; i++) {
        ...
        if (IsX360()) // Should we also do this for the PS3?
        {
            ...
            flSrgbGammaValue = SrgbLinearToGamma(
                    flLinearValue); // ( flLinearValue <= 0.0031308f ) ? ( flLinearValue * 12.92f ) : ( 1.055f * powf( flLinearValue, ( 1.0f / 2.4f ) ) ) - 0.055f;
...
//-----------------------------------------------------------------------------
IMesh *
CShaderDeviceDx8::CreateStaticMesh(VertexFormat_t vertexFormat, const char *pTextureBudgetGroup, IMaterial *pMaterial) {
    LOCK_SHADERAPI();
...
IIndexBuffer *
CShaderDeviceDx8::CreateIndexBuffer(ShaderBufferType_t bufferType, MaterialIndexFormat_t fmt, int nIndexCount,
                                    const char *pBudgetGroup) {
    LOCK_SHADERAPI();
