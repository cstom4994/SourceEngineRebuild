...
//-----------------------------------------------------------------------------
template
class CShaderBuffer<ID3DXBuffer>;
...
CreateD3DVertexShader(DWORD *pByteCode, int numBytes, const char *pShaderName, char *debugLabel = NULL) {
...
//-----------------------------------------------------------------------------
static HardwareShader_t
CreateD3DPixelShader(DWORD *pByteCode, unsigned int nCentroidMask, int numBytes, const char *pShaderName,
...
struct ShaderFileCache_t {
    ...
    int FindCombo(uint32 nStaticComboID) {
        int nSearchAliases = BinarySearchCombos(nStaticComboID, m_StaticComboDupRecords.Count(),
...
public:
    ...
    CreateVertexShader(const char *pVertexShaderFile, int nStaticVshIndex = 0, char *debugLabel = NULL);

    virtual PixelShader_t
    CreatePixelShader(const char *pPixelShaderFile, int nStaticPshIndex = 0, char *debugLabel = NULL);
...
private:
    ...
    bool LoadShaderCache(
            char *cacheName);    // read named file, establish compiled shader sets for each vertex+static and pixel+static, then link pairs as listed in table
...
bool CShaderManager::CreateDynamicCombos_Ver4(void *pContext, uint8 *pComboBuffer) {
    ...
    for (int i = 0; i < pHeader->m_nDynamicCombos; i++) {
        ...
        uint8 *pByteCode = (uint8 *) pComboBuffer + pLookup->m_nDataOffset + pLookup->m_pComboDictionary[i].m_Offset -
                           nStartingOffset;
        ...
        if (IsPC() && m_bCreateShadersOnDemand) {
            ...
            pLookup->m_ShaderStaticCombos.m_pCreationData[i].iCentroidMask = pFileCache->m_bVertexShader ? 0
                                                                                                         : pHeader->m_nCentroidMask;
        } else {
    ...
    while (bOK) {
        ...
        while (pReadPtr < pUnpackBuffer + nBlockSize) {
            ...
            if (IsPC() && m_bCreateShadersOnDemand) {
                ...
                pLookup->m_ShaderStaticCombos.m_pCreationData[iIndex].iCentroidMask = pFileCache->m_bVertexShader ? 0
...
//					}
                    ...
                    // pass binary code to d3d interface, on GL it will invoke the translator back to asm
                    hardwareShader = CreateD3DPixelShader(reinterpret_cast< DWORD *>( pReadPtr ),
...
bool CShaderManager::LoadAndCreateShaders(ShaderLookup_t &lookup, bool bVertexShader, char *debugLabel) {
    ...
    if (IsX360() && g_pQueuedLoader->IsMapLoading()) {
        ...
        uint8 *pOptimalBuffer = (uint8 *) g_pFullFileSystem->AllocOptimalReadBuffer(hFile, nAlignedBytesToRead,
                                                                                    nAlignedOffset);
