...
public:
    ...
    void
    DeferredGetDynamicMesh(VertexFormat_t vertexFormat, unsigned flags, IMesh *pVertexOverride, IMesh *pIndexOverride,
    ...
    bool OnGetDynamicMesh(VertexFormat_t vertexFormat, unsigned flags, IMesh *pVertexOverride, IMesh *pIndexOverride,
        ...
        if (!(flags & MQM_FLEX)) {
            if (pVertexOverride) {
                ...
                m_VertexFormat = (vertexFormat == 0) ? (pMaterial->GetVertexFormat() & ~VERTEX_FORMAT_COMPRESSED)
                                                     : vertexFormat;
    ...
    void
    ModifyBeginEx(bool bReadOnly, int firstVertex, int numVerts, int firstIndex, int numIndices, MeshDesc_t &desc) {
        CannotSupport();
    }
...
void CMatQueuedRenderContext::FogColor3f(float r, float g, float b) {
    FogColor3ub(clamp((int) (r * 255.0f), 0, 255), clamp((int) (g * 255.0f), 0, 255),
...
inline void CMatQueuedRenderContext::QueueMatrixSync() {
    void (IMatRenderContext::*pfnLoadMatrix)(
            const VMatrix &) = &IMatRenderContext::LoadMatrix; // need assignment to disambiguate overloaded function
...
void
CMatQueuedRenderContext::ReadPixels(int x, int y, int width, int height, unsigned char *data, ImageFormat dstFormat) {
    EndRender();
    MaterialLock_t hLock = m_pMaterialSystem->Lock();
