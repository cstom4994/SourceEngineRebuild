...
public:
    ...
    GetShaderAPITextureBindHandle(ITexture *pTexture, int nFrameVar, int nTextureChannel = 0);
    ...
    virtual bool
    InitRenderState(IShader *pShader, int numParams, IMaterialVar **params, ShaderRenderState_t *pRenderState,
    ...
    virtual void
    LoadTexture(IMaterialVar *pTextureVar, const char *pTextureGroupName, int nAdditionalCreationFlags = 0);
...
void CShaderSystem::CreateDebugMaterials() {
    ...
    for (i = 0; i < MATERIAL_DEBUG_COUNT; ++i) {
        ...
        m_pDebugMaterials[i] = static_cast<IMaterialInternal *>(MaterialSystem()->CreateMaterial(shaderName,
                                                                                                 pVMTKeyValues[i]));
...
//-----------------------------------------------------------------------------
void
CShaderSystem::DrawNormalMap(IShader *pShader, IMaterialVar **ppParams, VertexCompressionType_t vertexCompression) {
...
CShaderSystem::GetShaderAPITextureBindHandle(ITexture *pTexture, int nFrame, int nTextureChannel) {
...
void CShaderSystem::LoadTexture(IMaterialVar *pTextureVar, const char *pTextureGroupName,
    ...
    if (MaterialSystem()->CanUseEditorMaterials() && (stricmp(pName, "env_cubemap") == 0)) {
        ...
        pTexture = static_cast< ITextureInternal * >( MaterialSystem()->FindTexture(pName, pTextureGroupName, false,
