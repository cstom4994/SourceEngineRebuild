...
void CSessionBlockPublisher::WriteAndPublishSessionBlock() {
    ...
    g_pEngine->GetSessionRecordBuffer(&pSessionBuffer,
                                      &nSessionBufferSize);    // This will get called the last client disconnects from the server - but in waiting for players state we won't have a demo buffer
