...
void CPerformanceController::SetupPlaybackFromPerformance(CReplayPerformance *pPerformance) {
    AssertMsg(!m_pSavedPerformance,
              "This probably hit because either SaveNow() or Discard() were not called.  One of those should always be called on disconnect after watching or editing a replay.");
...
//				g_pEngineClient->Con_NPrintf( 0, "sample 0 time: %f", flEventTime );
                    ...
                    bShouldCut = bShouldCut || m_pCurEvent->GetBool(
                            "cut");    // We cut if any set-view event cuts, otherwise we will interpolate
                }
                break;
                ...
                if (pSearch->GetInt("type", EVENTTYPE_INVALID) == EVENTTYPE_CAMERA_SETVIEW) {
                    ...
                    if (flDiff > 0.0f) {
                        ...
                        if (pSearch->GetBool("cut")) {
                            const int iSrc = clamp((int) (.5f + t), 0,
                                                   1);    // Round t to 0 or 1, so we set the camera to the current frame if t < 0.5f, and we set the camera to the 'cut'/next frame if t >= 0.5.
