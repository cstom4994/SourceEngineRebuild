...
//		Q_strncpy( strippedModName, ModName, sizeof( strippedModName ) );
    ...
    CModWizardSubPanel_CopyFiles *pCopyPanel = dynamic_cast<CModWizardSubPanel_CopyFiles *>(GetWizardPanel()->FindChildByName(
...
void CModWizardSubPanel_GetModInfo::OnCommand(const char *command) {
    if (Q_stricmp(command, "SearchButton") == 0) {
        CModalPreserveDirectorySelectDialog *pDlg = vgui::SETUP_PANEL(
                new CModalPreserveDirectorySelectDialog(this, "#SelectInstallDirectory"));
