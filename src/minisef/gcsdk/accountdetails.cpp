...
//-----------------------------------------------------------------------------
    bool CAccountDetails::BIsExpired() const {
        int nCacheSeconds = BIsValid() ? cv_account_details_cache_time.GetInt()
        ...
        virtual bool BYieldingRunGCJob() {
            ...
            for (int iTries = 0; iTries < kMaxTries; iTries++) {
                ...
                if (msgReply.Body().has_accountid() && msgReply.Body().accountid() != m_SteamID.GetAccountID()) {
                    ...
                    if (!bHasAlerted) {
                        GGCBase()->PostAlert(k_EAlertTypeInfo, true,
                                             CFmtStr("GetAccountDetails got a response for account %d, but we were expecting a response for account %s\n",
                                                     msgReply.Body().accountid(), m_SteamID.Render()));
                        bHasAlerted = true;
                    }

                    EmitError(SPEW_GC,
                              "GetAccountDetails got a response for account %d, but we were expecting a response for account %s\n",
                              msgReply.Body().accountid(), m_SteamID.Render());
                    break;
                }
...
//-----------------------------------------------------------------------------
    ...
    public:
        CGCJobSendGetPersonaNamesRequest(CGCBase *pGC, CAccountDetailsManager *pManager,
...
//-----------------------------------------------------------------------------
    void CAccountDetailsManager::SendBatchedPersonaNamesRequest() {
        ...
        CGCJobSendGetPersonaNamesRequest *pJob = new CGCJobSendGetPersonaNamesRequest(GGCBase(), this,
                                                                                      m_vecPendingPersonaNameLookups);
        pJob->StartJob(NULL);
    }
...
//-----------------------------------------------------------------------------
    bool CAccountDetailsManager::BExpireRecords(CLimitTimer &limitTimer) {
        ...
        for (CCachedPersonaName *pPersonaName = m_hashPersonaNameCache.PvRecordRun();
             NULL != pPersonaName; pPersonaName = m_hashPersonaNameCache.PvRecordRun()) {
...
//-----------------------------------------------------------------------------
    void CAccountDetailsManager::Dump() const {
        ...
        EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
                 "\tPersona Names: %d cached, %d lookups in flight, %d jobs waiting\n", m_hashPersonaNameCache.Count(),
                 m_mapQueuedPersonaNameRequests.Count(), nJobsWaiting);
    }
