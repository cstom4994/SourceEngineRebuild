...
namespace GCSDK {
    uint32 Leaderboard_YieldingFind(const char *pName, ELeaderboardSortMethod eLeaderboardSortMethod,
                                    ELeaderboardDisplayType eLeaderboardDisplayType, bool bCreateIfNotFound) {
        ...
        const EResult eAPIResult =
                eCallResult == k_EResultOK ? static_cast<EResult>( kvAPIResponse->GetInt("result", k_EResultFail))
                                           : k_EResultFail;
    ...
    bool Leaderboard_YieldingSetScore(uint32 unLeaderboardID, const CSteamID &steamID,
        ...
        apiRequest.SetPOSTParamString("scoremethod",
        ...
        EmitError(SPEW_GC, __FUNCTION__ ": error code %u/%u setting leaderboard %u to %i (%s) for user '%s'.\n",
                  ...
                  PchNameFromELeaderboardUploadScoreMethod(eLeaderboardUploadScoreMethod), steamID.Render());
        return false;
    }
