...
void CRTime::UpdateRealTime() {
    ...
    if (sm_nTimeCur - sm_nTimeLastSystemTimeUpdate >= 1) {
        ...
        GetTimeFormat(LOCALE_USER_DEFAULT, 0, &systemTimeLocal, "HH:mm:ss", sm_rgchLocalTimeCur,
                      Q_ARRAYSIZE(sm_rgchLocalTimeCur));
        GetDateFormat(LOCALE_USER_DEFAULT, 0, &systemTimeLocal, "MM/dd/yy", sm_rgchLocalDateCur,
                      Q_ARRAYSIZE(sm_rgchLocalDateCur));
#elif defined(POSIX)
        time_t now;
        time( &now );
        struct tm tmStruct;
...
RTime32 CRTime::RTime32FromFmtString(const char *pchFmt, const char *pchValue) {
    ...
    if (pchMnt) {
        static const char *rgszMonthNames[] = {"Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct",
                                               "Nov", "Dec"};
...
const char *CRTime::RTime32ToRFC3339UTCString(const RTime32 rTime32, char (&buf)[k_RTimeRenderBufferSize]) {
    ...
    Q_snprintf(buf, k_RTimeRenderBufferSize, "%04u-%02u-%02uT%02u:%02u:%02uZ", ptm->tm_year + 1900, ptm->tm_mon + 1,
...
// STATIC
const char *
CRTime::RTime32ToString(const RTime32 rTime32, char (&buf)[k_RTimeRenderBufferSize], bool bNoPunct /*=false*/,
...
RTime32 CRTime::RTime32MonthAddChooseDay(int nNthDayOfMonth, RTime32 rtime32StartDate, int nMonthsToAdd) {
    ...
    RTime32 rtime32FirstDayOfTargetMonth = CRTime::RTime32DateAdd(rtime32FirstDayOfStartMonth, nMonthsToAdd,
                                                                  k_ETimeUnitMonth);
