...
//-------------------------------------------------------------------------
    ...
    void CFrameFunctionMgr::DumpProfile() {
        ...
        EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS, "%s",
                 "Name                                         Time(ms)      Calls  Time%     Time/F    Calls/F\n");
        EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS, "%s",
                 "---------------------------------------- ------------ ---------- ------ ---------- ----------\n");
        ...
        FOR_EACH_VEC(FrameFuncs, nFunc) {
            ...
            EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS, "%-40s %12.2f %10d %5.1f%% %10.3f %10.1f\n",
                     pFunc->m_sName.Get(), pFunc->m_nTrackedTime / 1000.0, pFunc->m_nNumCalls,
                     pFunc->m_nTrackedTime / (double) nTotalTime, pFunc->m_nTrackedTime * fInvFrame / 1000.0,
        ...
        EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS, "%s",
                 "---------------------------------------- ------------ ---------- ------ ---------- ----------\n");
        EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS, "%-40s %12.2f %10d %5.1f%% %10.3f %10.1f\n",
                 "Totals", nTotalTime / 1000.0, nTotalCalls, 100.0, nTotalTime * fInvFrame / 1000.0,
                 nTotalCalls * fInvFrame);
    }
...
//utility class for dumping out the profile results after time has expired
    ...
    GC_CON_COMMAND( framefunc_profile,
...
EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
         "Proper usage is framefunc_profile <n> where n is the number of seconds to profile for\n");
return;
}
...
s_DumpProfile.
ScheduleMS( DumpFrameFunctionProfile, fSeconds
* 1000.0f );
}
