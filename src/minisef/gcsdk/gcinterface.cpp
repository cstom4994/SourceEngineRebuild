...
namespace GCSDK {
    static GCConVar cv_assert_minidump_window("assert_minidump_window", "28800",
    ...
    static GCConVar cv_assert_max_minidumps_in_window("assert_max_minidumps_in_window", "5",
                                                      "The amount of times each unique assert will write a dump in assert_minidump_window seconds");
    static GCConVar enable_assert_minidumps("enable_assert_minidumps", "1",
...
//-----------------------------------------------------------------------------
    ...
    public:
        ...
        virtual void *
        AssertionFailure(const char *pFormattedMsg, const tchar *pchFile, int nLine, const tchar *pchFunction,
                         const tchar *pchRawExpression, int nInstanceReportCount, AssertionType_t nType,
        ...
        virtual void
        MiniDumpHandler(const MiniDumpHandlerData_t &HandlerData, const char *pFormattedMsg, const tchar *pchFile,
                        int nLine, const tchar *pchFunction, const tchar *pchRawExpression, int nInstanceReportCount,
    ...
    static void
    ProtobufLogHandler(::google::protobuf::LogLevel level, const char *filename, int line, const std::string &message) {
        EG_MSG(g_EGMessages, "Protobuf %s(%d): %s\n", filename, line, message.c_str());
        AssertFatalMsg(level != google::protobuf::LOGLEVEL_FATAL, "Fatal protobuf assert %s(%d): %s", filename, line,
                       message.c_str());
    }
...
//-----------------------------------------------------------------------------
    CGCInterface::CGCInterface()
            : m_pGCHost(NULL), m_pGC(NULL), m_pGCDirProcess(NULL), m_nAppID(k_uAppIdInvalid),
              m_eUniverse(k_EUniverseInvalid), m_bDevMode(false), m_ullGID(0), m_bLogCaptureEnabled(false),
...
//-----------------------------------------------------------------------------
    ...
    bool CGCInterface::BReadConvars(KeyValuesAD &configValues) {
        ...
        if (k_EUniverseDev != GetUniverse()) {
            ...
            if (pkvSavedConvars->LoadFromFile(g_pFullFileSystem, CFmtStr("%u_%s_%s_savedconvars.vdf", GetAppID(),
...
//-----------------------------------------------------------------------------
    bool CGCInterface::BSaveConvars() {
        ...
        return pkvConvars->SaveToFile(g_pFullFileSystem,
                                      CFmtStr("%u_%s_%s_savedconvars.vdf", GetAppID(), m_pGCDirProcess->GetName(),
                                              PchNameFromEUniverse(GetUniverse())), "CONFIG");
    }
...
//-----------------------------------------------------------------------------
    void
    CGCInterface::RecordAssert(const char *pchFile, int nLine, const char *pchMessage, bool *pbShouldWriteMinidump) {
...
//-----------------------------------------------------------------------------
    bool CGCInterface::BAsyncInit(uint32 unAppID, const char *pchDebugName, int iGCIndex, IGameCoordinatorHost *pHost) {
        ...
        pHost->EmitSpew(SPEW_GC.GetName(), SPEW_ERROR, SPEW_ALWAYS, LOG_ALWAYS,
                        CFmtStr("Initializing %d bit GC, Dir Index %d, PID:%u\n", (uint32) (sizeof(void *) * 8),
                                iGCIndex, GetCurrentProcessId()).Access());
        pHost->EmitSpew(SPEW_GC.GetName(), SPEW_ERROR, SPEW_ALWAYS, LOG_ALWAYS,
        ...
        //open our a handle to our parent
        {
            ...
            if (nParentPID == 0) {
                pHost->EmitSpew(SPEW_GC.GetName(), SPEW_ERROR, SPEW_ALWAYS, LOG_ALWAYS,
                                "Parent process ID was not specified via -parentpid, unable to get information about the launching process\n");
            } else {
                m_hParentProcess = OpenProcess(PROCESS_QUERY_INFORMATION | PROCESS_VM_READ, FALSE, nParentPID);
                if (!m_hParentProcess) {
                    pHost->EmitSpew(SPEW_GC.GetName(), SPEW_ERROR, SPEW_ALWAYS, LOG_ALWAYS,
                                    "Unable to open the parent process with read access. Unable to get information about the launching process\n");
                }
            }
        }
        ...
        AssertMsg(iGCIndex >= 0 && iGCIndex < 256,
        ...
        if (NULL == pfnFactory) {
            GCSDK::EmitError(SPEW_CONSOLE,
                             "Failed to start GC for index %d (type %s). Got a NULL factory function, likely missing registration for this type\n",
...
//-----------------------------------------------------------------------------
    bool CGCInterface::BSendMessageToClient(uint64 ullSteamID, uint32 unMsgType, const void *pubData, uint32 cubData) {
        ...
        if (ullSteamID == k_steamIDNil.ConvertToUint64()) {
            AssertMsg(false, "Message %d sent to invalid steam ID. This message will not be processed.",
...
//-----------------------------------------------------------------------------
    void CGCInterface::EmitSpew(const char *pchGroupName, SpewType_t spewType, int iSpewLevel, int iLevelLog,
                                const char *pchMsg) {
