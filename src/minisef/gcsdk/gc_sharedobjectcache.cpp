...
//----------------------------------------------------------------------------
    ...
    IMPLEMENT_CLASS_MEMPOOL( CCachedSubscriptionMessage,
    10 * 1000, UTLMEMORYPOOL_GROW_SLOW );
}
...
//----------------------------------------------------------------------------
    void
    CGCSharedObjectTypeCache::BuildCacheSubscribedMsg(CMsgSOCacheSubscribed_SubscribedType *pMsgType, uint32 unFlags,
        ...
        if (pObject) {
            ...
            if (BShouldSendToAnyClients(unTypeFlags)) {
                ...
                FOR_EACH_VEC(vecSubscribers, nSub) {
                    ...
                    if (filter.BShouldSendObject(pObject, unFlags)) { VPROF_BUDGET(
                                CSharedObject::PchClassCreateNodeName(pObject->GetTypeID()), VPROF_BUDGETGROUP_STEAM);
                        vecRecipients.AddToTail(vecSubscribers[nSub].m_steamID);
                    }
                }

                if (vecRecipients.Count() > 0) { VPROF_BUDGET(
                            CSharedObject::PchClassCreateNodeName(pObject->GetTypeID()), VPROF_BUDGETGROUP_STEAM);
#if 0
...
//----------------------------------------------------------------------------
    void CGCSharedObjectCache::AddSubscriber(const CSteamID &steamID, bool bForceSendSubscriptionMsg) {
        MEM_ALLOC_CREDIT_("StartPlaying - AddSubscriber");VPROF_BUDGET("CGCSharedObjectCache::AddSubscriber()",
    ...
    void CGCSharedObjectCache::SendAllNetworkUpdates() {
        ...
        { VPROF_BUDGET("CGCSharedObjectCache::SendAllNetworkUpdates - send all messages", VPROF_BUDGETGROUP_STEAM);
            ...
            FOR_EACH_VEC(vecSubscribers, nSub) {
                ...
                if (vecObjsToSendToThisUser.Count() > 0) { VPROF_BUDGET(
                            "CGCSharedObjectCache::SendAllNetworkUpdates - Sending Msg", VPROF_BUDGETGROUP_STEAM);
                    ...
                    FOR_EACH_VEC(vecObjsToSendToThisUser, idx) {
                        ...
                        g_SharedObjectStats.TrackSharedObjectSend(objectMsg.type_id(), 1,
                                                                  objectMsg.object_data().size());
...
//----------------------------------------------------------------------------
    void CGCSharedObjectCache::SendNetworkCreateInternal(CSharedObject *pObj) {
        ...
        FOR_EACH_VEC(vecSubscribers, nSub) {
            ...
            Assert(steamID.IsValid());
            uint32 unFlags = CalcSendFlags(steamID);
            if (filter.BShouldSendObject(pObj, unFlags)) { VPROF_BUDGET(
                        CSharedObject::PchClassCreateNodeName(pObj->GetTypeID()), VPROF_BUDGETGROUP_STEAM);
...
//----------------------------------------------------------------------------
    void CGCSharedObjectCache::MarkDirty() {
        ...
        AssertMsg(!m_bDetectVersionChanges,
                  "Warning: Detected a change to the version of the SO cache when it was not expected. This is often done when an SO cache is being loaded, and can result in costly unnecessary updates of caches");
...
//-----------------------------------------------------------------------------
    ...
    GC_CON_COMMAND( so_profile_on,
    "Starts the profiling of shared object stats" ) {
...
GC_CON_COMMAND( so_profile_off,
"Stops the profiling of shared object stats" )
{
g_SharedObjectStats.
