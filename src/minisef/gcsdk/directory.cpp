...
// Purpose: Constructor
//-----------------------------------------------------------------------------
    CGCDirTypeInstance::CGCDirTypeInstance(const char *pszTypeName, uint32 nType, uint32 nInstance,
                                           const KeyValues *pKVConfig, const CGCDirProcess *pProcess)
            : m_nType(nType), m_sTypeName(pszTypeName), m_nInstance(nInstance), m_pProcess(pProcess) {
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
    CGCDirProcess::CGCDirProcess(uint32 nGCDirIndex, const char *pchName, const char *pchProcessType,
...
//-----------------------------------------------------------------------------
    CDirectory::CDirectory()
            : m_bInitialized(false), m_mapGCsByType(DefLessFunc(int32)), m_mapRegisteredGCTypes(DefLessFunc(int32)) {
    }
    ...
    bool CDirectory::BInit(KeyValues *pKVDirectory) {
        ...
        FOR_EACH_TRUE_SUBKEY(pKVDirectory, pkvProcess) {
            ...
            if (!pchProcessType || !pchProcessType[0]) {
                AssertMsg(false, "Process %s defined in the config with no process type associated with it",
                          pchProcessName);
                return false;
            }

            //create our new process info
            CGCDirProcess *pProcess = new CGCDirProcess(m_vecProcesses.Count(), pchProcessName, pchProcessType,
                                                        pkvProcess);
            ...
            FOR_EACH_SUBKEY(pkvProcess, pkvType) {
                ...
                if (s_nInvalidGCType == nGCType) {
                    AssertMsg(false,
                              "GC defined on box \"%s\" with unknown type \"%s\" encountered while parsing the config",
                ...
                CGCDirTypeInstance *pGC = new CGCDirTypeInstance(pchGCType, nGCType,
                                                                 m_mapGCsByType[nGCTypeIndex].Count(), pkvType,
                                                                 pProcess);
                pProcess->AddGC(pGC);
        ...
        if (1 != GetGCCountForType(nMasterType)) {
            AssertMsg(false, "Incorrect number of master GCs in the config. Expected: 1 Actual: %d",
