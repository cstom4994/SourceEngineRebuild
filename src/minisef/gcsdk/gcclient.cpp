...
//-----------------------------------------------------------------------------
    ...
    public:
        ...
        virtual bool BAsyncSend(MsgType_t eMsg, const uint8 *pubMsgBytes, uint32 cubSize) {
            g_theMessageList.TallySendMessage(eMsg & ~k_EMsgProtoBufFlag, cubSize);VPROF_BUDGET("CGCClient",
...
//------------------------------------------------------------------------------
    void CGCClient::OnGCMessageAvailable(GCMessageAvailable_t *pCallback) {
        ...
        while (m_pSteamGameCoordinator && m_pSteamGameCoordinator->IsMessageAvailable(&cubData)) {
            ...
            EGCResults eResult = m_pSteamGameCoordinator->RetrieveMessage(&unMsgType, pPacketFromGC,
                                                                          m_memMsg.Count() - sizeof(GCMsgHdr_t),
                                                                          &cubData);
            ...
            if (eResult == k_EGCResultOK) {
                if (unMsgType & k_EMsgProtoBufFlag) {
                    ...
                    if (pMsgNetPacket.Get() != NULL) {
                        ...
                        // dispatch the packet
                        GetJobMgr().BRouteMsgToJob(this, pMsgNetPacket.Get(), JobMsgInfo_t(pMsgNetPacket->GetEMsg(),
...
//------------------------------------------------------------------------------
    ...
    public:
        ...
        virtual bool BYieldingRunGCJob(GCSDK::IMsgNetPacket *pNetPacket) {
            ...
            if (pSOCache) {
                pSOCache->BCreateFromMsg(msg.Body().type_id(), msg.Body().object_data().data(),
    ...
    public:
        ...
        virtual bool BYieldingRunGCJob(GCSDK::IMsgNetPacket *pNetPacket) {
            ...
            if (pCache) {
                pCache->BDestroyFromMsg(msg.Body().type_id(), msg.Body().object_data().data(),
                                        msg.Body().object_data().size());
                Assert(msg.Body().has_version());
    ...
    public:
        ...
        virtual bool BYieldingRunGCJob(GCSDK::IMsgNetPacket *pNetPacket) {
            ...
            CGCClientSharedObjectCache *pSOCache = m_pGCClient->FindSOCache(msg.Body().owner());
            if (pSOCache) {
                pSOCache->BUpdateFromMsg(msg.Body().type_id(), msg.Body().object_data().data(),
                                         msg.Body().object_data().size());
                Assert(msg.Body().has_version());
                pSOCache->SetVersion(msg.Body().version());
            }
        ...
        virtual bool BYieldingRunGCJob(GCSDK::IMsgNetPacket *pNetPacket) {
            ...
            if (pSOCache) {
                ...
                for (int i = 0; i < msg.Body().objects_size(); ++i) {
                    ...
                    SOCDebug("     type %d\n", objMessage.type_id());
                    pSOCache->BUpdateFromMsg(objMessage.type_id(), objMessage.object_data().data(),
