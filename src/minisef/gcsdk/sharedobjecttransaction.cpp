...
namespace GCSDK {
    ...
    public:
        CTrustedHelper_OutputAndSetErrorState(CSharedObjectTransactionEx &SharedObjectTransaction,
                                              const char *pszFunctionContext, const CSteamID &CacheOwnerSteamID,
                                              const CSharedObject *pObject)
                : m_SharedObjectTransaction(SharedObjectTransaction), m_pszFunctionContext(pszFunctionContext),
                  m_CacheOwnerSteamID(CacheOwnerSteamID), m_pObject(pObject) {
        }

        void operator()(const bool bExpResult, const char *pszExp) const {
            if (!bExpResult) {
                AssertMsg4(bExpResult, "Failed verification: %s (context '%s'; owner '%s'; object '%s')", pszExp,
                           m_pszFunctionContext, m_CacheOwnerSteamID.Render(),
...
#define CSOTVerifyObj(exp_) \
    ...
    // Purpose:
    //-----------------------------------------------------------------------------
    CSharedObjectTransactionEx::CSharedObjectTransactionEx(CGCSharedObjectCache *pLockedSOCache,
    ...
    bool CSharedObjectTransactionEx::BIsValidInternalState() {
        ...
        if (!CSOTVerify(m_pLockedSOCache) || !CSOTVerify(m_pLockedSOCache->GetOwner().IsValid()) ||
    ...
    // Purpose:
    //-----------------------------------------------------------------------------
    bool CSharedObjectTransactionEx::BTrackModifiedObjectInternal(CSharedObject *pObject,
    ...
    // Purpose:
    //-----------------------------------------------------------------------------
    const CSharedObject *CSharedObjectTransactionEx::InternalFindSharedObject(CGCSharedObjectCache *pSOCache,
    ...
    bool CSharedObjectTransactionEx::BYieldingCommit() {
        ...
        // SO cache, but are clones living outside of it) to queue up their updates to the SQL transaction.
        {
            ...
            FOR_EACH_VEC(m_vecObjects_Modified, i) {
                ...
                if (info.m_pWriteableObject->BIsDatabaseBacked()) {
                    ...
                    bGeneratedCommitSQL &= info.m_pWriteableObject->BYieldingAddWriteToTransaction(*m_pSQLAccess,
                                                                                                   vecDirtyFields);
                    bDBRelevantObjectsChanged |= true;
                }
        ...
        if (!bGeneratedCommitSQL) {
            EmitError(SPEW_GC,
                      "Failed to commit CSharedObjectTransaction '%s' for user '%s': failed to add inserts/writes.\n",
        ...
        if (!m_pSQLAccess->BCommitTransaction(!bDBRelevantObjectsChanged)) {
            EmitError(SPEW_GC,
                      "Failed to commit CSharedObjectTransaction '%s' for user '%s': SQL transaction failure.\n",
                      GetInternalTransactionDesc(), m_pLockedSOCache->GetOwner().Render());
        ...
        FOR_EACH_VEC(m_vecObjects_Modified, i) {
            ...
            info.m_pObject->Copy(
                    *info.m_pWriteableObject);                                // stomp the version that's already in the cache with the properties from where we've been writing
            delete info.m_pWriteableObject;
        }

        FOR_EACH_VEC(m_vecObjects_Removed, i) {
            CreateOrDestroyCommitInfo_t &info = m_vecObjects_Removed[i];
            Verify(m_pLockedSOCache->BDestroyObject(*info.m_pObject,
    ...
    CSharedObjectTransaction::FindObjectInVector(const CSharedObject *pObject, CUtlVector<undoinfo_t> &vec) const {
        FOR_EACH_VEC(vec, i) {
    ...
    bool CSharedObjectTransaction::AssertValidInput(const CGCSharedObjectCache *pSOCache, const CSharedObject *pObject,
        ...
        if (pSOCache == NULL || pObject == NULL) {
            SetError(CFmtStr("%s: attempt to manipulate invalid SO cache %s, object %s", pszContext,
        ...
        if (!bSOCachedLocked) {
            SetError(CFmtStr("%s: attempt to manipulate non-locked SO cache %s to add object %s", pszContext,
                             pSOCache->GetOwner().Render(), pObject->GetDebugString().String()));
            return false;
        }
    ...
    void CSharedObjectTransaction::RemoveObject(CGCSharedObjectCache *pSOCache, CSharedObject *pObject) {
        ...
        // make sure the object we're removing is in the cache we're trying to remove it from.
        AssertMsg1(pSOCache->FindSharedObject(*pObject), "Attempting to remove object '%s' from non-owning cache!",
        ...
        if (undoinfo_t * pInfo = FindObjectInVector(pObject, m_vecObjects_Added)) {
            EmitError(SPEW_GC,
                      "Attempting to add and remove the same object from the same CSharedObjectTransaction! Object: %s",
        ...
        if (undoinfo_t * pInfo = FindObjectInVector(pObject, m_vecObjects_Modified)) {
            EmitError(SPEW_GC,
                      "Attempting to modify and remove the same object from the same CSharedObjectTransaction! Object: %s",
                      pInfo->pObject->GetDebugString().Get());
    ...
    void CSharedObjectTransaction::ModifiedObject(CGCSharedObjectCache *pSOCache, CSharedObject *pObject,
                                                  uint32 unFieldIdx) {
        ...
        AssertMsg1(pSOCache->FindSharedObject(*pObject), "Attempting to modify object '%s' in non-owning cache!",
                   pObject->GetDebugString().Get());
    ...
    CSharedObject *
    CSharedObjectTransaction::FindSharedObject(CGCSharedObjectCache *pSOCache, const CSharedObject &soIndex) {
    ...
    bool CSharedObjectTransaction::BYieldingCommit(bool bAllowEmpty) {
        ...
        FOR_EACH_VEC(m_vecObjects_Removed, i) {
            ...
            DbgVerify(info.pSOCache->BDestroyObject(*info.pObject,
                                                    false));            // internally will assert if cache isn't locked
    ...
    void CSharedObjectTransaction::Undo() {
        ...
        FOR_EACH_VEC(m_vecObjects_Removed, i) {
            ...
            AssertMsg1(GGCBase()->IsSteamIDLockedByCurJob(info.pSOCache->GetOwner()),
                       "Attempt to modify in-memory object '%s' during CSharedObjectTransaction removal rollback.",
        ...
        FOR_EACH_VEC(m_vecObjects_Modified, i) {
            undoinfo_t &info = m_vecObjects_Modified[i];
            AssertMsg1(GGCBase()->IsSteamIDLockedByCurJob(info.pSOCache->GetOwner()),
                       "Attempt to modify in-memory object '%s' during CSharedObjectTransaction modify rollback.",
                       info.pObject->GetDebugString().Get());
            info.pObject->Copy(*info.pOriginalCopy);
            delete info.pOriginalCopy;
        }
