...
//-----------------------------------------------------------------------------
    void CJobMgr::SetIsShuttingDown() {
        m_WorkThreadPool.AllowTimeouts(
                true); // during shutdown, we might abort jobs before waiting for the work item to complete
...
// Output:	true if the message was routed to a job
//-----------------------------------------------------------------------------
    bool CJobMgr::BRouteWorkItemCompletedInternal(JobID_t jobID, bool bWorkItemCanceled, bool bShouldExist,
...
//-----------------------------------------------------------------------------
    void CJobMgr::AddDelayedJobToYieldList(CJob &job) {
        ...
        AssertMsg1(job.GetPauseReason() == k_EJobPauseReasonYield,
                   "Delayed job %s was added to yield list but was not in expected yield state\n", job.GetName());
        AddToYieldList(job);
    }
...
//-----------------------------------------------------------------------------
    bool
    CJobMgr::BResumeYieldingJobsFromList(CUtlLinkedList<JobYielding_t, int> &listJobsYielding, uint nCurrentIteration,
...
//-----------------------------------------------------------------------------
    int CJobMgr::DumpJobSummary() {
        ...
        V_qsort_s(vecSort.Base(), vecSort.Count(), sizeof(int), (QSortCompareFuncCtx_t) ProfileSortFunc,
                  &jobprofilestats);
...
//-----------------------------------------------------------------------------
    void CJobMgr::ProfileJobs(EJobProfileAction ejobProfileAction, EJobProfileSortOrder iSortOrder) {
        ...
        if (m_mapOrphanMessages.Count()) {
            EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
                     "Messages that arrived responding to jobs that no longer exists and were dropped\n");
