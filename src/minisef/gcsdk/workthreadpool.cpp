    ...
    int CWorkThread::Run() {
        ...
        while (!m_bExitThread) {
            ...
            while ((pPool->BNeverSetEventOnAdd() && nIterations < nMaxFastIterations) || nIterations == 0) {
                ...
                ++nIterations;
                if (pPool->BNeverSetEventOnAdd() && nIterations < nMaxFastIterations) { VPROF_BUDGET(
                            "CWorkThread -- Sleep", VPROF_BUDGETGROUP_SLEEPING);
                    ThreadSleepEx(2);
                }
            }
    ...
    void CWorkThreadPool::StopWorkThreads() {
        ...
        while (true) {
            ...
            if (!bJoined) {
                ...
                EmitWarning(SPEW_THREADS, 2, "Thread \"%s\" (ID %p) failed to shut down", pWorkThread->GetName(),
                            (void *) pWorkThread->GetThreadID());
            } else {
...
//-----------------------------------------------------------------------------
    bool CWorkThreadPool::AddWorkItem(CWorkItem *pWorkItem) {
        ...
        if (!BNeverSetEventOnAdd() && m_cActiveThreads == 0) { VPROF_BUDGET("SetEvent()",
                                                                            VPROF_BUDGETGROUP_THREADINGMAIN);
            m_EventNewWorkItem.Set();
        }
    ...
    bool CWorkThreadPool::BDispatchCompletedWorkItems(const CLimitTimer &limitTimer, CJobMgr *pJobMgr) {
        ...
        while (pWorkItem != NULL) {
            ...
            if (!pWorkItem->DispatchCompletedWorkItem(pJobMgr)) {
                EmitWarning(SPEW_THREADS, 2,
                            "Work Item for Work Pool %s completed but job no longer existed to notify\n",
