...
//function called when a message is tallied but not registered properly. Used to help consolidate behavior for untracked messages
    static void HandleUntrackedMsg(const char *pszMsgOperation, uint32 nMsgID) {
        ...
        EG_VERBOSE(g_EGMessages,
                   "Found %s message with ID %d that was not properly registered. Unable to tally information\n",
...
// Tally the receiving of a message
//-----------------------------------------------------------------------------
    void CMessageList::TallyMultiplexedMessage(MsgType_t eMsgType, uint32 uSent, uint32 cRecipients, uint32 uMsgSize,
                                               uint32 nSourceMask) {
        short nIndex = GetMessageIndex(eMsgType);
        if (nIndex == -1) {
        ...
        TallyMessageInternal(msgInfo, MsgInfo_t::k_EStatsTypeMultiplexedSendsRaw, uMsgSize * cRecipients, nSourceMask,
                             cRecipients);
    }
...
// Tally the receiving of a message
//-----------------------------------------------------------------------------
    void CMessageList::TallyMessageInternal(MsgInfo_t &msgInfo, MsgInfo_t::EStatsType eBucket, uint32 unMsgSize,
                                            uint32 nSourceMask, uint32 cMessages) {
...
//-----------------------------------------------------------------------------
    void CMessageList::PrintStats(bool bShowAll, bool bSortByFrequency, MsgInfo_t::EStatsGroup eGroup,
                                  MsgInfo_t::EStatsType eType, uint32 nSourceMask) const {
        ...
        EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS, "%s",
                 "EMsg  MessageName                                         Count      %          Bytes      %     MsgPS    Avg/Msg       KBPS\n");
        EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS, "%s",
                 "----- ---------------------------------------------- ---------- ------ -------------- ------ --------- ---------- ----------\n");

        for (uint16 iValue = mapValueToMsg.LastInorder();
             iValue != mapValueToMsg.InvalidIndex(); iValue = mapValueToMsg.PrevInorder(iValue)) {
        ...
        EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
                 "----- ---------------------------------------------- ---------- ------ -------------- ------ --------- ---------- ----------\n");
        EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
                 "Totals                                               %10u 100.0%% %14llu 100.0%% %9.1f %10u %10.2f\n",
                 unTotalMessages,
                 unTotalBytes,
                 fMsgPS,
                 uAvgMsgTotal,
...
//-----------------------------------------------------------------------------
    void
    CMessageList::PrintMultiplexStats(MsgInfo_t::EStatsGroup eGroup, bool bSortByFrequency, uint32 nSourceMask) const {
        // Figure out which time value we should use for rate calcs
        uint64 ulMicroseconds = GetGroupDuration(eGroup);
        ...
        EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS, "%s",
                 "EMsg  MessageName                                         Count      %             KB      %    Avg/Msg       KBPS  Msg Saved      %       KB Saved      %\n");
        EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS, "%s",
                 "----- ---------------------------------------------- ---------- ------ -------------- ------ ---------- ---------- ---------- ------ -------------- ------\n");
        ...
        for (uint16 iValue = mapValueToMsg.LastInorder();
            ...
            EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
                     "%5u %-46s %10u %5.1f%% %14llu %5.1f%% %10u %10.2f %10d %5.1f%% %14lld %5.1f%%\n",
                     msgInfo.eMsg,
                     msgInfo.pchMsgName,
                     nCount,
                     flCountPerc,
        ...
        EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
                 "----- ---------------------------------------------- ---------- ------ -------------- ------ ---------- ---------- ---------- ------ -------------- ------\n");
        EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
                 "Totals                                               %10u %5.1f%% %14llu %5.1f%% %10u %10.2f %10d %5.1f%% %14lld %5.1f%%\n",
                 unTotalMessages,
                 flTotalMessagesPct,
                 unTotalBytes / 1024,
                 flTotalBytesPct,
