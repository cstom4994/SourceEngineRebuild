...
// Purpose: Constructor
//-----------------------------------------------------------------------------
    CProtoBufNetPacket::CProtoBufNetPacket(CNetPacket *pNetPacket, GCProtoBufMsgSrc eMsgSrc, const CSteamID steamID,
        ...
        if (unLenProtoBufHeader + sizeof(ProtoBufMsgHeader_t) <= pNetPacket->CubData()) {
            if (!m_pHeader->ParseFromArray(pNetPacket->PubData() + sizeof(ProtoBufMsgHeader_t), unLenProtoBufHeader)) {
                AssertMsg4(false, "Failed parsing ProtoBuf extended header out of message: %s(%d) from %s size=%u",
                           PchMsgNameFromEMsg(GetEMsg()), GetEMsg(), steamID.Render(), pNetPacket->CubData());
            } else {
            ...
            AssertMsg4(false, "Unparseable protobuf message arrived: message %s(%u) from %s size=%u",
                       PchMsgNameFromEMsg(GetEMsg()), GetEMsg(), steamID.Render(), pNetPacket->CubData());
        }
    }
...
//-----------------------------------------------------------------------------
    CProtoBufMsgMemoryPoolBase::~CProtoBufMsgMemoryPoolBase() {
        AssertMsg(0 == m_pTSQueueFreeObjects->Count(),
                  "CProtoBufMsgMemoryPoolBase destructor called with items still in the queue. They must be freed by the derived class or they will be leaked");
...
//-----------------------------------------------------------------------------
    void CProtoBufMsgMemoryPoolMgr::DumpPoolInfo() {
        ...
        FOR_EACH_VEC(m_vecMsgPools, i) {
            ...
            Msg("%43s%12d%12d%12s%12s\n", m_vecMsgPools[i]->GetName().String(), m_vecMsgPools[i]->GetAllocated(),
                m_vecMsgPools[i]->GetFree(), Q_pretifymem((float) unEstimate, 2, true),
...
//-----------------------------------------------------------------------------
    bool CProtoBufMsgBase::InitFromPacket(IMsgNetPacket *pNetPacket) {
        ...
        AssertMsg(NULL == m_pProtoBufHdr,
                  "Encountered a packet being initialized that has outstanding memory. This will leak memory, and is often caused by receiving messages using the wrong constructor which preallocates or not clearing the message between uses");
...
//-----------------------------------------------------------------------------
    bool
    CProtoBufMsgBase::BAsyncSendProto(IProtoBufSendHandler &sender, MsgType_t eMsgType, const CMsgProtoBufHeader &hdr,
...
// Purpose: Sends a message using the given handler 
//-----------------------------------------------------------------------------
    bool CProtoBufMsgBase::BAsyncSendWithPreSerializedBody(IProtoBufSendHandler &pSender, const byte *pubBody,
...
//-----------------------------------------------------------------------------
    bool CProtoBufMsgBase::BAsyncSendWithPreSerializedBody(IProtoBufSendHandler &sender, MsgType_t eMsgType,
...
//-----------------------------------------------------------------------------
    uint8 *
    CProtoBufMsgBase::AllocateMessageMemory(MsgType_t eMsgType, const CMsgProtoBufHeader &hdr, uint32 cubBodySize,
