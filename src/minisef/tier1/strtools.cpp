...
char *V_strncat(char *pDest, const char *pSrc, size_t destBufferSize, int max_chars_to_copy) {
    ...
    ANALYZE_SUPPRESS(
            6059); // warning C6059: : Incorrect length parameter in call to 'strncat'. Pass the number of remaining characters, not the buffer size of 'argument 1'
                ...
                if (iInput + 1 < nInSize && pIn[iInput + 1] == '#') {
                    ...
                    for (int i = 0; pFullEntities[i].uCharCode != 0; ++i) {
                        if (nInSize - iInput - 1 >= pFullEntities[i].nEntityLength) {
                            if (Q_memcmp(pIn + iInput, pFullEntities[i].pchEntity, pFullEntities[i].nEntityLength) ==
                                0) {
...
//-----------------------------------------------------------------------------
void
V_StripAndPreserveHTMLCore(CUtlBuffer *pbuffer, const char *pchHTML, const char **rgszPreserveTags, uint cPreserveTags,
                           const char **rgszNoCloseTags, uint cNoCloseTags, uint cMaxResultSize) {
    uint cHTMLCur = 0;
