...
void CFavoriteGames::OnRemoveFromFavorites() {
    ...
    for (int iGame = 0; iGame < m_pGameList->GetSelectedItemsCount(); iGame++) {
        ...
        if (pServer) {
            steamapicontext->SteamMatchmaking()->RemoveFavoriteGame(pServer->m_nAppID, pServer->m_NetAdr.GetIP(),
                                                                    pServer->m_NetAdr.GetConnectionPort(),
                                                                    pServer->m_NetAdr.GetQueryPort(),
...
void CFavoriteGames::OnAddCurrentServer() {
    ...
    if (pConnected && steamapicontext->SteamMatchmaking()) {
        steamapicontext->SteamMatchmaking()->AddFavoriteGame(pConnected->m_nAppID, pConnected->m_NetAdr.GetIP(),
                                                             pConnected->m_NetAdr.GetConnectionPort(),
                                                             pConnected->m_NetAdr.GetQueryPort(),
