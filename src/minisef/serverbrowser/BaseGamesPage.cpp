...
void CBaseGamesPage::PrepareQuickListMap(const char *pMapName, int iListID) {
    ...
    if (m_quicklistserverlist.IsValidIndex(iIndex) == false) {
        ...
        const char *pszFriendlyGameTypeName = ServerBrowser().GetMapFriendlyNameAndGameType(szMapName, szFriendlyName,
    ...
    for (int i = 0; i < m_pGameFilter->GetItemCount(); i++) {
        ...
        if ((gameID == m_iLimitToAppID || m_iLimitToAppID.AppID() == 0) && (!m_szGameFilter[0] ||
                                                                            (pchGameDir && pchGameDir[0] &&
                                                                             !Q_strncmp(pchGameDir, m_szGameFilter,
                                                                                        Q_strlen(pchGameDir))))) {
...
void CBaseGamesPage::ApplyGameFilters() {
    ...
    FOR_EACH_MAP_FAST(m_mapServers, i) {
            ...
            gameserveritem_t *pServer = steamapicontext->SteamMatchmakingServers()->GetServerDetails(m_hRequest,
                                                                                                     server.m_iServerID);
            if (!pServer)
                continue;
...
void CBaseGamesPage::StartRefresh() {
    ...
    switch (m_eMatchMakingType) {
        case eFavoritesServer:
            ...
            break;
        case eHistoryServer:
            m_hRequest = steamapicontext->SteamMatchmakingServers()->RequestHistoryServerList(GetFilterAppID().AppID(),
                                                                                              &pFilters, nFilters,
                                                                                              this);
            break;
        case eInternetServer:
            m_hRequest = steamapicontext->SteamMatchmakingServers()->RequestInternetServerList(GetFilterAppID().AppID(),
                                                                                               &pFilters, nFilters,
                                                                                               this);
            break;
        case eSpectatorServer:
            ...
            break;
        case eFriendsServer:
            m_hRequest = steamapicontext->SteamMatchmakingServers()->RequestFriendsServerList(GetFilterAppID().AppID(),
                                                                                              &pFilters, nFilters,
                                                                                              this);
            break;
        case eLANServer:
            m_hRequest = steamapicontext->SteamMatchmakingServers()->RequestLANServerList(GetFilterAppID().AppID(),
                                                                                          this);
            break;
        default:
...
//-----------------------------------------------------------------------------
CDialogServerWarning::CDialogServerWarning(vgui::Panel *parent, IGameList *gameList, int serverID) : Frame(parent,
                                                                                                           "DialogServerWarning") {
    ...
    if (sb_dontshow_maxplayer_warning.IsValid()) {
        ...
        if (iMaxP && pKV && !sb_dontshow_maxplayer_warning.GetBool()) {
            ...
            if (iMaxCount > iMaxP) {
                ...
                g_pVGuiLocalize->ConstructString(wszWarning, sizeof(wszWarning),
                                                 g_pVGuiLocalize->Find("#ServerBrowser_ServerWarning_MaxPlayers"), 4,
                                                 wszServerMaxPlayers, wszGameName, wszDesignedMaxPlayers,
                                                 wszDesignedMaxPlayers);
