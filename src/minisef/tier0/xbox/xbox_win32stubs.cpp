...
LONG_PTR GetWindowLongPtr(HWND
hWnd,
int nIndex
)
{
...
return
GetWindowLong( hWnd, idx
);
}

LONG_PTR GetWindowLongPtrW(HWND
hWnd,
int nIndex
)
{
...
case GWL_STYLE:
oldLong = ((xWnd_t *) hWnd)->windowLongs[nIndex];
((xWnd_t*)hWnd)->windowLongs[nIndex] =
dwNewLong;
break;
default:
// not implemented
...
LONG_PTR SetWindowLongPtr(HWND
hWnd,
int nIndex, LONG_PTR
dwNewLong)
{
...
return
SetWindowLong( hWnd, idx, dwNewLong
);
}

LONG_PTR SetWindowLongPtrW(HWND
hWnd,
int nIndex, LONG_PTR
dwNewLong)
{
...
pWnd->
pWndClass = pWndClass;
pWnd->windowLongs[GWL_WNDPROC] = (LONG)pWndClass->
wndProc;
pWnd->windowLongs[GWL_STYLE] =
dwStyle;
pWnd->
x = x;
pWnd->
...
pWnd, WM_CREATE, 0, 0 );
SendMessage((HWND)
pWnd, WM_ACTIVATEAPP, TRUE, 0 );

return (HWND)
pWnd;
}

HWND CreateWindowEx(DWORD
dwExStyle,
...
memset(pWndClass,
0, sizeof(xWndClass_t));
pWndClass->
pClassName = new char[strlen(lpwcx->lpszClassName) + 1];
strcpy(pWndClass
->pClassName, lpwcx->lpszClassName);
pWndClass->
wndProc = lpwcx->lpfnWndProc;
...
0, sizeof(xWndClass_t));
pWndClass->
pClassName = new char[strlen(lpwc->lpszClassName) + 1];
strcpy(pWndClass
->pClassName, lpwc->lpszClassName);
pWndClass->
wndProc = lpwc->lpfnWndProc;
...
DWORD XBX_GetCurrentDirectory(DWORD
nBufferLength,
LPTSTR lpBuffer
)
{
...
DWORD XBX_GetModuleFileName(HMODULE
hModule,
