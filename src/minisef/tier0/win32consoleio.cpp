...
bool SetupWin32ConsoleIO() {
    ...
    if (GetFileType(GetStdHandle(STD_OUTPUT_HANDLE)) == FILE_TYPE_UNKNOWN) {
        ...
        pAttachConsole_t pAttachConsole((BOOL(_stdcall * )(DWORD))
        GetProcAddress(hInst, "AttachConsole"));
        ...
        *stdout = *_fdopen(_open_osfhandle(reinterpret_cast< intptr_t >( GetStdHandle(STD_OUTPUT_HANDLE)), _O_TEXT),
                           "w");
        ...
        *stderr = *_fdopen(_open_osfhandle(reinterpret_cast< intptr_t >( GetStdHandle(STD_ERROR_HANDLE)), _O_TEXT),
                           "w");
        setvbuf(stdout, NULL, _IONBF, 0);
