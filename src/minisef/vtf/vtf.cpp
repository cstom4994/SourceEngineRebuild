...
bool CVTFTexture::LoadImageData(CUtlBuffer &buf, const VTFFileHeader_t &header, int nSkipMipLevels) {
    ...
    TextureStreamSettings_t const *pStreamSettings = (TextureStreamSettings_t const *) GetResourceData(
            VTF_RSRC_TEXTURE_STREAM_SETTINGS, NULL);
    if (pStreamSettings) {
...
bool CVTFTexture::UnserializeEx(CUtlBuffer &buf, bool bHeaderOnly, int nForceFlags, int nSkipMipLevels) {
    ...
    if ((Q_log2(header.width) + Q_log2(header.height) + Q_log2(header.depth) + Q_log2(header.numFrames) >
         cMaxImageSizeLog2) || (header.numResources > MAX_RSRC_DICTIONARY_ENTRIES)) {
        STAGING_ONLY_EXEC(
                DevWarning("Asked for a large texture to be created (%d h x %d w x %d d x %d f). Nope.\n", header.width,
                           header.height, header.depth, header.numFrames));
        return false;
    }
    ...
    if (int(header.numResources) < m_arrResourcesData.Count()) {
        m_arrResourcesData_ForReuse.EnsureCapacity(
                m_arrResourcesData_ForReuse.Count() + m_arrResourcesData.Count() - header.numResources);
...
//	}
    ...
    if (TextureLODControlSettings_t const *pLodSettings = (TextureLODControlSettings_t const *) GetResourceData(
    ...
    for (int iMipLevel = 1; iMipLevel < m_nMipCount; ++iMipLevel) {
        ...
        for (int ch = 0; ch < 4; ++ch) {
            ...
            if (!(((iMipLevel == m_nMipCount - 1) || (iMipLevel > iLastNonDecayMip)) &&
                  // last 1x1 mip  or  past clamped and skipped
...
bool CVTFTexture::ConstructLowResImage() {
    ...
    return ImageLoader::ConvertImageFormat(tmpImage, IMAGE_FORMAT_RGBA8888,
                                           m_pLowResImageData, m_LowResImageFormat, m_nLowResImageWidth,
                                           m_nLowResImageHeight);
}
    ...
    if (m_Format == IMAGE_FORMAT_DXT1 || m_Format == IMAGE_FORMAT_DXT5) {
        if (iFace1Cur != incs.iFace1End) {
            while (iFace1Cur != incs.iFace1End) {
                ...
                S3PaletteIndex paletteIndex = S3TC_GetPaletteIndex(pFace1Data, m_Format, nMipWidth, iFace1Cur.x,
                                                                   iFace1Cur.y);
...
void CVTFTexture::BlendCubeMapFaceCorners(
    ...
    if (m_Format == IMAGE_FORMAT_DXT1 || m_Format == IMAGE_FORMAT_DXT5) {
        ...
        S3PaletteIndex paletteIndex = S3TC_GetPaletteIndex(pImageData[0], m_Format, nMipWidth, texelPos[0].x,
                                                           texelPos[0].y);
...
void CVTFTexture::BlendCubeMapEdgePalettes(
    ...
    unsigned char *pFace1Data = ImageData(iFrame, pMatch->m_iFaces[0], iMipLevel);
    unsigned char *pFace2Data = ImageData(iFrame, pMatch->m_iFaces[1], iMipLevel);
    S3RGBA *pFace1Original = &m_OriginalData[
            GetImageOffset(iFrame, pMatch->m_iFaces[0], iMipLevel, IMAGE_FORMAT_RGBA8888) / 4];
    S3RGBA *pFace2Original = &m_OriginalData[
            GetImageOffset(iFrame, pMatch->m_iFaces[1], iMipLevel, IMAGE_FORMAT_RGBA8888) / 4];
...
void CVTFTexture::BlendCubeMapCornerPalettes(
    ...
    for (int iEdge = 0; iEdge < 3; iEdge++) {
        ...
        S3RGBA *pFaceOriginal = &m_OriginalData[GetImageOffset(iFrame, iFaceIndex, iMipLevel, IMAGE_FORMAT_RGBA8888) /
                                                4];
