...
//-----------------------------------------------------------------------------
ImageFormat
PreferredFormat(IVTFTexture *pVTFTexture, ImageFormat fmt, int width, int height, int mipCount, int faceCount) {
    switch (fmt) {
...
bool ComputeTargetDimensions(const char *pDebugName, IVTFTexture *pVTFTexture, int picmip, int &width, int &height,
    ...
    int nClampX = 1 << 30;
    int nClampY = 1 << 30;
    TextureLODControlSettings_t const *pLODInfo = reinterpret_cast<TextureLODControlSettings_t const *> ( pVTFTexture->GetResourceData(
    ...
    if (bSrgbGammaConvert == true) {
        if (targetFormat == IMAGE_FORMAT_BGRA8888 || targetFormat == IMAGE_FORMAT_BGRX8888) {
            ...
            for (int i = 0; i < (targetImageSize / nStrideBytes); i++) // For each color or color/alpha block
            {
                ...
                n565Color0 |=
                        ((unsigned short) (unsigned char) (pTargetImage[(i * nStrideBytes) + nOffsetBytes + 0])) << 8;
                n565Color0 |= ((unsigned short) (unsigned char) (pTargetImage[(i * nStrideBytes) + nOffsetBytes + 1]));

                unsigned short n565Color1 = 0;
                n565Color1 |=
                        ((unsigned short) (unsigned char) (pTargetImage[(i * nStrideBytes) + nOffsetBytes + 2])) << 8;
                ...
                unsigned short n565Color0New =
                        ((unsigned short) v888Color0New[0] << 8) | ((unsigned short) v888Color0New[1] << 3) |
                        ((unsigned short) v888Color0New[2] >> 3);

                v888Color1New[0] &= 0xf8; // 5 bits
                v888Color1New[1] &= 0xfc; // 6 bits
                v888Color1New[2] &= 0xf8; // 5 bits
                unsigned short n565Color1New =
                        ((unsigned short) v888Color1New[0] << 8) | ((unsigned short) v888Color1New[1] << 3) |
...
bool ConvertVTFTo360Format(const char *pDebugName, CUtlBuffer &sourceBuf, CUtlBuffer &targetBuf,
    ...
    targetFormat = PreferredFormat(pSourceVTF, pSourceVTF->Format(), targetWidth, targetHeight, targetMipCount,
                                   targetFaceCount);
    ...
    if (targetLowResWidth && targetLowResHeight) {
        ...
        int targetLowResImageSize = ImageLoader::GetMemRequired(targetLowResWidth, targetLowResHeight, 1,
