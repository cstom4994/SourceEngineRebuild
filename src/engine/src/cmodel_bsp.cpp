...
IPhysicsCollision* _physcollision = NULL;
...
void CollisionBSPData_Destroy(CCollisionBSPData* pBSPData)
{
    ...
    {
        _physcollision->VCollideUnload(&pBSPData->map_cmodels[i].vcollisionData);
    }
...
void CollisionBSPData_LinkPhysics(void)
{
    ...
    if (!physprop)
    {
        ...
        _physcollision = (IPhysicsCollision*)g_AppSystemFactory(VPHYSICS_COLLISION_INTERFACE_VERSION, NULL);

        if (!physprop || !_physcollision)
        {
...
void CollisionBSPData_LoadPhysics(CCollisionBSPData* pBSPData)
{
    ...
    do
    {
        ...
        if (physModel.dataSize > 0)
        {
            ...
            _physcollision->VCollideLoad(&pModel->vcollisionData, physModel.solidCount, (const char*)ptr, physModel.dataSize + physModel.keydataSize);
