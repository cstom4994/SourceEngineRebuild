...
static ConVar r_fastzrejectdisp("r_fastzrejectdisp", "0", 0,
...
void Shader_SetChainTextureState(IMatRenderContext *pRenderContext, SurfaceHandle_t surfID, IClientEntity *pBaseEntity,
    ...
    if (bShadowDepth) {
        ...
        if (nAlphaTest == 1) {
            ...
            IMaterialVar *pOriginalTextureFrameVar = pDrawMaterial->FindVarFast("$frame",
                                                                                &originalTextureFrameVarCache);
            static unsigned int originalAlphaRefCache = 0;
            IMaterialVar *pOriginalAlphaRefVar = pDrawMaterial->FindVarFast("$AlphaTestReference",
                                                                            &originalAlphaRefCache);
...
void Shader_DrawChainsStatic(const CMSurfaceSortList &sortList, int nSortGroup, bool bShadowDepth) {
    ...
    while (listIndex < count) {
        ...
        for (int m = 0; m < meshTotal; m++) {
            ...
            IMaterial *pBindMaterial = materialSortInfoArray[MSurf_MaterialSortID(
                    batchList[mesh.firstbatch].surfID)].material;
#ifdef NEWMESH
...
//			IMesh *pMesh = pRenderContext->GetDynamicMesh( false, mesh.pMesh, pBuildMesh, pBindMaterial );
            ...
            for (int b = 0; b < mesh.numbatches; b++) {
                ...
                if (bShadowDepth) {
                    ...
                    if (nAlphaTest == 1) {
                        static unsigned int originalTextureVarCache = 0;
                        IMaterialVar *pOriginalTextureVar = pDrawMaterial->FindVarFast("$basetexture",
                                                                                       &originalTextureVarCache);
                        static unsigned int originalTextureFrameVarCache = 0;
                        IMaterialVar *pOriginalTextureFrameVar = pDrawMaterial->FindVarFast("$frame",
                                                                                            &originalTextureFrameVarCache);
                        static unsigned int originalAlphaRefCache = 0;
                        ...
                        IMaterialVar *pTextureVar = pDepthWriteMaterial->FindVarFast("$basetexture", &textureVarCache);
                        static unsigned int textureFrameVarCache = 0;
                        IMaterialVar *pTextureFrameVar = pDepthWriteMaterial->FindVarFast("$frame",
                                                                                          &textureFrameVarCache);
                        static unsigned int alphaRefCache = 0;
                        IMaterialVar *pAlphaRefVar = pDepthWriteMaterial->FindVarFast("$AlphaTestReference",
                                                                                      &alphaRefCache);
...
static void Shader_DrawChainNormals(const CUtlVector<msurface2_t *> &surfaceList) {
    ...
    for (int i = 0; i < surfaceList.Count(); i++) {
        ...
        for (vertID = 0; vertID < MSurf_VertCount(surfID); ++vertID) {
            ...
            Vector &norm = pBrushData->vertnormals[pBrushData->vertnormalindices[MSurf_FirstVertNormal(surfID) +
                                                                                 vertID]];
                ...
                MSL_FOREACH_SURFACE_IN_GROUP_BEGIN(sortList, group, surfID)
                            {
                                ...
                                if (OVERLAY_FRAGMENT_INVALID != MSurf_OverlayFragmentList(surfID)) {
                                    OverlayMgr()->AddFragmentListToRenderList(nSortGroup,
...
//-----------------------------------------------------------------------------
void
Shader_DrawDispChain(int nSortGroup, const CMSurfaceSortList &list, unsigned long flags, ERenderDepthMode DepthMode) {
                        ...
                        if (pPrim->vertCount == 0) {
                            ...
                            for (int primIndex = 0; primIndex < pPrim->indexCount; primIndex++) {
                                meshBuilder.FastIndex(
                                        host_state.worldbrush->primindices[pPrim->firstIndex + primIndex] + nStartVert);
                            }
                        }
                    } else {
...
void
Shader_DrawTranslucentSurfaces(IWorldRenderList *pRenderListIn, int sortIndex, unsigned long flags, bool bShadowDepth) {
...
JobStatus_t CBuildWorldListsJob::DoExecute() {
    if (!m_bDrawTopView) {
        if (m_bShadowDepth) {
            R_RecursiveWorldNodeNoCull(m_pRenderList, host_state.worldbrush->nodes,
...
void R_BuildWorldLists(IWorldRenderList *pRenderListIn, WorldListInfo_t *pInfo,
                       int iForceViewLeaf, const VisOverrideData_t *pVisData, bool bShadowDepth /* = false */,
...
bool
Shader_DrawBrushSurfaceOverride(IMatRenderContext *pRenderContext, SurfaceHandle_t surfID, IClientEntity *baseentity) {
...
public:
...
#else
                IMesh *pBuildMesh = pRenderContext->GetDynamicMesh(false, g_WorldStaticMeshes[batch.sortID], NULL,
                                                                   NULL);
    ...
    void
    DrawOpaqueBrushModel(IClientEntity *baseentity, model_t *model, const Vector &origin, ERenderDepthMode DepthMode);
...
void CBrushBatchRender::DrawOpaqueBrushModel(IClientEntity *baseentity, model_t *model, const Vector &origin,
    ...
    for (i = 0; i < pRender->planeCount; i++) {
        ...
        backface[i] = (DepthMode == DEPTH_MODE_NORMAL && (dot < BACKFACE_EPSILON)) ? true
    ...
    for (i = 0; i < pRender->meshCount; i++) {
        ...
        for (int j = 0; j < mesh.batchCount; j++) {
            ...
            if (DepthMode != DEPTH_MODE_NORMAL) {
                ...
                if (nAlphaTest == 1) {
                    static unsigned int originalTextureVarCache = 0;
                    IMaterialVar *pOriginalTextureVar = batch.pMaterial->FindVarFast("$basetexture",
                                                                                     &originalTextureVarCache);
                    static unsigned int originalTextureFrameVarCache = 0;
                    IMaterialVar *pOriginalTextureFrameVar = batch.pMaterial->FindVarFast("$frame",
                                                                                          &originalTextureFrameVarCache);
                    static unsigned int originalAlphaRefCache = 0;
                    IMaterialVar *pOriginalAlphaRefVar = batch.pMaterial->FindVarFast("$AlphaTestReference",
                                                                                      &originalAlphaRefCache);
...
void R_DrawBrushModel(IClientEntity *baseentity, model_t *model,
                      const Vector &origin, const QAngle &angles, ERenderDepthMode DepthMode, bool bDrawOpaque,
...
static int
ListLeafsInBox(mnode_t *RESTRICT node, const Vector &center, const Vector &extents, unsigned short *RESTRICT pList,
               int listMax) {
    int leafCount = 0;
    const int NODELIST_MAX = 1024;
    mnode_t *nodeList[NODELIST_MAX];
...
static ConVar opt_ListLeavesFastAlgorithm("opt_ListLeavesFastAlgorithm", "1", FCVAR_NONE,
                                          "Use the new SIMD version of CEngineBspTree::ListLeavesInBox.");
