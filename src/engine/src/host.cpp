...
void Snd_Restart_f() {
    ...
    ConCommand *pCommand = (ConCommand *) dynamic_cast< const ConCommand * >( g_pCVar->FindCommand(
            "cl_soundscape_flush"));
    if (pCommand) {
...
ConVar threadpool_affinity("threadpool_affinity", (IsX360()) ? "1" : "0", 0, "Enable setting affinity", 0, 0, 0, 0,
...
ConVar host_timer_spin_ms("host_timer_spin_ms", IsWindows() ? "1.5" : "0", FCVAR_NONE,
...
ConVar skill("skill", "1", FCVAR_ARCHIVE | FCVAR_ARCHIVE_XBOX, "Game skill level (1-3).", true, 1, true,
             3);            // 1 - 3
...
void SetupNewBindings() {
    ...
    FOR_EACH_TRUE_SUBKEY(pNewBindingsData, pSubKey) {
        ...
        for (int nCurButton = (int) KEY_0; nCurButton <= (int) KEY_LAST; ++nCurButton, ++nNumAttempts) {
            ...
            for (int iKeyIndex = 0;
                 iKeyIndex < sizeof(nInvalidKeyBindings) / sizeof(nInvalidKeyBindings[0]); iKeyIndex++) {
...
int SteamGameServer_GetIPCCallCount() { return 0; }
    ...
    if (sv.IsDedicated() || bIsServerOnly) {
        ...
        if (Host_IsSecureServerAllowed()) {
            if (Steam3Client().SteamUtils()) {
                ...
                if (pPathID) {
                    char szFullPath[512];
                    const char *pFullPath = g_pFileSystem->RelativePathToFullPath(szDllname, pPathID, szFullPath,
                                                                                  sizeof(szFullPath));
                    if (!pFullPath) {
...
bool Host_Changelevel(bool loadfromsavedgame, const char *mapname, const char *start) {
    ...
    if (bTransitionBySave) {
        ...
        saverestore->SetMostRecentElapsedMinutes(
                (int) (totalSeconds / 60.0f) + saverestore->GetMostRecentElapsedMinutes());
