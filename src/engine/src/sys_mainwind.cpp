...
private:
    ...
    void PlayVideoAndWait(const char *filename,
                          bool bNeedHealthWarning = false); // plays a video file and waits till it's done to return. Can be interrupted by user.
...
void CGame::PlayVideoAndWait(const char *filename, bool bNeedHealthWarning) {
    ...
    VideoResult_t status = g_pVideo->PlayVideoFileFullScreen(filename, "GAME", GetMainWindowPlatformSpecificHandle(),
                                                             m_width, m_height, m_iDesktopWidth, m_iDesktopHeight,
                                                             videomode->IsWindowedMode(),
                                                             forcedMinTime,
