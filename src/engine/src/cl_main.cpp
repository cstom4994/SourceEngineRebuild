...
#define SOUND_DMA_SPEED        44100        // hardware playback rate
        ...
        if (!cl_simulate_no_quicktime.GetBool() && g_pVideo && videoSystem != VideoSystem::NONE) {
            ...
            if (g_pVideoRecorder != NULL) {
                ...
                } else if (IsIntegralValue(
                        flFrameRate * 1001.0f / 1000.0f)) // 1001 is the ntsc divisor (30*1000/1001 = 29.97, etc)
                {
                ...
                if (cl_movieinfo.DoVideoSound()) {
                    g_pVideoRecorder->SetMovieSourceAudioParameters(AudioEncodeSourceFormat::AUDIO_16BIT_PCMStereo,
                                                                    SOUND_DMA_SPEED,
...
void CL_SetSteamCrashComment() {
    ...
    Q_snprintf(full, sizeof(full), "%sPP PAGES: used: %d, free %d\n", g_minidumpinfo,
               (int) g_pagedpoolinfo.numPagesUsed, (int) g_pagedpoolinfo.numPagesFree);
...
ConVar cl_cloud_settings("cl_cloud_settings", "0", FCVAR_HIDDEN,
...
void
Callback_DynamicModelsChanged(void *object, INetworkStringTable *stringTable, int stringNumber, const char *newString,
                              const void *newData) {
#ifndef SWDS
