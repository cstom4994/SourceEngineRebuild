...
CBaseClient *CBaseServer::CreateFakeClient(const char *name) {
    ...
    if (sv_stressbots.GetBool()) {
        netadr_t adrNull(0,
                         0); // 0.0.0.0:0 signifies a bot. It'll plumb all the way down to winsock calls but it won't make them.
...
void
CBaseServer::WriteTempEntities(CBaseClient *client, CFrameSnapshot *pCurrentSnapshot, CFrameSnapshot *pLastSnapshot,
