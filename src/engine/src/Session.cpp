...
void CSession::UpdateSlots(const CClientInfo *pClient, bool bAddPlayers) {
    ...
    if (bAddPlayers) {
        ...
        m_nPlayerSlots[SLOTS_FILLEDPRIVATE] = max(0, (int) (m_nPlayerSlots[SLOTS_FILLEDPRIVATE] -
                                                            pClient->m_numPrivateSlotsUsed));
        m_nPlayerSlots[SLOTS_FILLEDPUBLIC] = max(0, (int) (m_nPlayerSlots[SLOTS_FILLEDPUBLIC] -
...
uint CSession::GetSessionSlots(unsigned int nSlot) {
    Assert(nSlot < SLOTS_LAST);
