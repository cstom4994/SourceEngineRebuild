...
static ConVar con_filter_enable("con_filter_enable", "0", FCVAR_MATERIAL_SYSTEM_THREAD,
                                "Filters console output based on the setting of con_filter_text. 1 filters completely, 2 displays filtered text brighter than other text. 3 completely disables console output.");
...
static ConVar con_filter_text_out("con_filter_text_out", "", FCVAR_MATERIAL_SYSTEM_THREAD,
...
void CConPanel::PaintBackground() {
    ...
    int wide = GetWide();
    char ver[100];
    Q_snprintf(ver, sizeof(ver), "%s %s (build %d)", SOURCEENGINEREBUILD_NAME, SOURCEENGINEREBUILD_VERSION_TEXT,
               source_build_number());
    wchar_t unicode[200];
