...
ConVar mem_max_heapsize("mem_max_heapsize", "576", FCVAR_INTERNAL_USE,
                        "Maximum amount of memory to dedicate to engine hunk and datacache (in mb)");
ConVar mem_max_heapsize_dedicated("mem_max_heapsize_dedicated", "64", FCVAR_INTERNAL_USE,
...
void Sys_GetRegKeyValueUnderRoot(HKEY rootKey, const char *pszSubKey, const char *pszElement, OUT_Z_CAP(nReturnLength)
                                 char *pszReturnString, int nReturnLength, const char *pszDefaultValue) {
...
void Sys_SetRegKeyValueUnderRoot(HKEY rootKey, const char *pszSubKey, const char *pszElement, const char *pszValue) {
    ...
    if (dwDisposition == REG_CREATED_NEW_KEY) {
        ...
        */
        ...
        {
            // Just Set the Values according to the defaults
            lResult = VCRHook_RegSetValueEx(hKey, pszElement, 0, REG_SZ, (CONST BYTE *) pszValue,
                                            Q_strlen(pszValue) + 1);
        }
    };
