...
static ConVar vprof_dump_spikes("vprof_dump_spikes", "0", 0,
...
static ConVar vprof_dump_oninterval("vprof_dump_oninterval", "0", 0,
...
static ConVar vprof_report_oninterval("vprof_report_oninterval", "0", 0,
                                      "Interval (in seconds) at which vprof will batch up a full report to the console -- more detailed than vprof_dump_oninterval.");
        ...
        if ((s_flIntervalStartTime + flIntervalTime) < flCurrentTime) {
            if (vprof_report_oninterval.GetFloat() != 0.0f) {
                ...
                g_VProfCurrentProfile.OutputReport(
                        VPRT_SUMMARY | VPRT_LIST_BY_TIME | VPRT_LIST_BY_AVG_TIME | VPRT_LIST_BY_TIME_LESS_CHILDREN |
            ...
            if (g_VProfSignalSpike || (Sys_FloatTime() - LastSpikeTime > MAX_SPIKE_REPORT &&
                ...
                g_VProfCurrentProfile.OutputReport(
                        ...
                        ? g_VProfCurrentProfile.BudgetGroupNameToBudgetGroupID(
                                vprof_dump_spikes_budget_group.GetString()) : -1);
#ifdef _XBOX // X360TBD
...
CON_COMMAND(budget_toggle_group, "Turn a budget group on/off") {
    ...
    g_VProfCurrentProfile.HideBudgetGroup(budgetGroup, !(g_VProfCurrentProfile.GetBudgetGroupFlags(budgetGroup) &
                                                         BUDGETFLAG_HIDDEN));
}
