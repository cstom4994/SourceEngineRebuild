...
int
NET_SendToImpl(SOCKET s, const char FAR *buf, int len, const struct sockaddr FAR *to, int tolen, int iGameDataLength) {
    int nSend = 0;
#if defined( _X360 )
                                                                                                                            if ( X360SecureNetwork() )
	{
...
static int NET_SendTo(bool verbose, SOCKET s, const char FAR *buf, int len, const struct sockaddr FAR *to, int tolen,
                      int iGameDataLength) {
    int nSend = 0;
...
static int NET_QueuePacketForSend(CNetChan *chan, bool verbose, SOCKET s, const char FAR *buf, int len,
...
static int
NET_SendLong(INetChannel *chan, int sock, SOCKET s, const char FAR *buf, int len, const struct sockaddr FAR *to,
    ...
    while (nBytesLeft > 0) {
        ...
        if (netchan && (nFragmentsSent >= net_splitrate.GetInt() ||
            ...
            float flMaxSplitpacketDataRateBytesPerSecond = MIN((float) netchan->GetDataRate(),
            ...
            uint32 delay = (int) (1000.0f * ((float) (nPacketNumber * (nMaxRoutableSize + UDP_HEADER_SIZE)) /
                                             flMaxSplitpacketDataRateBytesPerSecond) + 0.5f);
...
int NET_SendPacket(INetChannel *chan, int sock, const netadr_t &to, const unsigned char *data, int length,
    ...
    if (pVoicePayload) { VPROF_BUDGET("NET_SendPacket_CompressVoice", VPROF_BUDGETGROUP_OTHER_NETWORKING);
        unsigned int nCompressedLength = COM_GetIdealDestinationCompressionBufferSize_Snappy(
    ...
    if (bUseCompression) { VPROF_BUDGET("NET_SendPacket_Compress", VPROF_BUDGETGROUP_OTHER_NETWORKING);
...
static bool
OpenSocketInternal(int nModule, int nSetPort, int nDefaultPort, const char *pName, int nProtocol, bool bTryAny,
