...
static ConVar mod_offline_hdr_switch("mod_offline_hdr_switch", "1", FCVAR_INTERNAL_USE,
                                     ...
                                     "pathways.");
static ConVar mod_touchalldata("mod_touchalldata", AsyncMdlCache() ? "0" : "1", 0,
...
struct dgamelump_internal_t {
    dgamelump_internal_t(dgamelump_t &other, unsigned int nCompressedSize)
            : id(other.id), flags(other.flags), version(other.version), offset(Max(other.fileofs, 0)),
              uncompressedSize(Max(other.filelen, 0)), compressedSize(nCompressedSize) {}
...
public:
    CModelLoader() : m_ModelPool(sizeof(model_t), MAX_KNOWN_MODELS, CUtlMemoryPool::GROW_FAST,
...
private:
    ...
    public:
        enum {
            QUEUED = 0x01, LOADING = 0x02, CLIENTREADY = 0x04, SERVERLOADING = 0x08, ALLREADY = 0x10, INVALIDFLAG = 0x20
        }; // flags
        CDynamicModelInfo() : m_iRefCount(0), m_iClientRefCount(0), m_nLoadFlags(INVALIDFLAG),
...
void CMapLoadHelper::Init(model_t *pMapModel, const char *loadname) {
    ...
    s_MapFileHandle = g_pFileSystem->OpenEx(s_szMapName, "rb", IsX360() ? FSOPEN_NEVERINPACK : 0,
                                            IsX360() ? "GAME" : NULL);
...
void Mod_LoadWorldlights(CMapLoadHelper &lh, bool bIsHDR) {
    ...
    lh.GetMap()->worldlights = (dworldlight_t *) Hunk_AllocName(lh.LumpSize(),
    ...
    if (r_lightcache_zbuffercache.GetInt()) {
        ...
        if (Hunk_Available() >= (int) zbufSize) {
            lh.GetMap()->shadowzbuffers = (lightzbuffer_t *) Hunk_AllocName(zbufSize, va("%s [%s]", lh.GetLoadName(),
                                                                                         "shadowzbuffers"));
...
void Mod_LoadVertNormalIndices(void) {
    ...
    unsigned short *out = (unsigned short *) Hunk_AllocName(lh.LumpSize(),
                                                            va("%s [%s]", lh.GetLoadName(), "vertnormalindices"));
...
void Mod_LoadFaces(void) {
    ...
    msurfacelighting_t *pLighting = Hunk_AllocNameAlignedClear<msurfacelighting_t>(count, 32,
                                                                                   va("%s [%s]", lh.GetLoadName(),
                                                                                      "surfacelighting"));

    lh.GetMap()->surfaces1 = out1;
    lh.GetMap()->surfaces2 = out2;
    lh.GetMap()->surfacelighting = pLighting;
    lh.GetMap()->surfacenormals = Hunk_AllocNameAlignedClear<msurfacenormal_t>(count, 2, va("%s [%s]", lh.GetLoadName(),
                                                                                            "surfacenormal"));
...
void Mod_LoadLeafs_Version_0(CMapLoadHelper &lh) {
    ...
    // one sample per leaf
    lh.GetMap()->m_pLeafAmbient = (mleafambientindex_t *) Hunk_AllocName(count * sizeof(*lh.GetMap()->m_pLeafAmbient),
                                                                         "LeafAmbient");
...
//-----------------------------------------------------------------------------
void
Mod_LoadLeafs_Version_1(CMapLoadHelper &lh, CMapLoadHelper &ambientLightingLump, CMapLoadHelper &ambientLightingTable) {
    Vector mins(0, 0, 0), maxs(0, 0, 0);
    dleaf_t *in;
    mleaf_t *out;
    int i, j, count, p;
    ...
    if (ambientLightingLump.LumpVersion() != LUMP_LEAF_AMBIENT_LIGHTING_VERSION ||
        ...
        lh.GetMap()->m_pLeafAmbient = (mleafambientindex_t *) Hunk_AllocName(ambientLightingTable.LumpSize(),
                                                                             "LeafAmbient");
        lh.GetMap()->m_pAmbientSamples = (mleafambientlighting_t *) Hunk_AllocName(ambientLightingLump.LumpSize(),
                                                                                   "LeafAmbientSamples");
...
class CResourcePreloadModel : public CResourcePreload {
    static void
    QueuedLoaderMapCallback(void *pContext, void *pContext2, const void *pData, int nSize, LoaderError_t loaderError) {
    ...
    virtual bool CreateResource(const char *pName) {
        ...
        if (modType == mod_brush) {
            ...
            if (pModel->nLoadFlags &
                (IModelLoader::FMODELLOADER_LOADED | IModelLoader::FMODELLOADER_LOADED_BY_PRELOAD)) {
    ...
    switch (mod->type) {
        ...
        case mod_brush: {
            ...
            if (IsX360()) {
                ...
                bQueuedLoader = g_pQueuedLoader->BeginMapLoading(mod->strName,
            ...
            if (pFilesToReload->IsFileInList(pModel->strName)) {
                ...
                if (g_pMDLCache->IsDataLoaded(pModel->studio, MDLCACHE_STUDIOHWDATA)) {
                    ...
                    if (pStudioHdr) {
                        ...
                        IMaterial *pMaterials[128];
                        int nMaterials = g_pStudioRender->GetMaterialList(pStudioHdr, ARRAYSIZE(pMaterials),
                                                                          &pMaterials[0]);
...
static void GetSpriteInfo(const char *pName, bool bIsVideo, int &nWidth, int &nHeight, int &nFrameCount) {
    ...
    if (bIsVideo && g_pVideo != NULL) {
        pVideoMaterial = g_pVideo->CreateVideoMaterial(pName, pName, "GAME",
                                                                                                                        ...
                                                                                                                        static bool BSPCreateCallback( const char *pSourceName, const char *pTargetName, const char *pPathID, void *pExtraData )
{
	...
	if( !iBSPPack )
	{
		Warning( "Can't load engine.dll - unable to swap bsp.\n" );
		return false;
	}
    ...
    if (int *pCallbackRegistrationCount = m_RegisteredDynamicCallbacks.GetPtr(callbackID)) {
        if (pModel) {
            ...
            for (UtlHashHandle_t i = m_DynamicModels.FirstHandle();
                 i != m_DynamicModels.InvalidHandle(); i = m_DynamicModels.NextHandle(i)) {
                CDynamicModelInfo &dyn = m_DynamicModels[i];
                if (dyn.m_Callbacks.FindAndFastRemove(callbackID)) {
                    if (dyn.m_Callbacks.Count() == 0) {
                        dyn.m_Callbacks.Purge();
...
void CModelLoader::DebugPrintDynamicModels() {
    ...
    for (UtlHashHandle_t h = m_DynamicModels.FirstHandle();
         h != m_DynamicModels.InvalidHandle(); h = m_DynamicModels.NextHandle(h)) {
