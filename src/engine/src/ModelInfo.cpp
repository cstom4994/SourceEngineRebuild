...
public:
    ...
    virtual bool
    RegisterModelLoadCallback(int modelindex, IModelLoadCallback *pCallback, bool bCallImmediatelyIfLoaded);
    ...
    virtual void
    RecomputeTranslucency(const model_t *model, int nSkin, int nBody, void /*IClientRenderable*/ *pClientRenderable,
    ...
    ComputeLevelScreenFade(const Vector &vecAbsOrigin, float flRadius, float flFadeScale) const { return 0; }

    virtual unsigned char
    ComputeViewScreenFade(const Vector &vecAbsOrigin, float flRadius, float flFadeScale) const { return 0; }
    ...
    virtual bool
    IsUsingFBTexture(const model_t *model, int nSkin, int nBody, void /*IClientRenderable*/ *pClientRenderable) const;
...
bool
CModelInfo::RegisterModelLoadCallback(int modelIndex, IModelLoadCallback *pCallback, bool bCallImmediatelyIfLoaded) {
    const model_t *pModel = GetModel(modelIndex);
    Assert(pModel);
    if (pModel && IsDynamicModelIndex(modelIndex)) {
        return modelloader->RegisterModelLoadCallback(const_cast< model_t *>( pModel ),
                                                      IsClientOnlyModelIndex(modelIndex), pCallback,
                                                      bCallImmediatelyIfLoaded);
...
void CModelInfo::UnregisterModelLoadCallback(int modelIndex, IModelLoadCallback *pCallback) {
    if (modelIndex == -1) {
        ...
        Assert(pModel);
        if (pModel) {
            modelloader->UnregisterModelLoadCallback(const_cast< model_t *>( pModel ),
                                                     IsClientOnlyModelIndex(modelIndex), pCallback);
        }
    }
}
        ...
        switch (model->type) {
            case mod_brush: {
                for (int i = 0; i < model->brush.nummodelsurfaces; ++i) {
                    SurfaceHandle_t surfID = SurfaceHandleFromIndex(model->brush.firstmodelsurface + i,
                                                                    model->brush.pShared);
...
void CModelInfoClient::OnDynamicModelsStringTableChange(int nStringIndex, const char *pString, const void *pData) {
    ...
    bool bServerLoaded = pData ? (*(char *) pData != 0) : (g_ClientGlobalVariables.network_protocol <=
                                                           PROTOCOL_VERSION_20);
...
inline unsigned char CModelInfoClient::ComputeScreenFade(const Vector &vecAbsOrigin,
                                                         float flRadius, float flFadeScale,
                                                         const ScreenFadeInfo_t &fade) const {
...
unsigned char
CModelInfoClient::ComputeLevelScreenFade(const Vector &vecAbsOrigin, float flRadius, float flFadeScale) const {
    if (IsXbox()) {
        return 255;
    } else {
...
unsigned char
CModelInfoClient::ComputeViewScreenFade(const Vector &vecAbsOrigin, float flRadius, float flFadeScale) const {
