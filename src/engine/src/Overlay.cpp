    ...
    while (iCurrentRenderOrder <= iHighestRenderOrder) {
        ...
        for (int i = m_nFirstRenderQueue[nSortGroup]; i != RENDER_QUEUE_INVALID; i = nNextRenderQueue) {
            ...
            int nMaxVertices = pRenderContext->GetMaxVerticesToRender(
                    !bWireframeFragments ? renderQueueHead.m_pMaterial : g_materialWorldWireframe);
            if (nMaxVertices == 0)
                continue;
    ...
    for (int iVert = 0; iVert < nVertCount; ++iVert) {
        ...
        for (int iTexCoord = 0; iTexCoord < NUM_OVERLAY_TEXCOORDS; iTexCoord++) {
            TexCoordInQuadFromBarycentric(origOverlay.m_aPrimVerts[0].texCoord[iTexCoord],
                                          ...
                                          origOverlay.m_aPrimVerts[1].texCoord[iTexCoord],
                                          vecUV, vecTexCoord);
...
void COverlayMgr::ReSortMaterials(void) {
    ...
    for (int iOverlay = 0; iOverlay < nOverlayCount; ++iOverlay) {
        ...
        while (hFrag != OVERLAY_FRAGMENT_INVALID) {
            ...
            moverlayfragment_t *pFrag = &m_aFragments[iFrag];
            if (pFrag) {
                const MaterialSystem_SortInfo_t &sortInfo = materialSortInfoArray[MSurf_MaterialSortID(
                        pFrag->m_SurfId)];
...
void COverlayMgr::DoClipFragment(moverlayfragment_t *pFragment, cplane_t *pClipPlane,
    ...
    for (iVert = 0; iVert < nVertCount; ++iVert) {
        ...
        overlayvert_t vert;
        vert.pos =
                pFragment->m_aPrimVerts[iVert].pos + fraction * (pFragment->m_aPrimVerts[(iVert + 1) % nVertCount].pos -
                                                                 pFragment->m_aPrimVerts[iVert].pos);
        for (int iTexCoord = 0; iTexCoord < NUM_OVERLAY_TEXCOORDS; iTexCoord++) {
            vert.texCoord[iTexCoord][0] = pFragment->m_aPrimVerts[iVert].texCoord[iTexCoord][0] + fraction *
                                                                                                  (pFragment->m_aPrimVerts[
                                                                                                           (iVert + 1) %
                                                                                                           nVertCount].texCoord[iTexCoord][0] -
                                                                                                   pFragment->m_aPrimVerts[iVert].texCoord[iTexCoord][0]);
            vert.texCoord[iTexCoord][1] = pFragment->m_aPrimVerts[iVert].texCoord[iTexCoord][1] + fraction *
                                                                                                  (pFragment->m_aPrimVerts[
                                                                                                           (iVert + 1) %
                                                                                                           nVertCount].texCoord[iTexCoord][1] -
