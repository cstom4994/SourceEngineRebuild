...
public:
    ...
    virtual void
    ComputeLighting(const Vector &pt, const Vector *pNormal, bool bClamp, Vector &color, Vector *pBoxColors);
    ...
    if (g_ClientDLL) {
        ...
        if (g_ClientFactory) {
            ...
            if (clientleafsystem) {
                ...
                clientleafsystem = (IClientLeafSystemEngine *) g_ClientFactory(CLIENTLEAFSYSTEM_INTERFACE_VERSION_1,
                                                                               NULL);
                if (!clientleafsystem) {
