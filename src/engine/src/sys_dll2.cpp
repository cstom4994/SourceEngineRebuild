...
static eSteamInfoInit
Sys_TryInitSteamInfo(void *pvAPI, SteamInfVersionInfo_t &VerInfo, const char *pchMod, const char *pchBaseDir,
                     bool bDedicated) {
...
void CEngineAPI::SetStartupInfo(StartupInfo_t &info) {
    ...
    eSteamInfoInit steamInfo = Sys_TryInitSteamInfo(this, g_SteamInfIDVersionInfo, info.m_pInitialMod,
    ...
    if (steamInfo != eSteamInfo_Initialized) {
        ...
        steamInfo = Sys_TryInitSteamInfo(this, g_SteamInfIDVersionInfo, info.m_pInitialMod, info.m_pBaseDirectory,
                                         bDedicated);
...
bool CModAppSystemGroup::Create() {
    ...
    IServerDLLSharedAppSystems *serverSharedSystems = (IServerDLLSharedAppSystems *) g_ServerFactory(
...
bool CDedicatedServerAPI::ModInit(ModInfo_t &info) {
    ...
    if (CommandLine()->CheckParm("+sv_pure"))
        ...
        pure_mode = CommandLine()->ParmValue("-sv_pure", 1);
    if (pure_mode)
        g_pFullFileSystem->EnableWhitelistFileTracking(true, true,
                                                       CommandLine()->FindParm("-sv_pure_verify_hashes") ? true
                                                                                                         : false);
    else
