...
public:
    ...
    virtual void
    TakeSnapshotTGARect(const char *pFilename, int x, int y, int w, int h, int resampleWidth, int resampleHeight,
...
private:
    ...
    void
    BlitGraphicToHDCWithAlpha(HDC hdc, byte *rgba, int imageWidth, int imageHeight, int x0, int y0, int x1, int y1);
...
void CVideoMode_Common::ComputeStartupGraphicName(char *pBuf, int nBufLen) {
    ...
    if (!g_pFileSystem->FileExists(pBuf, "GAME")) {
        Q_strncpy(pBuf, (aspectRatio >= 1.6f) ? "materials/console/background01_widescreen.vtf"
                                              : "materials/console/background01.vtf", nBufLen);
    }
}
...
void CVideoMode_Common::WriteScreenshotToSteam(uint8 *pImage, int cubImage, int width, int height) {
    ...
    if (cl_savescreenshotstosteam.GetBool()) {
        if (Steam3Client().SteamScreenshots()) {
            ScreenshotHandle hScreenshot = Steam3Client().SteamScreenshots()->WriteScreenshot(pImage, cubImage, width,
                                                                                              height);
...
void CVideoMode_Common::AddScreenshotToSteam(const char *pchFilename, int width, int height) {
    ...
    if (cl_savescreenshotstosteam.GetBool()) {
        if (Steam3Client().SteamScreenshots()) {
            ScreenshotHandle hScreenshot = Steam3Client().SteamScreenshots()->AddScreenshotToLibrary(pchFilename, NULL,
                                                                                                     width, height);
            ApplySteamScreenshotTags(hScreenshot);
        }
    }
...
//  resampleHeight = h;
    ...
    float *pFloatImage = (float *) malloc(
            resampleWidth * resampleHeight * ImageLoader::SizeInBytes(IMAGE_FORMAT_RGB323232F));
...
//-----------------------------------------------------------------------------
static void
VID_ProcessMovieFrame(const MovieInfo_t &info, bool jpeg, const char *filename, int width, int height, byte *data) {
    CUtlBuffer outBuf;
    bool bSuccess = false;
    if (jpeg) {
