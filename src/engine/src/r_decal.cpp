...
static void
R_DecalCreate(decalinfo_t *pDecalInfo, SurfaceHandle_t surfID, float x, float y, bool bForceForDisplacement);
...
void
R_DecalShoot(int textureIndex, int entity, const model_t *model, const Vector &position, const float *saxis, int flags,
...
static void R_DecalShoot_(IMaterial *pMaterial, int entity, const model_t *model,
                          ...
                          const Vector *pNormal, void *userdata = 0) {
...
void
R_DecalShoot(int textureIndex, int entity, const model_t *model, const Vector &position, const Vector *saxis, int flags,
             const color32 &rgbaColor, const Vector *pNormal) {
...
static void
R_DecalCreate(decalinfo_t *decalinfo, SurfaceHandle_t surfID, float x, float y, bool bForceForDisplacement) {
    decal_t *pdecal;
...
void R_DecalsGetMaxMesh(IMatRenderContext *pRenderContext, int &nDecalSortMaxVerts, int &nDecalSortMaxIndices) {
    ...
    nDecalSortMaxVerts = MIN(nDecalSortMaxVerts,
                             8192); // just a guess, you should be able to do 8K dynamic verts in any material and this is no big loss on batching
}
    ...
    for (int i = 0; i != iDecalCount; ++i) {
        ...
        if (bBatchInit) {
            ...
            if (bWireframe) {
                ...
                pBatch->m_iLightmapPage = materialSortInfoArray[MSurf_MaterialSortID(
                        pDecalHead->surfID)].lightmapPageID;
            }
...
void
R_DrawDecalsAll(IMatRenderContext *pRenderContext, int iGroup, int iTreeType, const Vector &vModelOrg, int nCheckCount,
                float flFade) {
    DecalMeshList_t meshList;
    CMeshBuilder meshBuilder;
    SurfaceHandle_t lastSurf = NULL;
...
//	VPROF_BUDGET( "Decals", "Decals" );
    ...
    for (decal_t *pDecal = pDecalList; pDecal; pDecal = pNext) {
        ...
        if (iPool != g_aDecalSortPool.InvalidIndex()) {
            ...
            DecalMaterialBucket_t &bucket = sortTree.m_aDecalSortBuckets[iGroup][iTreeType].Element(
