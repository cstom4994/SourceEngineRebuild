...
public:
    ...
    void
    Remove(ReplayHandle_t hObj);    // Remove() will remove the object, remove any .dmx associated with the object on disk, and usually delete attached files (like .dems or movies, etc, depending on what the manager implementation is)
    void Remove(T *pObj);
    ...
    virtual void
    Think();    // IReplayThinker implementation - NOTE: not meant to be called directly - called from think manager
...
protected:
    ...
    virtual const char *
    GetRelativeIndexPath() const = 0;    // Should return relative (to replay/client or replay/server) path where index file lives - NOTE: Last char should be a slash
    virtual const char *GetIndexFilename() const = 0;    // Should return just the name of the file, e.g. "replays.dmx"
    virtual const char *GetDebugName() const = 0;

    virtual bool
    ShouldDeleteObjects() const { return true; }    // TODO: Used by Clear() - I'm not convinced this is needed yet though.
    ...
    virtual int
    GetHandleBase() const { return 0; }    // Subclass can implement this to provide a base/minimum for handles
...
void CGenericPersistentManager<T>::RemoveFromIndex(int it) {
    ...
    AssertMsg(m_lstObjsToUnload.Find(pObj) == m_lstObjsToUnload.InvalidIndex(),
              "The object being removed was also in the unload list - is this OK?  If so, code should be added to remove from that list as well.");
...
template<class T>
const char *
CGenericPersistentManager<T>::GetIndexFullFilename() const        // Should return the full path to the main .dmx file
{
