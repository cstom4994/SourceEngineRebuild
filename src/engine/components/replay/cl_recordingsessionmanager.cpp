...
CClientRecordingSessionManager::OnSessionStart(int nCurrentRecordingStartTick, const char *pSessionName) {
...
void CClientRecordingSessionManager::FireGameEvent(IGameEvent *pEvent) {
    ...
    if (!V_stricmp("replay_sessioninfo", pEventName)) {
        ...
        m_ServerRecordingState.m_nCurrentBlock = nCurrentBlock +
                                                 1;        // This will account for any different between when the server actually dumps a block and the client predicts a dump
...
void CClientRecordingSessionManager::DownloadThink() {
    ...
    if (bKickedOffDownload) {
        m_flNextPossibleDownloadTime = flHostTime + MAX(MIN_SERVER_DUMP_INTERVAL,
                                                        CL_GetRecordingSessionManager()->m_ServerRecordingState.m_nDumpInterval);
    }
}
