...
void CClientReplayContext::GetPlaybackTimes(float &flOutTime, float &flOutLength, const CReplay *pReplay,
    ...
    const int nServerRecordStartTick = CL_GetRecordingSessionManager()->GetServerStartTickForSession(
            pReplay->m_hSession);
    ...
    const int nCurTick = MAX(g_pEngineClient->GetClientGlobalVars()->tickcount - nStartTick - nServerRecordStartTick,
                             0);
