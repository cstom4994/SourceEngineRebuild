...
bool CAudioSourceWave::GetXboxAudioStartupData() {
    ...
    if (!g_pFullFileSystem->ReadFile(fileName, "GAME", buf, sizeof(xwvHeader_t))) {
        ...
        if (pHeader->staticDataSize) {
            // get optional data
            if (!g_pFullFileSystem->ReadFile(fileName, "GAME", fileBuffer, pHeader->staticDataSize,
                                             sizeof(xwvHeader_t))) {
                return false;
            }
...
public:
    ...
    virtual int
    GetOutputData(void **pData, int samplePosition, int sampleCount, char copyBuf[AUDIOSOURCE_COPYBUF_SIZE]);
...
private:
    SearchPathCache *
    LookUpCacheEntry(const char *szCleanedFilename, int audiosourcetype, bool soundisprecached, CSfxTable *sfx);
...
CAudioSourceCache::LookUpCacheEntry(const char *fn, int audiosourcetype, bool soundisprecached, CSfxTable *sfx) {
    if (IsX360()) {
        return NULL;
    }
