...
#include "mouthinfo.h"
#include "../src/cl_main.h"
#include "icliententitylist.h"
#include "icliententity.h"
#include "../src/sys_dll.h"
...
portable_samplepair_t cubicfilter1[3] = {{0, 0},
                                         {0, 0},
                                         {0, 0}};
portable_samplepair_t cubicfilter2[3] = {{0, 0},
                                         {0, 0},
                                         {0, 0}};
...
void MIX_InitializePaintbuffer(paintbuffer_t *pPaintBuffer, bool bSurround, bool bSurroundCenter) {
    ...
    pPaintBuffer->pbuf = (portable_samplepair_t *) _aligned_malloc(PAINTBUFFER_MEM_SIZE * sizeof(portable_samplepair_t),
                                                                   16);
...
bool MIX_InitAllPaintbuffers(void) {
    ...
    g_temppaintbuffer = (portable_samplepair_t *) _aligned_malloc(TEMP_COPY_BUFFER_SIZE * sizeof(portable_samplepair_t),
                                                                  16);
...
void
S_Interpolate2xLinear_2(int count, portable_samplepair_t *pbuffer, portable_samplepair_t *pfiltermem, int cfltmem) {
    Assert (cfltmem >= 1);
...
void
SW_Mix8StereoDirectional(float soundfacing, portable_samplepair_t *pOutput, int *volume, byte *pData, int inputOffset,
                         fixedint rateScaleFix, int outCount) {
    int sampleIndex = 0;
    fixedint sampleFrac = inputOffset;
    int x;
...
void
SW_Mix16StereoDirectional(float soundfacing, portable_samplepair_t *pOutput, int *volume, short *pData, int inputOffset,
                          fixedint rateScaleFix, int outCount) {
    fixedint sampleIndex = 0;
    fixedint sampleFrac = inputOffset;
...
// pData buffer, ensuring we can always provide 'outCount' samples.
void
SW_Mix8StereoDistVar_Interp(float distmix, portable_samplepair_t *pOutput, int *volume, byte *pData, int inputOffset,
                            fixedint rateScaleFix, int outCount) {
    int x;
...
void
SW_Mix16StereoDistVar_Interp(float distmix, portable_samplepair_t *pOutput, int *volume, short *pData, int inputOffset,
                             fixedint rateScaleFix, int outCount) {
    int x;
...
// pData buffer, ensuring we can always provide 'outCount' samples.
void
SW_Mix8Mono_Interp(portable_samplepair_t *pOutput, int *volume, byte *pData, int inputOffset, fixedint rateScaleFix,
                   int outCount) {
    fixedint sampleIndex = 0;
    fixedint rateScaleFix14 = FIX_28TO14(rateScaleFix);        // convert 28 bit fixed point to 14 bit fixed point
    fixedint sampleFrac14 = FIX_28TO14(inputOffset);
...
// pData buffer, ensuring we can always provide 'outCount' samples.
void
SW_Mix8Stereo_Interp(portable_samplepair_t *pOutput, int *volume, byte *pData, int inputOffset, fixedint rateScaleFix,
                     int outCount) {
    fixedint sampleIndex = 0;
    fixedint rateScaleFix14 = FIX_28TO14(rateScaleFix);        // convert 28 bit fixed point to 14 bit fixed point
    fixedint sampleFrac14 = FIX_28TO14(inputOffset);
...
void
SW_Mix16Mono_Shift(portable_samplepair_t *pOutput, int *volume, short *pData, int inputOffset, fixedint rateScaleFix,
                   int outCount) {
    int vol0 = volume[0];
    int vol1 = volume[1];
                                                                                                                                ...
                                                                                                                                for ( int i = 0; i < outCount; i++ )
	{
        ...
        imul  edi, vol0; multiply pData[sampleIndex] by volume[0]
        sar   edi, 08h; divide by 256
        add DWORD PTR[eax], edi; add to pOutput[i].left; esi has a copy, use it now
...
// pData buffer, ensuring we can always provide 'outCount' samples.
    ...
    void
    SW_Mix16Stereo(portable_samplepair_t *pOutput, int *volume, short *pData, int inputOffset, fixedint rateScaleFix,
                   int outCount) {
        int sampleIndex = 0;
        fixedint sampleFrac = inputOffset;
...
//===============================================================================
    ...
    void
    Mix16MonoWavtype(channel_t *pChannel, portable_samplepair_t *pOutput, int *volume, short *pData, int inputOffset,
                     fixedint rateScaleFix, int outCount) {
        if (FUseHighQualityPitch(pChannel))
    ...
    void
    Mix8StereoWavtype(channel_t *pChannel, portable_samplepair_t *pOutput, int *volume, byte *pData, int inputOffset,
                      fixedint rateScaleFix, int outCount) {
        switch (pChannel->wavtype) {
            ...
            case CHAR_DIRECTIONAL:
                if (FUseHighQualityPitch(pChannel))
                    SW_Mix8StereoDirectional_Interp(pChannel->dspface, pOutput, volume, pData, inputOffset,
            ...
            case CHAR_DISTVARIANT:
                if (FUseHighQualityPitch(pChannel))
                    ...
                    SW_Mix8StereoDistVar(pChannel->distmix, pOutput, volume, pData, inputOffset, rateScaleFix,
                                         outCount);
                break;
    ...
    void
    Mix16StereoWavtype(channel_t *pChannel, portable_samplepair_t *pOutput, int *volume, short *pData, int inputOffset,
                       fixedint rateScaleFix, int outCount) {
        switch (pChannel->wavtype) {
            case CHAR_DOPPLER:
            ...
            case CHAR_DIRECTIONAL:
                if (FUseHighQualityPitch(pChannel))
                    SW_Mix16StereoDirectional_Interp(pChannel->dspface, pOutput, volume, pData, inputOffset,
                                                     rateScaleFix, outCount);
                else
            ...
            case CHAR_DISTVARIANT:
                if (FUseHighQualityPitch(pChannel))
                    ...
                    SW_Mix16StereoDistVar(pChannel->distmix, pOutput, volume, pData, inputOffset, rateScaleFix,
                                          outCount);
                break;
...
//#include "snd_wave_source.h"
    void SND_MoveMouth8(channel_t *ch, CAudioSource *pSource, int count) {
        ...
        if (pMouth->NeedsEnvelope()) {
            int availableSamples = pSource->GetOutputData((void **) &pdata, ch->pMixer->GetSamplePosition(), count,
                                                          NULL);
