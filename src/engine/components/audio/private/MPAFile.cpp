...
void CMPAFile::Open(const char *szFilename) {
    ...
    m_hFile = g_pFullFileSystem->Open(szFilename, "rb",
                                      "GAME");//::CreateFile( szFilename, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL );
    if (m_hFile == FILESYSTEM_INVALID_HANDLE) {
        // throw error
...
uint32 CMPAFile::ExtractBytes(uint32 &dwOffset, uint32 dwNumBytes, bool bMoveOffset) {
    ...
    for (uint32 n = dwOffset; n < dwOffset + dwNumBytes; n++) {
        dwResult |= ((byte) m_pBuffer[n]) << (8 *
                                              dwNumByteShifts); // the bit shift will do the correct byte order for you
        dwNumByteShifts--;
    }
