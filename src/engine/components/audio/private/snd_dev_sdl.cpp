...
extern void
S_SpatializeChannel( /*int nSlot,*/ int volume[6], int master_vol, const Vector *psourceDir, float gain, float mono);
...
void CAudioDeviceSDLAudio::AudioCallback(Uint8 *stream, int len) {
    ...
    while (len > 0) {
        ...
        m_readPos = len ? 0 : (m_readPos +
                               writeLen);  // if still bytes to write to stream, we're rolling around the ring buffer.
    }
