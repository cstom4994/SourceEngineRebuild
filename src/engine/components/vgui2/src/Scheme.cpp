...
void CScheme::LoadFonts() {
    ...
    for (KeyValues *kv = m_pData->FindKey("CustomFontFiles", true)->GetFirstSubKey();
    ...
    // add bitmap fonts
    for (KeyValues *kv = m_pData->FindKey("BitmapFontFiles", true)->GetFirstSubKey();
         kv != NULL; kv = kv->GetNextKey()) {
        const char *fontFile = kv->GetString();
        if (fontFile && *fontFile) {
    ...
    for (KeyValues *kv = m_pData->FindKey("Fonts", true)->GetFirstSubKey(); kv != NULL; kv = kv->GetNextKey()) {
        for (int i = 0; i < 2; i++) {
            ...
            const char *fontName = GetMungedFontName(kv->GetName(), tag,
                                                     proportionalFont); // first time it adds a normal font, and then a proportional one
...
IImage *CSchemeManager::GetImage(const char *imageName, bool hardwareFiltered) {
    if (!imageName || strlen(imageName) <=
                      0) // frame icons and the like are in the scheme file and may not be defined, so if this is null then fail silently
    {
        return NULL;
    }
