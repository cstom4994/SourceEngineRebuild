...
RichText::RichText(Panel *parent, const char *panelName) : BaseClass(parent, panelName) {
    ...
    if (IsProportional()) {
        ...
        _drawOffsetX = static_cast<int>( static_cast<float>( DRAW_OFFSET_X ) *
                                         (static_cast<float>( sw ) / static_cast<float>( width )));
        _drawOffsetY = static_cast<int>( static_cast<float>( DRAW_OFFSET_Y ) *
                                         (static_cast<float>( sw ) / static_cast<float>( width )));
    } else {
...
const wchar_t *
RichText::ResolveLocalizedTextAndVariables(char const *pchLookup, wchar_t *outbuf, size_t outbufsizeinbytes) {
    if (pchLookup[0] == '#') {
    ...
    if (m_FormatStream.IsValidIndex(renderState.formatStreamIndex)) {
        if (m_bResetFades == false) {
            if (m_FormatStream[renderState.formatStreamIndex].fade.flFadeLength != -1.0f) {
                float frac = (m_FormatStream[renderState.formatStreamIndex].fade.flFadeStartTime -
            ...
            if (renderState.textClickable != _currentTextClickable) {
                if (renderState.textClickable) {
                    ...
                    ClickPanel *clickPanel = _clickableTextPanels.IsValidIndex(_clickableTextIndex)
                                             ? _clickableTextPanels[_clickableTextIndex] : NULL;
...
int RichText::ParseTextStringForUrls(const char *text, int startPos, char *pchURLText, int cchURLText, char *pchURL,
    ...
    while (text[i] != 0) {
        ...
        if (!Q_strnicmp(text + i, "<a href=", 8)) {
            ...
            Q_strncpy(pchURL, text + i, std::min((uint32) (pchURLEnd - text - i + 1), (uint32) cchURL));
            i += (pchURLEnd - text - i + 1);

            // get the url text
            pchURLEnd = Q_strstr(text, "</a>");
            Q_strncpy(pchURLText, text + i, std::min((uint32) (pchURLEnd - text - i + 1), (uint32) cchURLText));
            i += (pchURLEnd - text - i);
            i += Q_strlen("</a>");
