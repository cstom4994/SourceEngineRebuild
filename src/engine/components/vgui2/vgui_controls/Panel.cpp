    ...
    while (map) {
        ...
        for (int i = 0; i < c; ++i) {
            ...
            if (list.Count() == 0) {
                ...
                if (defaultKey) {
                    KeyBindingMap_t *alreadyBound = LookupBindingByKeyCode((KeyCode) defaultKey->keycode,
...
#endif
            nPinnedCornerOffsetX = scheme()->GetProportionalScaledValueEx(GetScheme(), inResourceData->GetInt(
                    "PinnedCornerOffsetX"));
        }
...
#endif
            nPinnedCornerOffsetY = scheme()->GetProportionalScaledValueEx(GetScheme(), inResourceData->GetInt(
                    "PinnedCornerOffsetY"));
        }
...
#endif
            nUnpinnedCornerOffsetX = scheme()->GetProportionalScaledValueEx(GetScheme(), inResourceData->GetInt(
...
#endif
            nUnpinnedCornerOffsetY = scheme()->GetProportionalScaledValueEx(GetScheme(), inResourceData->GetInt(
                ...
                switch (pMap->numParams) {
                    ...
                    case 2: {
                        ...
                        if ((DATATYPE_INT == pMap->firstParamType) && (DATATYPE_INT == pMap->secondParamType)) {
                            ...
                            (this->*((MessageFunc_ConstCharPtrInt_t) pMap->func))(param1->GetString(),
                                                                                  param2->GetInt());
                        } else if ((DATATYPE_CONSTCHARPTR == pMap->firstParamType) &&
                            ...
                            (this->*((MessageFunc_IntConstCharPtr_t) pMap->func))(param1->GetInt(),
                                                                                  param2->GetString());
                ...
                switch (pMessageMap[i].numParams) {
                    case 2:
                        if ((DATATYPE_INT == pMessageMap[i].firstParamType) &&
                            ...
                            (this->*((MessageFunc_HandleConstCharPtr_t) pMessageMap[i].func))(vp, params->GetString(
                                    pMessageMap[i].secondParamName));
                        } else if ((DATATYPE_HANDLE == pMessageMap[i].firstParamType) &&
                                   (DATATYPE_CONSTWCHARPTR == pMessageMap[i].secondParamType)) {
                            typedef void (Panel::*MessageFunc_HandleConstCharPtr_t)(VPANEL, const wchar_t *);
                            VPANEL vp = ivgui()->HandleToPanel(params->GetInt(pMessageMap[i].firstParamName));
                            (this->*((MessageFunc_HandleConstCharPtr_t) pMessageMap[i].func))(vp, params->GetWString(
                                    pMessageMap[i].secondParamName));
                        } else {
                            // the message isn't handled
                            ivgui()->DPrintf("Message '%s', sent to '%s', has invalid parameter types\n",
...
public:
    CPanelMessageMapDictionary() : m_PanelMessageMapPool(sizeof(PanelMessageMap), 32, CUtlMemoryPool::GROW_FAST,
...
public:
    CPanelKeyBindingMapDictionary() : m_PanelKeyBindingMapPool(sizeof(PanelKeyBindingMap), 32,
...
namespace vgui {
    ...
    void VguiPanelGetSortedChildButtonList(Panel * pParentPanel, void * pSortedPanels, char * pchFilter /*= NULL*/, int
    nFilterType /*= 0*/ ) {
    CUtlSortVector<SortedPanel_t, CSortedPanelYLess> *pList = reinterpret_cast< CUtlSortVector<SortedPanel_t, CSortedPanelYLess> * >( pSortedPanels );
    ...
    if ( nFilterType == 0 ) {
    if ( !
    StringHasPrefix( szBuff, pchFilter
    )) {
    continue;
}
}
...
int ComputeWide(Panel * pPanel, unsigned int & nBuildFlags, KeyValues * inResourceData, int
nParentWide,
int nParentTall,
bool bComputingOther
)
...
{
nBuildFlags |=
Panel::BUILDMODE_SAVE_WIDE_FULL;
wstr++;
}
else
{
...
nBuildFlags |=
Panel::BUILDMODE_SAVE_WIDE_PROPORTIONAL_TALL;
...
{
nBuildFlags |=
Panel::BUILDMODE_SAVE_WIDE_PROPORTIONAL;
wstr++;
}
else if (wstr[0] == 's' || wstr[0] == 'S')
{
nBuildFlags |=
Panel::BUILDMODE_SAVE_WIDE_PROPORTIONAL_SELF;
wstr++;
}
}
...
int ComputeTall(Panel * pPanel, unsigned int & nBuildFlags, KeyValues * inResourceData, int
nParentWide,
int nParentTall,
bool bComputingOther
)
...
{
nBuildFlags |=
Panel::BUILDMODE_SAVE_TALL_FULL;
tstr++;
}
else
{
...
nBuildFlags |=
Panel::BUILDMODE_SAVE_TALL_PROPORTIONAL_WIDE;
...
{
nBuildFlags |=
Panel::BUILDMODE_SAVE_TALL_PROPORTIONAL;
tstr++;
}
else if (tstr[0] == 's' || tstr[0] == 'S')
{
nBuildFlags |=
Panel::BUILDMODE_SAVE_TALL_PROPORTIONAL_SELF;
tstr++;
}
}
...
int ComputePos(Panel * pPanel,
const char *pszInput,
int &nPos,
const int &nSize,
const int &nParentSize,
const bool &bX, EOperator
eOp)
{
...
const int nFlagProportionalSelf = bX ? Panel::BUILDMODE_SAVE_XPOS_PROPORTIONAL_SELF
                                     : Panel::BUILDMODE_SAVE_YPOS_PROPORTIONAL_SELF;
const int nFlagProportionalParent = bX ? Panel::BUILDMODE_SAVE_XPOS_PROPORTIONAL_PARENT
...
{
nFlags |=
nFlagProportionalSelf;
pszInput++;
}
else if (pszInput[0] == 'p' || pszInput[0] == 'P')
{
nFlags |=
nFlagProportionalParent;
pszInput++;
}
...
case '+':
ComputePos( pPanel,
++pszInput, nPos, nSize, nParentSize, bX, OP_ADD);
break;
case '-':
ComputePos( pPanel,
++pszInput, nPos, nSize, nParentSize, bX, OP_SUB);
break;
}
}
...
if (tf_debug_tabcontainer.

GetBool() &&

!Q_stricmp("TabContainer", pPanel->
...
nFlags, nPos, nParentSize, nPosDelta, nSize, pPanel->
