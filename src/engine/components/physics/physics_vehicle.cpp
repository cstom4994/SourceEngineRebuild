...
void CVehicleController::UpdatePowerslide(const vehicle_controlparams_t &controls, bool bPowerslide, float flSpeed) {
    ...
    if (flSpeed > 0 && (bPowerslideLeft != bPowerslideRight)) {
        ...
        float powerSlide = RemapValClamped(flSpeed, m_vehicleData.steering.speedSlow, m_vehicleData.steering.speedFast,
                                           0, 1);
...
void CVehicleController::UpdateEngineTurboFinish(void) {
    if (m_vehicleData.engine.boostDuration + m_vehicleData.engine.boostDelay > 0)    // watch out for div by zero
    {
        if (m_currentState.boostDelay > 0) {
            m_currentState.boostTimeLeft = 100 - 100 * (m_currentState.boostDelay /
...
void CVehicleController::CalcEngineTransmission(float flThrottle) {
    ...
    while ((flEstEngineRPM < m_vehicleData.engine.shiftDownRPM) && (m_currentState.gear > 0)) {
        m_currentState.gear--;
        flEstEngineRPM =
                flAvgRotSpeed * m_vehicleData.engine.axleRatio * m_vehicleData.engine.gearRatio[m_currentState.gear] *
                60;
    }
...
//-----------------------------------------------------------------------------
void
CVehicleController::CalcEngine(float throttle, float brake_val, bool handbrake, float steeringVal, bool torqueBoost) {
...
void CVehicleController::GetCarSystemDebugData(vehicle_debugcarsystem_t &debugCarSystem) {
    ...
    for (int iWheel = 0; iWheel < VEHICLE_DEBUGRENDERDATA_MAX_WHEELS; ++iWheel) {
        ...
        debugCarSystem.vecWheelRaycastImpacts[iWheel] =
                debugCarSystem.vecWheelRaycasts[iWheel][0] + (carSystemDebugData.wheelRaycastImpacts[iWheel] *
...
//-----------------------------------------------------------------------------
IPhysicsVehicleController *
CreateVehicleController(CPhysicsEnvironment *pEnv, CPhysicsObject *pBodyObject, const vehicleparams_t &params,
                        unsigned int nVehicleType, IPhysicsGameTrace *pGameTrace) {
