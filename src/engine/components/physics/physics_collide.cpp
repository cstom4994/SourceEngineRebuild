...
public:
    ...
    void
    ConvexesFromConvexPolygon(const Vector &vPolyNormal, const Vector *pPoints, int iPointCount, CPhysConvex **pOutput);
    ...
    CPhysCollide *
    ConvertConvexToCollideParams(CPhysConvex **pConvex, int convexCount, const convertconvexparams_t &convertParams);


    CPolyhedron *PolyhedronFromConvex(CPhysConvex *const pConvex, bool bUseTempPolyhedron);

    int
    GetConvexesUsedInCollideable(const CPhysCollide *pCollideable, CPhysConvex **pOutputArray, int iOutputArrayLimit);
    ...
    virtual Vector
    CollideGetExtent(const CPhysCollide *pCollide, const Vector &collideOrigin, const QAngle &collideAngles,
    ...
    void
    TraceBox(const Ray_t &ray, const CPhysCollide *pCollide, const Vector &collideOrigin, const QAngle &collideAngles,
             trace_t *ptr);
    ...
    void
    TraceCollide(const Vector &start, const Vector &end, const CPhysCollide *pSweepCollide, const QAngle &sweepAngles,
                 const CPhysCollide *pCollide, const Vector &collideOrigin, const QAngle &collideAngles, trace_t *ptr);
...
private:
    ...
    FastBboxCollide(const CPhysCollideCompactSurface *pCollide, const Vector &mins, const Vector &maxs);
...
CPhysConvex *CPhysicsCollision::RebuildConvexFromPlanes(CPhysConvex *pConvex, float mergeTolerance) {
    ...
    IVP_Compact_Ledge *pLedgeOut = IVP_SurfaceBuilder_Halfspacesoup::convert_halfspacesoup_to_compact_ledge(&halfspaces,
                                                                                                            mergeTolerance);
...
CPhysConvex *CPhysicsCollision::ConvexFromPlanes(float *pPlanes, int planeCount, float mergeDistance) {
    ...
    IVP_Compact_Ledge *pLedge = IVP_SurfaceBuilder_Halfspacesoup::convert_halfspacesoup_to_compact_ledge(&halfspaces,
                                                                                                         mergeDistance);
    return reinterpret_cast<CPhysConvex *>( pLedge );
}
...
CPolyhedron *CPhysicsCollision::PolyhedronFromConvex(CPhysConvex *const pConvex, bool bUseTempPolyhedron) {
    ...
    for (int i = 0; i != iTriangles; ++i) {
        ...
        pTriangles[i].Edges[2].iPointIndices[1] = pTriangles[i].Edges[0].iPointIndices[0] = pTri->get_edge(
                2)->get_start_point_index();
        pTriangles[i].Edges[0].iPointIndices[1] = pTriangles[i].Edges[1].iPointIndices[0] = pTri->get_edge(
                1)->get_start_point_index();
        pTriangles[i].Edges[1].iPointIndices[1] = pTriangles[i].Edges[2].iPointIndices[0] = pTri->get_edge(
                0)->get_start_point_index();
    ...
    int *pStartIndices = (int *) stackalloc(
            iNumPoints * sizeof(int)); //for quicker lookup of which edges to use in polygons
    ...
    *pWrite = &pIVP_Points[iReverseCounter];
    ...
    do {
        ...
        *pWrite = &pIVP_Points[iForwardCounter];
        ...
        pOutput[iOutputCount] = reinterpret_cast<CPhysConvex *>(IVP_SurfaceBuilder_Pointsoup::convert_triangle_to_compace_ledge(
...
CPhysCollideCompactSurface *
CPhysicsCollision::FastBboxCollide(const CPhysCollideCompactSurface *pCollide, const Vector &mins, const Vector &maxs) {
    Vector boxVerts[8];
...
// loads a set of solids into a vcollide_t
void
CPhysicsCollision::VCollideLoad(vcollide_t *pOutput, int solidCount, const char *pBuffer, int bufferSize, bool swap) {
