...
public:
    ...
    void
    Init(const CPhysCollide *pCollisionModel, IVP_Real_Object *pObject, int materialIndex, float volume, float drag,
         float angDrag);
    ...
    void
    UpdateShadow(const Vector &targetPosition, const QAngle &targetAngles, bool tempDisableGravity, float timeOffset);
...
private:
    ...
    friend bool CPhysicsEnvironment::TransferObject(IPhysicsObject *pObject,
                                                    IPhysicsEnvironment *pDestinationEnvironment); //need direct access to m_pShadow for Portal mod's physics object transfer system
};
...
extern IPhysicsObject *
CreateObjectFromBuffer(CPhysicsEnvironment *pEnvironment, void *pGameData, unsigned char *pBuffer,
