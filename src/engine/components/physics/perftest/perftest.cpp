...
struct physicstest_t {
    ...
    void CreateGround(float size) {
        ...
        for (int i = 0; i < 20; i++) {
            ...
            objectparams_t params = g_PhysDefaultObjectParams;
            params.mass = 150.0f;
            IPhysicsObject *pGround = physenv->CreatePolyObject(pCollide, physprops->GetSurfaceIndex("default"),
                                                                Vector(64 * (i % 4), 64 * (i % 5), 1024), vec3_angle,
                                                                &params);
            AddObject(pGround);
            pGround->Wake();
        }
...
void AppInit(void) {
    ...
    g_materialFlatshaded = g_MaterialSystemApp.m_pMaterialSystem->FindMaterial("debug/debugdrawflatpolygons",
                                                                               TEXTURE_GROUP_OTHER, true);
    g_pWireframeMaterial = g_MaterialSystemApp.m_pMaterialSystem->FindMaterial("shadertest/wireframevertexcolor",
                                                                               TEXTURE_GROUP_OTHER);
