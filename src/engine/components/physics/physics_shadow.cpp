...
void
ComputePDControllerCoefficients(float *coefficientsOut, const float frequency, const float damping, const float dt) {
...
void
ComputeController(IVP_U_Float_Point &currentSpeed, const IVP_U_Float_Point &delta, float maxSpeed, float maxDampSpeed,
                  float scaleDelta, float damping, IVP_U_Float_Point *pOutImpulse = NULL) {
...
void
ComputeController(IVP_U_Float_Point &currentSpeed, const IVP_U_Float_Point &delta, const IVP_U_Float_Point &maxSpeed,
                  float scaleDelta, float damping, IVP_U_Float_Point *pOutImpulse) {
    // scale by timestep
    IVP_U_Float_Point acceleration;
    acceleration.set_multiple(&delta, scaleDelta);
...
class CPlayerController
        : public IVP_Controller_Independent, public IPhysicsPlayerController, public IVP_Listener_Object {
public:
...
float
ComputeShadowControllerIVP(IVP_Real_Object *pivp, shadowcontrol_params_t &params, float secondsToArrival, float dt) {
    // resample fraction
...
class CShadowController
        : public IVP_Controller_Independent, public IPhysicsShadowController, public CAlignedNewDelete<16> {
public:
    CShadowController();
    ...
    virtual void
    SetPhysicallyControlled(bool isPhysicallyControlled) { m_isPhysicallyControlled = isPhysicallyControlled; }
...
void CShadowController::InitFromTemplate(const vphysics_save_cshadowcontroller_t &controllerTemplate) {
    ...
    m_pObject->GetObject()->get_environment()->get_controller_manager()->add_controller_to_core(this,
                                                                                                m_pObject->GetObject()->get_core());
}
...
bool
RestorePhysicsShadowControllerInternal(const physrestoreparams_t &params, IPhysicsShadowController **ppShadowController,
                                       CPhysicsObject *pObject) {
    vphysics_save_cshadowcontroller_t controllerTemplate;
...
void ControlPhysicsShadowControllerAttachment_Silent(IPhysicsShadowController *pController, IVP_Real_Object *pivp,
    ...
    if (bAttach) {
        ...
        pivp->get_environment()->get_controller_manager()->remove_controller_from_core(
