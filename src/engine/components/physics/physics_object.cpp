...
void
CPhysicsObject::Init(const CPhysCollide *pCollisionModel, IVP_Real_Object *pObject, int materialIndex, float volume,
...
void CPhysicsObject::SetInertia(const Vector &inertia) {
    ...
    if (ri.k[0] > 1e14f) ri.k[0] = 1e14f;
    if (ri.k[1] > 1e14f) ri.k[1] = 1e14f;
    if (ri.k[2] > 1e14f) ri.k[2] = 1e14f;
    if (ri.k[0] <= 0) ri.k[0] = 1.f;
    if (ri.k[1] <= 0) ri.k[1] = 1.f;
    if (ri.k[2] <= 0) ri.k[2] = 1.f;
...
void
CPhysicsObject::CalculateVelocityOffset(const Vector &forceVector, const Vector &worldPosition, Vector *centerVelocity,
...
void
CPhysicsObject::SetShadow(float maxSpeed, float maxAngularSpeed, bool allowPhysicsMovement, bool allowPhysicsRotation) {
    if (m_pShadow) {
...
CPhysicsObject *
CreatePhysicsObject(CPhysicsEnvironment *pEnvironment, const CPhysCollide *pCollisionModel, int materialIndex,
    ...
    IVP_Polygon *realObject = pEnvironment->GetIVPEnvironment()->create_polygon(pSurman, &objectTemplate, &rotation,
                                                                                &pos);
...
CPhysicsObject *
CreatePhysicsSphere(CPhysicsEnvironment *pEnvironment, float radius, int materialIndex, const Vector &position,
    ...
    MEM_ALLOC_CREDIT();
    IVP_Ball *realObject = pEnvironment->GetIVPEnvironment()->create_ball(&ballTemplate, &objectTemplate, &rotation,
                                                                          &pos);
...
IPhysicsObject *
CreateObjectFromBuffer_UseExistingMemory(CPhysicsEnvironment *pEnvironment, void *pGameData, unsigned char *pBuffer,
