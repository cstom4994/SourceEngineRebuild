...
public:
    ...
    CPhysCollideVirtualMesh(const virtualmeshparams_t &params) : m_params(params), m_hMemory(INVALID_MEMHANDLE),
                                                                 m_ledgeCount(0) {
        m_pHull = NULL;
    ...
    CreateBoundingSurfaceFromRange(const virtualmeshlist_t &list, int firstIndex, int indexCount);
    ...
    if (list.triangleCount) {
        ...
        if (pSurface) {
            ...
            if (CVPhysicsVirtualMeshWriter::LedgeCanBePacked(pLedge, list)) {
                ...
                IVP_Compact_Surface *pSurface1 = CreateBoundingSurfaceFromRange(list, list.indexCount / 2,
                                                                                list.indexCount / 2);
...
IVP_Compact_Surface *
CPhysCollideVirtualMesh::CreateBoundingSurfaceFromRange(const virtualmeshlist_t &list, int firstIndex, int indexCount) {
...
void
IVP_SurfaceManager_VirtualMesh::insert_all_ledges_hitting_ray(IVP_Ray_Solver *ray_solver, IVP_Real_Object *object) {
