...
protected:
    ...
    IVP_Real_Object_Fast_Static(IVP_Cluster *father, const IVP_Template_Object *templ) : IVP_Object(father,
                                                                                                    templ) { ; };
public:
...
public:
    ...
    IVP_Real_Object_Fast(IVP_Cluster *father, const IVP_Template_Object *templ) : IVP_Real_Object_Fast_Static(father,
                                                                                                              templ) { ; };
};
...
public:
    virtual void set_new_m_object_f_core(
            const IVP_U_Matrix *new_m_object_f_core);     /* specifies new objects coordinate system in core space */
...
public:
    ...
    void
    revive_object_for_simulation(); // wake up object immediately ; @@@IK don't use this - use ensure_in_simulation() instead!
    ...
    IVP_FLOAT
    get_extra_radius() const { return extra_radius; };    // returns an extra collision radius around the object#
 ...
 **********************************************************************************/
    ...
    void
    calc_at_quaternion(IVP_Time current_time_in, IVP_U_Quat *q_world_f_object_out, IVP_U_Point *position_ws_out) const;
 ...
 **********************************************************************************/
    void beam_object_to_new_position(const IVP_U_Quat *rotation, const IVP_U_Point *position,
 ...
 ****************************************************************************************************/
    ...
    void get_quat_world_f_object_AT(IVP_U_Quat *quat_world_f_object,
 ...
 **************************************************************************************/
    ...
    IVP_Core *
    get_original_core() const { return original_core; }; // Returns the core that was valid at creation of object.

    IVP_Movement_Type
    get_movement_state() { return (IVP_Movement_Type) flags.object_movement_state; }; // See declaration of IVP_Movement_Type.


    //////// Simulation
    IVP_BOOL
    disable_simulation();          // Freeze object immediately disregarding the speed, return IVP_TRUE when success
 ...
 ********************************************************************************/
    void async_push_object_ws(const IVP_U_Point *position_ws,
                              const IVP_U_Float_Point *impulse_ws); // impulse vec's length = impulse strength [kg*m/s]
    void async_add_speed_object_ws(
            const IVP_U_Float_Point *speed_ws);       // add a value to the core's speed_change variable
    void async_add_rot_speed_object_cs(
            const IVP_U_Float_Point *rotation_vec_cs); // add a value to the core's rot_speed_change variable

    IVP_BOOL
    is_collision_detection_enabled() { // Returns IVP_FALSE if IVP_Environment::enable_collision_detection was not called
