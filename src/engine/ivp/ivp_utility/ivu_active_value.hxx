...
public:
    ...
    virtual void delay_active_float(
            IVP_U_Active_Float_Delayed *mod);  // put into delay_active_IVP_FLOAT queue, so update can be executed later (next PSI)
    virtual void delay_active_int(
            IVP_U_Active_Int_Delayed *mod);  // put into delay_active_IVP_FLOAT queue, so update can be executed later (next PSI)
...
public:
    IVP_U_Active_Test_Range(const char *name,
                            IVP_U_Active_Float *mod_test, IVP_U_Active_Float *mod_low_val,
                            IVP_U_Active_Float *mod_high_val);
...
class IVP_U_Active_Switch
        : public IVP_U_Active_Float, public IVP_U_Active_Float_Listener, public IVP_U_Active_Int_Listener {
