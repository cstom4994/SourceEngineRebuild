        ...
        if (pos > last_pos) {        // not wrapped situation
            if (ideal_pos < ideal_pos_of_last_element) {    // shift wrapped elements also
                if (ideal_pos > last_pos) continue;  // no way to shift elements left to their ideal position
                       ...
                       ideal_pos_of_last_element) {    // further checks needed: are we at the start or at the end of the hash ??
...
void IVP_VHash_Store::print() {
    ...
    printf("%i:", size);
    for (i = 0; i < size; i++) {
        printf(" %i:%X:%X:%X  ", elems_store[i].hash_index & size_mm, (int) elems_store[i].key_elem,
               (int) elems_store[i].elem, elems_store[i].hash_index);
    }
    printf("\n");
}
