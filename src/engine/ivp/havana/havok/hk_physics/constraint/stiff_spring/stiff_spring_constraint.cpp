...
hk_Stiff_Spring_Constraint::hk_Stiff_Spring_Constraint(
        ...
        : hk_Constraint(constraint_system, a, b, HK_PRIORITY_LOCAL_CONSTRAINT,
                        HK_NEXT_MULTIPLE_OF(16, sizeof(hk_Stiff_Spring_Work))) {
    init_stiff_spring_constraint(bp);
}
...
int hk_Stiff_Spring_Constraint::setup_and_step_constraint(hk_PSI_Info &pi, void *mem, hk_real tau_factor,
                                                          hk_real damp_factor) {
    ...
    if (this->m_min_length == this->m_stiff_spring_length || work.current_dist != 0) {
        ...
        translation_ws_ks[0]._set_transformed_pos(b0->get_transform_next_PSI(pi.get_delta_time()),
                                                  m_translation_os_ks[0]);
        translation_ws_ks[1]._set_transformed_pos(b1->get_transform_next_PSI(pi.get_delta_time()),
        ...
        bool skipSolve = this->m_min_length <= len &&
                         (this->m_min_length = this->m_stiff_spring_length, len <= this->m_min_length) ||
                         (float) (len - this->m_min_length) == 0.0;
    ...
    { // step
        ...
        hk_real delta_dist = tau_factor * m_tau * pi.get_inv_delta_time() * work.current_dist -
