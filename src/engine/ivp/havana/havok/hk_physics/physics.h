...
//	hk_Rigid_Body(const IVP_Real_Object &real):IVP_Real_Object(real) {}
    hk_Rigid_Body(IVP_Cluster *father, IVP_SurfaceManager *surface, const IVP_Template_Real_Object *obj,
                  const IVP_U_Quat *q_world_f_obj, const IVP_U_Point *position) : IVP_Real_Object(father, surface, obj,
                                                                                                  q_world_f_obj,
                                                                                                  position) {}
    ...
    inline hk_Transform get_transform_next_PSI(float deltaTime) {
        ...
        next_m_world_f_core.vv.k[0] =
                this->get_core()->speed.k[0] * deltaTime + this->get_core()->m_world_f_core_last_psi.vv.k[0];
        next_m_world_f_core.vv.k[1] =
                this->get_core()->speed.k[1] * deltaTime + this->get_core()->m_world_f_core_last_psi.vv.k[1];
        next_m_world_f_core.vv.k[2] =
                this->get_core()->speed.k[2] * deltaTime + this->get_core()->m_world_f_core_last_psi.vv.k[2];
...
public:
    ...
    virtual void anchor_will_be_deleted_event(
            IVP_Anchor *del_anchor) { // when an object is deleted it sends events to its connected actuators
        delete this;
    }
...
class hk_Constraint *ivp_create_ragdoll_constraint_from_local_constraint_system(class hk_Local_Constraint_System *,
