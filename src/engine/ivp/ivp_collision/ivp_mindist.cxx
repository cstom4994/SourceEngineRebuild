...
void IVP_Mindist_Settings::set_collision_tolerance(IVP_DOUBLE t) {
    ...
    for (int i = IVP_MAX_STEPS_FOR_COLLDIST_DECREASE - 1; i >= 0; i--) {
        coll_dists[i] = min_coll_dists + (i / IVP_DOUBLE(IVP_MAX_STEPS_FOR_COLLDIST_DECREASE)) *
    ...
    speed_after_keeper_dist = IVP_Inline_Math::ivp_sqrtf(
            2.0f * (keeper_dist - min_coll_dists) * 9.81f); //speed when falling down from keeper dist to coll dist
...
void IVP_Mindist::init_mindist(IVP_Real_Object *pop0, IVP_Real_Object *pop1, const IVP_Compact_Edge *e0,
...
void IVP_Mindist_Manager::create_exact_mindists(IVP_Real_Object *pop0, IVP_Real_Object *pop1,
    ...
    if (!single_ledge0) {
        ...
        pop0->get_surface_manager()->get_all_ledges_within_radius(&visitor_position_os0, search_radius_0, root_ledge0,
                                                                  NULL, single_ledge1, &ledges0);
    } else {
    ...
    if (!single_ledge1) {
        ...
        pop1->get_surface_manager()->get_all_ledges_within_radius(&visitor_position_os1, search_radius_1, root_ledge1,
                                                                  NULL, single_ledge0, &ledges1);
    } else {
    ...
    IVP_Diff_Hash<IVP_MM_CMP, IVP_U_FVector<IVP_Collision>, IVP_Collision, IVP_MM_CMP_Key> diff_hash(mindists, a_buffer,
                                                                                                     buffer_size, NULL);
...
void IVP_Mindist_Manager::recheck_ov_element(IVP_Real_Object *object) {
    ...
    if (!scanning_universe) {    // check for new objects in the universe manager
        ...
        radius = environment->ov_tree_manager->insert_ov_element(elem, real_check_sphere, real_check_sphere,
                                                                 &colliding_elements);
    ...
    // convert old list into hash
    IVP_Diff_Hash<IVP_OO_CMP, IVP_U_FVector<IVP_Collision>, IVP_Collision, IVP_Real_Object> diff_hash(
            &elem->collision_fvector, a_buffer, buffer_size, object);
...
void IVP_Mindist::update_exact_mindist_events(IVP_BOOL allow_hull_conversion, IVP_MINDIST_EVENT_HINT event_hint) {
...

    {
        ...
        if (get_length() > hull_check_len) {
            if (allow_hull_conversion) {
                ...
                my_manager->insert_hull_mindist(this, get_length() -
                                                      coll_dist); // secdist is subtracted, because hull manager is already checked till next PSI
            }
            return;
        }
    }
    ...
    {
        mim.projected_center_speed = core1->speed.dot_product(&contact_plane) -
        ...
        rot0_projected = IVP_Inline_Math::ivp_sqrtf(1.001f - rot0_projected) *
                         core0->max_surface_rot_speed;    // make sure sqrt does not crash -> increase result
            ...
            if (event_hint == IVP_EH_NOW) {
                ...
                if ((event_hint == IVP_EH_BIG_DELAY) || (mim.event_type_out & 0x0f)) {    //
                    if (mdist >= P_DOUBLE_RES) {
                        ...
                        next_event =
                                env->get_current_time() + (ivp_mindist_settings.event_queue_min_delta_time_base * 1e-3f)
                                                          * env->get_delta_PSI_time(); //@@CB
                    }
                } else {
                    if (mdist >= P_DOUBLE_RES) {
                        ...
                        next_event =
                                env->get_current_time() + (ivp_mindist_settings.event_queue_min_delta_time_base * 1e-5f)
                                                          * env->get_delta_PSI_time(); //@@CB
                    }
                }
...
void IVP_Mindist::mindist_hull_limit_exceeded_event(IVP_HTIME hull_intrusion_value) {
    ...
    if (!disable_halfspace_optimization) {
        ...
        IVP_DOUBLE estimated_distance =
                this->get_length() - angular_movement - delta_center_distance + hull_intrusion_value;
        ...
        if (estimated_distance > speed * obj0->get_environment()->get_delta_PSI_time() * look_ahead_factor) {
            ...
            IVP_IF(ivp_check_debug_mindist(this)) {
                ivp_message("%32s estimated_distance into hull dist: %f error %f speed %f\n",
                            "hull_limit_exceeded_event", IVP_Inline_Math::ivp_sqrtf(qdistance), estimated_distance,
                            speed);
            }
...
void IVP_Mindist::create_cp_in_advance_pretension(IVP_Real_Object *robject, float gap_len) {
    ...
    if (old_fr_syn) {
        ...
        IVP_Contact_Point *new_cp = try_to_generate_managed_friction(&associated_fs, &success, sim_unit_not_destroy,
                                                                     call_recalc_svals);
    ...
    if (l_mindist->recalc_result == IVP_MDRR_OK) {
        if ((l_mindist->coll_type & 0x0f) == 0) {        // real collision might have happened
            ...
            if (md_len < sec_dist) {
                ...
                l_mindist->update_exact_mindist_events(IVP_FALSE,
...
void IVP_Mindist_Manager::insert_lazy_hull_mindist(IVP_Mindist *md, IVP_HTIME hull_time) {
    ...
    if (!IVP_MTIS_IS_MOVING(obj0->get_movement_state())) {
        ...
        IVP_IF(ivp_check_debug_mindist(md)) {
            ivp_message("%32s into hull, htimes %f: %f\n", "insert_lazy_hull_mindist", factor * speed_corr_0,
                        factor * speed_corr_1);
        }
    }
}
