...
class IVP_OV_Tree_Manager {
    ...
    IVP_BOOL box_contains_box(const struct IVP_OV_Node_Data *master_data, const IVP_OV_Node *sub_node,
                              int const rasterlevel_diff) const;

    IVP_BOOL
    box_overlaps_with_box(const IVP_OV_Node *largenode, const IVP_OV_Node *smallnode, int const rasterlevel_diff) const;
    ...
    void collect_collision_partners(const IVP_OV_Element *center_element, const IVP_OV_Node *masternode,
                                    const IVP_OV_Node *new_node);

    void get_luf_coordinates_ws(const IVP_OV_Node *node, IVP_U_Float_Point *p,
                                IVP_FLOAT *cubesize); // fills vars with left-upper-front corner's coordinates and cube's size
    void get_center_coordinates_ws(const IVP_OV_Node *node, IVP_U_Float_Point *p,
                                   IVP_FLOAT *cubesize); // fills vars with center's coordinates and cube's size
