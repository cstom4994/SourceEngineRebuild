...
IVP_MRC_TYPE IVP_Mindist_Minimize_Solver::p_minimize_FF(const IVP_Compact_Edge *A, const IVP_Compact_Edge *B,
        ...
        for (i = 0; i < 2; i++) {
            ...
            for (pB = tabB[i], b = 0; b < 3; pB = pB->get_next(), b++) {
                ...
                for (pA = tabA[i], a = 0; a < 3; pA = pA->get_next(), a++) {
                    ...
                    IVP_DOUBLE qdist = IVP_CLS.quad_dist_edge_to_point_K_space(cc_A[i]->get_compact_ledge(), pA,
                                                                               &pB_Aos);
                    if (qdist * (1 + P_DOUBLE_RES) < min_qdist) {
...
const IVP_Compact_Edge *IVP_Compact_Ledge_Solver::minimize_on_other_side(
        const IVP_Compact_Edge *edge, const IVP_U_Point *partner_os) {    // manage pierce count
    ...
    memset(pierce_visited_array, 0,
           n_triangles);        // @@@OG could be optimized, depending on how often piercing takes place
...
IVP_MRC_TYPE (*IVP_Mindist_Minimize_Solver::mms_function_table[IVP_ST_MAX_LEGAL][IVP_ST_MAX_LEGAL])(
