...
void IVP_Constraint_Solver_Car_Builder::calc_pushing_behavior(int A_obj_idx, int push_vec_idx) {
    ...
    m_B_from_A.vmult4(&A_push_pos_in_A,
                      &B_push_pos_in_B);    // means: objects are regarded as placed at the desired positions
...
//		rot_change_A_in_A.print("rot_change_A_in_A should be zero:");
        ...
        /**** Push B (contrary) ****/
        core_B->test_push_core(&B_push_pos_in_B, &inv_impulse_B, &inv_impulse_world, &speed_change_B_world,
                               &rot_change_B_in_B);

        // calc speed at push pos
        core_B->get_surface_speed_on_test(&B_push_pos_in_B, &speed_change_B_world, &rot_change_B_in_B,
                                          &sur_speed_B_world);
    ...
    for (int fill_app_idx = 0; fill_app_idx < car_solver->wheel_objects.len(); fill_app_idx++) {
        ...
        if (fill_app_idx == A_obj_idx) {
            ...
            const IVP_U_Point *app_center_world = &this->car_solver->wheel_objects.element_at(
                    fill_app_idx)->get_core()->get_m_world_f_core_PSI()->vv;
            IVP_U_Float_Point app_center_B, B_at_app_speed_world;
            m_world_f_B->vimult4(app_center_world, &app_center_B);
            core_B->get_surface_speed_on_test(&app_center_B, &speed_change_B_world, &rot_change_B_in_B,
