...
IVP_RETURN_TYPE
IVP_Friction_Solver::test_gauss_solution_suggestion(IVP_DOUBLE *push_results, int *active_is_at_pos, int total_actives,
...
void IVP_Friction_Solver::solve_linear_equation_and_push(IVP_Friction_System *my_fs, int *active_is_at_pos,
                                                         int total_actives, IVP_U_Memory *mem_friction) {
    ...
    if (actives_matrix.solve_great_matrix_many_zero() == IVP_OK) {
        if ((test_gauss_solution_suggestion(actives_matrix.result_vector, active_is_at_pos, total_actives,
                                            mem_friction) == IVP_OK)) {
            IVP_IF(1) {
                this->gauss_succed++;
            }
    ...
    if (constraint_unilateral.init_and_solve_lc(dist_change_mat.matrix_values, dist_change_mat.desired_vector,
...
IVP_FLOAT IVP_Friction_Solver::do_penalty_step(IVP_FLOAT *impulses, IVP_FLOAT *pretension, IVP_FLOAT force_factor,
...

        {
            ...
            for (int n = fr_pairs_of_objs.len() - 1; n >= 0; n--) {
                ...
                for (int m = fr_pair->fr_dists.len() - 1; m >= 0; m--) {
                    ...
                    int found_mine = 0;
                    for (IVP_Contact_Point *mindist2 = fs->get_first_friction_dist(); mindist2; mindist2 = fs->get_next_friction_dist(
                            mindist2)) {
                        if (mindist2 == fr_dist) {
                            found_mine = 1;
                        }
...
void IVP_Friction_Solver::setup_coords_mindists(IVP_Friction_System *fs) {
    ...
    for (IVP_Contact_Point *my_dist = fs->get_first_friction_dist();
        ...
        info->friction.friction_infos[0] = my_dist->get_synapse(0)->get_object()->get_core()->get_friction_info(
                fs); // #+# bad bad
...
void out_friction_info_obj(IVP_Core *obj) {
    ...
    IVP_IF(1) {
        ...
        while (fr_info) {
            printf("obj %lx fs %lx obj_ma %d", (long) obj & 0x0000ffff, (long) fr_info->l_friction_system & 0x0000ffff,
    ...
    for (int col = 0; col < fr_info->friction_springs.n_elems; col++) {
        ...
        if (info->contact_core[which_obj]) {
            ...
            dist_change_mat.matrix_values[line_num * dist_change_mat.aligned_row_len +
                                          current_contact_point_index] += speed_change_scalar;
            //if ( line_num != current_contact_point_index){
...
int IVP_Friction_Solver::calc_solver_PSI(IVP_Friction_System *fs, int *active_is_at_pos) {
    ...
    for (int i = 0; i < contact_info_vector.len(); i++) {
        ...
        IVP_DOUBLE distance_off =
                ivp_mindist_settings.friction_dist - info->friction.dist_len;  // distance to my optimal position
        ...
        if (info->contact_core[0]) {
            ...
            calc_distance_matrix_column(i, info->contact_core[0], info->friction.friction_infos[0], &rotation_vec,
                                        &translation_vec);
        }
        ...
        if (info->contact_core[1]) {
            ...
            calc_distance_matrix_column(i, info->contact_core[1], info->friction.friction_infos[1], &rotation_vec,
                                        &translation_vec);
        }
...
void IVP_Friction_System::reorder_mindists_for_complex() {
    ...
    while (my_frdist) {
        ...
        if (my_frdist->get_gap_length() >
            ivp_mindist_settings.friction_dist + ivp_mindist_settings.distance_keepers_safety) {
...
void IVP_Friction_System::ivp_debug_fs_pointers() {
    IVP_IF(1) {
        ...
        for (IVP_Contact_Point *fr_d = get_first_friction_dist(); fr_d; fr_d = get_next_friction_dist(fr_d)) {
            printf("%lx %lx %d %lx  ", (long) fr_d->prev_dist_in_friction & 0x0000ffff, (long) fr_d & 0x0000ffff,
                   fr_d->has_negative_pull_since, (long) fr_d->next_dist_in_friction & 0x0000ffff);
        }
        printf("\n");
    }
}
    ...
    while (1) {
        ...
        if (first_dist->has_negative_pull_since > second_dist->has_negative_pull_since) {
            ...
            while ((test_dist = get_prev_friction_dist(second_dist)) &&
                   (test_dist->has_negative_pull_since > second_dist->has_negative_pull_since)) {
