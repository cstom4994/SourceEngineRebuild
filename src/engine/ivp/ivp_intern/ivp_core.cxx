...
void IVP_Core::get_diff_surface_speed_of_two_cores(const IVP_Core *this_core, const IVP_Core *other_core,
    ...
    if (this_core && !this_core->physical_unmoveable) {
        this_core->get_surface_speed_on_test(obj_point_this, &this_core->speed, &this_core->rot_speed,
                                             delta_velocity_ws);
    } else {
    ...
    if (other_core && !other_core->physical_unmoveable) {
        ...
        other_core->get_surface_speed_on_test(obj_point_other, &other_core->speed, &other_core->rot_speed,
                                              &velocity_ws_2);
        ...
        while (fr_synapse) {
            ...
            if (other_core->physical_unmoveable) {
                ...
                fr_mindist->read_materials_for_contact_situation(
                        &info); //this is done only to get friction_factor ; maybe make extra function for friction_factor
    ...
    if (!this->physical_unmoveable) {
        ...
        for (d = this->objects.len() - 1; d >= 0; d--) {
            ...
            for (; fr_synapse; fr_synapse = fr_synapse->get_next()) {
                ...
                fr_mindist->recalc_friction_s_vals();
                fr_mindist->read_materials_for_contact_situation(
                        &info); //this is only done to calculate friction_factor ; maybe make extra function for friction_factor
                fr_mindist->calc_virtual_mass_of_mindist();
...
IVP_DOUBLE
IVP_Core::calc_correct_virt_mass(const IVP_U_Float_Point *core_point, const IVP_U_Float_Point *direction_core,
...
void p_calc_2d_distances_to_axis(const IVP_U_Float_Point *point, const IVP_U_Float_Point *direction,
...
IVP_DOUBLE IVP_Core::calc_virt_mass(const IVP_U_Float_Point *core_point, const IVP_U_Float_Point *direction) const {
    ...
    if (direction) {
        ...
        distances_2d.set_pairwise_mult(&distances_2d,
...
void IVP_Core::calc_at_matrix(IVP_Time current_time, IVP_U_Matrix *m_world_f_core_out) const {
    while (IVP_MTIS_SIMULATED(movement_state)) {
        ...
        // quaternion based solution
        {
            ...
            m_world_f_core_out->get_position()->add_multiple(&pos_world_f_core_last_psi, &delta_world_f_core_psis,
                                                             d_time);
        }
        return;
    }
...
void IVP_Core::async_push_core(const IVP_U_Float_Point *point_cs,
    ...
    this->test_push_core(point_cs, impulse_in_core, impulse_in_world, &center_change,
...
IVP_Core::IVP_Core(IVP_Real_Object *io, const IVP_U_Quat *q_world_f_object_init, const IVP_U_Point *position,
...
IVP_Movement_Type IVP_Core::calc_movement_state(IVP_Time psi_time) {
    ...
    while (1) {    // break if object has moved
        ...
        if (qrot_sum * upper_limit_radius * upper_limit_radius >
            IVP_MINIMAL_ROT_SPEED_PER_PSI_SHORT * IVP_MINIMAL_ROT_SPEED_PER_PSI_SHORT)
            break;
