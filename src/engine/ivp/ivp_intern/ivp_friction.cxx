...
IVP_DOUBLE IVP_Contact_Point::two_values_friction(IVP_U_Float_Point *real_world_friction_vec) {
    ...
    for (i = 0; i < 2; i++) {
        if (mtl[i]->second_friction_x_enabled) {
            ...
            IVP_DOUBLE max_len_in_x_direction =
                    IVP_MINIMAL_REAL_FRICTION_LEN * effective_friction / real_friction_factor;
...
void
IVP_Contact_Point::static_friction_single(const IVP_Event_Sim *es, IVP_FLOAT desired_gap, IVP_FLOAT speedup_factor) {
    IVP_Impact_Solver_Long_Term *info = this->tmp_contact_info;
...
bool IVP_Contact_Point::friction_force_local_constraint_2d_wheel(IVP_Core *core_a, IVP_Impact_Solver_Long_Term *info,
...

    {
        ...
        sc_body.init_reaction_solver_translation_ws(solver->body_object->get_core(), NULL, info->contact_point_ws,
                                                    &span_v_1, NULL, NULL);
    ...
    IVP_RETURN_TYPE ret = IVP_Inline_Math::invert_2x2_matrix(tpm00, tpm01, tpm01, tpm11, &inv_mat2x2[0], &inv_mat2x2[1],
    ...
    if (core_a->car_wheel->fix_wheel_constraint) {
        ...
        if (square_impulse > maximum_impulse_force * maximum_impulse_force) // test for sliding
        {
            ...
            core_a->car_wheel->last_skid_value =
                    (1.0f - f) * this->now_friction_pressure * solver->body_object->get_core()->get_inv_mass();
        ...
        if (1 && (square_impulse > maximum_impulse_force * maximum_impulse_force)) {
            ...
            if (1 && (square2 > maximum_impulse_force * maximum_impulse_force)) {
                ...
                if (1) // update reference point
                {
                    ...
                    core_a->car_wheel->last_skid_value = 0.3f * (1.0f - s_factor) * this->now_friction_pressure *
...
IVP_FLOAT IVP_Contact_Point::friction_force_local_constraint_2d(const IVP_Event_Sim *es) {
    ...
    if (1 && core_a && core_a->car_wheel && !core_b &&
        (next_dist_in_friction == NULL || next_dist_in_friction->now_friction_pressure == 0.0f)) {
    ...
    IVP_Solver_Core_Reaction tcb;
    tcb.init_reaction_solver_translation_ws(core_a, core_b, info->contact_point_ws, &info->span_friction_v[0],
...
void IVP_Contact_Point::friction_force_local_constraint_1d(const IVP_Event_Sim *es) {
    ...
    for (i = 0; i < 2; i++) {
        ...
        if (!c->physical_unmoveable) {
            ...
            c->test_push_core(&tmp_contact_info->contact_point_cs[i], &push_core_vec[i], &push_world_vec, &test_trans,
                              &test_rot);
...
void IVP_Contact_Point::calc_pretension(IVP_FLOAT len) { // clip pretension
    ...
    if (qlen_of_spring > len * len + P_FLOAT_RES) {
        ...
        this->integrated_destroyed_energy +=
                (qlen_of_spring * ilen_of_spring - len) * real_friction_factor * now_friction_pressure;
...
void IVP_Friction_Solver::ease_test_two_mindists(IVP_Contact_Point *dist0, IVP_Contact_Point *dist1,
                ...
                for (int j = i + 1; j < total_n; j++) {
                    ...
                    if ((fr_dist2->now_friction_pressure > P_DOUBLE_EPS) || 1) {
                        ...
                        if ((dir_conform < 1E-3f) ||
                            0) //due to random stray in distances the vectors between point-surface and edge-edge are not really parallel
                        {
                            //printf("teest ");
            ...
            if ((fr_dist->now_friction_pressure > P_DOUBLE_EPS) || 1) {
                for (int j = i + 1; j < total_n; j++) {
                    ...
                    if ((fr_dist2->now_friction_pressure > P_DOUBLE_EPS) || 1) {
                        ...
                        {
                            //printf("teest ");
                            IVP_Friction_Solver::ease_test_two_mindists(fr_dist2, fr_dist,
...
IVP_Friction_Solver::IVP_Friction_Solver(IVP_Friction_System *fri_sys, const IVP_Event_Sim *es_in) {
    ...
    dist_change_mat.matrix_values = (IVP_DOUBLE *) my_mem->get_mem(
            (size_t) (dist_change_mat.aligned_row_len * dist_change_mat.columns + IVP_VECFPU_SIZE - 1) *
            sizeof(IVP_DOUBLE));
...
void IVP_Friction_System::fusion_friction_systems(IVP_Friction_System *second_sys) {
    ...
    IVP_IF(1) {
        ...
        IVP_IF(env->get_debug_manager()->check_fs) {
            printf("fusion_fs %f %lx %lx  cores ", env->get_current_time().get_time(), (long) first_sys,
                   (long) second_sys);
    ...
    while (my_dist) {
        ...
        second_sys->dist_removed_update_pair_info(
                my_dist); //is a little bit slow. remove all core pairs directly. But this way it is saver
...
void IVP_Friction_System::split_friction_system(IVP_Core *split_father) {
    ...
    IVP_IF(fr_sys->l_environment->get_debug_manager()->check_fs) {
        printf("split_fs %f %lx %lx  cores ", fr_sys->l_environment->get_current_time().get_time(), (long) fr_sys,
               (long) new_fr_sys);
        ...
        for (int n = fr_sys->fr_pairs_of_objs.len() - 1; n >= 0; n--) {
            ...
            if (robj->union_find_get_father() == split_father) {
                ...
                for (int k = fr_pair->fr_dists.len() - 1; k >= 0; k--) {
                    ...
                    for (mindist = fr_sys->get_first_friction_dist(); mindist; mindist = fr_sys->get_next_friction_dist(
                            mindist)) {
                        if (mindist == fr_dist) {
                            found_mine = 1;
                        }
...
void IVP_Friction_System::print_all_dists() {
    IVP_IF(1) {
        ...
        for (IVP_Contact_Point *mindist = this->get_first_friction_dist(); mindist; mindist = this->get_next_friction_dist(
                mindist)) {
...
IVP_DOUBLE IVP_Mutual_Energizer::calc_impulse_to_reduce_energy_level(IVP_DOUBLE speed_pot, IVP_DOUBLE inv_mass0,
                                                                     IVP_DOUBLE inv_mass1, IVP_DOUBLE delta_e) {
    delta_e *= 2.0f;
    IVP_DOUBLE divisor = 1.0f / (inv_mass0 + inv_mass1);
    IVP_DOUBLE root = (speed_pot * speed_pot -
                       (inv_mass0 + inv_mass1) * delta_e); ///(mass1*speed_pot*speed_pot-target_e));
...
IVP_DOUBLE IVP_Mutual_Energizer::calc_energy_potential(IVP_DOUBLE speed_pot, IVP_DOUBLE mass0, IVP_DOUBLE mass1,
                                                       IVP_DOUBLE inv_mass0, IVP_DOUBLE inv_mass1) {
    ...
    x = speed_pot / (inv_mass0 +
                     inv_mass1); //solution of E1'(x) = -E0'(x) -> speed of both objects get identical, speed_pot gets zero
...
void IVP_Mutual_Energizer::get_rot_inertia(IVP_Core *core, IVP_U_Float_Point *rot_vec_obj, IVP_DOUBLE *rot_inertia,
...
void IVP_Mutual_Energizer::calc_energy_potential() {
    rot_energy_potential = calc_energy_potential(rot_speed_potential, rot_inertia[0], rot_inertia[1],
                                                 inv_rot_inertia[0], inv_rot_inertia[1]);
    trans_energy_potential = calc_energy_potential(trans_speed_potential, trans_inertia[0], trans_inertia[1],
...
void IVP_Mutual_Energizer::destroy_percent_energy(IVP_DOUBLE percent_energy_to_destroy) {
    IVP_DOUBLE rot_impulse = calc_impulse_to_reduce_energy_level(rot_speed_potential, inv_rot_inertia[0],
                                                                 inv_rot_inertia[1],
                                                                 percent_energy_to_destroy * rot_energy_potential);
    IVP_DOUBLE trans_impulse = calc_impulse_to_reduce_energy_level(trans_speed_potential, inv_trans_inertia[0],
                                                                   inv_trans_inertia[1],
...
void IVP_Friction_Core_Pair::debug_read_vector_after_ease() {
    ...
    if (diff.real_length() > 0.01f) {
        printf("easingerror: %f %f %f should equal %f %f %f\n", test_vec.k[0], test_vec.k[1], test_vec.k[2],
               span_vector_sum.k[0], span_vector_sum.k[1], span_vector_sum.k[2]);
    }
}
    ...
    for (c = objects.len() - 1; c >= 0; c--) {
        ...
        for (my_synapse = r_obj->get_first_exact_synapse(); my_synapse; my_synapse = next_synapse) {
            ...
            if (mindist_distance < ivp_mindist_settings.max_distance_for_friction) {
                ...
                my_mindist->try_to_generate_managed_friction(&affected_friction_system, &having_new_dist,
...
void IVP_Friction_Sys_Static::do_simulation_single_friction(IVP_Event_Sim *es) {
    ...
    if ((cp->get_gap_length() >= ivp_mindist_settings.max_distance_for_friction) ||
