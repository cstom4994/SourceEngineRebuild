...
void IVP_Real_Object::set_new_quat_object_f_core(const IVP_U_Quat *new_quat_object_f_core,
...
void IVP_Real_Object::update_exact_mindist_events_of_object() {
    ...
    for (syn = this->get_first_exact_synapse(); syn; syn = syn_next) {
        ...
        if (core0->mindist_event_already_done !=
            core1->mindist_event_already_done) {      /// #+# OS: vector for recalc mindists
            mindist->recalc_mindist();            // should not be needed as the current position has not changed
            if (mindist->recalc_result == IVP_MDRR_OK) {
                mindist->update_exact_mindist_events(IVP_FALSE,
                                                     IVP_EH_BIG_DELAY);    // check length, do not check for hull, update time manager ...
            }
        }
    }
}
...
IVP_BOOL IVP_Real_Object::disable_simulation() {
    ...
    if (core->movement_state < IVP_MT_NOT_SIM) {
        ...
        IVP_Time ref_time =
                now_time - 20; //bad hack: this ensures that 'sim_unit_calc_movement_state' switches off simulation
...
IVP_Real_Object::IVP_Real_Object(IVP_Cluster *cluster, IVP_SurfaceManager *surface_manager_,
                                 const IVP_Template_Real_Object *templ_obj, const IVP_U_Quat *q_world_f_object,
...
//	ivp_message("IVP_Real_Object::IVP_Real_Object 0x%x - %s\n", this, templ_obj->get_name());
    ...
    physical_core = new IVP_Core(this, q_world_f_object, position, templ_obj->physical_unmoveable,
...
void IVP_Real_Object::get_quat_world_f_object_AT(IVP_U_Quat *quat_world_f_object,
...
void IVP_Real_Object::beam_object_to_new_position(const IVP_U_Quat *rotation_world_f_object,
