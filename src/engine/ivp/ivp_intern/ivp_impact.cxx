...
IVP_Contact_Point *
IVP_Mindist::try_to_generate_managed_friction(IVP_Friction_System **associated_fs, IVP_BOOL *having_new,
...
void IVP_Impact_Solver::get_world_push_direction() {
    ...
    if (two_friction_values == IVP_FALSE) {
        ...
        if ((angle > -cos_friction)) //negativ value
        {
            ...
            world_push_direction.add_multiple(&part_in_direction_surf, surf_normal,
...
void IVP_Impact_Solver::get_world_push_direction_two_friction(IVP_DOUBLE part_direction_surf_normal) {
    ...
    IVP_FLOAT quad_len_allowed = axis_first_friction * axis_first_friction * sin_friction * sin_friction +
...
void IVP_Impact_Solver::undo_push() {
    ...
    if (!core[1]->physical_unmoveable) {
        ...
        IVP_IF(core[0]->environment->debug_information->debug_impact) {
            printf("undoing trans %.3f %.3f %.3f\n", trans_speed_change[1].k[0], trans_speed_change[1].k[1],
                   trans_speed_change[1].k[2]);
        }
    }
}
...
//	IVP_DOUBLE dist_len_now=diff_vec_world.fast_real_length();
    ...
    IVP_DOUBLE dist_velocity = rel_speed_world.dot_product(
            &diff_vec_world); //velocity of distance change. negative means getting closer.
...
void IVP_Impact_Solver::do_push_on_core(IVP_U_Float_Point *push_vec_world, int num_core) {
    ...
    core[num_core]->test_push_core(obj_point[num_core], &push_vec_obj, push_vec_world, &trans_speed_change[num_core],
                                   &rot_speed_change[num_core]);
    rot_speed[num_core].add(
            &rot_speed_change[num_core]); // ...speed0 has sum of pushes and beginning speed before impact
...
void IVP_Impact_Solver::do_impact(IVP_Core *pushed_cores[2], IVP_BOOL allow_delaying, int pushes_while_system,
    ...
    this->rescue_speed_impact_solver =
            (IVP_FLOAT) (ivp_mindist_settings.min_vertical_speed_at_collision + rescue_speed_addon) *
    ...
    IVP_IF(core[0]->environment->debug_information->debug_impact) {
        printf("speed %.4f %.4f %.4f rot %.4f %.4f %.4f\n", trans_speed[1].k[0], trans_speed[1].k[1],
               trans_speed[1].k[2], rot_speed[1].k[0], rot_speed[1].k[1], rot_speed[1].k[2]);
    }
    ...
    IVP_IF(1 == 1) {
        ...
        IVP_Core::get_diff_surface_speed_of_two_cores_on_test(core[0], core[1], obj_point[0], obj_point[1],
                                                              &trans_speed[0], &rot_speed[0], &trans_speed[1],
                                                              &rot_speed[1], &rel_speed_now);
...
void IVP_Impact_Solver::calc_virt_masses_impact_solver(const IVP_U_Float_Point *world_direction_normal) {
    ...
    for (i = 1; i >= 0; i--) {
        ...
        this->virt_mass[i] = core[i]->calc_correct_virt_mass(obj_point[i], (IVP_Vec_PCore *) &direction_obj,
                                                             &direction_world);
...
void IVP_Impact_Solver_Long_Term::do_impact_of_two_objects(IVP_Mindist *mindist, IVP_Real_Object *obj0,
    ...
    IVP_Contact_Point *my_fr_dist = mindist->try_to_generate_managed_friction(&affected_friction_system,
                                                                              &having_new_dist, sim_u,
...
void IVP_Impact_Solver::get_cos_sin_for_impact(IVP_FLOAT friction_val, IVP_FLOAT percent_energy_conservation,
...
void IVP_Impact_Solver_Long_Term::do_impact_long_term(IVP_Core *pushed_cores[2], IVP_FLOAT rescue_speed_val,
    ...
    if (imp_solver_stack.core[0]->car_wheel || imp_solver_stack.core[1]->car_wheel) {
        imp_solver_stack.get_cos_sin_for_impact(0.0f, this->impact.percent_energy_conservation,
                                                &imp_solver_stack.cos_friction, &imp_solver_stack.sin_friction);
    } else {
        imp_solver_stack.get_cos_sin_for_impact(cp->real_friction_factor, this->impact.percent_energy_conservation,
                                                &imp_solver_stack.cos_friction, &imp_solver_stack.sin_friction);
...
// remove
void IVP_Impact_System::init_and_solve_impact_system(IVP_Mindist *mindist, IVP_Friction_System *fs_system,
...
void IVP_Impact_System::debug_check_all_dists_at_end() {
    IVP_Contact_Point *my_dist;
    for (my_dist = associated_fs_system->get_first_friction_dist(); my_dist; my_dist = associated_fs_system->get_next_friction_dist(
            my_dist)) {
        my_dist->recalc_friction_s_vals();
...
IVP_BOOL IVP_Impact_System::test_loop_all_pairs() {
    ...
    if (worst_impact_dist) {
        IVP_ASSERT(worst_impact_dist->tmp_contact_info->impact.distance_reached_in_time <
        ...
        worst_impact_dist->tmp_contact_info->do_impact_long_term(impacting_cores,
                                                                 worst_impact_dist->tmp_contact_info->impact.rescue_speed_addon,
                                                                 worst_impact_dist);
...
void IVP_Impact_System::invalidate_impact_mindists(IVP_Core *my_core) {    //there is a cheaper way: via synapses
...
void IVP_Impact_Solver::get_world_direction_second_friction(
        IVP_Contact_Point *cp) { //IVP_U_Float_Point *ret_second_friction_vec,IVP_FLOAT *sin_second_friction) {
    IVP_Material *mtl[2];
    ...
    for (i = 0; i < 2; i++) {
        if (mtl[i]->second_friction_x_enabled) {
            ...
            IVP_FLOAT dummy_cos;
            get_cos_sin_for_impact(effective_friction_second, (IVP_FLOAT) percent_energy_conservation, &dummy_cos,
                                   &sin_second_friction);
        }
    }
    ...
    for (i = 0; i < 2; i++) {
        ...
        if (my_core) {
            ...
            if (my_syn->get_status() != IVP_ST_BALL) {
                ...
                //tan_quad*=tan_quad;
                IVP_DOUBLE tan_quad = my_core->rot_speed.quad_length() * IVP_COLL_DETECT_MIN_TIME *
                                      IVP_COLL_DETECT_MIN_TIME; //approximate tan quad alpha with alpha*alpha
                ...
                IVP_DOUBLE uncertain_speed = uncertain_way *
                                             my_core->environment->get_inv_delta_PSI_time(); //later speed is transformed back to way with get_delta_PSI_time
...
void IVP_Contact_Point::calc_coll_distance() {
    ...
    info->impact.distance_reached_in_time =
            get_gap_length() - (closing_speed + info->impact.rescue_speed_addon * 0.5f) * env->get_delta_PSI_time();
    ...
    IVP_IF(1) {
        IVP_IF(env->get_debug_manager()->file_out_impacts) {
            ...
            fprintf(fp, "  %lx %lx-%lx: ", 0x0000ffff & (long) this, 0x0000ffff & (long) core0,
                    0x0000ffff & (long) core1);
