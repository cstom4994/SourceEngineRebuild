...
public:
    void
    set_transformation(const IVP_U_Quat *rotation, const IVP_U_Point *position, IVP_BOOL optimize_for_repeated_calls);

    void
    calc_next_PSI_matrix(class IVP_Event_Sim *event_sim, IVP_U_Vector<IVP_Hull_Manager_Base> *active_hull_managers_out);

    static void
    commit_all_calc_next_PSI_matrix(IVP_Environment *, IVP_U_Vector<IVP_Core> *cores_which_needs_calc_next_psi,
                                    IVP_U_Vector<IVP_Hull_Manager_Base> *active_hull_managers_out);

    static void commit_one_hull_manager(IVP_Environment *,
                                        IVP_U_Vector<IVP_Hull_Manager_Base> *active_hull_managers); // if you now there is only one hull_manager
