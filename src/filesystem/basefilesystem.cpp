...
//-----------------------------------------------------------------------------
inline void
CBaseFileSystem::ComputeFullWritePath(char *pDest, int maxlen, const char *pRelativePath, const char *pWritePathID) {
...
void CBaseFileSystem::AddVPKFile(char const *pPath, const char *pPathID, SearchPathAdd_t addType) {
    ...
    CSearchPath *sp = &m_SearchPaths[(addType == PATH_ADD_TO_TAIL) ? m_SearchPaths.AddToTail()
...
void CBaseFileSystem::AddMapPackFile(const char *pPath, const char *pPathID, SearchPathAdd_t addType) {
    ...
    int iStoreId = CRC32_ProcessSingleBuffer(fullpath, V_strlen(fullpath)) |
                   0x80000000; // set store ID based on .bsp filename, so if we remount the same map file, it will have the same storeid
...
void CBaseFileSystem::PrintSearchPaths(void) {
    ...
    for (int i = 0; i < c; i++) {
        ...
        Msg("\"%s\" \"%s\" %s%s\n", pSearchPath->GetPathString(), (const char *) pSearchPath->GetPathIDString(),
            pszType, pszPack);
    }
...
//-----------------------------------------------------------------------------
bool
CBaseFileSystem::ReadFile(const char *pFileName, const char *pPath, CUtlBuffer &buf, int nMaxBytes, int nStartingByte,
...
public:
    CFileOpenInfo(CBaseFileSystem *pFileSystem, const char *pFileName, const CBaseFileSystem::CSearchPath *path,
            ...
            m_pFileSystem(pFileSystem), m_pFileName(pFileName), m_pSearchPath(path), m_pOptions(pOptions),
            m_Flags(flags), m_ppszResolvedFilename(ppszResolvedFilename) {
        m_pFileHandle = NULL;
...
CBaseFileSystem::OpenForRead(const char *pFileNameT, const char *pOptions, unsigned flags, const char *pathID,
    ...
    for (openInfo.m_pSearchPath = iter.GetFirst();
         openInfo.m_pSearchPath != NULL; openInfo.m_pSearchPath = iter.GetNext()) {
...
//-----------------------------------------------------------------------------
KeyValues *
CBaseFileSystem::LoadKeyValues(KeyValuesPreloadType_t type, char const *filename, char const *pPathID /*= 0*/ ) {
    KeyValues *kv = NULL;
...
//-----------------------------------------------------------------------------
bool
CBaseFileSystem::LookupKeyValuesRootKeyName(char const *filename, char const *pPathID, char *rootName, size_t bufsize) {
...
//-----------------------------------------------------------------------------
bool
CBaseFileSystem::ExtractRootKeyName(KeyValuesPreloadType_t type, char *outbuf, size_t bufsize, char const *filename,
                                    char const *pPathID /*= 0*/ ) {
    char tempPathID[MAX_PATH];
    ParsePathID(filename, pPathID, tempPathID);
...
EFileCRCStatus
CBaseFileSystem::CheckCachedFileHash(const char *pPathID, const char *pRelativeFilename, int nFileFraction,
...
void
CBaseFileSystem::EnableWhitelistFileTracking(bool bEnable, bool bCacheAllVPKHashes, bool bRecalculateAndCheckHashes) {
    if (IsX360()) {
...
//-----------------------------------------------------------------------------
const char *CBaseFileSystem::RelativePathToFullPath(const char *pFileName, const char *pPathID, OUT_Z_CAP(maxLenInChars)
                                                    char *pDest, int maxLenInChars, PathTypeFilter_t pathFilter,
...
const char *
CBaseFileSystem::GetLocalPath(const char *pFileName, OUT_Z_CAP(maxLenInChars) char *pDest, int maxLenInChars) {
    CHECK_DOUBLE_SLASHES(pFileName);
...
//-----------------------------------------------------------------------------
bool CBaseFileSystem::FullPathToRelativePathEx(const char *pFullPath, const char *pPathId, OUT_Z_CAP(maxLenInChars)
                                               char *pDest, int maxLenInChars) {
...
void
CBaseFileSystem::CFileCacheObject::IOCallback(const FileAsyncRequest_t &request, int nBytesRead, FSAsyncStatus_t err) {
