...
public:
    ...
    virtual void
    DynamicLoadMapResource(const char *pFilename, DynamicResourceCallback_t pCallback, void *pContext, void *pContext2);
    ...
    virtual bool
    ClaimAnonymousJob(const char *pFilename, QueuedLoaderCallback_t pCallback, void *pContext, void *pContext2);
...
//-----------------------------------------------------------------------------
bool
CQueuedLoader::CResourceNameLessFunc::Less(const FileNameHandle_t &hFilenameLHS, const FileNameHandle_t &hFilenameRHS,
                                           void *pCtx) {
    switch ((int) pCtx) {
    ...
    for (int i = 0; i < sortedFiles.Count(); i++) {
        ...
        if (pFileJob->m_bFileExists) {
            ...
            if (IsDynamic())
                ...
                g_pThreadPool->QueueCall(IOComputationJob, pFileJob, pFileJob->m_pTargetData, 0,
                                         LOADERERROR_FILEOPEN)->Release();
        }
    }
}
...
void CQueuedLoader::GetJobRequests() {
    ...
    if (!IsPC() && !m_bDynamic) {
        ...
        CJob *jobs[5];
        jobs[0] = g_pThreadPool->QueueCall(BuildResources, m_pLoaders[RESOURCEPRELOAD_SOUND],
        ...
        jobs[3] = g_pThreadPool->QueueCall(BuildResources, m_pLoaders[RESOURCEPRELOAD_MODEL],
