...
public:
    ...
    void
    NoteFileLoadedFromDisk(const char *pFilename, const char *pPathID, int iSearchPathStoreId, FILE *fp, int64 nLength);
...
public:
    ...
    EFileCRCStatus CheckCachedFileHash(const char *pPathID, const char *pRelativeFilename, int nFileFraction,
                                       FileHash_t *pFileHash) { return k_eFileCRCStatus_CantOpenFile; }

#ifdef SUPPORT_PACKED_STORE

    virtual int SubmitThreadedMD5Request(uint8 *pubBuffer, int cubBuffer, int PackFileID, int nPackFileNumber,
                                         int nPackFileFraction) { return 0; }
