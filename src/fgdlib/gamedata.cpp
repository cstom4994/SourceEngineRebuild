...
//-----------------------------------------------------------------------------
GDclass *
GameData::BeginInstanceRemap(const char *pszClassName, const char *pszInstancePrefix, Vector &Origin, QAngle &Angle) {
...
bool GameData::RemapNameField(const char *pszInValue, char *pszOutValue, TNameFixup NameFixup) {
    ...
    if (pszInValue[0] &&
        pszInValue[0] != '@') {    // ! at the start of a value means it is global and should not be remaped
        switch (NameFixup) {
