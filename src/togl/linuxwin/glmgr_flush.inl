...
GLMContext::FindSamplerObject(const GLMTexSamplingParams &desiredParams) {
...
{
    Assert(m_drawingLang ==
           kGLMGLSL); // no support for ARB shaders right now (and NVidia reports that they aren't worth targeting under Windows/Linux for various reasons anyway)
    ...
    if (m_bDirtyPrograms) {
        ...
        if (pNewPair != m_pBoundPair) {
            ...
            if (!m_pBoundPair) {
                ...
                GL_BATCH_PERF(
                if (pNewPair->m_fragmentProg != m_pBoundPair->m_fragmentProg) m_FlushStats.m_nNewPS++; )
                GL_BATCH_PERF(
                if (pNewPair->m_vertexProg != m_pBoundPair->m_vertexProg) m_FlushStats.m_nNewVS++; )
            }
    ...
    if (m_bUseBoneUniformBuffers) {
        ...
        if (m_programParamsF[kGLMVertexProgram].m_dirtySlotHighWaterNonBone) {
            ...
            if ((vconstLoc >= 0) && (dirtySlotHighWater > firstDirtySlot)) {
                ...
                if (numSlots > 0) {
                    gGL->glUniform4fv(
                            m_pBoundPair->m_UniformBufferParams[kGLMVertexProgram][DXABSTRACT_VS_FIRST_BONE_SLOT],
                            numSlots,
        ...
        if (m_programParamsF[kGLMVertexProgram].m_dirtySlotHighWaterBone) {
            ...
            if (vconstBoneLoc >= 0) {
                ...
                int dirtySlotHighWaterBone = MIN(shaderSlotsBone,
        ...
        if (m_programParamsF[kGLMVertexProgram].m_dirtySlotHighWaterNonBone) {
            ...
            int firstDirtySlot = m_programParamsF[kGLMVertexProgram].m_firstDirtySlotNonBone;
            int dirtySlotHighWater = MIN(nMaxUsedShaderSlots,
                                         m_programParamsF[kGLMVertexProgram].m_dirtySlotHighWaterNonBone);
    ...
    if (m_pBoundPair->m_bHasBoolOrIntUniforms) {
        ...
        {
            const uint nLimit = MIN(CGLMShaderPair::cMaxVertexShaderBoolUniforms,
                                    m_programParamsB[kGLMVertexProgram].m_dirtySlotCount);
    ...
    if ((nCurTotalBufferRevision != m_CurAttribs.m_nTotalBufferRevision) ||
        ...
        for (int nMask = 1, nIndex = 0; nIndex < nMaxVertexAttributesToCheck; ++nIndex, nMask <<= 1) {
            ...
            SetBufAndVertexAttribPointer(nIndex, pBuf->GetHandle(),
                                         pStream->m_stride, pDeclElem->m_gldecl.m_datatype,
                                         pDeclElem->m_gldecl.m_normalized, pDeclElem->m_gldecl.m_nCompCount,
                                         reinterpret_cast< const GLvoid * >(
                                                 reinterpret_cast< int >( pBuf->m_pPseudoBuf ) + nBufOffset ),
                                         pBuf->m_nRevision);
