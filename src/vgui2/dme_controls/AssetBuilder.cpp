...
const char *pFileName, KeyValues
*pOkCommand )
{
...
{
pActionTarget->
PostMessage( pActionTarget
->

GetVPanel(), pOkCommand
...
bool
CAssetBuilderFrame::OnReadFileFromDisk(const char *pFileName, const char *pFileFormat, KeyValues *pContextKeyValues) {
    CDmElement * pRoot;
...
bool
CAssetBuilderFrame::OnWriteFileToDisk(const char *pFileName, const char *pFileFormat, KeyValues *pContextKeyValues) {
...
void CAssetBuilderFrame::OnFileSelected(KeyValues *kv) {
    ...
    if (pDialogKeys) {
        KeyValues * pOkCommand = new KeyValues("PerformFileNew", "makefileType",
...
void CAssetBuilderFrame::OnFileStateMachineFinished(KeyValues *pKeyValues) {
    ...
    if (pNewFile) {
        ...
        if (pKeyValues->GetInt("completionState", FileOpenStateMachine::IN_PROGRESS) ==
