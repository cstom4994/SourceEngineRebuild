...
void CDmePresetRemapPanel::RefreshPresetList() {
    ...
    bool bUseRemap = (pRemap && m_hSourceGroup.Get() &&
...
//-----------------------------------------------------------------------------
static int
__cdecl IndexSortFunc(vgui::ListPanel *pPanel, const vgui::ListPanelItem &item1, const vgui::ListPanelItem &item2) {
...
//-----------------------------------------------------------------------------
void
CDmePresetGroupEditorPanel::SetupFileOpenDialog(vgui::FileOpenDialog *pDialog, bool bOpenFile, const char *pFileFormat,
...
void CDmePresetGroupEditorPanel::OnAddPhonemeGroup() {
    vgui::InputDialog *pInput = new vgui::InputDialog(this, "Add Phoneme Preset Group",
    ...
    if (msgList.Count() > 0) {
        ...
        if (m_pPresetGroupPanel) {
            ...
            if (pPreset) {
                ...
                CDmePresetGroup *pDropGroup = pKeyValues ? GetElementKeyValue<CDmePresetGroup>(pKeyValues,
                                                                                               "presetGroup") : NULL;
...
void CDmePresetGroupListPanel::OnPanelDropped(CUtlVector<KeyValues *> &msgList) {
    ...
    if (pDragGroup) {
        CDmePresetGroup *pDropGroup = pKeyValues ? GetElementKeyValue<CDmePresetGroup>(pKeyValues, "presetGroup")
                                                 : NULL;
...
//-----------------------------------------------------------------------------
void
CDmePresetGroupListPanel::OnDroppablePanelPaint(CUtlVector<KeyValues *> &msglist, CUtlVector<Panel *> &dragPanels) {
    // Discover the cell the panel is over
