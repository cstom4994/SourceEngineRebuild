...
public:
    ...
    void
    DoModal(CDmeParticleSystemDefinition *pParticleSystem, ParticleFunctionType_t type, KeyValues *pContextKeyValues);
...
//-----------------------------------------------------------------------------
void
CParticleChildrenPickerFrame::DoModal(CDmeParticleSystemDefinition *pParticleSystem, KeyValues *pContextKeyValues) {
    CleanUpMessage();
...
void CParticleChildrenPickerFrame::OnCommand(const char *pCommand) {
    if (!Q_stricmp(pCommand, "Ok")) {
        ...
        CDmeParticleSystemDefinition *pParticleSystem = GetElementKeyValue<CDmeParticleSystemDefinition>(pKeyValues,
                                                                                                         "particleSystem");
...
void CParticleFunctionBrowser::SelectParticleFunction(CDmeParticleFunction *pFind) {
    ...
    for (int nItemID = m_pFunctionList->FirstItem();
         nItemID != m_pFunctionList->InvalidItemID(); nItemID = m_pFunctionList->NextItem(nItemID)) {
...
void CParticleFunctionBrowser::OnAdd() {
    if (m_FuncType != FUNCTION_CHILDREN) {
        ...
        CParticleChildrenPickerFrame *pPicker = new CParticleChildrenPickerFrame(this, "Select Child Particle Systems",
                                                                                 m_pQuery);
...
void CParticleFunctionBrowser::OnRename() {
    ...
    vgui::InputDialog *pInput = new vgui::InputDialog(this, "Rename Particle Function",
                                                      "Enter new name of particle function");
...
CParticleSystemPropertiesPanel::CParticleSystemPropertiesPanel(IParticleSystemPropertiesPanelQuery *pQuery,
    ...
    m_pFunctionTypeCombo = new vgui::ComboBox(pSplitterLeftSide, "FunctionTypeCombo", PARTICLE_FUNCTION_COUNT + 1,
                                              false);
