...
void CLocalizationDialog::OnFileSave() {
    if (g_pVGuiLocalize->SaveToFile(m_szFileName)) {
        ...
        // failure
        MessageBox *box = new MessageBox("Error during save - VLocalize", "Error - File was not successfully saved.",
                                         false);
        box->DoModal();
    }
}
...
void CLocalizationDialog::OnTokenCreated(const char *tokenName) {
    ...
    int itemId = m_pTokenList->AddItem(new KeyValues("LString", "Token", g_pVGuiLocalize->GetNameByIndex(idx)), idx,
                                       true, true);
