...
bool
CModWizardSubPanel_CopyFiles::BuildCopyFiles_R(const char *pSourceDir, const char *pMask, const char *pOutputDirName) {
    char mask[512];
...
void CModWizardSubPanel_CopyFiles::OnTick() {
    if (m_iCurCopyFile == -1) {
        ...
        CModWizardSubPanel_Intro *pIntro = dynamic_cast<CModWizardSubPanel_Intro *>(GetWizardPanel()->FindChildByName(
        ...
        while (nCopied < 10) {
            if (m_iCurCopyFile >= m_FileCopyInfos.Count()) {
                ...
                CModWizardSubPanel_Finished *pNextPanel = dynamic_cast<CModWizardSubPanel_Finished *>(GetWizardPanel()->FindChildByName(
...
bool
CModWizardSubPanel_CopyFiles_Source2009::HandleReplacements_GameProjectFiles(CFileCopyInfo *pInfo, bool &bErrorStatus) {
...
bool
CModWizardSubPanel_CopyFiles_Source2007::HandleReplacements_GameProjectFiles(CFileCopyInfo *pInfo, bool &bErrorStatus) {
    // Speed up the copy process a bit, if it's not a vcproj, get out.
    if (!IsVCProjFile(pInfo->m_InFilename))
        return false;
    ...
    // No 'dx8' shaders in orange box, at least any time soon, just keeping it cleaned up.
    if (Q_stricmp(pInfo->m_InFilename, "src_mod\\orangebox\\materialsystem\\stdshaders\\stdshader_dx9-2005.vcproj") ==
        0) {
        bErrorStatus = true;
        Q_snprintf(replaceWith, sizeof(replaceWith), "%s", m_OutModGamedirName);
        bErrorStatus = CopyWithReplacements(pInfo->m_InFilename, replacements, ARRAYSIZE(replacements), "%s",
...
bool
CModWizardSubPanel_CopyFiles_Source2006::HandleReplacements_GameProjectFiles(CFileCopyInfo *pInfo, bool &bErrorStatus) {
    // Speed up the copy process a bit, if it's not a vcproj, get out.
    if (!IsVCProjFile(pInfo->m_InFilename))
        return false;
...
bool
CModWizardSubPanel_CopyFiles_Source2009::HandleReplacements_TemplateOptions(CFileCopyInfo *pInfo, bool &bErrorStatus) {
    ...
    if (Q_stricmp(pInfo->m_InFilename, "src_mod\\source2009\\game\\shared\\sdk\\sdk_shareddefs.h") == 0) {
        ...
        CModWizardSubPanel_TemplateOptions *pTemplate = dynamic_cast<CModWizardSubPanel_TemplateOptions *>(GetWizardPanel()->FindChildByName(
...
bool
CModWizardSubPanel_CopyFiles_Source2007::HandleReplacements_TemplateOptions(CFileCopyInfo *pInfo, bool &bErrorStatus) {
    // Copy over the sdk_shareddefs.h with replacements for the defines.
