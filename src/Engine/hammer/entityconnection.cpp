...
//------------------------------------------------------------------------------
bool
CEntityConnection::ValidateTarget(const CMapEntityList *pEntityList, bool bVisibilityCheck, const char *pszTarget) {
...
void CEntityConnection::FindBadConnections(CMapEntity *pEntity, bool bVisibilityCheck,
    ...
    for (int i = 0; i < nConnCount; i++) {
        ...
        if (pConnection != NULL) {
            ...
            else if (!CEntityConnection::ValidateInput(pConnection->GetTargetName(), pConnection->GetInputName(),
                                                       true)) {
                BadConnectionList.AddToTail(pConnection);
            }
        }
...
//------------------------------------------------------------------------------
int
CEntityConnection::ValidateOutputConnections(CMapEntity *pEntity, bool bVisibilityCheck, bool bIgnoreHiddenTargets) {
    if (!pEntity) {
