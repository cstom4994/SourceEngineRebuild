...
//-----------------------------------------------------------------------------
bool
CMapDoc::FindEntitiesByKeyValue(CMapEntityList &Found, const char *pszKey, const char *pszValue, bool bVisiblesOnly) {
...
void CMapDoc::Postload(void) {
    ...
    if (CMapSolid::GetBadSolidCount() > 0) {
        char szError[80];
        sprintf(szError, "For your information, %d solid(s) were not loaded due to errors in the file.",
...
BOOL CMapDoc::OnSaveDocument(LPCTSTR lpszPathName) {
    ...
    if (access(lpszPathName, 0) != -1) {
        if (!CopyFile(lpszPathName, szFile, FALSE)) {
            ...
            wsprintf(szError,
                     "Hammer was unable to backup the existing file \"%s\" (Error: 0x%lX). Please verify that the there is space on the hard drive and that the path still exists.",
    ...
    if (!file.is_open()) {
        char szError[_MAX_PATH];
        wsprintf(szError,
                 "Hammer was unable to open the file \"%s\" for writing. Please verify that the file is writable and that the path exists.",
                 lpszPathName);
        AfxMessageBox(szError);
        return (FALSE);
    }
...
void CMapDoc::OnUpdateEditApplytexture(CCmdUI *pCmdUI) {
    pCmdUI->Enable(
            (m_pToolManager->GetActiveToolID() != TOOL_FACEEDIT_MATERIAL) && !GetMainWnd()->IsShellSessionActive());
}
    ...
    for (int i = 0; i < pSelList->Count(); i++) {
        ...
        if ((pobj->GetParent() != NULL) && (!IsWorldObject(pobj->GetParent()))) {
            if (GetMainWnd()->MessageBox(
                    "Some selected objects are part of an entity or belong to a group. Grouping them now will remove them from the entity or group! Continue?",
...
void CMapDoc::OnFileExport(void) {
    ...
    if (dlg.DoModal() == IDOK) {
        ...
        if (strFile.CompareNoCase(dlg.GetPathName()) == 0) {
            if (GetMainWnd()->MessageBox(
                    "You are about to export over your current work file. Some data loss will occur if you have any objects hidden. Continue?",
...
void CMapDoc::OnMapEntityGallery(void) {
    if (GetMainWnd()->MessageBox(
            "This will place one of every possible point entity in the current map! Performing this operation in an empty map is recommended. Continue?",
...
void CMapDoc::OnToolsCreateprefab(void) {
    if (m_pSelection->IsEmpty()) {
        AfxMessageBox(
                "This feature creates a prefab with the selected objects. You must select some objects before you can use it.",
