...
CMapFace *
CMapFrustum::CreateMapFace(const Vector &v1, const Vector &v2, const Vector &v3, const Vector &v4, float flAlpha) {
...
void CMapFrustum::BuildFrustumFaces(void) {
    ...
    for (int i = 0; i < 4; i++) {
        m_Faces.AddToTail(CreateMapFace(Vector(0, 0, 0), vNearFace[i], vNearFace[(i + 1) % 4], Vector(0, 0, 0),
                                        flOriginFacesAlpha));
    }
}
