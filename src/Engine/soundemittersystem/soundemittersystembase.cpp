...
void CSoundEmitterSystemBase::InternalModShutdown() {
    ...
    for (UtlHashHandle_t nIndex = m_Sounds.FirstHandle();
         nIndex != m_Sounds.InvalidHandle(); nIndex = m_Sounds.NextHandle(nIndex)) {
...
static void
SplitName(char const *input, int splitchar, int splitlen, char *before, int beforelen, char *after, int afterlen) {
    char const *in = input;
    char *out = before;
            ...
            if (pKeys->GetFirstSubKey()) {
                ...
                if (m_Sounds[lookup] != pEntry) {
                    if (bIsOverride || bRefresh) {
                        ...
                        pEntry->m_SoundParams.SetShouldPreload(bIsOverride ? bPreload
                                                                           : m_Sounds[lookup]->m_SoundParams.ShouldPreload()); // this gets handled by game code after initing.
