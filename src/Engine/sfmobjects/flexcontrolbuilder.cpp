...
//-----------------------------------------------------------------------------
void
CFlexControlBuilder::CleanupExistingFlexController(CDmeGameModel *pGameModel, CDmeGlobalFlexControllerOperator *pOp) {
...
void
CFlexControlBuilder::RemoveUnusedControlsAndChannels(CDmeAnimationSet *pAnimationSet, CDmeChannelsClip *pChannelsClip) {
...
void CFlexControlBuilder::GetExistingMonoLog(ExistingLogInfo_t *pExistingLog,
    ...
    if (ComputeChannelTimeTransform(&pExistingLog->m_GlobalOffset, &pExistingLog->m_flGlobalScale, pClip,
                                    pMonoChannel)) {
...
//-----------------------------------------------------------------------------
void
CFlexControlBuilder::ComputeChannelTimeTransform(DmeTime_t *pOffset, double *pScale, CDmeChannelsClip *pChannelsClip) {
...
static void
AddKeyToLogs(CDmeTypedLog<float> *valueLog, CDmeTypedLog<float> *balanceLog, const DmeTime_t &keyTime, float lval,
             float rval) {
...
static void
ConvertLRToVBLog(CDmeFloatLog *pValueLog, CDmeFloatLog *pBalanceLog, CDmeFloatLog *pLeftLog, CDmeFloatLog *pRightLog,
...
void CFlexControlBuilder::GetExistingStereoLog(ExistingLogInfo_t *pLogs, CDmeFilmClip *pClip,
    ...
    // Sometimes a stereo op will be read from by this channel
    CDmeBalanceToStereoCalculatorOperator *pStereoOp = CastElement<CDmeBalanceToStereoCalculatorOperator>(
    ...
    for (int gi = 0; gi < nTrackGroups; ++gi) {
        ...
        DMETRACKGROUP_FOREACH_CLIP_TYPE_START(CDmeChannelsClip, pTrackGroup, pTrack, pChannelsClip)
                    ...
                    for (int ci = 0; ci < nChannels; ++ci) {
                        ...
                        CDmeGlobalFlexControllerOperator *pOp = CastElement<CDmeGlobalFlexControllerOperator>(
...
void CFlexControlBuilder::BuildExistingFlexControlLogList(CDmeFilmClip *pCurrentClip, CDmeGameModel *pGameModel) {
    ...
    for (int i = 0; i < nCount; ++i) {
        ...
        if (info.m_bIsStereo) {
            ...
            CDmeGlobalFlexControllerOperator *pRightOp = pGameModel->FindGlobalFlexController(
                    pRightInfo->m_nGlobalIndex);
        ...
        if (info.m_bIsMulti) {
            ...
            CDmeGlobalFlexControllerOperator *pMultiOp = pGameModel->FindGlobalFlexController(
                    pMultiInfo->m_nGlobalIndex);
            if (pMultiOp) {
...
void CFlexControlBuilder::BuildFlexControllerOps(CDmeGameModel *pGameModel, CDmeChannelsClip *pChannelsClip,
    ...
    info.m_ppControlChannel[field] = pChannelsClip->CreatePassThruConnection(pName,
                                                                             info.m_pControl,
                                                                             flexInfo.m_pControlAttributeName,
...
void CFlexControlBuilder::BuildStereoFlexControllerOps(CDmeAnimationSet *pAnimationSet,
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        info.m_ppControlChannel[i] = pChannelsClip->CreatePassThruConnection(pChannelName,
                                                                             info.m_pControl, s_pStereoInputPrefix[i],
                                                                             pStereoCalcOp, s_pStereoInputPrefix[i]);
