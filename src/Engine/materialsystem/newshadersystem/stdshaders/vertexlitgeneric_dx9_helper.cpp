...
void InitVertexLitGeneric_DX9(CBaseVSShader *pShader, IMaterialVar **params, bool bVertexLitGeneric,
    ...
    if (info.m_nBaseTexture != -1 && params[info.m_nBaseTexture]->IsDefined()) {
        pShader->LoadTexture(info.m_nBaseTexture,
                             (info.m_nGammaColorRead != -1) && (params[info.m_nGammaColorRead]->GetIntValue() == 1) ? 0
                                                                                                                    : TEXTUREFLAGS_SRGB);
...
/*^*/ // 	printf("\t\t[1] snapshotting=%d  pContextData=%08x  pContextData->m_bMaterialVarsChanged=%d \n",(int)pShader->IsSnapshotting(), (int)pContextData, pContextData ? (int)pContextData->m_bMaterialVarsChanged : -1 );
        ...
        bool bHasSelfIllumFresnel =
                (!IsTextureSet(info.m_nDetail, params)) && (bHasSelfIllum) && (info.m_nSelfIllumFresnel != -1) &&
                (params[info.m_nSelfIllumFresnel]->GetIntValue() != 0);
...
/*^*/ // 		printf("\t\t[3] pShaderAPI && ( (! pContextData ) || ( pContextData->m_bMaterialVarsChanged ) )  TRUE \n");
            ...
            if (bDistanceAlpha) {
                ...
                if (bScaleEdges || bScaleOutline) {
                    ...
                    flResScale = max(0.5f, max(1024.0f / (float)nWidth, 768.0f / (float)nHeight));
            ...
            if (bHasEnvmap) {
                pContextData->m_SemiStaticCmdsOut.SetEnvMapTintPixelShaderDynamicStateGammaToLinear(0,
                                                                                                    info.m_nEnvmapTint,
                                                                                                    fTintReplaceFactor);
            } else {
            ...
            if (bHasSelfIllumFresnel && (!bHasFlashlight || IsX360())) {
                ...
                float flMin = IS_PARAM_DEFINED(info.m_nSelfIllumFresnelMinMaxExp)
                              ? params[info.m_nSelfIllumFresnelMinMaxExp]->GetVecValue()[0] : 0.0f;
                float flMax = IS_PARAM_DEFINED(info.m_nSelfIllumFresnelMinMaxExp)
                              ? params[info.m_nSelfIllumFresnelMinMaxExp]->GetVecValue()[1] : 1.0f;
                float flExp = IS_PARAM_DEFINED(info.m_nSelfIllumFresnelMinMaxExp)
                              ? params[info.m_nSelfIllumFresnelMinMaxExp]->GetVecValue()[2] : 1.0f;
...
//					SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo() );
...
#ifndef _X360
            {
                ...
                SET_DYNAMIC_VERTEX_SHADER_COMBO(LIGHTING_PREVIEW,
                                                pShaderAPI->GetIntRenderingParameter(
...
//				SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo() );
        ...
        if ((info.m_nHDRColorScale != -1) && pShader->IsHDREnabled()) {
            pShader->SetModulationPixelShaderDynamicState_LinearColorSpace_LinearScale(1,
...
void DrawVertexLitGeneric_DX9(CBaseVSShader *pShader, IMaterialVar **params, IShaderDynamicAPI *pShaderAPI,
    ...
    DrawVertexLitGeneric_DX9_Internal(pShader, params, pShaderAPI,
                                      pShaderShadow, bVertexLitGeneric, bHasFlashlight, info, vertexCompression,
                                      pContextDataPtr);
}
