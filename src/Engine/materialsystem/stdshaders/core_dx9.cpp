...
#include "pp_core_vs20.inc"
#include "pp_core_ps20.inc"
#include "pp_core_ps20b.inc"

#define MAXBLUR 1

DEFINE_FALLBACK_SHADER( SDK_Core, PP_Core_DX90 )
DEFINE_FALLBACK_SHADER( PP_Core, PP_Core_DX90 )

BEGIN_VS_SHADER( PP_Core_DX90, 
			  "Help for Core" )
	...
	BEGIN_SHADER_PARAMS
		...
		SHADER_PARAM(ENTITYORIGIN, SHADER_PARAM_TYPE_VEC3, "[2688 12139 5170]", "");
		SHADER_PARAM(DIAMETER, SHADER_PARAM_TYPE_FLOAT, "430", "");
	END_SHADER_PARAMS
	SHADER_INIT_PARAMS()
	{
		...
		{
			float vec[] = { 2688.0f, 12139.0f, 5170.0f };
			params[ENTITYORIGIN]->SetVecValue(vec, 3);
		}
		if (!params[DIAMETER]->IsDefined())
		{
			params[DIAMETER]->SetFloatValue(430);
		}
	}
		...
		IShaderDynamicAPI* pShaderAPI, int nPass, VertexCompressionType_t vertexCompression ) 
	{
		...
		SHADOW_STATE
		{
			...
			DECLARE_STATIC_VERTEX_SHADER( pp_core_vs20 );
			SET_STATIC_VERTEX_SHADER_COMBO( MODEL,  bIsModel );
			SET_STATIC_VERTEX_SHADER( pp_core_vs20 );

			if ( g_pHardwareConfig->SupportsPixelShaders_2_b() )
			{
				DECLARE_STATIC_PIXEL_SHADER( pp_core_ps20b );
				...
				SET_STATIC_PIXEL_SHADER( pp_core_ps20b );
			}
			else
			{
				DECLARE_STATIC_PIXEL_SHADER( pp_core_ps20 );
				SET_STATIC_PIXEL_SHADER_COMBO( CUBEMAP,  bHasEnvmap && ( nPass == 1 ) );
				SET_STATIC_PIXEL_SHADER_COMBO( FLOWMAP, bHasFlowmap );
				SET_STATIC_PIXEL_SHADER_COMBO( CORECOLORTEXTURE, bHasCoreColorTexture && ( nPass == 0 ) );
				SET_STATIC_PIXEL_SHADER_COMBO( REFRACT, nPass == 0 );
				SET_STATIC_PIXEL_SHADER( pp_core_ps20 );
			}
		...
		DYNAMIC_STATE
		{
			...
			DECLARE_DYNAMIC_VERTEX_SHADER( pp_core_vs20 );
			SET_DYNAMIC_VERTEX_SHADER_COMBO( SKINNING,  pShaderAPI->GetCurrentNumBones() > 0 );
			SET_DYNAMIC_VERTEX_SHADER_COMBO( COMPRESSED_VERTS, (int)vertexCompression );
			SET_DYNAMIC_VERTEX_SHADER( pp_core_vs20 );

			if ( g_pHardwareConfig->SupportsPixelShaders_2_b() )
			{
				DECLARE_DYNAMIC_PIXEL_SHADER( pp_core_ps20b );
				SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo() );
				SET_DYNAMIC_PIXEL_SHADER( pp_core_ps20b );
			}
			else
			{
				DECLARE_DYNAMIC_PIXEL_SHADER( pp_core_ps20 );
				SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo() );
				SET_DYNAMIC_PIXEL_SHADER( pp_core_ps20 );
			}
			...
			if( bHasFlowmap )
			{
				...
				float c10[4];
				params[ENTITYORIGIN]->GetVecValue(c10, 3);
				c10[3] = params[DIAMETER]->GetFloatValue();
