...
#include "pp_downsample_ps20.inc"
#include "pp_downsample_ps20b.inc"

// NOTE: This has to be the last file included!
#include "tier0/memdbgon.h"


BEGIN_VS_SHADER_FLAGS( PP_Downsample, "Help for Downsample", SHADER_NOT_EDITABLE )
	BEGIN_SHADER_PARAMS
	END_SHADER_PARAMS
	...
	SHADER_DRAW
	{
		...
		SHADOW_STATE
		{
			...
			{
				DECLARE_STATIC_PIXEL_SHADER( pp_downsample_ps20b );
				SET_STATIC_PIXEL_SHADER(pp_downsample_ps20b );
			}
			else
			{
				DECLARE_STATIC_PIXEL_SHADER(pp_downsample_ps20 );
				SET_STATIC_PIXEL_SHADER(pp_downsample_ps20 );
			}
		}
...
//			v[0] = 1.0f / MAX_HDR_OVERBRIGHT;
			...
			if( g_pHardwareConfig->SupportsPixelShaders_2_b() )
			{
				DECLARE_DYNAMIC_PIXEL_SHADER(pp_downsample_ps20b );
				SET_DYNAMIC_PIXEL_SHADER(pp_downsample_ps20b );
			}
			else
			{
				DECLARE_DYNAMIC_PIXEL_SHADER(pp_downsample_ps20 );
				SET_DYNAMIC_PIXEL_SHADER(pp_downsample_ps20 );
			}
		}
		Draw();
	}
