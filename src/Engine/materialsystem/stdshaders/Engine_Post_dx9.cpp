...
#include "pp_engine_post_ps20.inc"
#include "pp_engine_post_ps20b.inc"
...
BEGIN_VS_SHADER_FLAGS( PP_Engine_Post_dx9, "Engine post-processing effects (software anti-aliasing, bloom, color-correction", SHADER_NOT_EDITABLE )
	...
	SHADER_DRAW
	{
		...
		SHADOW_STATE
		{
			...
			{
				DECLARE_STATIC_PIXEL_SHADER( pp_engine_post_ps20b );
				SET_STATIC_PIXEL_SHADER_COMBO( TOOL_MODE, bToolMode );
				SET_STATIC_PIXEL_SHADER_COMBO( DEPTH_BLUR_ENABLE, bDepthBlurEnable );
				SET_STATIC_PIXEL_SHADER( pp_engine_post_ps20b );
			}
			else
			{
				DECLARE_STATIC_PIXEL_SHADER( pp_engine_post_ps20 );
				SET_STATIC_PIXEL_SHADER_COMBO( TOOL_MODE, bToolMode );
				SET_STATIC_PIXEL_SHADER_COMBO( DEPTH_BLUR_ENABLE, false );
				SET_STATIC_PIXEL_SHADER( pp_engine_post_ps20 );
			}
		}
		DYNAMIC_STATE
		{
			...
			if ( bVomitEnable )
			{
				...
				vViewportMad[0] = ( float )nRtWidth / ( float )viewport.m_nWidth;
				vViewportMad[1] = ( float )nRtHeight / ( float )viewport.m_nHeight;
				vViewportMad[2] = -( float )viewport.m_nTopLeftX / ( float )viewport.m_nWidth;
				vViewportMad[3] = -( float )viewport.m_nTopLeftY / ( float )viewport.m_nHeight;
			...
			if ( g_pHardwareConfig->SupportsPixelShaders_2_b() )
			{
				DECLARE_DYNAMIC_PIXEL_SHADER( pp_engine_post_ps20b );
			...
			{
				DECLARE_DYNAMIC_PIXEL_SHADER( pp_engine_post_ps20 );
				SET_DYNAMIC_PIXEL_SHADER_COMBO( AA_ENABLE,						aaEnabled );
				SET_DYNAMIC_PIXEL_SHADER_COMBO( COL_CORRECT_NUM_LOOKUPS,		colCorrectNumLookups );
				SET_DYNAMIC_PIXEL_SHADER_COMBO( VOMIT_ENABLE,					bVomitEnable );
				SET_DYNAMIC_PIXEL_SHADER( pp_engine_post_ps20 );
			}
