...
#include "pp_downsample_nohdr_ps20.inc"
#include "pp_downsample_nohdr_ps20b.inc"
...
BEGIN_VS_SHADER_FLAGS( PP_Downsample_NOHDR, "Help for Downsample_nohdr", SHADER_NOT_EDITABLE )
	...
	SHADER_DRAW
	{
		...
		SHADOW_STATE
		{
			...
			pShaderShadow->SetVertexShader( "pp_downsample_vs20", 0 );
			
			if( g_pHardwareConfig->SupportsPixelShaders_2_b() || g_pHardwareConfig->ShouldAlwaysUseShaderModel2bShaders() )
			{
				DECLARE_STATIC_PIXEL_SHADER( pp_downsample_nohdr_ps20b );
				SET_STATIC_PIXEL_SHADER_COMBO( BLOOMTYPE, params[BLOOMTYPE]->GetIntValue() + 1 );
				SET_STATIC_PIXEL_SHADER_COMBO( SRGB_INPUT_ADAPTER, bForceSRGBReadAndWrite );
				SET_STATIC_PIXEL_SHADER_COMBO( PS3REGCOUNT48, 0 );
				SET_STATIC_PIXEL_SHADER(pp_downsample_nohdr_ps20b );
			}
			else
			{
				DECLARE_STATIC_PIXEL_SHADER(pp_downsample_nohdr_ps20 );
				SET_STATIC_PIXEL_SHADER_COMBO( BLOOMTYPE, params[BLOOMTYPE]->GetIntValue() + 1 );
				//SET_STATIC_PIXEL_SHADER_COMBO( SRGB_INPUT_ADAPTER, bForceSRGBReadAndWrite );
				SET_STATIC_PIXEL_SHADER_COMBO( PS3REGCOUNT48, 0 );
				SET_STATIC_PIXEL_SHADER(pp_downsample_nohdr_ps20 );
			}
		}
		...
		DYNAMIC_STATE
		{
			...
			if( g_pHardwareConfig->SupportsPixelShaders_2_b() || g_pHardwareConfig->ShouldAlwaysUseShaderModel2bShaders() )
			{
				...
				DECLARE_DYNAMIC_PIXEL_SHADER(pp_downsample_nohdr_ps20b );
				SET_DYNAMIC_PIXEL_SHADER_COMBO( FLOAT_BACK_BUFFER, floatBackBuffer );
				SET_DYNAMIC_PIXEL_SHADER(pp_downsample_nohdr_ps20b );
			}
			else
			{
				DECLARE_DYNAMIC_PIXEL_SHADER(pp_downsample_nohdr_ps20 );
				SET_DYNAMIC_PIXEL_SHADER(pp_downsample_nohdr_ps20 );
			}
		}
		Draw();
	}
