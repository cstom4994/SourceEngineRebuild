...
//-----------------------------------------------------------------------------
void
CMatRenderContextBase::PushRenderTargetAndViewport(ITexture *pTexture, int nViewX, int nViewY, int nViewW, int nViewH) {
...
//-----------------------------------------------------------------------------
void
CMatRenderContextBase::PushRenderTargetAndViewport(ITexture *pTexture, ITexture *pDepthTexture, int nViewX, int nViewY,
    ...
    if ((HardwareConfig()->GetHDRType() != HDR_TYPE_NONE) && (m_FrameTime > 0.0f)) {
        ...
        if (rate == 0.0f) // Zero indicates instantaneous tonemap scaling
        {
            ...
            if (mat_tonemap_algorithm.GetInt() == 1) {
                ...
                flRateTimesTime = min(flRateTimesTime, (1.0f / 16.0f) *
                                                       0.25f); // 16 is number of HDR sample bins defined in viewpostprocess.cpp
                //Warning( " --> %.4f\n", flRateTimesTime );
            }
...
void
CMatRenderContext::PerspectiveOffCenterX(double flFovX, double flAspect, double flZNear, double flZFar, double bottom,
...
IMesh *
CMatRenderContext::GetDynamicMesh(bool buffered, IMesh *pVertexOverride, IMesh *pIndexOverride, IMaterial *pAutoBind) {
...
void CMatRenderContext::CopyRenderTargetToTextureEx(ITexture *pTexture, int nRenderTargetID, Rect_t *pSrcRect,
    ...
    if (IsPC() || !IsX360()) {
        ...
        ITextureInternal *pFullFrameFB = (ITextureInternal *) GetMaterialSystem()->FindTexture("_rt_FullFrameFB",
...
void
CMatRenderContext::AsyncMap(ITextureInternal *pTexToMap, IAsyncTextureOperationReceiver *pRecipient, void *pExtraArgs) {
    if (g_pMaterialSystem->GetThreadMode() == MATERIAL_SINGLE_THREADED) {
