...
#include "../stdshaders_old/common_hlsl_cpp_consts.h" // hack hack hack!
#include "KeyValues.h"
...
static void
CommitSetViewports(IDirect3DDevice9 *pDevice, const DynamicState_t &desiredState, DynamicState_t &currentState,
                   bool bForce);
...
public:
    ...
    virtual void
    ClearBuffers(bool bClearColor, bool bClearDepth, bool bClearStencil, int renderTargetWidth, int renderTargetHeight);
...
public:
    ...
    void
    PerspectiveOffCenterX(double fovx, double aspect, double zNear, double zFar, double bottom, double top, double left,
                          double right);
    ...
    // Set to SHADER_RENDERTARGET_BACKBUFFER if you want to use the regular framebuffer.
    void
    SetRenderTargetEx(int nRenderTargetID, ShaderAPITextureHandle_t colorTextureHandle = SHADER_RENDERTARGET_BACKBUFFER,
                      ShaderAPITextureHandle_t depthTextureHandle = SHADER_RENDERTARGET_DEPTHBUFFER);
    ...
    virtual void
    ReadPixels(Rect_t *pSrcRect, Rect_t *pDstRect, unsigned char *data, ImageFormat dstFormat, int nDstStride);
    ...
    void
    CopyRenderTargetToTextureEx(ShaderAPITextureHandle_t textureHandle, int nRenderTargetID, Rect_t *pSrcRect = NULL,
                                Rect_t *pDstRect = NULL);

    void
    CopyTextureToRenderTargetEx(int nRenderTargetID, ShaderAPITextureHandle_t textureHandle, Rect_t *pSrcRect = NULL,
                                Rect_t *pDstRect = NULL);
    ...
    virtual void
    LockRect(void **pOutBits, int *pOutPitch, ShaderAPITextureHandle_t texHandle, int mipmap, int x, int y, int w,
             int h, bool bWrite, bool bRead);
    ...
    // Scissor Rect
    void
    SetScissorRect(const int nLeft, const int nTop, const int nRight, const int nBottom, const bool bEnableScissor);
...
// ------------ New Vertex/Index Buffer interface ----------------------------
    void
    BindVertexBuffer(int streamID, IVertexBuffer *pVertexBuffer, int nOffsetInBytes, int nFirstVertex, int nVertexCount,
...
private:
    ...
    void CopyBitsFromHostSurface(IDirect3DSurface *pSurfaceBits,
                                 const Rect_t &dstRect, unsigned char *pData, ImageFormat srcFormat,
...
//-----------------------------------------------------------------------------
static void
CommitSetScissorRect(IDirect3DDevice9 *pDevice, const DynamicState_t &desiredState, DynamicState_t &currentState,
                     bool bForce) {
...
//-----------------------------------------------------------------------------
static void
CommitVertexTextures(IDirect3DDevice9 *pDevice, const DynamicState_t &desiredState, DynamicState_t &currentState,
                     bool bForce) {
    ...
    for (int i = 0; i < nCount; ++i) {
        ...
        Texture_t *pTexture = (textureHandle != INVALID_SHADERAPI_TEXTURE_HANDLE) ? &g_ShaderAPIDX8.GetTexture(
                textureHandle) : NULL;
...
// 			RECORD_INT( 0 );
            ...
            IDirect3DBaseTexture *pD3DTexture = (textureHandle >= 0) ? g_ShaderAPIDX8.GetD3DTexture(textureHandle)
                                                                     : NULL;
...
//-----------------------------------------------------------------------------
inline void
CShaderAPIDx8::SetVertexDecl(VertexFormat_t vertexFormat, bool bHasColorMesh, bool bUsingFlex, bool bUsingMorph) {
...
ConVar r_reduce_frame_latency("r_reduce_frame_latency", "0", 0,
                              "Reduces frame latency at the cost of rendering performance.");
...
void CShaderAPIDx8::ExportTextureList() {
    ...
    if (IsPC() || !IsX360()) {
        ...
        for (ShaderAPITextureHandle_t hTexture = m_Textures.Head();
             hTexture != m_Textures.InvalidIndex(); hTexture = m_Textures.Next(hTexture)) {
    ...
    for (int i = 0; i < nCount; ++i) {
        ...
        if (!bDifferentValue) {
            if (nCommitCount != 0) {
                ...
                pDevice->SetVertexShaderConstantF(nFirstCommit,
...
#define ALLOW_FOR_FASTCLIPDUMPS 0
    ...
    if ((clipPlane.z * clipPlane.w) <=
        -0.4f) // a plane with (z*w) > -0.4 at this point is behind the camera and will cause graphical glitches. Toss it. (0.4 found through experimentation)
    {
#if (ALLOW_FOR_FASTCLIPDUMPS == 1)
        if( shader_dumpfastclipprojectioncoords.GetBool() )
            ...
            if (g_pHardwareConfig->NeedsShaderSRGBConversion()) {
                ...
                for (int i = 0; i != 3; ++i)
                    fogColor[i] = GammaToLinear_HardwareSpecific(
                            m_DynamicState.m_PixelFogColor[i]); //this is how water fog color has always been setup in the past
            }
...
int CShaderAPIDx8::GetPixelFogCombo(void) {
    if ((m_SceneFogMode != MATERIAL_FOG_NONE) && ShouldUsePixelFogForMode(m_SceneFogMode))
        return m_SceneFogMode -
               1; //PIXELFOGTYPE dynamic combos are shifted down one. MATERIAL_FOG_NONE is simulated by range fog with rigged parameters. Gets rid of a dynamic combo across the ps2x set
    else
        return MATERIAL_FOG_NONE;
}
    ...
    switch (m_SceneFogMode) {
        case MATERIAL_FOG_LINEAR:
            return (g_pHardwareConfig->SupportsPixelShaders_2_b() &&
                    //lightmappedgeneric_ps20.fxc can't handle the instruction count
...
FORCEINLINE void CShaderAPIDx8::SetVertexShaderConstantInternal(int var, float const *pVec, int numVecs, bool bForce) {
    ...
    if (IsPC() || IsPS3()) {
        ...
        if (!bForce &&
            memcmp(pVec, &m_DynamicState.m_pVectorVertexShaderConstant[var], numVecs * 4 * sizeof(float)) == 0)
            return;
...
FORCEINLINE void
CShaderAPIDx8::SetPixelShaderConstantInternal(int nStartConst, float const *pValues, int nNumConsts, bool bForce) {
    ...
    if (IsPC() || IsPS3()) {
        if (!bForce) {
            ...
            while (nNumConsts && (pSrc[0] == pDst[0]) && (pSrc[1] == pDst[1]) && (pSrc[2] == pDst[2]) &&
                   (pSrc[3] == pDst[3])) {
                pSrc += 4;
                pDst += 4;
                nNumConsts--;
    ...
    if (IsPC() || !IsX360()) {
        if (isCubeMap) {
            ...
            for (int i = 0; i < numLevels; ++i) {
                ...
                Assert(!FAILED(hr));
                textureData.m_SizeBytes += 6 * ImageLoader::GetMemRequired(desc.Width, desc.Height, 1,
            ...
            for (int i = 0; i < numLevels; ++i) {
                ...
                HRESULT hr = pTex->GetLevelDesc(i, &desc);
                Assert(!FAILED(hr));
                textureData.m_SizeBytes += ImageLoader::GetMemRequired(desc.Width, desc.Height, 1,
                                                                       textureData.GetImageFormat(), false);
                textureData.m_SizeTexels += desc.Width * desc.Height;
            }
        }
...
void CShaderAPIDx8::CreateTextureHandles(ShaderAPITextureHandle_t *pHandles, int nCount) {
    bool bReuseHandles = false;
...
void CShaderAPIDx8::CreateTextures(
    ...
    usSetFlags |= (IsPosix() || (creationFlags & (TEXTURE_CREATE_DYNAMIC | TEXTURE_CREATE_MANAGED)))
...
void CShaderAPIDx8::SetupTextureGroup(ShaderAPITextureHandle_t hTexture, const char *pTextureGroupName) {
    ...
    pTexture->m_pTextureGroupCounterGlobal = g_VProfCurrentProfile.FindOrCreateCounter(counterName,
    ...
    pTexture->m_pTextureGroupCounterFrame = g_VProfCurrentProfile.FindOrCreateCounter(counterName,
                                                                                      COUNTER_GROUP_TEXTURE_PER_FRAME);
#else
...
void CShaderAPIDx8::TexMagFilter(ShaderTexFilterMode_t texFilterMode) {
    ...
    switch (texFilterMode) {
        ...
        case SHADER_TEXFILTERMODE_LINEAR_MIPMAP_LINEAR:
            ...
            break;
        case SHADER_TEXFILTERMODE_ANISOTROPIC:
            GetTexture(hModifyTexture).m_MagFilter = g_pHardwareConfig->Caps().m_bSupportsMagAnisotropicFiltering
...
void CShaderAPIDx8::CopyRenderTargetToTextureEx(ShaderAPITextureHandle_t textureHandle, int nRenderTargetID,
    ...
    LOCK_SHADERAPI();VPROF_BUDGET("CShaderAPIDx8::CopyRenderTargetToTexture", "Refraction overhead");
...
//-------------------------------------------------------------------------
void
CShaderAPIDx8::LockRect(void **pOutBits, int *pOutPitch, ShaderAPITextureHandle_t texHandle, int mipmap, int x, int y,
...
void
CShaderAPIDx8::PerspectiveOffCenterX(double fovx, double aspect, double zNear, double zFar, double bottom, double top,
...
void CShaderAPIDx8::SetVertexShaderViewProj() {
    ...
    if (g_pHardwareConfig->Caps().m_SupportsPixelShaders) {
        ...
        transpose = GetTransform(MATERIAL_VIEW) *
    ...
    if (nNumLights > 0) {
        ...
        if (nNumLights > 1)                                                            // At least two lights
        {
            ...
            if (nNumLights > 2)                                                        // At least three lights
            {
                ...
                if (nNumLights >
                    3)                                                    // At least four lights (our current max)
                {
...
#endif
    {
        ...
        if (bUsingFixedFunction) {
            if (m_DynamicState.m_bUserClipTransformOverride) {
                ...
                worldToProjectionInvTrans = viewInverse *
                                            GetUserClipTransform(); //taking a cue from the multiplication below, multiplication goes left into right
...
CShaderAPIDx8::GetFlashlightStateEx(VMatrix &worldToTexture, ITexture **ppFlashlightDepthTexture) const {
    worldToTexture = m_FlashlightWorldToTexture;
