...
void FX_MuzzleEffectAttached(
    ...
    pEmitter->SetString("material",
                        "effects/muzzleflash2"); // FIXME - create DmeMultiMaterialSpriteEmitter to support the 4 materials of muzzleflash
...
FX_Smoke(const Vector &origin, const Vector &velocity, float scale, int numParticles, float flDietime,
         unsigned char *pColor, int iAlpha, const char *pMaterial, float flRoll, float flRollDelta) {
...
//-----------------------------------------------------------------------------
void
FX_BuildSmoke(Vector &vecOrigin, QAngle &vecAngles, ClientEntityHandle_t hEntity, int nAttachment, float flLifeTime,
    ...
    for (int i = 0; i < iTotalBeams; i++) {
        ...
        if (tr.fraction < 1.0) {
            if (!EffectOccluded(tr.endpos, 0)) {
                ...
                SimpleParticle *pParticle;
                pParticle = (SimpleParticle *) pSimple->AddParticle(sizeof(SimpleParticle),
                                                                    pSimple->GetPMaterial("effects/tesla_glow_noz"),
                                                                    vecFlash);
                if (pParticle != NULL) {
                    pParticle->m_flLifetime = 0.0f;
