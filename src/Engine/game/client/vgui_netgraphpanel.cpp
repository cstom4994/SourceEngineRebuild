...
public:
    ...
    inline void
    DrawLine2(vrect_t *rect, unsigned char *color, unsigned char *color2, unsigned char alpha, unsigned char alpha2);
...
//-----------------------------------------------------------------------------
void
CNetGraphPanel::DrawTextFields(int graphvalue, int x, int y, int w, netbandwidthgraph_t *graph, cmdinfo_t *cmdinfo) {
...
void CNetGraphPanel::DrawHatches(int x, int y, int maxmsgbytes) {
    ...
    for (starty = rcHatch.y;
         rcHatch.y > 0 && ((starty - rcHatch.y) * net_scale.GetFloat() < (maxmsgbytes + 50)); rcHatch.y -= ystep) {
