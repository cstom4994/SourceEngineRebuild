...
ConVar vr_moveaim_mode("vr_moveaim_mode", "3", FCVAR_ARCHIVE,
                       "0=move+shoot from face. 1=move with torso. 2,3,4=shoot with face+mouse cursor. 5+ are probably not that useful.");
ConVar vr_moveaim_mode_zoom("vr_moveaim_mode_zoom", "3", FCVAR_ARCHIVE,
                            "0=move+shoot from face. 1=move with torso. 2,3,4=shoot with face+mouse cursor. 5+ are probably not that useful.");
...
ConVar vr_zoom_scope_scale("vr_zoom_scope_scale", "6.0", 0,
                           "Something to do with the default scope HUD overlay size.");        // Horrible hack - should work out the math properly, but we need to ship.
...
void CClientVirtualReality::DrawMainMenu() {
    ...
    for (int nView = STEREO_EYE_LEFT; nView <= STEREO_EYE_RIGHT; nView++) {
        ...
        ITexture *pColor = g_pSourceVR->GetRenderTarget((ISourceVirtualReality::VREye) (nView - 1),
                                                        ISourceVirtualReality::RT_Color);
        ITexture *pDepth = g_pSourceVR->GetRenderTarget((ISourceVirtualReality::VREye) (nView - 1),
    ...
    switch (m_hmmMovementActual) {
        ...
        case HMM_SHOOTBOUNDEDMOUSE_LOOKFACE_MOVEMOUSE: {
            ...
            *pNewMotion = curMotion;
            ...
            break;
        case HMM_SHOOTMOUSE_MOVEFACE: {
            (*pNewAngles)[PITCH] = clamp((*pNewAngles)[PITCH], m_PlayerViewAngle[PITCH] - 15.f,
...
void CClientVirtualReality::OverrideViewModelTransform(Vector &vmorigin, QAngle &vmangles, bool bUseLargeOverride) {
    ...
    float fForward = bUseLargeOverride ? vr_viewmodel_offset_forward_large.GetFloat()
...
void CClientVirtualReality::PostProcessFrame(StereoEye_t eEye) {
    ...
    g_pSourceVR->DoDistortionProcessing(
            eEye == STEREO_EYE_LEFT ? ISourceVirtualReality::VREye_Left : ISourceVirtualReality::VREye_Right);
}
...
void CClientVirtualReality::OverlayHUDQuadWithUndistort(const CViewSetup &eyeView, bool bDoUndistort, bool bBlackout,
    ...
    ISourceVirtualReality::VREye sourceVrEye = (eyeView.m_eStereoEye == STEREO_EYE_LEFT)
                                               ? ISourceVirtualReality::VREye_Left : ISourceVirtualReality::VREye_Right;
