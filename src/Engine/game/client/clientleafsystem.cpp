...
public:
    ...
    virtual void
    ComputeTranslucentRenderLeaf(int count, const LeafIndex_t *pLeafList, const LeafFogVolume_t *pLeafFogVolumeList,
                                 int frameNumber, int viewID);

    virtual void
    CollateViewModelRenderables(CUtlVector<IClientRenderable *> &opaque, CUtlVector<IClientRenderable *> &translucent);
    ...
    void
    AddRenderableToLeaves(ClientRenderHandle_t handle, int nLeafCount, unsigned short *pLeaves, bool bReceiveShadows);
...
public:
    ...
    virtual bool
    GetRenderableLeaf(ClientRenderHandle_t handle, int *pOutLeaf, const int *pInIterator = 0, int *pOutIterator = 0);
...
void DefaultRenderBoundsWorldspace(IClientRenderable *pRenderable, Vector &absMins, Vector &absMaxs) {
    ...
    if (pEnt && (pParent = pEnt->GetMoveParent()) != NULL &&
        (pEnt->GetParentAttachment() > 0 || pEnt->IsFollowingEntity())) {
...
void CClientLeafSystem::ComputeAllBounds(void) {
    ...
    if (bThreaded) {
        ParallelProcess("CClientLeafSystem::ComputeAllBounds", renderablesToUpdate.Base(), renderablesToUpdate.Count(),
                        this, &CClientLeafSystem::ComputeBounds);
    ...
    while (m_DirtyRenderables.Count()) {
        ...
        if (!bThreaded) {
            ...
            for (i = nDirty; --i >= 0;) {
                ...
                if (info.m_vecPendingBloatedAbsMins != info.m_vecBloatedAbsMins ||
                    info.m_vecPendingBloatedAbsMaxs != info.m_vecBloatedAbsMaxs) {
                    // Update position in leaf system
                    RemoveFromTree(handle);
                    if (bDebugLeafSystem) {
                        debugoverlay->AddBoxOverlay(vec3_origin, info.m_vecPendingBloatedAbsMins,
                                                    info.m_vecPendingBloatedAbsMaxs, QAngle(0, 0, 0), 0, 255, 0, 0, 0);
                    }
            ...
            if (nDirty) {
                ...
                ClientRenderHandle_t *pDirtyRenderables = (ClientRenderHandle_t *) alloca(
                        sizeof(ClientRenderHandle_t) * nDirty);
                memcpy(pDirtyRenderables, m_DirtyRenderables.Base(), sizeof(ClientRenderHandle_t) * nDirty);
                ParallelProcess("CClientLeafSystem::RecomputeRenderableLeaves", pDirtyRenderables, nDirty, this,
        ...
        if (m_DeferredInserts.Count()) {
            ...
            while (m_DeferredInserts.PopItem(&enumResultList)) {
                m_ShadowEnum++;
                const bool bReceiveShadows = ShouldRenderableReceiveShadow(enumResultList.handle,
...
int CClientLeafSystem::GetRenderableLeaves(ClientRenderHandle_t handle, int leaves[128]) {
    ...
    for (int i = m_RenderablesInLeaf.FirstBucket(handle);
         i != m_RenderablesInLeaf.InvalidIndex(); i = m_RenderablesInLeaf.NextBucket(i)) {
...
bool CClientLeafSystem::EnumerateLeaf(int leaf, int context) {
    ...
    if (ThreadInMainThread()) {
        const bool bReceiveShadows = ShouldRenderableReceiveShadow(pList->handle,
                                                                   SHADOW_FLAGS_PROJECTED_TEXTURE_TYPE_MASK);
...
void CClientLeafSystem::InsertIntoTree(ClientRenderHandle_t &handle) {
    ...
    int leafCount = pQuery->ListLeavesInBox(info.m_vecBloatedAbsMins, info.m_vecBloatedAbsMaxs, leafList,
                                            ARRAYSIZE(leafList));
...
void CClientLeafSystem::ComputeTranslucentRenderLeaf(int count, const LeafIndex_t *pLeafList,
                                                     ...
                                                     int viewID) {
    ASSERT_NO_REENTRY();VPROF_BUDGET("CClientLeafSystem::ComputeTranslucentRenderLeaf", "ComputeTranslucentRenderLeaf");
...
static RenderGroup_t DetectBucketedRenderGroup(RenderGroup_t group, float fDimension) {
    ...
    RenderGroup_t bucketedGroup = RenderGroup_t(
            group - ((RENDER_GROUP_CFG_NUM_OPAQUE_ENT_BUCKETS - 1) - bucketedGroupIndex) * 2);
                    ...
                    if (info.m_bDrawTranslucentObjects)    // Don't draw translucent objects into shadow depth maps
                    {
                        ...
                        if (pRenderable->GetFxBlend() > 0) {
                            AddRenderableToRenderList(*info.m_pRenderList, pRenderable,
                                                      worldListLeafIndex, RENDER_GROUP_TRANSLUCENT_ENTITY,
                                                      DETAIL_PROP_RENDER_HANDLE);
                        }
                    }
                } else {
                    AddRenderableToRenderList(*info.m_pRenderList, pRenderable,
                                              worldListLeafIndex, RENDER_GROUP_OPAQUE_ENTITY,
                                              DETAIL_PROP_RENDER_HANDLE);
                }
            }
            ++idx;
