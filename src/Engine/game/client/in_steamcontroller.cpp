...
static ControllerDigitalActionToCommand g_ControllerDigitalGameActions[] =
        {
                ...
                {"invprev",                       "invprev",                       CONTROLLER_ACTION_FLAGS_STOPS_TAUNT |
                                                                                   CONTROLLER_ACTION_FLAGS_NEEDS_DEBOUNCE},
                {"invnext",                       "invnext",                       CONTROLLER_ACTION_FLAGS_STOPS_TAUNT |
                                                                                   CONTROLLER_ACTION_FLAGS_NEEDS_DEBOUNCE},
                {"reload",                        "+reload",                       CONTROLLER_ACTION_FLAGS_NONE},
                {"dropitem",                      "dropitem",                      CONTROLLER_ACTION_FLAGS_STOPS_TAUNT |
                                                                                   CONTROLLER_ACTION_FLAGS_NEEDS_DEBOUNCE},
...
bool CInput::InitializeSteamControllerGameActionSets() {
    ...
    for (int i = 0; i < ARRAYSIZE(g_ControllerDigitalGameActions); ++i) {
        ...
        bGotHandle = bGotHandle &&
                     (state.handle != 0);                        // We're only successful if we get *all* the handles.
        state.cmd = cmd;
        state.bState = false;
