...
void CGlowOverlay::Draw(bool bCacheFullSceneState) {
    ...
    for (int iSprite = 0; iSprite < m_nSprites; iSprite++) {
        ...
        if (m_Sprites[iSprite].m_pMaterial == NULL) {
            m_Sprites[iSprite].m_pMaterial = materials->FindMaterial("sprites/light_glow02_add_noz",
                                                                     TEXTURE_GROUP_CLIENT_EFFECTS);
        }

        Assert(m_Sprites[iSprite].m_pMaterial);
        static unsigned int nHDRColorScaleCache = 0;
        IMaterialVar *pHDRColorScaleVar = m_Sprites[iSprite].m_pMaterial->FindVarFast("$hdrcolorscale",
                                                                                      &nHDRColorScaleCache);
...
void CGlowOverlay::DrawOverlays(bool bCacheFullSceneState) {
    ...
    for (unsigned short i = g_GlowOverlaySystem.m_GlowOverlays.Head();
        ...
        if (pOverlay->Update()) {
            pRenderContext->EnableClipping(((pOverlay->m_bInSky) ? (false)
                                                                 : (bClippingEnabled))); //disable clipping in skybox, restore clipping to pre-existing state when not in skybox (it may be off as well)
