...
void CScriptedIconLesson::ProcessOpenGameEvents(const CScriptedIconLesson *pRootLesson, const char *name,
    ...
    if (pRootLesson->InstanceType() == LESSON_INSTANCE_SINGLE_OPEN &&
        GetGameInstructor().IsLessonOfSameTypeOpen(this)) {
...
void CScriptedIconLesson::ProcessCloseGameEvents(const CScriptedIconLesson *pRootLesson, const char *name,
                                                 IGameEvent *event) {
    for (int iLessonEvent = 0;
         iLessonEvent < pRootLesson->m_CloseEvents.Count() && IsOpenOpportunity(); ++iLessonEvent) {
...
bool
CScriptedIconLesson::ProcessElement(IGameEvent *event, const LessonElement_t *pLessonElement, bool bInFailedScope) {
                ...
                switch (pInfo->varType) {
                    ...
                    case FIELD_STRING:
                        eventParam_float = static_cast<float>(
                                atoi(&LESSON_VARIABLE_GET_FROM_OFFSET(CGameInstructorSymbol,
                                                                      pInfo->iOffset)->String()));
                        break;
                    case FIELD_EHANDLE:
                    case FIELD_FUNCTION:
        ...
        case FIELD_EHANDLE:
            if (pLessonElement->iParamVarIndex < LESSON_VARIABLE_TOTAL) {
                ...
                if (iEntID >= NUM_ENT_ENTRIES) {
                    ...
                    DevWarning("Invalid entity ID used in game event (%s) for param (%s).", event->GetName(),
                               pParamName);
                    return false;
                }
            ...
            if (!pBaseCombatCharacter) {
                if (gameinstructor_verbose.GetInt() > 0 && ShouldShowSpew()) {
                    ConColorMsg(CBaseLesson::m_rgbaVerbosePlain, "\t[%s] = [%s]->GetActiveWeapon()", pchParamNameTemp,
                                pchVarName);
                    ConColorMsg(CBaseLesson::m_rgbaVerboseClose,
...
bool CScriptedIconLesson::ProcessElementAction(int iAction, bool bNot, const char *pchVarName,
    ...
    switch (iAction) {
        ...
        case LESSON_ACTION_SET:
            if (gameinstructor_verbose.GetInt() > 0 && ShouldShowSpew()) {
                ConColorMsg(CBaseLesson::m_rgbaVerbosePlain, "\tQ_strcpy([%s], [%s] ", pchVarName,
                            pchParamName->String());
                ConColorMsg(CBaseLesson::m_rgbaVerboseName, "\"%s\"", pchParam);
                ConColorMsg(CBaseLesson::m_rgbaVerbosePlain, ")\n");
            }
        ...
        case LESSON_ACTION_ADD:
            if (gameinstructor_verbose.GetInt() > 0 && ShouldShowSpew()) {
                ConColorMsg(CBaseLesson::m_rgbaVerbosePlain, "\tQ_strcat([%s], [%s] ", pchVarName,
                            pchParamName->String());
                ConColorMsg(CBaseLesson::m_rgbaVerboseName, "\"%s\"", pchParam);
                ConColorMsg(CBaseLesson::m_rgbaVerbosePlain, ")\n");
            }
        ...
        case LESSON_ACTION_HAS_PREFIX:
            ...
            return (bNot) ? (!StringHasPrefix(pchVar->String(), pchParam)) : (StringHasPrefix(pchVar->String(),
                                                                                              pchParam));
