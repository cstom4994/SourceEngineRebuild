...
void CAchievementsDialog_XBox::OnKeyCodePressed(vgui::KeyCode code) {
    // Handle close here, CCBasePanelBasePanel parent doesn't support "DialogClosing" command
...
CAchievementsDialog::CAchievementsDialog(vgui::Panel *parent) : BaseClass(parent, "AchievementsDialog") {
    ...
    m_pSelectionHighlight = new ImagePanel(m_pAchievementsList->FindChildByName("PanelListEmbedded"),
                                           "SelectionHighlight");
    ...
    if (pData) {
        ...
        if (achievementmgr) {
            ...
            for (int i = 0; i < achievements.Count(); i++) {
                CAchievementDialogItemPanel *achievementItemPanel = new CAchievementDialogItemPanel(m_pAchievementsList,
...
void CAchievementsDialog::ApplySchemeSettings(IScheme *pScheme) {
    ...
    Color cProgressBar = Color(static_cast<float>( clrHighlight.r()) * (1.0f - flCompletion) +
...
void CAchievementsDialog::OnKeyCodePressed(vgui::KeyCode code) {
    ...
    if (nButtonCode == KEY_XBUTTON_B || nButtonCode == STEAMCONTROLLER_B) {
        ...
        if (GameSupportsAchievementTracker() && m_nScrollItem >= 0 &&
            ...
            CAchievementDialogItemPanel *pItem = dynamic_cast< CAchievementDialogItemPanel * >( m_pAchievementsList->GetItemPanel(
                    m_nScrollItem));
    ...
    if (bCreate && (m_iNumAchievementGroups > NUM_COMBO_BOX_LINES_DEFAULT)) {
        if (m_pAchievementPackCombo) {
            m_pAchievementPackCombo->SetNumberOfEditLines(
                    (m_iNumAchievementGroups <= NUM_COMBO_BOX_LINES_MAX) ? m_iNumAchievementGroups
...
CAchievementDialogItemPanel::CAchievementDialogItemPanel(vgui::PanelListPanel *parent, const char *name,
                                                         int iListItemID) : BaseClass(parent, name) {
