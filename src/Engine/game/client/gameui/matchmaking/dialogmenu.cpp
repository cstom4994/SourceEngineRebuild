...
void COptionsItem::PerformLayout() {
    ...
    int optionWide = max(m_nOptionsMinWide,
                         GetWide() - m_nOptionsXPos - m_pRightArrow->GetWide() - m_nOptionsLeftMargin);
...
void CSectionedItem::PerformLayout() {
    ...
    for (int i = 0; i < m_Sections.Count(); ++i) {
        ...
        pLabel->SetTextInset(10, m_bHeader ? 5 : m_pParent->GetColumnYPos(
...
CBrowserItem *
CDialogMenu::AddBrowserItem(const char *pHost, const char *pPlayers, const char *pScenario, const char *pPing) {
...
CAchievementItem *
CDialogMenu::AddAchievementItem(const wchar_t *pName, const wchar_t *pDesc, uint points, bool bUnlocked,
