...
public:
    ...
    void
    CreateMessageDialog(const uint nType, const char *pTitle, const char *pMsg, const char *pCmdA, const char *pCmdB,
...
//-----------------------------------------------------------------------------
// Purpose: EditablePanel that can replace the GameMenuButtons in CCBasePanelBasePanel
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
class CCBasePanelBasePanel : public vgui::Panel {
    DECLARE_CLASS_SIMPLE(CCBasePanelBasePanel, vgui::Panel);

public:
    CCBasePanelBasePanel();

    virtual ~CCBasePanelBasePanel();
...
public:
    ...
    public:
        CAsyncJobContext(float flLeastExecuteTime = 0.0f) : m_flLeastExecuteTime(flLeastExecuteTime),
...
public:
    ...
    void
    UpdatePlayerInfo(uint64 nPlayerId, const char *pName, int nTeam, byte cVoiceState, int nPlayersNeeded, bool bHost);
...
#if defined( _X360 )
    CON_COMMAND_MEMBER_F( CCBasePanelBasePanel, "gameui_reload_resources", Reload_Resources, "Reload the Xbox 360 UI res files", 0 );
#endif
...
//-----------------------------------------------------------------------------
extern CCBasePanelBasePanel *BasePanel();
