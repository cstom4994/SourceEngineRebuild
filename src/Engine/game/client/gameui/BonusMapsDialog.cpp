...
CBonusMapsDialog::CBonusMapsDialog(vgui::Panel *parent) : BaseClass(parent, "BonusMapsDialog") {
    ...
    CCBasePanelBasePanel *pBasePanel = BasePanel();
    if (pBasePanel)
...
void CBonusMapsDialog::BuildMapsList(void) {
    ...
    if (iMapCount <= 0) {
        vgui::Label *pNoSavesLabel = SETUP_PANEL(
                new Label(m_pGameList, "NoBonusMapsLabel", "#GameUI_NoBonusMapsToDisplay"));
...
void CBonusMapsDialog::RefreshData(void) {
    for (int iMap = 0; iMap < m_pGameList->GetItemCount(); ++iMap) {
        ...
        if (pBonusMapPanel)
            pBonusMapPanel->SetBonusMapInfo(BonusMapsDatabase()->GetPath(), *(BonusMapsDatabase()->GetBonusData(
    ...
    if (iChallenge < 0) {
        ...
        for (int iChallengeIndex = 0; iChallengeIndex < m_pChallengeSelection->GetItemCount(); ++iChallengeIndex) {
            ...
            if (iChallengeNum >= 0) {
                ...
                CBitmapImagePanel *pBitmap = dynamic_cast<CBitmapImagePanel *>( GetChild(
                        iFirstChildIndex + iChallengeNum));
            ...
            if (shortName && shortName[0]) {
                if (pBonusMap->bIsFolder) {
                    ...
                    if (pBonusMap->szMapFileName[0] != '.') {
                        ...
                        const char *pchSubDir = Q_strnchr(BonusMapsDatabase()->GetPath(), '/',
