...
void CHudBaseDeathNotice::Paint() {
    ...
    for (int i = 0; i < iCount; i++) {
        ...
        int iDeathInfoTextWide = msg.wzInfoText[0] ? UTIL_ComputeStringWidth(m_hTextFont, msg.wzInfoText) + xSpacing
                                                   : 0;
        int iDeathInfoEndTextWide = msg.wzInfoTextEnd[0] ? UTIL_ComputeStringWidth(m_hTextFont, msg.wzInfoTextEnd) +
                                                           xSpacing : 0;
        ...
        int iPreKillerTextWide = msg.wzPreKillerText[0] ? UTIL_ComputeStringWidth(m_hTextFont, msg.wzPreKillerText) -
                                                          xSpacing : 0;
        ...
        int iTotalWide = iKillerTextWide + iconWide + iVictimTextWide + iDeathInfoTextWide + iDeathInfoEndTextWide +
                         (xMargin * 2);
...
void CHudBaseDeathNotice::FireGameEvent(IGameEvent *event) {
    ...
    if (bPlayerDeath || bObjectDeath) {
        ...
        if (event->GetInt("death_flags") & TF_DEATH_AUSTRALIUM) {
            ...
            m_DeathNotices[iMsg].iconCritDeath = GetIcon("d_australium",
                                                         bLocalPlayerInvolved ? kDeathNoticeIcon_Inverted
                                                                              : kDeathNoticeIcon_Standard);
        } else if (event->GetInt("damagebits") & DMG_CRITICAL) {
            m_DeathNotices[iMsg].bCrit = true;
            m_DeathNotices[iMsg].iconCritDeath = GetIcon("d_crit", bLocalPlayerInvolved ? kDeathNoticeIcon_Inverted
                                                                                        : kDeathNoticeIcon_Standard);
        } else {
    ...
    if (!bSpecialScore && !bTeamLeaderKilled) {
        if (!m_DeathNotices[iMsg].iconDeath && m_DeathNotices[iMsg].szIcon) {
            ...
            m_DeathNotices[iMsg].iconDeath = GetIcon(m_DeathNotices[iMsg].szIcon,
