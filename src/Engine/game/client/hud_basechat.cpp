...
ConVar cl_mute_all_comms("cl_mute_all_comms", "1", FCVAR_ARCHIVE,
...
//-----------------------------------------------------------------------------
wchar_t *
ReadLocalizedString(bf_read &msg, OUT_Z_BYTECAP(outSizeInBytes) wchar_t *pOut, int outSizeInBytes, bool bStripNewline,
                    OUT_Z_CAP(originalSize) char *originalString, int originalSize) {
    char szString[2048];
    szString[0] = 0;
        ...
        if (pBuf) {
            ...
            if (i) {
                StripEndNewlineFromString(
                        tmpStr);  // these strings are meant for subsitution into the main strings, so cull the automatic end newlines
            }
...
void CBaseHudChat::StartMessageMode(int iMessageModeType) {
    ...
    const wchar_t *pszPrompt = (m_nMessageMode == MM_SAY) ? g_pVGuiLocalize->Find("#chat_say") : g_pVGuiLocalize->Find(
            "#chat_say_team");
    if (pszPrompt) {
