...
void CClient_Precipitation::CreateAshParticle(void) {
    ...
    while (m_tAshParticleTimer.NextEvent(curTime)) {
        ...
        if (bEmberTime == true) {
            ...
            pParticle->m_vecVelocity = Vector(RandomFloat(-20.0f, 20.0f), RandomFloat(-20.0f, 20.0f),
                                              RandomFloat(-10, -15));
        }
...
void CSnowFallManager::CreateSnowFallParticle(const Vector &vecParticleSpawn, int iSnow) {
    SimpleParticle *pParticle = (SimpleParticle *) m_pSnowFallEmitter->AddParticle(sizeof(SimpleParticle),
                                                                                   m_aSnow[iSnow].m_hMaterial,
                                                                                   vecParticleSpawn);
    if (pParticle == NULL)
        return;
    ...
    pParticle->m_flDieTime = fabs(
            (vecParticleSpawn.z - m_aSnow[iSnow].m_vecMin.z) / (pParticle->m_vecVelocity.z - 0.1));
