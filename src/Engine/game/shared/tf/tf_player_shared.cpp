...
static ConVar tf_demoman_charge_frametime_scaling("tf_demoman_charge_frametime_scaling", "1",
...
ConVar tf_spy_max_cloaked_speed("tf_spy_max_cloaked_speed", "999",
                                FCVAR_DEVELOPMENTONLY | FCVAR_REPLICATED);    // no cap
...
ConVar tf_allow_taunt_switch("tf_allow_taunt_switch", "0", FCVAR_REPLICATED,
...
BEGIN_RECV_TABLE_NOBASE(CTFPlayerShared, DT_TFPlayerSharedLocal)
                    ...
                    RecvPropDataTable(RECVINFO_DT(m_ScoreData), 0,
                                      &REFERENCE_RECV_TABLE(DT_TFPlayerScoringDataExclusive)),
                    RecvPropDataTable(RECVINFO_DT(m_RoundScoreData), 0,
                                      &REFERENCE_RECV_TABLE(DT_TFPlayerScoringDataExclusive)),
END_RECV_TABLE()
...
BEGIN_RECV_TABLE_NOBASE(CTFPlayerShared, DT_TFPlayerShared)
                    ...
                    RecvPropDataTable(RECVINFO_DT(m_ConditionList), 0,
                                      &REFERENCE_RECV_TABLE(DT_TFPlayerConditionListExclusive)),
...
//	DEFINE_PRED_FIELD( m_hDisguiseWeapon, FIELD_EHANDLE, FTYPEDESC_INSENDTABLE ),
                    ...
                    DEFINE_PRED_FIELD(m_bIsTargetedForPasstimePass, FIELD_BOOLEAN,
                                      FTYPEDESC_INSENDTABLE), // does this belong here?
...
extern void
HandleRageGain(CTFPlayer *pPlayer, unsigned int iRequiredBuffFlags, float flDamage, float fInverseRageGainScale);
...
void CTFPlayerShared::RemoveCond(ETFCond eCond, bool ignore_duration) {
    ...
    if (m_ConditionData[eCond].m_nPreventedDamageFromCondition) {
        ...
        if (pEvent) {
            pEvent->SetInt("preventor",
                           m_ConditionData[eCond].m_pProvider ? m_ConditionData[eCond].m_pProvider->entindex()
                                                              : m_pOuter->entindex());
...
//-----------------------------------------------------------------------------
void
CTFPlayerShared::SyncConditions(int nPreviousConditions, int nNewConditions, int nForceConditions, int nBaseCondBit) {
...
//		m_pOuter->ParticleProp()->StopEmission( m_pOuter->m_pDisguisingEffect );
    ...
    if (!m_pOuter->IsLocalPlayer() && (!IsStealthed() || !m_pOuter->IsEnemyPlayer())) {
        const char *pEffectName = (m_pOuter->GetTeamNumber() == TF_TEAM_RED) ? "spy_start_disguise_red"
...
void CTFPlayerShared::OnAddUrine(void) {
    ...
    if (!m_pOuter->m_pUrineEffect) {
        m_pOuter->m_pUrineEffect = m_pOuter->ParticleProp()->Create("peejar_drips",
                                                                    PATTACH_ABSORIGIN_FOLLOW); // pEffect! Kek!
    }
    ...
    if (unTauntSourceItemID != INVALID_ITEM_ID) {
        ...
        if (pTauntItem) {
            ...
            if (iQualityParticleType > 0) {
                const attachedparticlesystem_t *pParticleSystem = GetItemSchema()->GetAttributeControlledParticleSystem(
                        iQualityParticleType);
                if (pParticleSystem) {
            ...
            for (int i = 0; i < m_pOuter->GetNumWearables(); ++i) {
                ...
                CEconItemView *pItem =
                        ...
                        ? pWearable->GetAttributeContainer()->GetItem() : NULL;
...
void CTFPlayerShared::FireClientTauntParticleEffects() {
    ...
    if (TauntParticleState.first) {
        ...
        m_pOuter->m_pTauntEffect = m_pOuter->ParticleProp()->Create(TauntParticleState.first,
                                                                    iAttachment != INVALID_PARTICLE_ATTACHMENT
                                                                    ? PATTACH_POINT_FOLLOW : PATTACH_ABSORIGIN_FOLLOW,
                                                                    iAttachment, vec3_origin);
...
#ifdef CLIENT_DLL
    if (m_pOuter->IsLocalPlayer()) {
        IMaterial *pMaterial = materials->FindMaterial(TF_SCREEN_OVERLAY_MATERIAL_BLEED, TEXTURE_GROUP_CLIENT_EFFECTS,
                                                       false);
        if (!IsErrorMaterial(pMaterial)) {
            view->SetScreenOverlayMaterial(pMaterial);
        }
...
void CTFPlayerShared::OnAddSoldierOffensiveBuff(void) {
    ...
    if (!m_pOuter->m_pSoldierOffensiveBuffEffect) {
        m_pOuter->m_pSoldierOffensiveBuffEffect = m_pOuter->ParticleProp()->Create(strBuffName,
...
void CTFPlayerShared::OnAddSoldierDefensiveBuff(void) {
    ...
    if (!m_pOuter->m_pSoldierDefensiveBuffEffect) {
        m_pOuter->m_pSoldierDefensiveBuffEffect = m_pOuter->ParticleProp()->Create(strBuffName,
                                                                                   PATTACH_ABSORIGIN_FOLLOW);
    }
#endif
}
...
void CTFPlayerShared::OnAddSoldierOffensiveHealthRegenBuff(void) {
    ...
    if (!m_pOuter->m_pSoldierOffensiveHealthRegenBuffEffect) {
        m_pOuter->m_pSoldierOffensiveHealthRegenBuffEffect = m_pOuter->ParticleProp()->Create(strBuffName,
                                                                                              PATTACH_ABSORIGIN_FOLLOW);
    }
#endif
}
...
void CTFPlayerShared::OnAddSoldierNoHealingDamageBuff(void) {
    ...
    if (!m_pOuter->m_pSoldierNoHealingDamageBuffEffect) {
        m_pOuter->m_pSoldierNoHealingDamageBuffEffect = m_pOuter->ParticleProp()->Create(strBuffName,
                                                                                         PATTACH_ABSORIGIN_FOLLOW);
    }
#endif
}
...
static void
AddResistParticle(CTFPlayer *pPlayer, medigun_resist_types_t nResistType, ETFCond eYeildToCond = TF_COND_LAST) {
...
static void RemoveResistShield(C_LocalTempEntity **pShield, CTFPlayer *pPlayer) {
    if (*pShield) {
        ...
        eCond = pPlayer->m_Shared.InCond(TF_COND_MEDIGUN_UBER_BULLET_RESIST) ? TF_COND_MEDIGUN_UBER_BULLET_RESIST
                                                                             : eCond;
        ...
        eCond = (pPlayer->m_Shared.InCond(TF_COND_RUNE_RESIST) && !pPlayer->m_Shared.IsStealthed())
                ? TF_COND_RUNE_RESIST : eCond;

        C_TFPlayer *pLocalPlayer = C_TFPlayer::GetLocalTFPlayer();
        if (pLocalPlayer) {
            eCond = (pPlayer->IsEnemyPlayer() && pPlayer->m_Shared.InCond(TF_COND_RUNE_PLAGUE) &&
                     pLocalPlayer->m_Shared.InCond(TF_COND_PLAGUE)) ? TF_COND_RUNE_PLAGUE : eCond;
        }
...
void CTFPlayerShared::OnAddMVMBotRadiowave(void) {
    ...
    if (!m_pOuter->m_pMVMBotRadiowave) {
        m_pOuter->m_pMVMBotRadiowave = m_pOuter->ParticleProp()->Create("bot_radio_waves", PATTACH_POINT_FOLLOW,
                                                                        "head");
    }
#else
                                                                                                                            if ( !m_pOuter->IsBot() )
...
#ifdef CLIENT_DLL
    if (m_pOuter->IsLocalPlayer()) {
        IMaterial *pMaterial = materials->FindMaterial(TF_SCREEN_OVERLAY_MATERIAL_SWIMMING_CURSE,
...
void CTFPlayerShared::OnAddPasstimeInterception(void) {
    ...
    if (m_pOuter->IsLocalPlayer()) {
        IMaterial *pMaterial = materials->FindMaterial(TF_SCREEN_OVERLAY_MATERIAL_PHASE, TEXTURE_GROUP_CLIENT_EFFECTS,
                                                       false);
        if (!IsErrorMaterial(pMaterial)) {
            view->SetScreenOverlayMaterial(pMaterial);
        }
...
void CTFPlayerShared::OnAddRunePlague(void) {
    ...
    m_pOuter->m_pRunePlagueEffect = m_pOuter->ParticleProp()->Create("powerup_plague_carrier",
                                                                     PATTACH_ABSORIGIN_FOLLOW);
...
void CTFPlayerShared::OnAddRadiusHeal(void) {
    ...
    if (InCond(TF_COND_RADIUSHEAL)) {
        ...
        if (!m_pOuter->m_pRadiusHealEffect) {
            m_pOuter->m_pRadiusHealEffect = m_pOuter->ParticleProp()->Create(pszRadiusHealEffect,
                                                                             PATTACH_ABSORIGIN_FOLLOW);
        }
    }
#endif
...
void CTFPlayerShared::OnAddMegaHeal(void) {
    ...
    if (InCond(TF_COND_MEGAHEAL)) {
        ...
        if (!m_pOuter->m_pMegaHealEffect) {
            m_pOuter->m_pMegaHealEffect = m_pOuter->ParticleProp()->Create(pszMegaHealEffectName,
                                                                           PATTACH_ABSORIGIN_FOLLOW);
        }
    }
#endif
...
void CTFPlayerShared::ClientKillStreakBuffThink(void) {
    int nLoadoutSlot = m_pOuter->GetActiveTFWeapon()
                       ? m_pOuter->GetActiveTFWeapon()->GetAttributeContainer()->GetItem()->GetStaticData()->GetLoadoutSlot(
                    m_pOuter->GetPlayerClass()->GetClassIndex()) : LOADOUT_POSITION_PRIMARY;
...
void CTFPlayerShared::OnAddStealthed(void) {
    ...
    if (InCond(TF_COND_STEALTHED_USER_BUFF) && m_pOuter->IsLocalPlayer()) {
        IMaterial *pMaterial = materials->FindMaterial(TF_SCREEN_OVERLAY_MATERIAL_STEALTH, TEXTURE_GROUP_CLIENT_EFFECTS,
                                                       false);
        if (!IsErrorMaterial(pMaterial)) {
            view->SetScreenOverlayMaterial(pMaterial);
        }
...
void CTFPlayerShared::OnAddBurning(void) {
    ...
    if (!m_pOuter->m_pBurningEffect) {
        if (!(IsLocalPlayerUsingVisionFilterFlags(TF_VISION_FILTER_PYRO) && m_pOuter->IsLocalPlayer())) {
            const char *pEffectName = (m_pOuter->GetTeamNumber() == TF_TEAM_RED) ? "burningplayer_red"
                                                                                 : "burningplayer_blue";
    ...
    if (m_pOuter->IsLocalPlayer()) {
        IMaterial *pMaterial = materials->FindMaterial(TF_SCREEN_OVERLAY_MATERIAL_BURNING, TEXTURE_GROUP_CLIENT_EFFECTS,
                                                       false);
        if (!IsErrorMaterial(pMaterial)) {
            view->SetScreenOverlayMaterial(pMaterial);
        }
        ...
        if (GetActiveStunInfo()) {
            if (!m_pOuter->m_pStunnedEffect && !(GetActiveStunInfo()->iStunFlags & TF_STUN_NO_EFFECTS)) {
                if ((GetActiveStunInfo()->iStunFlags & TF_STUN_BY_TRIGGER) || InCond(TF_COND_HALLOWEEN_BOMB_HEAD)) {
                    ...
                    m_pOuter->m_pStunnedEffect = m_pOuter->ParticleProp()->Create(pEffectName, PATTACH_POINT_FOLLOW,
                                                                                  "head");
                } else {
        ...
        if (pWeapon) {
            if (!m_pOuter->m_pCritBoostEffect) {
                if (InCond(TF_COND_DISGUISED) && !m_pOuter->IsLocalPlayer() &&
                    ...
                    const char *pEffectName = (GetDisguiseTeam() == TF_TEAM_RED) ? "critgun_weaponmodel_red"
                                                                                 : "critgun_weaponmodel_blu";
                    m_pOuter->m_pCritBoostEffect = pWeapon->ParticleProp()->Create(pEffectName,
                                                                                   PATTACH_ABSORIGIN_FOLLOW);
                } else {
...
void CTFPlayerShared::FadeInvis(float fAdditionalRateScale) {
    ...
    if (flInvisFadeTime < 0.15) {
        ...
        m_flStealthNoAttackExpire = gpGlobals->curtime + (tf_spy_cloak_no_attack_time.GetFloat() * flDecloakRateScale *
                                                          fAdditionalRateScale);
    }
...
int CTFPlayerShared::GetDisguiseTeam(void) const {
    return InCond(TF_COND_DISGUISED_AS_DISPENSER) ? (int) ((m_pOuter->GetTeamNumber() == TF_TEAM_RED) ? TF_TEAM_BLUE
                                                                                                      : TF_TEAM_RED)
                                                  : m_nDisguiseTeam;
}
...
#else // !GAME_DLL
        CSmartPtr<CNewParticleEffect> pEffect = pWeapon->ParticleProp()->Create(pParticleSystemName,
...
static bool OnOpposingTFTeams(int iTeam0, int iTeam1) {
    ...
    return iTeam0 !=
           iTeam1;                    // if we're neither red nor blue, then anyone different from us is opposing
}
...
#ifdef CLIENT_DLL
static ConVar tf_impactwatertimeenable("tf_impactwatertimeenable", "1", 0,
                                       "Limit time between new water impact effects.");
...
bool CTFPlayer::CanPlayerMove() const {
    ...
    if (TFGameRules() && TFGameRules()->BInMatchStartCountdown()) { return false; }
        ...
        if (pWeapon) {
            ...
            if (pMedigun) {
                ...
                if (pHealTarget) {
                    ...
                    const float flHealTargetMaxSpeed = (bCharge) ? tf_max_charge_speed.GetFloat()
                                                                 : pHealTarget->TeamFortress_CalculateMaxSpeed(true);
...
bool CTFPlayer::CanAttack(int iCanAttackFlags) {
    ...
    if ((m_Shared.GetStealthNoAttackExpireTime() > gpGlobals->curtime &&
         !m_Shared.InCond(TF_COND_STEALTHED_USER_BUFF)) || m_Shared.InCond(TF_COND_STEALTHED)) {
...
bool CTFPlayer::DoClassSpecialSkill(void) {
    ...
    if (m_Shared.IsRuneCharged() && GetActiveTFWeapon() &&
        GetActiveTFWeapon()->GetWeaponID() == TF_WEAPON_GRAPPLINGHOOK) {
#ifdef GAME_DLL
    ...
    switch (GetPlayerClass()->GetClassIndex()) {
        ...
        case TF_CLASS_DEMOMAN:
            if (!m_Shared.HasPasstimeBall()) {
                CTFPipebombLauncher *pPipebombLauncher = static_cast<CTFPipebombLauncher *>( Weapon_OwnsThisID(
        ...
        switch (pPickupObject->GetType()) {
            ...
            case OBJ_TELEPORTER:
                return pPickupObject->GetObjectMode() == MODE_TELEPORTER_ENTRANCE
                       ? training_can_pickup_tele_entrance.GetBool() : training_can_pickup_tele_exit.GetBool();
            case OBJ_SENTRYGUN:
...
int CTFPlayer::GetMaxAmmo(int iAmmoIndex, int iClassIndex /*= -1*/ ) {
    int iMax = (iClassIndex == -1) ? m_PlayerClass.GetData()->m_aAmmoMax[iAmmoIndex] : GetPlayerClassData(
            iClassIndex)->m_aAmmoMax[iAmmoIndex];
...
int CTFPlayer::GetNumActivePipebombs(void) {
    if (IsPlayerClass(TF_CLASS_DEMOMAN)) {
        CTFPipebombLauncher *pWeapon = dynamic_cast < CTFPipebombLauncher *>( Weapon_OwnsThisID(
                TF_WEAPON_PIPEBOMBLAUNCHER));
...
void CTFPlayerShared::UpdatePhaseEffects(void) {
    ...
    if (m_pOuter) {
        if (!bRunning && !m_pOuter->m_pPhaseStandingEffect && m_flEnergyDrinkMeter < 100.0f) {
            m_pOuter->m_pPhaseStandingEffect = m_pOuter->ParticleProp()->Create("warp_version",
                                                                                PATTACH_ABSORIGIN_FOLLOW);
...
void CTFPlayerShared::CreateVehicleMove(float flInputSampleTime, CUserCmd *pCmd) {
    ...
    if (flNormalizedSpeed == 0.f) {
        ...
        const float flSmoothCurveVal = SmoothCurve_Tweak(flNormalizedSpeed,
                                                         tf_halloween_kart_turning_curve_peak_position.GetFloat());
    ...
    } else {    // When not trying to turn, or turning the opposite way you're already turning
    ...
    const float flMaxPossibleTurnSpeed = Max(tf_halloween_kart_slow_turn_speed.GetFloat(),
                                             tf_halloween_kart_fast_turn_speed.GetFloat());
...
float CTFPlayer::GetKartSpeedBoost(void) {
    ...
    // Calculate time
    return RemapValClamped(gpGlobals->curtime, m_flKartNextAvailableBoost, m_flKartNextAvailableBoost, 0.0f, 1.0f);
}
...
void CTFPlayerShared::UpdateEnergyDrinkMeter(void) {
    ...
    if (bIsLocalPlayer) {
        ...
        if (InCond(TF_COND_PHASE) || InCond(TF_COND_ENERGY_BUFF)) {
            ...
            CTFLunchBox_Drink *pDrink = static_cast< CTFLunchBox_Drink * >( m_pOuter->Weapon_OwnsThisID(
                    TF_WEAPON_LUNCHBOX));
...
void CTFPlayerShared::PulseKingRuneBuff(void) {
    ...
    if (m_bKingRuneBuffActive && !InCond(TF_COND_PLAGUE)) {
        if (!m_pOuter->m_pKingRuneRadiusEffect) {
            ...
            m_pOuter->m_pKingRuneRadiusEffect = m_pOuter->ParticleProp()->Create(pszRadiusEffect,
                                                                                 PATTACH_ABSORIGIN_FOLLOW, NULL,
                                                                                 Vector(0, 0, 0));
        }
    } else {
        EndKingBuffRadiusEffect();
...
CEconItemView *
CTFPlayerSharedUtils::GetEconItemViewByLoadoutSlot(CTFPlayer *pTFPlayer, int iSlot, CEconEntity **pEntity) {
