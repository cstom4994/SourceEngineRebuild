...
DEFINE_ARRAY( m_pBall, FIELD_CLASSPTR,
2 ),
DEFINE_ARRAY( m_iBall, FIELD_INTEGER,
2 ),
DEFINE_ARRAY( m_iBallTime, FIELD_TIME,
2 ),
DEFINE_ARRAY( m_iBallCurrent, FIELD_INTEGER,
2 ),
...
void CNPC_Controller::Spawn() {
    ...
    CapabilitiesAdd(
            bits_CAP_MOVE_FLY | bits_CAP_INNATE_RANGE_ATTACK1 | bits_CAP_INNATE_RANGE_ATTACK2 | bits_CAP_MOVE_SHOOT);
        ...
        while (m_flShootTime < m_flShootEnd && m_flShootTime < gpGlobals->curtime) {
            ...
            if (GetEnemy() != NULL) {
                ...
                vecDir = vecDir + Vector(random->RandomFloat(-delta, delta), random->RandomFloat(-delta, delta),
                                         random->RandomFloat(-delta, delta)) * sk_controller_speedball.GetFloat();
