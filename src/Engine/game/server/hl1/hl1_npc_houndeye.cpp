...
void CNPC_Houndeye::SonicAttack(void) {
    ...
    while ((pEntity = gEntList.FindEntityInSphere(pEntity, GetAbsOrigin(), HOUNDEYE_MAX_ATTACK_RADIUS)) != NULL) {
        if (pEntity->m_takedamage != DAMAGE_NO) {
            if (!FClassnameIs(pEntity, "monster_houndeye")) {// houndeyes don't hurt other houndeyes with their attack
...
bool CNPC_Houndeye::ShouldGoToIdleState(void) {
    if (m_pSquad) {
        AISquadIter_t iter;
        for (CAI_BaseNPC *pMember = m_pSquad->GetFirstMember(&iter); pMember; pMember = m_pSquad->GetNextMember(
                &iter)) {
...
void CNPC_Houndeye::PrescheduleThink(void) {
    ...
    if (m_pSquad && m_pSquad->IsLeader(this)) {
        ...
        AISquadIter_t iter;
        for (CAI_BaseNPC *pSquadMember = m_pSquad->GetFirstMember(
                &iter); pSquadMember; pSquadMember = m_pSquad->GetNextMember(&iter)) {
            iSquadCount++;
...
int CNPC_Houndeye::SquadRecruit(int searchRadius, int maxMembers) {
    ...
    if (m_SquadName != NULL_STRING) {
        ...
        while (pEntity && squadCount < maxMembers) {
            CNPC_Houndeye * pRecruit = (CNPC_Houndeye * )
            pEntity->MyNPCPointer();
