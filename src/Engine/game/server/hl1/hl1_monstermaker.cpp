...
DEFINE_KEYFIELD( m_iMaxNumNPCs, FIELD_INTEGER,
"monstercount" ),
DEFINE_KEYFIELD( m_iMaxLiveChildren, FIELD_INTEGER,
"MaxLiveChildren" ),
DEFINE_KEYFIELD( m_flSpawnFrequency, FIELD_FLOAT,
"delay" ),
DEFINE_KEYFIELD( m_bDisabled, FIELD_BOOLEAN,
"StartDisabled" ),
DEFINE_KEYFIELD( m_iszNPCClassname, FIELD_STRING,
"monstertype" ),
...
bool CNPCMaker::CanMakeNPC(void) {
    if (m_iMaxLiveChildren > 0 &&
        m_cLiveChildren >= m_iMaxLiveChildren) {// not allowed to make a new one yet. Too many live ones out right now.
        return false;
    }
...
void CNPCMaker::MakeNPC(void) {
    ...
    CBaseEntity * pent = (CBaseEntity * )
    CreateEntityByName(STRING(m_iszNPCClassname));
