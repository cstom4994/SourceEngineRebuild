...
DEFINE_ARRAY( m_pFlame, FIELD_CLASSPTR,
4 ),
...
CStomp *CStomp::StompCreate(Vector &origin, Vector &end, float speed, CBaseEntity *pOwner) {
    CStomp * pStomp = (CStomp * )
    CreateEntityByName("garg_stomp");
    ...
    switch (pEvent->event) {
        case GARG_AE_SLASH_LEFT: {
            ...
            CBaseEntity *pHurt = GargantuaCheckTraceHullAttack(GARG_ATTACKDIST + 10.0,
                                                               sk_gargantua_dmg_slash.GetFloat(), DMG_SLASH);
    ...
    while ((pEntity = gEntList.FindEntityInSphere(pEntity, GetAbsOrigin(), searchRadius)) != NULL) {
        ...
        if (pEntity->m_takedamage != DAMAGE_NO) {
            ...
            if (iClassIgnore != CLASS_NONE &&
                pEntity->Classify() == iClassIgnore) {// houndeyes don't hurt other houndeyes with their attack
                continue;
            }
...
//			NDebugOverlay::Line( vecStart, vecSpot, 0, 255, 0, false, 10.0f );
            ...
            if (tr.fraction == 1.0 || tr.m_pEnt == pEntity) {// the explosion can 'see' this entity, so hurt them!
