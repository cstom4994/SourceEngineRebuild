...
void CNPC_Hornet::TrackTouch(CBaseEntity *pOther) {
    ...
    if (pOther == GetOwnerEntity() || pOther->GetModelIndex() == GetModelIndex()) {// bumped into the guy that shot it.
        //SetSolid( SOLID_NOT );
        return;
    }
...
void CNPC_Hornet::TrackTarget(void) {
    ...
    if (flDelta <= 0 && m_iHornetType ==
                        HORNET_TYPE_RED) {// no flying backwards, but we don't want to invert this, cause we'd go fast when we have to turn REAL far.
        flDelta = 0.25;
    }
