...
public:
    ...
    int GetUpAxis(int &nSign) const {
        nSign = mUpVector.mSign;
        return mUpVector.mAxis;
    };

    int GetFrontAxis(int &nSign) const {
        nSign = mFrontVector.mSign;
        return mFrontVector.mAxis;
    };

    int GetLeftAxis(int &nSign) const {
        nSign = mCoorSystem.mSign;
        return mCoorSystem.mAxis;
    };
...
CDmFbxSerializer::CDmFbxSerializer()
        : m_nOptVerbosity(0), m_bOptUnderscoreForCorrectors(false), m_bAnimation(false), m_bReturnDmeModel(false),
          m_flOptScale(1.0f) {
    // Initialize Axis System To Maya Y Up
    CDmeAxisSystem::GetPredefinedAxisSystem(m_eOptUpAxis, m_eOptForwardParity, m_eCoordSys,
                                            CDmeAxisSystem::AS_MAYA_YUP);
}
...
void CDmFbxSerializer::LoadModelAndSkeleton_R(
        FbxToDmxMap_t &fbxToDmxMap, CDmeModel *pDmeModel, CDmeDag *pDmeDagParent, FbxNode *pFbxNode, bool bAnimation,
        int nDepth) const {
    FbxString sIndent;
...
//
//-----------------------------------------------------------------------------
CDmeDag *CDmFbxSerializer::FbxNodeToDmeDag(CDmeDag *pDmeDagParent, FbxNode *pFbxNode, const char *pszDmeType,
...
//
//-----------------------------------------------------------------------------
bool AddPositionData(CDmeVertexData *pDmeVertexData, const CUtlVector<int> &nIndices, const FbxMesh *pFbxMesh,
                     const FbxMatrix &mScale) {
...
//
//-----------------------------------------------------------------------------
void AddUVData(CDmeVertexData *pDmeVertexData, const CUtlVector<int> &nIndices, const FbxGeometryElementUV *pFbxElement,
               int nSemanticIndex) {
...
//
//-----------------------------------------------------------------------------
void AddNormalData(CDmeVertexData *pDmeVertexData, const CUtlVector<int> &nIndices,
                   const FbxGeometryElementNormal *pFbxElement, const FbxMatrix &mScale) {
    if (!pFbxElement || nIndices.Count() <= 0)
        return;
...
//
//-----------------------------------------------------------------------------
void CDmFbxSerializer::AddColorData(CDmeVertexData *pDmeVertexData, const CUtlVector<int> &nIndices,
...
void CDmFbxSerializer::AddUVColorData(CDmeVertexData *pDmeVertexData, const UVColorChannelData_t &uvColorData) const {
    ...
    for (int i = 0; i < ARRAYSIZE(uvColorData.m_uvChannelData); ++i) {
        ...
        for (int j = 0; j < indices.Count(); ++j) {
            const int nDmxDataIndex = indices[j];
            Assert(nDmxDataIndex == j);
            ...
            if (bExportColor) {
                ...
                vDmxColorData[i] = clamp(static_cast< uint8 >( floor(vFbxData[0] * 255.0)), 0,
                                         255);    // Always use value 0
            }
        }
    }

    {
        ...
        const FieldIndex_t nFieldIndex = pDmeVertexData->CreateField(uvColorData.m_sChannelName.Get(),
                                                                     ValueTypeToArrayType(dmAttributeType));
    ...
    if (bExportColor) {
        ...
        const FieldIndex_t nFieldIndex = pDmeVertexData->CreateField(sFieldName.Get(),
                                                                     ValueTypeToArrayType(dmAttributeType));
...
void HandleUV(CUtlVector<int> &uvIndices, FbxMesh *pFbxMesh, FbxGeometryElementUV *peUV, int nControlPointIndex,
              int nPolygonVertexIndex) {
    // Do Nothing
}
...
void HandleUV<FbxGeometryElement::eByControlPoint, FbxGeometryElement::eDirect>(
        CUtlVector<int> &uvIndices, FbxMesh *pFbxMesh, FbxGeometryElementUV *peUV, int nControlPointIndex,
        int nPolygonVertexIndex) {
...
CDmFbxSerializer::HandleUVFunc_t GetHandleUVFunc(FbxGeometryElementUV *pFbxElementUV) {
    if (pFbxElementUV) {
        ...
        } else if (nMappingMode == FbxGeometryElement::eByControlPoint &&
                   nReferenceMode == FbxGeometryElement::eIndexToDirect) {
            return HandleUV<FbxGeometryElement::eByControlPoint, FbxGeometryElement::eIndexToDirect>;
        } else if (nMappingMode == FbxGeometryElement::eByPolygonVertex &&
                   nReferenceMode == FbxGeometryElement::eDirect) {
...
template<FbxLayerElement::EMappingMode M, FbxLayerElement::EReferenceMode R>
void HandleNormal(
        CUtlVector<int> &nNormalIndices, FbxMesh *pFbxMesh, FbxGeometryElementNormal *pFbxElementNormal,
        int nControlPointIndex, int nPolygonVertexIndex) {
    // Do Nothing
}
...
void HandleVertexColor(
        CUtlVector<int> &vertexColorIndices, FbxMesh *pFbxMesh, FbxGeometryElementVertexColor *peVertexColor,
        int nControlPointIndex, int nPolygonVertexIndex) {
    // Do Nothing
}
...
public:
    ColorChannelData_t()
            : m_pFbxElementVertexColor(NULL), m_pFunc(NULL) {
    }
        ...
        if (nUVElementCount >= 4) {
            ...
            if (bValid && nUVFoundCount == 3) {
                ...
                for (int i = 0; i < ARRAYSIZE(pUVs); ++i) {
                    ...
                    uvElementList.FindAndRemove(
                            pUVs[i]);    // Remove these funky 3ds max channels from the normal UV channels
                }
            } else if (nUVFoundCount > 0) {
                Warning("Warning! Fbx 3DSMax UV encoded VertexPaint \"%s\" invalid, expected %llu maps, found %d, on mesh \"%s\"\n",
                        pszPrefix, static_cast< uint64 >(ARRAYSIZE(szChannelNames)), nUVFoundCount,
                        pFbxMesh->GetName());
            }
        }
    ...
    for (int nPolygon = 0; nPolygon < nPolygonCount; ++nPolygon) {
        ...
        for (int nPolygonVert = 0; nPolygonVert < nPolygonVertCount; ++nPolygonVert) {
            ...
            for (int nUV = 0; nUV < nUVElementCount; ++nUV) {
                ...
                (*(uvChannelData.m_pFunc))(uvChannelData.m_nIndices, pFbxMesh, uvChannelData.m_pFbxElementUV,
                                           nControlPointIndex, nPolygonVertexIndex);
            }
            ...
            for (int nColor = 0; nColor < nColorCount; ++nColor) {
                ...
                (*colorChannelData.m_pFunc)(colorChannelData.m_nIndices, pFbxMesh,
                                            colorChannelData.m_pFbxElementVertexColor, nControlPointIndex,
                                            nPolygonVertexIndex);
            }
            ...
            for (int nUVColor = 0; nUVColor < nUVColorCount; ++nUVColor) {
                ...
                for (int nUVChannel = 0; nUVChannel < ARRAYSIZE(uvColorChannelData.m_uvChannelData); ++nUVChannel) {
                    (*(uvColorChannelData.m_uvChannelData[nUVChannel].m_pFunc))(
                            uvColorChannelData.m_uvChannelData[nUVChannel].m_nIndices, pFbxMesh,
                            uvColorChannelData.m_uvChannelData[nUVChannel].m_pFbxElementUV, nControlPointIndex,
                            nPolygonVertexIndex);
                }
            }
...
//
//-----------------------------------------------------------------------------
bool CDmFbxSerializer::FbxMeshToDmeFaceSets(CDmeDag *pDmeDag, CDmeMesh *pDmeMesh, FbxMesh *pFbxMesh,
    ...
    typedef FbxMap<FbxString, FbxLayerElementArrayTemplate <
                              void * > * > HoudiniMatDataMap_t;    // data to figure out material assignments from houdini
        ...
        for (int j = 0; j < pFbxUserData->GetDirectArrayCount(); ++j) {
            if (pFbxUserData->GetDataType(j).GetType() == eFbxInt) {
                ...
                if (sDataName.FindAndReplace(szSuffix, ".vmat", sDataName.GetLen() - nSuffixCount)) {
                    bool bArray = false;
                    FbxLayerElementArrayTemplate<void *> *pDirectArrayVoid = pFbxUserData->GetDirectArrayVoid(j,
                                                                                                              &bArray);
    ...
    if (pMatElement) {
        if (pMatElement->GetMappingMode() == FbxGeometryElement::eByPolygon) {
            ...
            if (houdiniMatData.Empty()) {
                ...
                for (int i = 0; i < nPolygonCount; ++i) {
                    ...
                    if (materialList[nPolygonMaterialIndex].IsEmpty()) {
                        ...
                        for (HoudiniMatDataMap_t::Iterator hmdIt = houdiniMatData.Begin();
                             hmdIt != houdiniMatData.End(); ++hmdIt) {
                            FbxLayerElementArrayTemplate<void *> *pDirectArrayVoid = hmdIt->GetValue();
                            int *pDirectArrayInt = NULL;

                            pDirectArrayInt = pDirectArrayVoid->GetLocked(pDirectArrayInt,
            ...
            for (int i = 0; i < nMaterialCount; ++i) {
                ...
                CDmeFaceSet *pDmeFaceSet = CreateElement<CDmeFaceSet>(sByPolygonMaterialName.String(),
                                                                      pDmeMesh->GetFileId());
                CDmeMaterial *pDmeMaterial = CreateElement<CDmeMaterial>(sByPolygonMaterialName.String(),
                                                                         pDmeMesh->GetFileId());
            ...
            AddConversionError(pDmeMesh->GetFileId(), CFmtStrMax(
                    "GetFbxMaterialPath Failed! FbxMesh: %s, Unsupported material mapping mode, using: %s",
                    pDmeMesh->GetName(), sMaterialPath.Get()).Get());
        }
    } else {
        AddConversionError(pDmeMesh->GetFileId(),
                           CFmtStrMax("GetFbxMaterialPath Failed! FbxMesh: %s, No FbxMaterial assigned, using: %s",
                                      pDmeMesh->GetName(), sMaterialPath.Get()).Get());
    }
...
//
//-----------------------------------------------------------------------------
bool CDmFbxSerializer::FindMaterialResource(CUtlString &sOutMaterialPath, const char *pszInMaterialName,
...
//
//-----------------------------------------------------------------------------
bool CDmFbxSerializer::GetFbxMaterialPathFromFbxFileTexture(CUtlString &sMaterialPath, FbxFileTexture *pFileTexture,
                                                            CUtlVector<CUtlString> &materialSearchErrorList) const {
...
//
//-----------------------------------------------------------------------------
bool CDmFbxSerializer::GetFbxMaterialPath(CUtlString &sMaterialPath, FbxSurfaceMaterial *pFbxMat,
                                          CUtlVector<CUtlString> &materialSearchErrorList) const {
    if (!pFbxMat) {
        ...
        if (fbx_vmatPath.IsValid()) {
            ...
            if (eFbxType == eFbxString) {
                ...
                if (!sVMatPath.IsEmpty()) {
                    ...
                    materialSearchErrorList.AddToTail(
                            CFmtStr("FBXSurfaceMaterial( %s ).FBX_vmatPath found but empty", pFbxMat->GetName()).Get());
                }
            } else {
                materialSearchErrorList.AddToTail(
                        CFmtStr("FBXSurfaceMaterial( %s ).FBX_vmatPath found but not a string attribute",
                                pFbxMat->GetName()).Get());
            }
        }
    }
    ...
    if (nLayeredTextureCount > 0) {
        for (int j = 0; j < nLayeredTextureCount; ++j) {
            FbxLayeredTexture *pLayeredTexture = FbxCast<FbxLayeredTexture>(
                    fbxProperty.GetSrcObject(FbxLayeredTexture::ClassId, j));
            const int nTexCount = pLayeredTexture->GetSrcObjectCount(FbxTexture::ClassId);

            for (int k = 0; k < nTexCount; ++k) {
                FbxFileTexture *pFileTexture = FbxCast<FbxFileTexture>(
                        pLayeredTexture->GetSrcObject(FbxTexture::ClassId, k));
        ...
        if (nTexCount > 0) {
            for (int j = 0; j < nTexCount; ++j) {
                FbxFileTexture *pFileTexture = FbxCast<FbxFileTexture>(
                        fbxProperty.GetSrcObject(FbxTexture::ClassId, j));
...
//
//-----------------------------------------------------------------------------
void CDmFbxSerializer::SkinMesh(CDmeDag *pDmeDag, const FbxToDmxMap_t &fbxToDmxMap, CDmeModel *pDmeModel,
    ...
    for (int i = 0; i < nDeformerCount; ++i) {
        ...
        for (int j = 0; j < nClusterCount; ++j) {
            ...
            if (nLinkMode != FbxCluster::eNormalize && nLinkMode != FbxCluster::eTotalOne) {
                Warning("Warning! FBX mesh (%s) skinned to joint (%s) but mode %s isn't supported, only %s & %s\n",
                        pFbxNode->GetName(), pDmeJoint->GetName(), szClusterModes[nLinkMode],
                        szClusterModes[FbxCluster::eNormalize], szClusterModes[FbxCluster::eTotalOne]);
                continue;
            }
        ...
        for (int j = 0; j < vertexSkinWeights.Count(); ++j) {
            ...
            switch (nErr) {
                case CVertexSkin::kSkinErrorTooManyWeights:
                    AddConversionError(pDmRoot->GetFileId(),
                                       CFmtStr("Too many skin weights on some vertices, maximum is %d weights per vertex, will renormalize and discard smallest weights",
                                               CVertexSkin::kMaxWeights).Access());
                    break;
        ...
        pDmeVertexData->CreateJointWeightsAndIndices(CVertexSkin::kMaxWeights, &nJointWeightsIndex,
                                                     &nJointIndicesIndex);
...
//-----------------------------------------------------------------------------
void
CDmFbxSerializer::AddBlendShapes_R(const FbxToDmxMap_t &fbxToDmxMap, CDmElement *pDmeRoot, FbxNode *pFbxNode) const {
    FbxNodeAttribute *pFbxNodeAttribute = pFbxNode->GetNodeAttribute();
...
//
//-----------------------------------------------------------------------------
void CDmFbxSerializer::AddBlendShape(CDmeDag *pDmeDag, const FbxToDmxMap_t &fbxToDmxMap, CDmElement *pDmeRoot,
                                     FbxNode *pFbxNode) const {
    if (!pDmeDag)
        return;
    ...
    for (int nBlendShapeIndex = 0; nBlendShapeIndex < nBlendShapeCount; ++nBlendShapeIndex) {
        FbxBlendShape *pFbxBlendShape = FbxCast<FbxBlendShape>(
                pFbxMesh->GetDeformer(nBlendShapeIndex, FbxDeformer::eBlendShape));
        ...
        for (int nBlendShapeChannelIndex = 0;
             nBlendShapeChannelIndex < nBlendShapeChannelCount; ++nBlendShapeChannelIndex) {
            ...
            const char *pszExt = V_GetFileExtension(
                    pszDeltaName);        // FBX will probably name this blendShape.<THING> we just want <THING>
            if (pszExt) {
                pszDeltaName = pszExt;
            }
            ...
            for (int nTargetShapeIndex = 0; nTargetShapeIndex < nTargetShapeCount; ++nTargetShapeIndex) {
                ...
                CDmeVertexDeltaData *pDmeVertexDeltaData = pDmeMesh->FindOrCreateDeltaState(pszDeltaName,
                                                                                            m_bOptUnderscoreForCorrectors);
...
bool CDmFbxSerializer::FindOrCreateControl(CDmeCombinationOperator *pDmeComboOp, const char *pszName) const {
    if (m_bOptUnderscoreForCorrectors) {
        if (strchr(pszName,
                   '_'))        // Don't create a control if m_bOptUnderscoreForCorrectors is true and name has '_' in it
            return false;
    }
...
struct FbxDmxAnimData_t {
    FbxDmxAnimData_t()
            : m_pFbxNode(nullptr), m_pDmePosLog(nullptr), m_pDmeRotLog(nullptr), m_pParent(nullptr),
              m_pFbxAnimCurve(nullptr), m_pDmeFloatLog(nullptr), m_flMin(0.0f), m_flMax(1.0f), m_bNormalize(false) {
...
static CDmeFloatLog *CreateMorphChannel(const char *pszChannelName, CDmeChannelsClip *pDmeChannelsClip) {
    CDmeChannel *pDmeFloatChannel = CreateElement<CDmeChannel>(CFmtStr("%s_flex_channel", pszChannelName).Access(),
...
void CDmFbxSerializer::ComputeVstFlexSliderAnimDataList_R(
    ...
    if (pFbxNodeAttribute) {
        ...
        if (nAttributeType == FbxNodeAttribute::eNull) {
            ...
            if (sNodeName == "vstFlexSlider" || pFbxNode->FindProperty("vstFlexSlider", false).IsValid()) {
                for (FbxProperty fbxProperty = pFbxNode->GetFirstProperty(); fbxProperty.IsValid(); fbxProperty = pFbxNode->GetNextProperty(
                        fbxProperty)) {
                    ...
                    if (fbxProperty.GetFlag(FbxPropertyFlags::eUserDefined) &&
                        (eFbxType == eFbxDouble || eFbxType == eFbxFloat)) {
    ...
    for (int i = 0; i < pFbxNode->GetChildCount(); ++i) {
        ComputeVstFlexSliderAnimDataList_R(pFbxAnimLayer, animDataList, pDmeChannelsClip, fbxToDmxMap,
                                           pFbxNode->GetChild(i));
    }
}
...
void CDmFbxSerializer::ComputeAnimDataList_R(
    ...
    if (pFbxNodeAttribute && pFbxNodeAttribute->GetAttributeType() == FbxNodeAttribute::eMesh) {
        ...
        for (int lBlendShapeIndex = 0; lBlendShapeIndex < nBlendShapeDeformerCount; ++lBlendShapeIndex) {
            FbxBlendShape *pFbxBlendShape = static_cast< FbxBlendShape * >( pFbxGeometry->GetDeformer(lBlendShapeIndex,
            ...
            if (pFbxBlendShape) {
                ...
                for (int nChannelIndex = 0; nChannelIndex < nBlendShapeChannelCount; ++nChannelIndex) {
                    ...
                    const char *pszExt = V_GetFileExtension(
                            pszChannelName);        // FBX will probably name this blendShape.<THING> we just want <THING>
                    if (pszExt) {
                        pszChannelName = pszExt;
                    }

                    FbxAnimCurve *pFbxAnimCurve = pFbxGeometry->GetShapeChannel(lBlendShapeIndex, nChannelIndex,
                                                                                pFbxAnimLayer, true);
                    if (pFbxAnimCurve) {
                        bool bDefined = false;
                        ...
                        FOR_EACH_VEC(animDataList, aIt) {
                            ...
                            if (pFbxDmxAnimData->m_pDmeFloatLog && pFbxDmxAnimData->m_fbxProperty.IsValid() &&
                                !V_stricmp(pFbxDmxAnimData->m_fbxProperty.GetName(), pszChannelName)) {
                                if (Verbose1()) {
    ...
    for (int i = 0; i < pFbxNode->GetChildCount(); ++i) {
        ComputeAnimDataList_R(pFbxAnimLayer, animDataList, pDmeChannelsClip, fbxToDmxMap, pFbxNode->GetChild(i),
                              pAnimData);
    }
}
...
void CDmFbxSerializer::LoadAnimation(
        CDmElement *pDmeRoot, CDmeModel *pDmeModel, const FbxToDmxMap_t &fbxToDmxMap, FbxScene *pFbxScene,
    ...
    if (Verbose1()) {
        Msg(" * Animation Stack %s Layer %s - %s\n", pFbxAnimStack->GetName(), pFbxAnimLayer->GetName(),
            mAnimStackNameArray[0]->Buffer());
    ...
    for (int i = 0; i < pFbxRootNode->GetChildCount(); ++i) {
        ComputeVstFlexSliderAnimDataList_R(pFbxAnimLayer, animList, pDmeChannelsClip, fbxToDmxMap,
                                           pFbxRootNode->GetChild(i));
    }
