...
bool CDmElement::FindReferer(DmElementHandle_t hElement, CUtlVector<ElementPathItem_t> &elementPath,
    ...
    for (; hAttr != DMATTRIBUTE_REFERENCE_ITERATOR_INVALID; hAttr = g_pDataModel->NextAttributeReferencingElement(
            hAttr)) {
...
//-----------------------------------------------------------------------------
void
CDmElement::FixupReferences(CUtlHashFast<DmElementHandle_t> &visited, const CRefMap &refmap, TraversalDepth_t depth) {
...
void CDmElement::RemoveAttribute(CDmAttribute **pAttrRef) {
    ...
    if (g_pDataModel->UndoEnabledForElement(this)) {
        ...
        CUndoAttributeRemove *pUndo = new CUndoAttributeRemove(this, pAttrToDelete, pAttrToDelete->GetName(),
...
void CopyElements(const CUtlVector<CDmElement *> &from, CUtlVector<CDmElement *> &to,
    ...
    for (int i = 0; i < c; ++i) {
        ...
        CDmElement *pCopy = GetElement<CDmElement>(
                g_pDataModel->CreateElement(pFrom->GetType(), pFrom->GetName(), pFrom->GetFileId()));
        if (pCopy) {
