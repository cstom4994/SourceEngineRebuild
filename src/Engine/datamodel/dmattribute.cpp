...
public:
    ...
    virtual void
    SetMultiple(CDmAttribute *pAttribute, int i, int nCount, DmAttributeType_t valueType, const void *pValue) = 0;
...
public:
    ...
    virtual void
    SetMultiple(CDmAttribute *pAttribute, int i, int nCount, DmAttributeType_t valueType, const void *pValue);
...
public:
    ...
    CDmArrayAttributeOp(CDmAttribute *pAttribute) : m_pAttribute(pAttribute),
...
template<class T>
void
CDmArrayAttributeOp<T>::OnAttributeArrayElementAdded(int nFirstElem, int nLastElem, bool bUpdateElementReferences) {
...
int CDmArrayAttributeOp<T>::InsertBefore(int elem, const T &src) {
    ...
    if (g_pDataModel->UndoEnabledForElement(m_pAttribute->GetOwner())) {
        CUndoAttributeArrayInsertCopyBefore<T> *pUndo = new CUndoAttributeArrayInsertCopyBefore<T>(m_pAttribute, elem,
                                                                                                   src);
...
void CDmArrayAttributeOp<T>::FastRemove(int elem) {
    ...
    // UNDO HOOK
    if (g_pDataModel->UndoEnabledForElement(m_pAttribute->GetOwner())) {
        CUndoAttributeArrayRemoveElement<T> *pUndo = new CUndoAttributeArrayRemoveElement<T>(m_pAttribute, true, elem,
                                                                                             1);
        g_pDataModel->AddUndoElement(pUndo);
    }
...
void CDmArrayAttributeOp<T>::Remove(int elem) {
    ...
    // UNDO HOOK
    if (g_pDataModel->UndoEnabledForElement(m_pAttribute->GetOwner())) {
        CUndoAttributeArrayRemoveElement<T> *pUndo = new CUndoAttributeArrayRemoveElement<T>(m_pAttribute, false, elem,
                                                                                             1);
        g_pDataModel->AddUndoElement(pUndo);
    }
...
void CDmArrayAttributeOp<T>::Purge() {
    ...
    // UNDO HOOK
    if (g_pDataModel->UndoEnabledForElement(m_pAttribute->GetOwner())) {
        CUndoAttributeArrayCopyAllElement<T> *pUndo = new CUndoAttributeArrayCopyAllElement<T>(m_pAttribute, NULL,
                                                                                               true);
        g_pDataModel->AddUndoElement(pUndo);
    }
...
void CDmArrayAttributeOp<T>::CopyArray(const T *pArray, int nCount) {
    ...
    // UNDO HOOK
    if (g_pDataModel->UndoEnabledForElement(m_pAttribute->GetOwner())) {
        CUndoAttributeArrayCopyAllElement<T> *pUndo = new CUndoAttributeArrayCopyAllElement<T>(m_pAttribute, pArray,
                                                                                               nCount);
        g_pDataModel->AddUndoElement(pUndo);
    }
...
void CDmArrayAttributeOp<T>::SwapArray(CUtlVector<T> &src) {
    ...
    // UNDO HOOK
    if (g_pDataModel->UndoEnabledForElement(m_pAttribute->GetOwner())) {
        CUndoAttributeArrayCopyAllElement<T> *pUndo = new CUndoAttributeArrayCopyAllElement<T>(m_pAttribute, src.Base(),
                                                                                               src.Count());
        g_pDataModel->AddUndoElement(pUndo);
    }
...
void CDmArrayAttributeOp<T>::Set(int i, const T &value) {
    ...
    if (g_pDataModel->UndoEnabledForElement(m_pAttribute->GetOwner())) {
        CUndoArrayAttributeSetValueElement<T> *pUndo = new CUndoArrayAttributeSetValueElement<T>(m_pAttribute, i,
                                                                                                 value);
        g_pDataModel->AddUndoElement(pUndo);
    }
...
void CDmArrayAttributeOp<T>::SetMultiple(int i, int nCount, const T *pValue) {
    ...
    if (g_pDataModel->UndoEnabledForElement(m_pAttribute->GetOwner())) {
        CUndoArrayAttributeSetMultipleValueElement<T> *pUndo = new CUndoArrayAttributeSetMultipleValueElement<T>(
                m_pAttribute, i, nCount, pValue);
        g_pDataModel->AddUndoElement(pUndo);
    }
...
CDmAttribute *
CDmAttribute::CreateExternalAttribute(CDmElement *pOwner, DmAttributeType_t type, const char *pAttributeName,
...
void CDmAttribute::OnChanged(bool bArrayCountChanged, bool bIsTopological) {
    ...
    if (bIsTopological || IsTopological(GetType())) {
        ...
        g_pDataModelImp->NotifyState(
                bArrayCountChanged ? NOTIFY_CHANGE_ATTRIBUTE_ARRAY_SIZE : NOTIFY_CHANGE_ATTRIBUTE_VALUE);
    }
}
...
void CDmaDecorator<T, B>::Init(CDmElement *pOwner, const char *pAttributeName, int nFlags) {
    Assert(pOwner);
    this->m_pAttribute = pOwner->AddExternalAttribute(pAttributeName, CDmAttributeInfo<CUtlVector<T> >::AttributeType(),
                                                      &this->Value());
    Assert(m_pAttribute);
    if (nFlags) {
...
void CDmrGenericArrayConst::Init(const CDmElement *pElement, const char *pAttributeName) {
    const CDmAttribute *pAttribute = (pElement && pAttributeName && pAttributeName[0]) ? pElement->GetAttribute(
            pAttributeName) : NULL;
    Init(pAttribute);
}
...
template
class CDmaDecorator<CUtlString, CDmaStringArrayBase<CDmaDataInternal<CUtlVector<CUtlString> > > >;

template
class CDmrDecorator<CUtlString, CDmaStringArrayBase<CDmaDataExternal<CUtlVector<CUtlString> > > >;
