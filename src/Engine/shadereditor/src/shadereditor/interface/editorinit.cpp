...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(ShaderEditorInterface, IVShaderEditor, SHADEREDIT_INTERFACE_VERSION,
                                  gShaderEditorInterface);
...
void
UpdateScreenEffectTexture(ITexture *pTexture, int x, int y, int w, int h, bool bDestFullScreen, Rect_t *pActualRect) {
    Rect_t srcRect;
    srcRect.x = x;
    srcRect.y = y;
    srcRect.width = w;
...
#if defined( SHADER_EDITOR_DLL_SWARM ) || defined( SHADER_EDITOR_DLL_SER )
        qMode.SetValue("0");
#else
        qMode.SetValue( "-1" );
#endif
...
void ShaderEditorInterface::RegisterViewRenderCallback(const char *pszVrCName, pFnVrCallback(callback),
                                                       const char **pszBoolNames, const bool *pBoolDefaults,
                                                       const int numBoolParams,
                                                       const char **pszIntNames, const int *pIntDefaults,
                                                       const int numIntParams,
                                                       const char **pszFloatNames, const float *pFloatDefaults,
                                                       const int numFloatParams,
                                                       const char **pszStringNames, const char **pStringDefaults,
                                                       const int numStringParams) {
    char tmp[MAX_PATH];
...
IMaterialVar *ShaderEditorInterface::GetPPEMaterialVarFast(ShaderEditVarToken &token,
                                                           const char *pszPPEName, const char *pszNodeName,
...
void CheckDirectories() {
    ...
    if (!g_pFullFileSystem->FileExists(checkSCompile)) {
        ...
        if (!g_pFullFileSystem->IsDirectory(GetBinaryPath()))
            ...
            Q_snprintf(errormsg, MAXTARGC, "shadercompile.exe cannot be found at %s! Try reinstalling the Source SDK.",
                       GetBinaryPath());
