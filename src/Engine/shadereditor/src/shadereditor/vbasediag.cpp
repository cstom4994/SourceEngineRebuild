...
CBaseDiag::CBaseDiag(vgui::Panel *parent, CNodeView *nodeview, const char *pElementName, bool bDoAutoSorting)
...
void CBaseDiag::OnThink(void) {
    ...
    if (m_bDoAutoSorting) {
        ...
        const bool bMShouldActive = e >= 0 && e <= c && f >= 0 && f <= d && bGlobalInput &&
                                    (g_pVGuiInput->GetAppModalSurface() == GetVPanel() ||
                                     !g_pVGuiInput->GetAppModalSurface());
