...
CNodeView::CNodeView(Panel *parent, CEditorRoot *root, /*FlowGraphType_t type,*/ const char *pElementName) : BaseClass(
        parent, pElementName) {
...
void CNodeView::InitCanvas(FlowGraphType_t type, GenericShaderData *newData) {
    ...
    if (type == FLOWGRAPH_HLSL) {
        ...
        n->SetPosition(Vector2D(CRUCIALNODE_CANVAS_BASE_X - CRUCIALNODE_MIN_SIZE_X,
                                CRUCIALNODE_CANVAS_BASE_Y + CRUCIALNODE_CANVAS_OFFSET_Y));
...
void CNodeView::UpdateUserErrors() {
    ...
    if (type == FLOWGRAPH_HLSL) {
        ...
        if (pNodeStart) {
            ...
            for (CNodePP_Base *pNext = pNodeStart->GetNextPPNode();
                 pNext && !bGotError; pNext = pNext->GetNextPPNode()) {
...
int CNodeView::CreatePPSolversFromFile(const char *fileName, CUtlVector<CHLSL_SolverBase *> &hOutput,
...
bool CNodeView::IsCursorOutOfBounds(Vector2D *delta) {
    ...
    if (delta) {
        ...
        float out = 0;
        CBaseTrace tr;
        // FIXME:: IntersectRayWithBox was from public/collisionutils.cpp
...
void CNodeView::HandleSelectionInputReleasedBox() {
    ...
    for (int i = 0; i < m_hNodeList.Count(); i++) {
        ...
        // FIXME:: IsBoxIntersectingBox was from public/collisionutils.cpp
...
void CNodeView::Paint_Grid() {
    ...
    //for ( int i = 1; i < 2; i++ )
    {
        ...
        int num_vertical = max(1, (int) ceil(abs(delta.x) / GRID_CELL_SIZE)) + 1;
        int num_horizontal = max(1, (int) ceil(abs(delta.y) / GRID_CELL_SIZE)) + 1;
        ...
        thickness.x = max(1.0f, thickness.x - orig.x) * 0.5f;
...
void CNodeView::AddNodesToContextMenu(Menu *pNodeMenu) {
    ...
    if (type == FLOWGRAPH_HLSL || type == FLOWGRAPH_HLSL_TEMPLATE) {
        ...
        padd_const->AddMenuItem("Callback constant", new KeyValues("spawnNode", "type", HLSLNODE_CONSTANT_CALLBACK),
                                this);
        padd_const->AddMenuItem("VParam static", new KeyValues("spawnNode", "type", HLSLNODE_CONSTANT_VP_STATIC), this);
        padd_const->AddMenuItem("VParam mutable", new KeyValues("spawnNode", "type", HLSLNODE_CONSTANT_VP_MUTABLE),
                                this);
        ...
        padd_const->AddMenuItem("View forward", new KeyValues("spawnNode", "type", HLSLNODE_CONSTANT_VIEW_FORWARD),
                                this);
        ...
        padd_const->AddMenuItem("View world distance",
                                new KeyValues("spawnNode", "type", HLSLNODE_CONSTANT_VIEW_WORLDSPACE_DEPTH), this);
        padd_const->AddMenuItem("Time", new KeyValues("spawnNode", "type", HLSLNODE_CONSTANT_TIME), this);
        padd_const->AddMenuItem("FB texelsize", new KeyValues("spawnNode", "type", HLSLNODE_CONSTANT_FB_PIXELSIZE),
                                this);
        ...
        padd_const->AddMenuItem("Flashlight pos", new KeyValues("spawnNode", "type", HLSLNODE_CONSTANT_FLASHLIGHTPOS),
                                this);
        pNodeMenu->AddCascadingMenuItem("Constants", this, padd_const);

        padd_matrix->AddMenuItem("Matrix - ModelViewProj",
                                 new KeyValues("spawnNode", "type", HLSLNODE_MATRIX_MODELVIEWPROJ), this);
        padd_matrix->AddMenuItem("Matrix - ViewProj", new KeyValues("spawnNode", "type", HLSLNODE_MATRIX_VIEWPROJ),
                                 this);
        padd_matrix->AddMenuItem("Matrix - Model", new KeyValues("spawnNode", "type", HLSLNODE_MATRIX_MODEL), this);
        padd_matrix->AddMenuItem("Matrix - ViewModel", new KeyValues("spawnNode", "type", HLSLNODE_MATRIX_VIEWMODEL),
                                 this);
        padd_matrix->AddMenuItem("Matrix - Flashlight VP",
                                 new KeyValues("spawnNode", "type", HLSLNODE_MATRIX_FLASHLIGHT), this);
        ...
        padd_tex->AddMenuItem("Texture transform", new KeyValues("spawnNode", "type", HLSLNODE_TEXTURE_TRANSFORM),
                              this);
        padd_tex->AddMenuItem("Sampler object", new KeyValues("spawnNode", "type", HLSLNODE_TEXTURE_SAMPLEROBJECT),
                              this);
        padd_tex->AddMenuItem("Parallax UV", new KeyValues("spawnNode", "type", HLSLNODE_TEXTURE_PARALLAX), this);
        padd_tex->AddMenuItem("Parallax shadow", new KeyValues("spawnNode", "type", HLSLNODE_TEXTURE_PARALLAX_SHADOW),
                              this);
        ...
        padd_controlflow->AddMenuItem("Condition", new KeyValues("spawnNode", "type", HLSLNODE_CONTROLFLOW_CONDITION),
                                      this);
        padd_controlflow->AddMenuItem("Shader combo", new KeyValues("spawnNode", "type", HLSLNODE_CONTROLFLOW_COMBO),
                                      this);
        ...
        padd_utility_studio->AddMenuItem("Vertex lighting",
                                         new KeyValues("spawnNode", "type", HLSLNODE_STUDIO_VERTEXLIGHTING), this);
        padd_utility_studio->AddMenuItem("Pixelshader lighting",
                                         new KeyValues("spawnNode", "type", HLSLNODE_STUDIO_PIXELSHADER_LIGHTING),
                                         this);
        padd_utility_studio->AddMenuItem("Pixelshader specular",
                                         new KeyValues("spawnNode", "type", HLSLNODE_STUDIO_PIXELSHADER_SPECLIGHTING),
                                         this);
        padd_utility_studio->AddMenuItem("Skinning", new KeyValues("spawnNode", "type", HLSLNODE_STUDIO_SKINNING),
                                         this);
        padd_utility_studio->AddMenuItem("Morph", new KeyValues("spawnNode", "type", HLSLNODE_STUDIO_MORPH), this);
        padd_utility_studio->AddMenuItem("Vertex decompress",
                                         new KeyValues("spawnNode", "type", HLSLNODE_STUDIO_VCOMPRESSION), this);
        ...
        padd_render->AddMenuItem("Draw material",
                                 new KeyValues("spawnNode", "type", HLSLNODE_POSTPROCESSING_DRAW_MATERIAL), this);
        padd_render->AddMenuItem("Clear buffers",
                                 new KeyValues("spawnNode", "type", HLSLNODE_POSTPROCESSING_CLEAR_BUFFERS), this);
        padd_render->AddMenuItem("Update framebuffer texture",
                                 new KeyValues("spawnNode", "type", HLSLNODE_POSTPROCESSING_UPDATE_FB), this);
        padd_render->AddMenuItem("Render view", new KeyValues("spawnNode", "type", HLSLNODE_POSTPROCESSING_RENDER_VIEW),
                                 this);
        pNodeMenu->AddCascadingMenuItem("Render", this, padd_render);

        padd_stack->AddMenuItem("Push viewport", new KeyValues("spawnNode", "type", HLSLNODE_POSTPROCESSING_PUSH_VP),
                                this);
        padd_stack->AddMenuItem("Pop viewport", new KeyValues("spawnNode", "type", HLSLNODE_POSTPROCESSING_POP_VP),
                                this);
        padd_stack->AddMenuItem("Set rendertarget", new KeyValues("spawnNode", "type", HLSLNODE_POSTPROCESSING_SET_RT),
                                this);
        pNodeMenu->AddCascadingMenuItem("Viewport stack", this, padd_stack);

        padd_resource->AddMenuItem("Rendertarget",
                                   new KeyValues("spawnNode", "type", HLSLNODE_POSTPROCESSING_CREATE_RT), this);
        padd_resource->AddMenuItem("Material", new KeyValues("spawnNode", "type", HLSLNODE_POSTPROCESSING_CREATE_MAT),
                                   this);
        padd_resource->AddMenuItem("Copy rendertarget",
                                   new KeyValues("spawnNode", "type", HLSLNODE_POSTPROCESSING_COPY_RT), this);
