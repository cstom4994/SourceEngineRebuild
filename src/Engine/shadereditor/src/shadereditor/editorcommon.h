...
float GaussianWeight_2D(const int &iPos_X, const int &iPos_Y, const int &iSize_X, const int &iSize_Y,
                        const float &flBias = 0);
...
void ComposeShaderName(GenericShaderData *data, bool bPS, bool bExtension, char *out, int maxbuf,
                       bool bPosOverride = false); // fxc
void
ComposeShaderPath(GenericShaderData *data, bool bPS, bool bExtension, char *out, int maxbuf, bool bPosOverride = false);

void ComposeShaderName_Compiled(GenericShaderData *data, bool bPS, bool bExtension, char *out, int maxbuf,
                                bool bPosOverride = false); // vcs
void ComposeShaderPath_Compiled(GenericShaderData *data, bool bPS, bool bExtension, char *out, int maxbuf,
                                bool bPosOverride = false);
...
void
BuildUserFunctionName(const char *pFunctionName, const CUtlVector<__funcParamSetup *> &sharedOutputs, char *pszDest,
                      int buflen);

void BuildUserFunctionName(const char *pFunctionName, const CUtlVector<__funcParamSetup *> &input,
                           const CUtlVector<__funcParamSetup *> &output,
...
void CleanupString(const char *pSrc, char *pOut, int maxLength, bool AllowSpaces = false, bool AllowUnderscore = true,
                   bool AllowNumbers = true, bool AllowNumberInFirstPlace = false);
...
void BuildFormatFlags(BasicShaderCfg_t &data, CUtlVector<CHLSL_SolverBase *> &hVertexshaderStack,
                      CUtlVector<CHLSL_SolverBase *> &hPixelhaderStack);
...
public:
    ...
    void Modify(const char *pszName = NULL,
                ImageFormat *format = NULL, RenderTargetSizeMode_t *sizemode = NULL,
                MaterialRenderTargetDepth_t *depthmode = NULL,
...
bool
SetMaterialVar_Vec(IMaterial *pMat, const char *varName, const float a, const float b, const float c, const float d);
...
void UpdateScreenEffectTexture(ITexture *pTexture, int x, int y, int w, int h, bool bDestFullScreen = false,
