...
void CSmartObjectList::ParseCode_Stream(CUtlBuffer &hStream, bool bRecursive, CUtlVector<char *> *hIncludes,
                                        const char *sourceFile, int scopeOffset) {
...
void CSmartObjectList::ParseCode_PreProc(CUtlVector<char *> &hLines, bool bRecursive, CUtlVector<char *> *hIncludes,
                ...
                if (!IS_BRACKET(*pScopeReader) && !IS_SPACE(*pScopeReader) && !bEvaluatedExpression) {
                    ...
                    if (bRegexTmpNeedsRebuild) {
                        ...
                        for (int iKeyword = 0; iKeyword <= __REGEX_KEYWORDS_LASTACCESS_MOD; iKeyword++) {
                            Q_strcat(accessModRegexTmp, CRegex::GetSyntaxList_KeyWords()[iKeyword],
                                     sizeof(accessModRegexTmp));
                        ...
                        for (int iDatatype = 0; iDatatype < iDType; iDatatype++) {
                            if (iDatatype < iDefDType)
                                Q_strcat(accessModRegexTmp, CRegex::GetSyntaxList_Datatypes_Regex()[iDatatype],
                                         sizeof(accessModRegexTmp));
                            else {
                        ...
                        Q_snprintf(regexTmp_Var, sizeof(regexTmp_Var), "%s[ \\t]+[\\w\\d]+.*?", accessModRegexTmp);
                        Q_snprintf(regexTmp_Func, sizeof(regexTmp_Func),
                                   "%s[ \\t]+[\\w\\d]+[ \\t]*\\((?:[[:print:]\\t]+)?\\)[[:print:]\\t]*",
                                   accessModRegexTmp);
