...
void CSheet_Custom_IO::AllocNodeData() {
    ...
    for (int i = 0; i < m_hCtrl_In.Count(); i++) {
        ...
        m_hszVarNames_In.AddToTail(
                CNodeCustom::AllocOutputSetup(false, tmp, ctrl->pType ? GetVarTypeFlag(ctrl->pType->GetActiveItem())
                                                                      : HLSLVAR_FLOAT1)
        );
    }
    for (int i = 0; i < m_hCtrl_Out.Count(); i++) {
        ...
        m_hszVarNames_Out.AddToTail(
                CNodeCustom::AllocOutputSetup(true, tmp, ctrl->pType ? GetVarTypeFlag(ctrl->pType->GetActiveItem())
                                                                     : HLSLVAR_FLOAT1)
        );
    }
}
