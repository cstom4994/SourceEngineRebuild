...
void CBaseContainerNode::UpdateSize() {
    ...
    m_vecSize.Init(max(m_flMinSizeX, 0.0f), -sizeMin);
    TouchJacks();
...
void CBaseContainerNode::OnDrag(Vector2D &delta) {
    ...
    if (m_vecContainerExtents[0] < m_flMinSizeY) {
        m_vecAccumulatedExtent[0] += min(m_vecContainerExtents[0] - m_flMinSizeY, 0.0f);
        m_vecAccumulatedExtent[4] -= min(m_vecContainerExtents[0] - m_flMinSizeY, 0.0f) * Sign(pos_delta.y);

        float _save = oldExtents.x - m_flMinSizeY;
        _save = max(_save, 0.0f);
        pos_delta.y = _save * Sign(pos_delta.y);
        m_vecContainerExtents[0] = m_flMinSizeY;
    }
    if (m_vecContainerExtents[2] > -m_flMinSizeY) {
        m_vecAccumulatedExtent[2] -= min(abs(m_vecContainerExtents[2]) - (m_flMinSizeY), 0.0f);
    ...
    if (m_vecContainerExtents[1] < m_flMinSizeX) {
        ...
        m_vecAccumulatedExtent[3] -= min(m_vecContainerExtents[1] - m_flMinSizeX, 0.0f);
        m_vecAccumulatedExtent[1] += min(m_vecContainerExtents[1] - m_flMinSizeX, 0.0f);
...
bool CBaseContainerNode::IsSolvable(bool bHierachyUp, CUtlVector<CBaseNode *> *hCallerList,
                                    CUtlVector<CBaseNode *> *hTested) {
...
#define TITEL_INSET_X NODE_DRAW_TITLE_SPACE
    ...
    if (bShadow) {
        ...
        surface()->DrawFilledRect(rects_[7].x - borderSize, rects_[0].y - borderSize, rects_[2].x + borderSize,
                                  rects_[0].y);
        surface()->DrawFilledRect(rects_[7].x - borderSize, rects_[5].y, rects_[2].x + borderSize,
                                  rects_[5].y + borderSize);
    }
