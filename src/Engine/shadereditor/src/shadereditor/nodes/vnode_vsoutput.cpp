...
void CNodeVSOutput::Setup(SETUP_HLSL_VS_Output_PS_Input info) {
    ...
    LockJackInput_Flags(curInput, HLSLVAR_FLOAT4, "ProjPosition");
    curInput++;
    ...
    for (int i = 0; i < info.iTexcoords_num - extraRows; i++) {
        ...
        LockJackInput_Flags(curInput, info.iTexCoordFlag[texCoordNameOffset],
                            VarArgs("TexCoords %i", texCoordNameOffset));
...
void CNodeVSOutput::RestoreFromKeyValues_Specific(KeyValues *pKV) {
    ...
    for (int i = 0; i < VSTOPS_NUM_TEXCOORDS; i++) {
        ...
        Q_snprintf(curNames.names_texcoords[i], sizeof(curNames.names_texcoords[i]), "%s",
                   pKV->GetString(VarArgs("numTexcoord_Name_%i", i)));
    }
    for (int i = 0; i < VSTOPS_NUM_COLORS; i++) {
        info.iColorFlag[i] = pKV->GetInt(VarArgs("numColor_Flag_%i", i));
        Q_snprintf(curNames.names_colors[i], sizeof(curNames.names_colors[i]), "%s",
                   pKV->GetString(VarArgs("numColor_Name_%i", i)));
    }
