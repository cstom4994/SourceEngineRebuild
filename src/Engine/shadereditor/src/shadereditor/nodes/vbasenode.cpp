...
bool CBaseNode::RecursiveTestContainerError_Internal(CUtlVector<CBaseNode *> &m_hNodesProcessed,
                                                     bool &bLeftContainerOnce, const bool &bHierachyUp,
                                                     CBaseContainerNode *container) {
    bool bError = false;
    ...
    for (int i = 0; i < numJacks; i++) {
        ...
        for (int b = 0; b < j->GetNumBridges(); b++) {
            ...
            bError = bError || pNodeEnd->RecursiveTestContainerError_Internal(m_hNodesProcessed,
                                                                              bLeftContainerOnce, bHierachyUp,
                                                                              container);
        }
    }
    ...
    for (int i = 0; i < numJacks; i++) {
        ...
        for (int x = 0; x < j->GetNumBridges(); x++) {
            ...
            if (next && !m_hProcessedNodes.IsValidIndex(m_hProcessedNodes.Find(next)) &&
                ...
                next->Recursive_AddTailNodes_Internal(m_hProcessedNodes, m_hList, bHierachyUp, bOnlyWithoutSolver,
                                                      pContainer, bAddContainers);
        }
    }
}
	...
	*/
    ...
    for (int i = 0; i < GetNumContainers(); i++) {
        if (RecursiveTestContainerError(false, GetContainer(i)))
            inputErrorLevel = max(inputErrorLevel, (int)ERRORLEVEL_FAIL);
...
int
CBaseNode::GetHierachyTypeIterateFullyRecursive_Internal(CUtlVector<CBaseNode *> &m_hNodesProcessed, int *iAllowances) {
...
void CBaseNode::VguiDraw_Preview() {
    ...
    Vector2D prev_max(m_vecPosition.x + m_vecSize.x - m_vecBorderInfo.y - preview_inset,
                      GetBoundsBoxMax().y + preview_inset);
    ...
    if (GetErrorLevel() == ERRORLEVEL_NONE && GetNumSolvers() && !GetSolver(0)->IsDummy() &&
        sedit_2dprev_Enable.GetInt()) {
        Vector2D a, b;
