...
EditorPostProcessingEffect::EditorPostProcessingEffect(CUtlVector<CHLSL_SolverBase *> &hSolvers, GenericPPEData &config,
                                                       bool bCopySolvers) {
    AllocCheck_Alloc();
...
CPostProcessingCache::PPE_Error CPostProcessingCache::LoadPostProcessingEffect(const char *path, bool bStartEnabled) {
    ...
    CPostProcessingCache::PPE_Error error = (CPostProcessingCache::PPE_Error) effect.LoadPostProcessingChain(path,
                                                                                                             bStartEnabled);
...
void CPostProcessingCache::RenderSinglePPE(EditorPostProcessingEffect *effect, bool bPreviewMode, bool bSceneMode,
                                           bool bOwnsEffect) {
...
void CPostProcessingCache::RenderSinglePPE(EditorPostProcessingEffect *effect, int x, int y, int w, int h,
    ...
    if (!bSceneMode && r_queued_post_processing.GetInt()) {
        ...
        if (pCallQueue) {
            ...
            return pCallQueue->QueueCall(CPostProcessingCache::RenderSinglePPE, pNewEffect, x, y, w, h, bPreviewMode,
                                         bSceneMode, true);
        }
    }
#endif
