...
//-----------------------------------------------------------------------------
    ...
    public:
        ...
        virtual VPANEL
        GetDefaultButton();                    // panel which receives default input when ENTER is hit, if current focused item cannot accept ENTER
        ...
        virtual bool RequestFocusPrev(
                VPANEL panel = NULL); // if panel is NULL, then the tab increment is based last known panel that had key focus
