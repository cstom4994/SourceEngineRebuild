        ...
        while (token && token[0]) {
            ...
            if (!stricmp(token, "animate")) {
                ...
                if (cmdAnimate.variable == m_sPosition) {
                    ...
                    if (0 ==
                        sscanf(token, "%f %f %f %f", &cmdAnimate.target.a, &cmdAnimate.target.b, &cmdAnimate.target.c,
                               &cmdAnimate.target.d)) {
                ...
                if (cmdAnimate.variable == m_sSize) {
                    if (IsProportional()) {
                        cmdAnimate.target.a = static_cast<float>( vgui::scheme()->GetProportionalScaledValueEx(
                                GetScheme(), cmdAnimate.target.a));
                        cmdAnimate.target.b = static_cast<float>( vgui::scheme()->GetProportionalScaledValueEx(
...
AnimationController::GetInterpolatedValue(int interpolator, float interpolatorParam, float currentTime, float startTime,
...
//-----------------------------------------------------------------------------
void
AnimationController::RemoveQueuedAnimationByType(vgui::Panel *panel, UtlSymId_t variable, UtlSymId_t sequenceToIgnore) {
    for (int i = 0; i < m_ActiveAnimations.Count(); i++) {
...
void AnimationController::SetValue(ActiveAnimation_t &anim, Panel *panel, UtlSymId_t var, Value_t &value) {
    if (var == m_sPosition) {
        ...
        if (value.b == 0.0f && value.c == 0.0f && value.d == 0.0f) {
            ...
            Color col((unsigned char) value.a, (unsigned char) value.b, (unsigned char) value.c,
