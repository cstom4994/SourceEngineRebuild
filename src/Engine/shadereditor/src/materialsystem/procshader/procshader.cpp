...
void UpdateConstantByIdentifier(CBaseVSShader *pShader, IShaderDynamicAPI *pShaderAPI, IMaterialVar **params,
                                SimpleEnvConstant *pConst, CProceduralContext *pContext,
    ...
    switch (pConst->iEnvC_ID) {
        ...
        case HLSLENV_STUDIO_MORPHING: {
            pShader->SetHWMorphVertexShaderState_NoTex(VERTEX_SHADER_SHADER_SPECIFIC_CONST_10,
...
int CalcShaderIndex(CBaseVSShader *pShader, IShaderShadow *pShaderShadow, IShaderDynamicAPI *pShaderAPI,
...
void
DrawFallback(CBaseVSShader *pShader, IMaterialVar **params, IShaderShadow *pShaderShadow, IShaderDynamicAPI *pShaderAPI,
             VertexCompressionType_t vertexCompression) {
    SHADOW_STATE {
...
BEGIN_VS_SHADER(EDITOR_SHADER, "")
    ...
    END_SHADER_PARAMS
        ...
        void InitFlags2(BasicShaderCfg_t *cfg, IMaterialVar **params) {
            ...
            if (cfg->iVFMT_flags & VERTEX_TANGENT_S || cfg->iVFMT_flags & VERTEX_TANGENT_T ||
                cfg->iVFMT_flags & VERTEX_NORMAL)
                SET_FLAGS2(MATERIAL_VAR2_NEEDS_TANGENT_SPACES);
...
#define __IS_NUM(x) ( ( x >= ((int)'0') && x <= ((int)'9') ) || x == '.' || x == '-' )
        ...
        void InitSmartConstants_Static(CUtlVector<SimpleEnvConstant *> &hList, IMaterialVar **params, KeyValues *data,
                                       bool bPreloaded) {
        ...
        void ShaderInit_Textures(CUtlVector<SimpleTexture *> &hList, IMaterialVar **params, KeyValues *data,
                                 int startRegister) {
        ...
        SHADER_INIT {
            ...
            if (bCustomShader) {
                if (bInlineMaterial) {
                    ...
                    pKV = new KeyValues("");
                    if (!pKV->LoadFromFile(g_pFullFileSystem, fullpath) &&
                        !pKV->LoadFromFile(g_pFullFileSystem, localpath)) {
                        pKV->deleteThis();
        ...
        void ShaderInitParams_Textures(CUtlVector<SimpleTexture *> &hList, IMaterialVar **params, int startRegister,
                                       int numregister) {
...
//#endif
            ...
            SHADOW_STATE {
                ...
                int index_vs_static = CalcShaderIndex(this, pShaderShadow, pShaderAPI, vertexCompression, params,
                                                      hCombo_VS,
                                                      true, cfg->pVS_Identifiers->inum_DynamicCombos, bPreviewMode);
                int index_ps_static = CalcShaderIndex(this, pShaderShadow, pShaderAPI, vertexCompression, params,
                                                      hCombo_PS,
            ...
            DYNAMIC_STATE {
                ...
                int index_vs_dynamic = CalcShaderIndex(this, pShaderShadow, pShaderAPI, vertexCompression, params,
                                                       hCombo_VS,
                                                       false, cfg->pVS_Identifiers->inum_DynamicCombos, bPreviewMode);
                int index_ps_dynamic = CalcShaderIndex(this, pShaderShadow, pShaderAPI, vertexCompression, params,
                                                       hCombo_PS,
                ...
                for (; constCount > 0; constCount--, _constBase++) {
                    _const = *_constBase;
                    UpdateConstantByIdentifier(this, pShaderAPI, params, _const, pContext, true, MUTABLE_01,
                                               VPSTATIC_00);
                }
                ...
                for (; constCount > 0; constCount--, _constBase++) {
                    _const = *_constBase;
                    UpdateConstantByIdentifier(this, pShaderAPI, params, _const, pContext, false, MUTABLE_01,
                                               VPSTATIC_00);
                }
