...
ResponseRulePartition::tIndex ResponseRulePartition::IndexFromDictElem(tRuleDict *pDict, int elem) {
    ...
    AssertMsg1(elem <= kIDX_ELEM_MASK,
               "A rule dictionary has %d elements; this exceeds the 255 that can be packed into an index.\n", elem);
...
void ResponseRulePartition::RemoveAll(void) {
    for (int bukkit = 0; bukkit < N_RESPONSE_PARTITIONS; ++bukkit) {
        for (int i = m_RuleParts[bukkit].FirstInorder();
             i != m_RuleParts[bukkit].InvalidIndex(); i = m_RuleParts[bukkit].NextInorder(i)) {
...
void ResponseRulePartition::GetDictsForCriteria(CUtlVectorFixed<ResponseRulePartition::tRuleDict *, 2> *pResult,
