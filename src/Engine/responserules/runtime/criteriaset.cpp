...
// Purpose: 
//-----------------------------------------------------------------------------
CriteriaSet::CriteriaSet(const CriteriaSet &src) : m_Lookup(0, 0, CritEntry_t::LessFunc),
                                                   m_nNumPrefixedContexts(src.m_nNumPrefixedContexts) {
...
CriteriaSet::CriteriaSet(const char *criteria, const char *value) : m_Lookup(0, 0, CritEntry_t::LessFunc),
                                                                    m_bOverrideOnAppend(true) {
...
int CriteriaSet::InterceptWorldSetContexts(CriteriaSet *RESTRICT pFrom, CriteriaSet *RESTRICT pSetOnWorld) {
    ...
    for (int i = pFrom->Head(); pFrom->IsValidIndex(i); i = pFrom->Next(i)) {
        ...
        if (pszName[0] == CriteriaSet::kAPPLYTOWORLDPREFIX) {    // redirect to the world contexts
            ...
            buf[PREFIXLEN] = 0;
        } else {    // does not need to be fiddled; do not write back to world
            rewrite.AppendCriteria(pFrom->GetNameSymbol(i), pFrom->GetValue(i), pFrom->GetWeight(i));
        }
    }
    AssertMsg2(pSetOnWorld->GetCount() == nPrefixedContexts,
               "Count of $ persistent RR contexts is inconsistent (%d vs %d)! Call Elan.",
