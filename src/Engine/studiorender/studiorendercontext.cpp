...
void CStudioRenderContext::LoadMaterials(studiohdr_t *phdr,
                                         OptimizedModel::FileHeader_t *pVtxHeader, studioloddata_t &lodData,
                                         int lodID) {
...
//-----------------------------------------------------------------------------
int
CStudioRenderContext::CountDeltaFlexedStripGroups(mstudiomodel_t *pModel, OptimizedModel::ModelLODHeader_t *pVtxLOD) {
        ...
        for (i = 0; i < pStripGroup->numVerts; ++i) {
            ...
            switch (compressionType) {
                case VERTEX_COMPRESSION_ON:
                    success = R_AddVertexToMesh<VERTEX_COMPRESSION_ON>(pModelName, bNeedsTangentSpace, meshBuilder,
                                                                       pStripGroup->pVertex(i), pMesh, vertData,
                                                                       hwSkin);
                    break;
                case VERTEX_COMPRESSION_NONE:
                default:
                    success = R_AddVertexToMesh<VERTEX_COMPRESSION_NONE>(pModelName, bNeedsTangentSpace, meshBuilder,
                                                                         pStripGroup->pVertex(i), pMesh, vertData,
                                                                         hwSkin);
                    break;
            }
...
//-----------------------------------------------------------------------------
VertexFormat_t
CStudioRenderContext::CalculateVertexFormat(const studiohdr_t *pStudioHdr, const studioloddata_t *pStudioLodData,
    ...
    if (bIsDX7) {
        ...
        for (int i = 0; i < pStudioHdr->numskinfamilies; i++) {
            ...
            VertexFormat_t vertexFormat = bIsDX7 ? MATERIAL_VERTEX_FORMAT_MODEL_DX7 : (MATERIAL_VERTEX_FORMAT_MODEL &
                                                                                       ~VERTEX_COLOR);
...
void CStudioRenderContext::R_StudioCreateSingleMesh(studiohdr_t *pStudioHdr, studioloddata_t *pStudioLodData,
    ...
    for (int i = 0; i < pVtxMesh->numStripGroups; ++i) {
        ...
        VertexFormat_t vertexFormat = CalculateVertexFormat(pStudioHdr, pStudioLodData, pMesh, pStripGroup,
                                                            bIsHwSkinned);
...
//-----------------------------------------------------------------------------
void
CStudioRenderContext::BuildDecalBoneMap(studiohdr_t *pStudioHdr, int *pUsedBones, int *pBoneRemap, int *pMaxBoneCount,
                ...
                for (stripID = 0; stripID < pMeshGroup->m_NumStrips; stripID++) {
                    ...
                    if (bIsHWSkinned) {
                        ...
                        for (boneID = 0; boneID < pStripData->numBoneStateChanges; boneID++) {
                            OptimizedModel::BoneStateChangeHeader_t *pBoneStateChange = pStripData->pBoneStateChange(
                                    boneID);
...
void CStudioRenderContext::DrawModelArray(const DrawModelInfo_t &drawInfo, int arrayCount,
    ...
    if (!pCallQueue || studio_queue_mode.GetInt() == 0) {
        ...
        pCallQueue->QueueFunctor(
                StudioRenderFunctor(g_pStudioRenderImp, &CStudioRender::DrawModelArray, drawInfo, m_RC, arrayCount,
                                    pInstanceData, instanceStride, flags));
    }
}
...
//-----------------------------------------------------------------------------
void
CStudioRenderContext::DrawModelStaticProp(const DrawModelInfo_t &info, const matrix3x4_t &modelToWorld, int flags) {
    ...
    if (!pCallQueue || studio_queue_mode.GetInt() == 0) {
        ...
        InvokeBindProxies(info);
        pCallQueue->QueueFunctor(
                StudioRenderFunctor(g_pStudioRenderImp, &CStudioRender::DrawModelStaticProp, info, m_RC, modelToWorld,
                                    flags));
    }
}
...
void
CStudioRenderContext::DrawStaticPropShadows(const DrawModelInfo_t &info, const matrix3x4_t &modelToWorld, int flags) {
...
void CStudioRenderContext::AddShadow(IMaterial *pMaterial, void *pProxyData,
    ...
    if (!pCallQueue || studio_queue_mode.GetInt() == 0) {
        ...
        pCallQueue->QueueFunctor(StudioRenderFunctor(g_pStudioRenderImp, &CStudioRender::AddShadow, pMaterial,
                                                     (void *) NULL, rdFlashlight.Base(), rdMatrix.Base(),
                                                     pFlashlightDepthTexture));
    }
}
        ...
        if (pDecalInfo) {
            ...
            if (decalInfo.handle) {
                ...
                QUEUE_STUDIORENDER_CALL_RC(AddDecal, CStudioRender, g_pStudioRenderImp, pRenderContext,
                                           decalInfo.handle, m_RC, decalInfo.pBoneToWorld, decalInfo.pStudioHdr,
                                           decalInfo.ray, decalInfo.decalUp, decalInfo.pDecalMaterial, decalInfo.radius,
...
StudioRenderDecalInfo_t::StudioRenderDecalInfo_t() : handle(nullptr), pStudioHdr(nullptr), pBoneToWorld(nullptr),
                                                     ray(Ray_t()), decalUp(Vector()),
...
void CStudioRenderContext::AddDecal(StudioDecalHandle_t handle, studiohdr_t *pStudioHdr,
                                    ...
                                    IMaterial *pDecalMaterial, float radius, int body, bool noPokethru,
                                    int maxLODToDecal) {
    m_addDecalRequests.PushItem(
            new StudioRenderDecalInfo_t(handle, pStudioHdr, pBoneToWorld, ray, decalUp, pDecalMaterial, radius, body,
