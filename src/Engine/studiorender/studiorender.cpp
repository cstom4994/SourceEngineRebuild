...
void CStudioRender::InitDebugMaterials(void) {
    ...
    m_pMaterialMRMWireframeZBuffer =
            g_pMaterialSystem->FindMaterial("//platform/materials/debug/debugmrmwireframezbuffer", TEXTURE_GROUP_OTHER,
                                            true);
    ...
    m_pMaterialTranslucentModelHulls =
            g_pMaterialSystem->FindMaterial("//platform/materials/debug/debugtranslucentmodelhulls",
                                            TEXTURE_GROUP_OTHER, true);
    m_pMaterialTranslucentModelHulls->IncrementReferenceCount();

    m_pMaterialSolidModelHulls =
            g_pMaterialSystem->FindMaterial("//platform/materials/debug/debugsolidmodelhulls", TEXTURE_GROUP_OTHER,
                                            true);
    m_pMaterialSolidModelHulls->IncrementReferenceCount();

    m_pMaterialAdditiveVertexColorVertexAlpha =
            g_pMaterialSystem->FindMaterial("//platform/materials/debug/additivevertexcolorvertexalpha",
                                            TEXTURE_GROUP_OTHER, true);
    ...
    m_pMaterialWorldWireframe =
            g_pMaterialSystem->FindMaterial("//platform/materials/debug/debugworldwireframe", TEXTURE_GROUP_OTHER,
                                            true);
    ...
    if (g_pMaterialSystemHardwareConfig->GetDXSupportLevel() >= 90) {
        ...
        m_pDepthWrite[0][0] = g_pMaterialSystem->FindProceduralMaterial("__DepthWrite00", TEXTURE_GROUP_OTHER,
                                                                        pVMTKeyValues);
        ...
        m_pDepthWrite[0][1] = g_pMaterialSystem->FindProceduralMaterial("__DepthWrite01", TEXTURE_GROUP_OTHER,
                                                                        pVMTKeyValues);
        ...
        pVMTKeyValues->SetInt("$nocull", 0);
        m_pDepthWrite[1][0] = g_pMaterialSystem->FindProceduralMaterial("__DepthWrite10", TEXTURE_GROUP_OTHER,
                                                                        pVMTKeyValues);
        ...
        pVMTKeyValues->SetInt("$alphatest", 1);
        pVMTKeyValues->SetInt("$nocull", 1);
        m_pDepthWrite[1][1] = g_pMaterialSystem->FindProceduralMaterial("__DepthWrite11", TEXTURE_GROUP_OTHER,
                                                                        pVMTKeyValues);
        ...
        m_pSSAODepthWrite[0][0] = g_pMaterialSystem->FindProceduralMaterial("__ColorDepthWrite00", TEXTURE_GROUP_OTHER,
                                                                            pVMTKeyValues);
        ...
        pVMTKeyValues->SetInt("$nocull", 1);
        pVMTKeyValues->SetInt("$color_depth", 1);
        m_pSSAODepthWrite[0][1] = g_pMaterialSystem->FindProceduralMaterial("__ColorDepthWrite01", TEXTURE_GROUP_OTHER,
                                                                            pVMTKeyValues);
        ...
        pVMTKeyValues->SetInt("$nocull", 0);
        pVMTKeyValues->SetInt("$color_depth", 1);
        m_pSSAODepthWrite[1][0] = g_pMaterialSystem->FindProceduralMaterial("__ColorDepthWrite10", TEXTURE_GROUP_OTHER,
                                                                            pVMTKeyValues);
        ...
        pVMTKeyValues->SetInt("$nocull", 1);
        pVMTKeyValues->SetInt("$color_depth", 1);
        m_pSSAODepthWrite[1][1] = g_pMaterialSystem->FindProceduralMaterial("__ColorDepthWrite11", TEXTURE_GROUP_OTHER,
                                                                            pVMTKeyValues);
    ...
    for (int body = 0; body < pStudioHdr->numbodyparts; ++body) {
        ...
        for (int meshIndex = 0; meshIndex < pSubmodel->nummeshes; ++meshIndex) {
            ...
            IMaterial *pMaterial = R_StudioSetupSkinAndLighting(pRenderContext, pskinref[pmesh->material], ppMaterials,
                                                                materialFlags, drawInfo.m_pClientEntity, NULL,
                                                                lighting);
            if (!pMaterial)
                continue;
                    ...
                    for (int i = 0; i < arrayCount; i++, pData += instanceStride) {
                        ...
                        for (int j = 0; j < pGroup->m_NumStrips; ++j) {
                            ...
                            MaterialPrimitiveType_t localStripType =
                                    pStrip->flags & OptimizedModel::STRIP_IS_TRISTRIP ? MATERIAL_TRIANGLE_STRIP
                                                                                      : MATERIAL_TRIANGLES;
