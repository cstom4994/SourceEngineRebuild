...
//-----------------------------------------------------------------------------
FORCEINLINE StudioModelLighting_t
CStudioRender::R_StudioComputeLighting(IMaterial *pMaterial, int materialFlags, ColorMeshInfo_t *pColorMeshes) {
...
IMaterial *
CStudioRender::R_StudioSetupSkinAndLighting(IMatRenderContext *pRenderContext, int index, IMaterial **ppMaterials,
                                            int materialFlags,
    ...
    if (m_pRC->m_Config.bWireframe && !m_pRC->m_pForcedMaterial) {
        ...
        if (!m_pRC->m_pForcedMaterial && (m_pRC->m_nForcedMaterialType != OVERRIDE_DEPTH_WRITE &&
            ...
            if (m_pRC->m_nForcedMaterialType == OVERRIDE_BUILD_SHADOWS) {
                ...
                IMaterialVar *pOriginalTextureVar = ppMaterials[index]->FindVarFast("$basetexture",
                ...
                if (nAlphaTest != 0) {
                    ...
                    IMaterialVar *pOriginalTextureFrameVar = ppMaterials[index]->FindVarFast("$frame",
                                                                                             &originalTextureFrameVarCache);
                    static unsigned int originalAlphaRefCache = 0;
                    IMaterialVar *pOriginalAlphaRefVar = ppMaterials[index]->FindVarFast("$AlphaTestReference",
