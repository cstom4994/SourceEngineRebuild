...
static ConVar r_eyeglintlodpixels("r_eyeglintlodpixels", "20.0", FCVAR_CHEAT,
...
ConVar r_debugrandomstaticlighting("r_debugrandomstaticlighting", "0", FCVAR_CHEAT,
...
*/
float
Engine_WorldLightAngle(const dworldlight_t *wl, const Vector &lnormal, const Vector &snormal, const Vector &delta) {
    ...
    static CColorMeshData *CreateResource(const colormeshparams_t &params) {
        ...
        for (int i = 0; i < params.m_nMeshes; i++) {
            ...
            if (params.m_pPooledVBAllocator != NULL) {
                ...
                data->m_pMeshInfos[i].m_nVertOffsetInBytes = params.m_pPooledVBAllocator->Allocate(
                        params.m_nVertexes[i]);
            ...
            if (data->m_pMeshInfos[i].m_pMesh == NULL) {
                ...
                data->m_pMeshInfos[i].m_pMesh = pRenderContext->CreateStaticMesh(vertexFormat,
...
public:
    ...
    virtual void AddDecal(ModelInstanceHandle_t handle, Ray_t const &ray,
                          const Vector &decalUp, int decalIndex, int body, bool noPokethru = false,
                          int maxLODToDecal = ADDDECAL_TO_ALL_LODS);

    virtual void AddColoredDecal(ModelInstanceHandle_t handle, Ray_t const &ray,
                                 const Vector &decalUp, int decalIndex, int body, Color cColor, bool noPokethru = false,
                                 int maxLODToDecal = ADDDECAL_TO_ALL_LODS);
    ...
    virtual matrix3x4_t *
    DrawModelShadowSetup(IClientRenderable *pRenderable, int body, int skin, DrawModelInfo_t *pInfo,
                         matrix3x4_t *pBoneToWorld);

    virtual void
    DrawModelShadow(IClientRenderable *pRenderable, const DrawModelInfo_t &info, matrix3x4_t *pBoneToWorld);

    // Used to allow the shadow mgr to manage a list of shadows per model
    unsigned short &
    FirstShadowOnModelInstance(ModelInstanceHandle_t handle) { return m_ModelInstances[handle].m_FirstShadow; }
...
private:
    ...
    void
    InitColormeshParams(ModelInstance_t &instance, studiohwdata_t *pStudioHWData, colormeshparams_t *pColorMeshParams);
    ...
    bool
    LoadStaticPropColorData(IHandleEntity *pProp, DataCacheHandle_t colorMeshHandle, studiohwdata_t *pStudioHWData);
    ...
    LightingState_t *TimeAverageLightingState(ModelInstanceHandle_t handle,
                                              LightingState_t *pLightingState, int nEntIndex,
    ...
    // Internal Decal
    void
    AddDecalInternal(ModelInstanceHandle_t handle, Ray_t const &ray, const Vector &decalUp, int decalIndex, int body,
                     bool bUseColor, Color cColor, bool noPokeThru, int maxLODToDecal);
...
void CModelRender::TimeAverageAmbientLight(LightingState_t &actualLightingState,
                                           ModelInstance_t &inst, float flAttenFactor, LightingState_t *pLightingState,
                                           const Vector *pLightingOrigin) {
...
#endif
        inst.m_CurrentLightingState.r_boxcolor[i].x = clamp(inst.m_CurrentLightingState.r_boxcolor[i].x, 0.f,
                                                            AMBIENT_MAX);
        inst.m_CurrentLightingState.r_boxcolor[i].y = clamp(inst.m_CurrentLightingState.r_boxcolor[i].y, 0.f,
                                                            AMBIENT_MAX);
        inst.m_CurrentLightingState.r_boxcolor[i].z = clamp(inst.m_CurrentLightingState.r_boxcolor[i].z, 0.f,
                                                            AMBIENT_MAX);
    }
...
//-----------------------------------------------------------------------------
LightingState_t *
CModelRender::TimeAverageLightingState(ModelInstanceHandle_t handle, LightingState_t *pLightingState, int nEntIndex,
                                       const Vector *pLightingOrigin) {
...
int
GetLightingConditions(const Vector &vecLightingOrigin, Vector *pColors, int nMaxLocalLights, LightDesc_t *pLocalLights,
...
static ConVar r_drawmodelstatsoverlaymin("r_drawmodelstatsoverlaymin", "0.1", FCVAR_ARCHIVE,
                                         "time in milliseconds that a model must take to render before showing an overlay in r_drawmodelstatsoverlay 2");
static ConVar r_drawmodelstatsoverlaymax("r_drawmodelstatsoverlaymax", "1.5", FCVAR_ARCHIVE,
...
int CModelRender::DrawStaticPropArrayFast(StaticPropRenderInfo_t *pProps, int count, bool bShadowDepth) {
    ...
    if (!bShadowDepth) {
        for (int i = 0; i < objectList.Count(); i++) {
            ...
            bool bHasDecals = (m_ModelInstances[obj.instance].m_DecalHandle != STUDIORENDER_DECAL_INVALID) ? true
                                                                                                           : false;
            ...
            if (bStaticLighting) {
                ...
                if (pColorMeshData && (pColorMeshData->m_bColorMeshValid || pColorMeshData->m_bColorTextureValid)) {
                    ...
                    if (pCallQueue) {
                        if (CacheLock(
                                hColorMeshData)) // CacheCreate above will call functions that won't take place until later. If color mesh isn't used right away, it could get dumped
                        {
...
//-----------------------------------------------------------------------------
matrix3x4_t *
CModelRender::DrawModelShadowSetup(IClientRenderable *pRenderable, int body, int skin, DrawModelInfo_t *pInfo,
...
void
CModelRender::DrawModelShadow(IClientRenderable *pRenderable, const DrawModelInfo_t &info, matrix3x4_t *pBoneToWorld) {
#ifndef SWDS
...
void CModelRender::ProtectColorDataIfQueued(DataCacheHandle_t hColorMesh) {
    if (hColorMesh != DC_INVALID_HANDLE) {
        ...
        if (pCallQueue) {
            if (CacheLock(
                    hColorMesh)) // CacheCreate above will call functions that won't take place until later. If color mesh isn't used right away, it could get dumped
            {
...
void CModelRender::ComputeModelVertexLightingOld(mstudiomodel_t *pModel,
                                                 matrix3x4_t &matrix, const LightingState_t &lightingState,
                                                 color24 *pLighting,
                                                 bool bUseConstDirLighting, float flConstDirLightAmount) {
...
void CModelRender::ComputeModelVertexLighting(IHandleEntity *pProp,
    ...
    for (i = 0; i < pModel->numvertices; ++i) {
        ...
        VectorRotate(normal, matrix, worldNormal);
        bool bNonSolid = ComputeVertexLightingFromSphericalSamples(worldPos, worldNormal, pProp,
                                                                   &(pTempMem[i].AsVector3D()));
...
//-----------------------------------------------------------------------------
static void
StaticPropColorMeshCallback(const FileAsyncRequest_t &request, int numReadBytes, FSAsyncStatus_t asyncStatus) {
...
//-----------------------------------------------------------------------------
static void
StaticPropColorTexelCallback(const FileAsyncRequest_t &request, int numReadBytes, FSAsyncStatus_t asyncStatus) {
...
bool CModelRender::UpdateStaticPropColorData(IHandleEntity *pProp, ModelInstanceHandle_t handle) {
    ...
    if (!bDebugColor) {
        ...
        vertexFileHeader_t *pVertexHdr = g_pMDLCache->GetVertexData(
                (MDLHandle_t) (int) pStudioHdr->virtualModel & 0xffff);
        if (!pVertexHdr) {
            ...
            for (int lodID = pStudioHWData->m_RootLOD; lodID < pStudioHWData->m_NumLODs; ++lodID) {
                ...
                for (int meshID = 0; meshID < pModel->nummeshes; ++meshID) {
                    ...
                    for (int stripGroupID = 0;
                         stripGroupID < pStudioMeshData[pMesh->meshid].m_NumGroup; ++stripGroupID) {
...
bool CModelRender::RecomputeStaticLighting(ModelInstanceHandle_t handle) {
    ...
    if (pStudioHdr->flags & STUDIOHDR_FLAGS_STATIC_PROP) {
        ...
        if (r_decalstaticprops.GetBool() && instance.m_LightCacheHandle) {
            instance.m_AmbientLightingState = *(LightcacheGetStatic(instance.m_LightCacheHandle, NULL,
...
void CModelRender::AddColoredDecal(ModelInstanceHandle_t handle, Ray_t const &ray,
                                   const Vector &decalUp, int decalIndex, int body, Color cColor, bool noPokeThru,
                                   int maxLODToDecal) {
...
void CModelRender::AddDecalInternal(ModelInstanceHandle_t handle, Ray_t const &ray,
                                    const Vector &decalUp, int decalIndex, int body, bool bUseColor, Color cColor,
...
static void CreateLightmapsFromData(CColorMeshData *_colorMeshData) {
    ...
    for (int mesh = 0; mesh < _colorMeshData->m_nMeshes; ++mesh) {
        ...
        meshInfo->m_pLightmap = g_pMaterialSystem->CreateTextureFromBits(cti->m_nWidth, cti->m_nHeight,
                                                                         cti->m_nMipmapCount, cti->m_ImageFormat,
