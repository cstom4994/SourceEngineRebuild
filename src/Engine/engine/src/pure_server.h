...
private:
    ...
    void
    UpdateCommandStats(CUtlDict<CPureServerWhitelist::CCommand *, int> &commands, int *pHighest, int *pLongestPathName);

    void
    PrintCommand(const char *pFileSpec, const char *pExt, int maxPathnameLen, CPureServerWhitelist::CCommand *pCommand);
    ...
    void
    DecodeCommandList(CUtlDict<CPureServerWhitelist::CCommand *, int> &theList, CUtlBuffer &buf, uint32 nFormatVersion);
