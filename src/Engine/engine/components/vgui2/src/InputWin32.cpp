...
class CVGUIInputSystem : public IInputInternal {
public:
    CVGUIInputSystem();
    ...
    //   if it's set
    virtual void
    SetModalSubTree(VPANEL subTree, VPANEL unhandledMouseClickListener, bool restrictMessagesToSubTree = true);
...
CVGUIInputSystem g_Input;

EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CVGUIInputSystem, IInput, VGUI_INPUT_INTERFACE_VERSION,
                                  g_Input); // export IInput to everyone else, not IInputInternal!
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CVGUIInputSystem, IInputInternal, VGUI_INPUTINTERNAL_INTERFACE_VERSION,
...
CVGUIInputSystem::CVGUIInputSystem() {
    m_nDebugMessages = -1;
#ifdef DO_IME
    _imeWnd = NULL;
    _imeCandidates = NULL;
...
//-----------------------------------------------------------------------------
void CVGUIInputSystem::InitInputContext(InputContext_t *pContext) {
...
void CVGUIInputSystem::ResetInputContext(HInputContext context) {
...
// about which controls have mouse + key focus, for example.
//-----------------------------------------------------------------------------
HInputContext CVGUIInputSystem::CreateInputContext() {
    HInputContext i = m_Contexts.AddToTail();
    InitInputContext(&m_Contexts[i]);
    return i;
}

void CVGUIInputSystem::DestroyInputContext(HInputContext context) {
...
//-----------------------------------------------------------------------------
CVGUIInputSystem::InputContext_t *CVGUIInputSystem::GetInputContext(HInputContext context) {
...
// Associating NULL is valid; it disconnects the panel from the context
//-----------------------------------------------------------------------------
void CVGUIInputSystem::AssociatePanelWithInputContext(HInputContext context, VPANEL pRoot) {
...
// to get the one normally used by VGUI
//-----------------------------------------------------------------------------
void CVGUIInputSystem::ActivateInputContext(HInputContext context) {
...
//-----------------------------------------------------------------------------
VPanel *CVGUIInputSystem::CalculateNewKeyFocus() {
    InputContext_t *pContext = GetInputContext(m_hContext);
...
// Input  : *focus -
//-----------------------------------------------------------------------------
void CVGUIInputSystem::PanelDeleted(VPANEL focus) {
    HInputContext i;
...
// Input  : newMouseFocus -
//-----------------------------------------------------------------------------
void CVGUIInputSystem::SetMouseFocus(VPANEL newMouseFocus) {
    // check if we are in modal state,
    // and if we are make sure this panel is a child of us.
...
VPanel *CVGUIInputSystem::GetMouseFocusIgnoringModalSubtree() {
...
//-----------------------------------------------------------------------------
void CVGUIInputSystem::UpdateMouseFocus(int x, int y) {
    // find the panel that has the focus
    VPanel *focus = NULL;
...
// Passes in a keycode which allows hitting other mouse buttons w/o cancelling capture mode
void CVGUIInputSystem::SetMouseCaptureEx(VPANEL panel, MouseCode captureStartMouseCode) {
...
VPANEL CVGUIInputSystem::GetMouseCapture() {
    InputContext_t *pContext = GetInputContext(m_hContext);
...
//-----------------------------------------------------------------------------
void CVGUIInputSystem::SetMouseCapture(VPANEL panel) {
    // check if we are in modal state,
    // and if we are make sure this panel is a child of us.
    if (!IsChildOfModalPanel(panel)) {
        return;
...
// returns true if the specified panel is a child of the current modal panel
// if no modal panel is set, then this always returns TRUE
bool CVGUIInputSystem::IsChildOfModalSubTree(VPANEL panel) {
    if (!panel)
        return true;
...
//-----------------------------------------------------------------------------
bool CVGUIInputSystem::IsChildOfModalPanel(VPANEL panel, bool checkModalSubTree /*= true*/ ) {
    // NULL is ok.
    if (!panel)
        return true;
...
// events, etc.
//-----------------------------------------------------------------------------
void CVGUIInputSystem::UpdateCursorPosInternal(int x, int y) {
...
//-----------------------------------------------------------------------------
void CVGUIInputSystem::SetCursorPos(int x, int y) {
...
void CVGUIInputSystem::GetCursorPos(int &x, int &y) {
    if (IsDispatchingMessageQueue()) {
...
void CVGUIInputSystem::GetCursorPosition(int &x, int &y) {
    InputContext_t *pContext = GetInputContext(m_hContext);
    x = pContext->m_nCursorX;
    y = pContext->m_nCursorY;
}
...
//-----------------------------------------------------------------------------
void CVGUIInputSystem::GetKeyCodeText(KeyCode code, char *buf, int buflen) {
    if (!buf)
        return;
...
//-----------------------------------------------------------------------------
void CVGUIInputSystem::SurfaceSetCursorPos(int x, int y) {
    if (g_pSurface->HasCursorPosFunctions()) // does the surface export cursor functions for us to use?
    {
		...
		Assert( !"CVGUIInputSystem::SurfaceSetCursorPos NYI" );
#endif
    }
}

void CVGUIInputSystem::SurfaceGetCursorPos(int &x, int &y) {
    ...
    if (g_pSurface->HasCursorPosFunctions()) // does the surface export cursor functions for us to use?
    {
		...
		//   For l4d2, the vguimatsurface/cursor.cpp functions fire in the engine, the vgui2 ones
		// should be dormant (this isn't true for Steam however).
		Assert( !"CVGUIInputSystem::SurfaceGetCursorPos NYI" );
		x = 0;
		y = 0;
#endif
    }
...
void CVGUIInputSystem::SetCursorOveride(HCursor cursor) {
    _cursorOverride = cursor;
}

HCursor CVGUIInputSystem::GetCursorOveride() {
...
//-----------------------------------------------------------------------------
bool CVGUIInputSystem::InternalCursorMoved(int x, int y) {
...
//-----------------------------------------------------------------------------
void CVGUIInputSystem::HandleExplicitSetCursor() {
    InputContext_t *pContext = GetInputContext(m_hContext);
...
// Called when we've detected cursor has moved via a windows message
//-----------------------------------------------------------------------------
void CVGUIInputSystem::PostCursorMessage() {
    InputContext_t *pContext = GetInputContext(m_hContext);
...
bool CVGUIInputSystem::InternalMousePressed(MouseCode code) {
...
bool CVGUIInputSystem::InternalMouseDoublePressed(MouseCode code) {
    // True means we've processed the message and other code shouldn't see this message
    bool bFilter = false;
    ...
    if (pContext->_mouseCapture && IsChildOfModalPanel((VPANEL) pContext->_mouseCapture)) {
        ...
        // the panel with mouse capture gets all messages
        g_pIVgui->PostMessage((VPANEL) pContext->_mouseCapture, new KeyValues("MouseDoublePressed", "code", code),
                              NULL);
        pTargetPanel = pContext->_mouseCapture;
        bFilter = true;
    } else if ((pContext->_mouseFocus != NULL) && IsChildOfModalPanel((VPANEL) pContext->_mouseFocus)) {
...
bool CVGUIInputSystem::InternalMouseReleased(MouseCode code) {
    // True means we've processed the message and other code shouldn't see this message
    bool bFilter = false;
...
bool CVGUIInputSystem::InternalMouseWheeled(int delta) {
    // True means we've processed the message and other code shouldn't see this message
    bool bFilter = false;
...
//-----------------------------------------------------------------------------
void CVGUIInputSystem::SetMouseCodeState(MouseCode code, MouseCodeState_t state) {
    if (!IsMouseCode(code))
        return;
...
void CVGUIInputSystem::SetKeyCodeState(KeyCode code, bool bPressed) {
...
void CVGUIInputSystem::UpdateButtonState(const InputEvent_t &event) {
...
bool CVGUIInputSystem::InternalKeyCodePressed(KeyCode code) {
    InputContext_t *pContext = GetInputContext(m_hContext);
...
void CVGUIInputSystem::InternalKeyCodeTyped(KeyCode code) {
    InputContext_t *pContext = GetInputContext(m_hContext);
    // mask out bogus keys
    if (!IsKeyCode(code) && !IsJoystickCode(code))
        return;
...
void CVGUIInputSystem::InternalKeyTyped(wchar_t unichar) {
    InputContext_t *pContext = GetInputContext(m_hContext);
    // set key state
    if (unichar <= KEY_LAST) {
...
bool CVGUIInputSystem::InternalKeyCodeReleased(KeyCode code) {
    InputContext_t *pContext = GetInputContext(m_hContext);
...
//-----------------------------------------------------------------------------
bool CVGUIInputSystem::PostKeyMessage(KeyValues *message) {
    InputContext_t *pContext = GetInputContext(m_hContext);
...
VPANEL CVGUIInputSystem::GetAppModalSurface() {
    InputContext_t *pContext = GetInputContext(m_hContext);
    return (VPANEL) pContext->_appModalPanel;
}

void CVGUIInputSystem::SetAppModalSurface(VPANEL panel) {
    InputContext_t *pContext = GetInputContext(m_hContext);
    pContext->_appModalPanel = (VPanel *) panel;
}


void CVGUIInputSystem::ReleaseAppModalSurface() {
    InputContext_t *pContext = GetInputContext(m_hContext);
    pContext->_appModalPanel = NULL;
}
...
// Input  : *hwnd -
//-----------------------------------------------------------------------------
void CVGUIInputSystem::SetIMEWindow(void *hwnd) {
#ifdef DO_IME
    _imeWnd = hwnd;
#endif
}
...
// Change keyboard layout type
void CVGUIInputSystem::OnChangeIME(bool forward) {
    ASSERT_IF_IME_NYI();
...
int CVGUIInputSystem::GetCurrentIMEHandle() {
    ASSERT_IF_IME_NYI();
...
int CVGUIInputSystem::GetEnglishIMEHandle() {
#ifdef DO_IME
...
void CVGUIInputSystem::OnChangeIMEByHandle(int handleValue) {
    ASSERT_IF_IME_NYI();
...
// Returns the Language Bar label (Chinese, Korean, Japanese, Russion, Thai, etc.)
void CVGUIInputSystem::GetIMELanguageName(wchar_t *buf, int unicodeBufferSizeInBytes) {
    ASSERT_IF_IME_NYI();
...
// Returns the short code for the language (EN, CH, KO, JP, RU, TH, etc. ).
void CVGUIInputSystem::GetIMELanguageShortCode(wchar_t *buf, int unicodeBufferSizeInBytes) {
#ifdef DO_IME
...
// Call with NULL dest to get item count
int CVGUIInputSystem::GetIMELanguageList(LanguageItem *dest, int destcount) {
    ASSERT_IF_IME_NYI();
...
int CVGUIInputSystem::GetIMEConversionModes(ConversionModeItem *dest, int destcount) {
    ASSERT_IF_IME_NYI();
...
static IMESettingsTransform g_SentenceMode_JP_None(
        0,
        0,
        IME_SMODE_PLAURALCLAUSE | IME_SMODE_SINGLECONVERT | IME_SMODE_AUTOMATIC | IME_SMODE_PHRASEPREDICT |
...
static IMESettingsTransform g_SentenceMode_JP_General(
        0,
        0,
        IME_SMODE_NONE | IME_SMODE_PLAURALCLAUSE | IME_SMODE_SINGLECONVERT | IME_SMODE_AUTOMATIC |
        IME_SMODE_CONVERSATION,
...
static IMESettingsTransform g_SentenceMode_JP_BiasNames(
        0,
        0,
        IME_SMODE_NONE | IME_SMODE_PHRASEPREDICT | IME_SMODE_SINGLECONVERT | IME_SMODE_AUTOMATIC |
        IME_SMODE_CONVERSATION,
...
int CVGUIInputSystem::GetIMESentenceModes(SentenceModeItem *dest, int destcount) {
    ASSERT_IF_IME_NYI();
...
void CVGUIInputSystem::OnChangeIMEConversionModeByHandle(int handleValue) {
    ASSERT_IF_IME_NYI();
...
void CVGUIInputSystem::OnChangeIMESentenceModeByHandle(int handleValue) {
}

void CVGUIInputSystem::OnInputLanguageChanged() {
}

void CVGUIInputSystem::OnIMEStartComposition() {
}
...
void CVGUIInputSystem::OnIMEComposition(int flags) {
    ASSERT_IF_IME_NYI();
...
void CVGUIInputSystem::OnIMEEndComposition() {
    InputContext_t *pContext = GetInputContext(m_hContext);
    if (pContext) {
        // tell the current focused panel that a key was typed
...
void CVGUIInputSystem::DestroyCandidateList() {
#ifdef DO_IME
    if (_imeCandidates) {
...
void CVGUIInputSystem::OnIMEShowCandidates() {
    ASSERT_IF_IME_NYI();
...
void CVGUIInputSystem::OnIMECloseCandidates() {
    ASSERT_IF_IME_NYI();
...
void CVGUIInputSystem::OnIMEChangeCandidates() {
    ASSERT_IF_IME_NYI();
...
void CVGUIInputSystem::CreateNewCandidateList() {
    ASSERT_IF_IME_NYI();
...
int CVGUIInputSystem::GetCandidateListCount() {
    ASSERT_IF_IME_NYI();
...
void CVGUIInputSystem::GetCandidate(int num, wchar_t *dest, int destSizeBytes) {
    ASSERT_IF_IME_NYI();
...
int CVGUIInputSystem::GetCandidateListSelectedItem() {
    ASSERT_IF_IME_NYI();
...
int CVGUIInputSystem::GetCandidateListPageSize() {
    ASSERT_IF_IME_NYI();
...
int CVGUIInputSystem::GetCandidateListPageStart() {
    ASSERT_IF_IME_NYI();
...
void CVGUIInputSystem::SetCandidateListPageStart(int start) {
    ASSERT_IF_IME_NYI();
...
void CVGUIInputSystem::OnIMERecomputeModes() {
}
...
//-----------------------------------------------------------------------------
bool CVGUIInputSystem::CandidateListStartsAtOne() {
    ASSERT_IF_IME_NYI();
...
void CVGUIInputSystem::SetCandidateWindowPos(int x, int y) {
    ASSERT_IF_IME_NYI();
...
void CVGUIInputSystem::InternalSetCompositionString(const wchar_t *compstr) {
    InputContext_t *pContext = GetInputContext(m_hContext);
    if (pContext) {
        // tell the current focused panel that a key was typed
...
void CVGUIInputSystem::InternalShowCandidateWindow() {
    InputContext_t *pContext = GetInputContext(m_hContext);
    if (pContext) {
...
void CVGUIInputSystem::InternalHideCandidateWindow() {
    InputContext_t *pContext = GetInputContext(m_hContext);
    if (pContext) {
...
void CVGUIInputSystem::InternalUpdateCandidateWindow() {
    InputContext_t *pContext = GetInputContext(m_hContext);
    if (pContext) {
...
bool CVGUIInputSystem::GetShouldInvertCompositionString() {
#ifdef DO_IME
    LanguageIds *info = GetLanguageInfo(LOWORD(GetKeyboardLayout(0)));
    if (!info)
        return false;
...
void CVGUIInputSystem::RegisterKeyCodeUnhandledListener(VPANEL panel) {
    if (!panel)
        return;
...
void CVGUIInputSystem::UnregisterKeyCodeUnhandledListener(VPANEL panel) {
    if (!panel)
        return;
...
// Posts unhandled message to all interested panels
void CVGUIInputSystem::OnKeyCodeUnhandled(int keyCode) {
    InputContext_t *pContext = GetInputContext(m_hContext);
    if (!pContext)
        return;
...
void CVGUIInputSystem::PostModalSubTreeMessage(VPanel *subTree, bool state) {
    InputContext_t *pContext = GetInputContext(m_hContext);
...
//   however, if a mouse click occurs outside of the subtree, and "UnhandleMouseClick" message is sent to unhandledMouseClickListener panel
//   if it's set
void CVGUIInputSystem::SetModalSubTree(VPANEL subTree, VPANEL unhandledMouseClickListener,
...
void CVGUIInputSystem::ReleaseModalSubTree() {
    InputContext_t *pContext = GetInputContext(m_hContext);
    if (!pContext)
        return;
...
VPANEL CVGUIInputSystem::GetModalSubTree() {
    InputContext_t *pContext = GetInputContext(m_hContext);
    if (!pContext)
        return 0;
...
// These toggle whether the modal subtree is exclusively receiving messages or conversely whether it's being excluded from receiving messages
void CVGUIInputSystem::SetModalSubTreeReceiveMessages(bool state) {
    InputContext_t *pContext = GetInputContext(m_hContext);
    if (!pContext)
        return;
...
bool CVGUIInputSystem::ShouldModalSubTreeReceiveMessages() const {
    InputContext_t *pContext = const_cast< CVGUIInputSystem * >( this )->GetInputContext(m_hContext);
    if (!pContext)
        return true;
