...
//-----------------------------------------------------------------------------
    ...
    public:
        TreeNodeText(Panel *parent, const char *panelName, TreeView *tree) : BaseClass(parent, panelName),
                                                                             m_pTree(tree) {
...
bool TreeView::ModifyItem(int itemIndex, KeyValues *data) {
    ...
    if (bReSort) {
        ...
        bool bRightBad = (nChildIndex < nChildren - 1) &&
                         m_pSortFunc(pParent->m_Children[nChildIndex + 1]->m_pData, pNode->m_pData);
