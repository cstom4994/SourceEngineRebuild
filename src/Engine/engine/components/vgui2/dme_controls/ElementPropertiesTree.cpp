...
void CPropertiesTreeToolbar::UpdateButtonState() {
    m_pBack->SetEnabled(
            m_pTree->GetHistoryMenuItemCount(CElementPropertiesTreeInternal::DME_PROPERTIESTREE_MENU_BACKWARD) > 0
            ? true : false);
    m_pFwd->SetEnabled(
...
CElementPropertiesTreeInternal::CElementPropertiesTreeInternal(
        vgui::Panel *parent, IDmNotify *pNotify, CDmElement *pObject, bool autoApply /* = true */,
...
void CElementPropertiesTreeInternal::GetPathToItem(CUtlVector<TreeItem_t> &path, int itemIndex) {
    for (int idx = itemIndex;
         idx != m_pTree->GetTree()->GetRootItemIndex(); idx = m_pTree->GetTree()->GetItemParent(idx)) {
...
struct OwnerAttribute_t {
    ...
    OwnerAttribute_t(const OwnerAttribute_t &src) : sortedData(0, 0, ArrayIndexLessFunc) {
        ...
        for (int i = src.sortedData.FirstInorder();
             i != src.sortedData.InvalidIndex(); i = src.sortedData.NextInorder(i)) {
...
bool CElementPropertiesTreeInternal::OnRemoveFromData(CUtlVector<KeyValues *> &list) {
    ...
    for (i = sorted.m_Sorted.FirstInorder(); i != sorted.m_Sorted.InvalidIndex(); i = sorted.m_Sorted.NextInorder(i)) {
        ...
        for (int j = entry.sortedData.LastInorder();
             j != entry.sortedData.InvalidIndex(); j = entry.sortedData.PrevInorder(j)) {
...
void
CElementPropertiesTreeInternal::FindMatchingElements_R(CUtlRBTree<CDmElement *, int> &visited, const char *searchstr,
...
bool CElementPropertiesTreeInternal::ShowSetElementAttributeDialog(CDmElement *pOwner,
                                                                   const char *pAttributeName, int nArrayIndex,
    ...
    bool bRefreshRequired = false;
    {
        ...
        for (int i = 0; i < c; ++i) {
            ...
            if (attType == AT_ELEMENT_ARRAY) {
                ...
                DmElementHandle_t newElement = g_pDataModel->CreateElement(pElementType, elementName,
                                                                           pOwner->GetFileId());
                if (newElement != DMELEMENT_HANDLE_INVALID) {
                    array.AddToTail(newElement);
                    bRefreshRequired = true;
...
vgui::Panel *CElementPropertiesTreeInternal::CreateAttributeDataWidget(CDmElement *pElement,
                                                                       const char *pWidgetName, CDmElement *obj,
...
void CElementPropertiesTreeInternal::GenerateContextMenu(int itemIndex, int x, int y) {
    ...
    if (bIsElementAttribute || bIsElementArrayAttribute || bIsElementArrayItem) {
        ...
        for (int i = g_pDataModel->GetFirstFactory(); g_pDataModel->IsValidFactory(i); i = g_pDataModel->GetNextFactory(
                i)) {
...
void CElementPropertiesTreeInternal::DropItemsIntoArray(CDmrElementArray<> &array, CUtlVector<KeyValues *> &msglist,
    ...
    if (op == DO_COPY) {
        ...
        for (int i = 0; i < nMsgs; ++i) {
            ...
            bool isArrayElement = !itemData->IsEmpty("arrayIndex");
            CDmAttribute *pAttribute = pOwner->GetAttribute(pAttributeName,
                                                            isArrayElement ? AT_ELEMENT_ARRAY : AT_ELEMENT);
            if (!pAttribute)
                continue;
        ...
        for (int i = arrayItemSorter.LastInorder();
             i != arrayItemSorter.InvalidIndex(); i = arrayItemSorter.PrevInorder(i)) {
...
CElementPropertiesTreeInternal::GetDropOperation(int itemIndex, CUtlVector<KeyValues *> &msglist) {
...
bool
CElementPropertiesTreeInternal::GetItemDropContextMenu(int itemIndex, Menu *menu, CUtlVector<KeyValues *> &msglist) {
    KeyValues *itemData = m_pTree->GetItemData(itemIndex);
...
void
CElementPropertiesTreeInternal::SpewOpenItems(int depth, OpenItemTree_t &tree, int nOpenTreeIndex, int nItemIndex) {
...
//-----------------------------------------------------------------------------
void
CElementPropertiesTreeInternal::SetupWidgetInfo(AttributeWidgetInfo_t *pInfo, CDmElement *obj, CDmAttribute *pAttribute,
                                                int nArrayIndex) {
...
//-----------------------------------------------------------------------------
void
CElementPropertiesTreeInternal::InsertSingleAttribute(int parentNodeIndex, CDmElement *obj, CDmAttribute *pAttribute,
                                                      int nArrayIndex) {
