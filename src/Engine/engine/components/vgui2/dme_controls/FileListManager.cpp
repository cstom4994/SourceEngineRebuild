...
template<int C>
int
ListPanelStringSortFunc(vgui::ListPanel *pPanel, const vgui::ListPanelItem &item1, const vgui::ListPanelItem &item2);
...
int
AddMenuItemHelper(vgui::Menu *pMenu, const char *pItemName, const char *pKVName, vgui::Panel *pTarget, bool bEnabled) {
...
void CFileListManager::OnOpenFile(KeyValues *pParams) {
    ...
    vgui::FileOpenDialog *pFileOpenDialog = new vgui::FileOpenDialog(this, "Save .dmx File As", false,
                                                                     pContextKeyValues);
...
void CFileListManager::OnDataChanged(KeyValues *pParams) {
    ...
    if (!bPerformFullRefresh) {
        ...
        for (int i = 0; i < nCount; ++i) {
            ...
            bPerformFullRefresh = (fileid != (DmFileId_t) GetItemUserData(i)) ||
