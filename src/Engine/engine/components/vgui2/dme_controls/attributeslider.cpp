...
void CAttributeSlider::PaintBackground() {
    ...
    if (viewType == ANIM_CONTROL_VALUE && IsControlActive(ANIM_CONTROL_BALANCE)) {
        float flBalance = bUsePreview ? m_Preview.m_Current.m_pValue[ANIM_CONTROL_BALANCE] : GetValue(
    ...
    if (IsControlActive(ANIM_CONTROL_MULTILEVEL)) {
        float flMultiValue = bUsePreview ? m_Preview.m_Current.m_pValue[ANIM_CONTROL_MULTILEVEL] : GetValue(
