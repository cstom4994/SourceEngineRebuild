...
public:
    ...
    virtual void
    PlayIncomingVoiceData(XUID xuid, const byte *pbData, unsigned int dwDataSize, const bool *bAudiblePlayers = NULL);
...
void
CEngineVoiceSteam::GetVoiceData(int iController, const byte **ppvVoiceDataBuffer, unsigned int *pnumVoiceDataBytes) {
    ...
    *ppvVoiceDataBuffer = pbVoiceData;
    ...
    EVoiceResult res = Steam3Client().SteamUser()->GetVoice(true, pbVoiceData, size, pnumVoiceDataBytes, false, NULL, 0,
                                                            NULL, 0);
    switch (res) {
        case k_EVoiceResultNoData:
        case k_EVoiceResultOK:
