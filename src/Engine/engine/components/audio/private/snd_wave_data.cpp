...
ConVar snd_async_spew_blocking("snd_async_spew_blocking", "0", 0,
...
ConVar snd_async_fullyasync("snd_async_fullyasync", "1", 0,
...
public:
    ...
    static void
    QueuedLoaderCallback(void *pContext, void *pContext2, const void *pData, int nSize, LoaderError_t loaderError);
...
public:
    ...
    virtual bool
    CopyDataIntoMemory(char const *filename, int datasize, int startpos, void *buffer, int bufsize, int copystartpos,
                       int bytestocopy, bool *pbPostProcessed);

    virtual bool
    CopyDataIntoMemory(memhandle_t &handle, char const *filename, int datasize, int startpos, void *buffer, int bufsize,
    ...
    virtual StreamHandle_t
    OpenStreamedLoad(char const *pFileName, int dataSize, int dataStart, int startPos, int loopPos, int bufferSize,
...
//-----------------------------------------------------------------------------
StreamHandle_t
CAsyncWavDataCache::OpenStreamedLoad(char const *pFileName, int dataSize, int dataStart, int startPos, int loopPos,
...
//-----------------------------------------------------------------------------
bool
CAsyncWavDataCache::GetDataPointer(memhandle_t &handle, char const *filename, int datasize, int startpos, void **pData,
                                   int copystartpos, bool *pbPostProcessed) {
...
void CAsyncWavDataCache::SpewMemoryUsage(int level) {
    ...
    if (IsPC()) {
        ...
        if (level >= 1) {
            for (int i = m_CacheHandles.FirstInorder(); m_CacheHandles.IsValidIndex(i); i = m_CacheHandles.NextInorder(
                    i)) {
                char name[MAX_PATH];
...
bool CWaveDataMemoryAsync::IsReadyToMix() {
    ...
    const bool bReady =
            !m_source.IsAsyncLoad() && !snd_async_fullyasync.GetBool() && iStatus != CAudioSource::AUDIO_IS_LOADED;
...
//-----------------------------------------------------------------------------
IWaveData *
CreateWaveDataStream(CAudioSource &source, IWaveStreamSource *pStreamSource, const char *pFileName, int dataStart,
                     int dataSize, CSfxTable *pSfx, int startOffset) {
    CWaveDataStreamAsync *pStream = new CWaveDataStreamAsync(source, pStreamSource, pFileName, dataStart, dataSize,
                                                             pSfx, startOffset);
