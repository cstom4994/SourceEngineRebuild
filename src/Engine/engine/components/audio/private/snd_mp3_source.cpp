...
CAudioMixer *CAudioSourceStreamMP3::CreateMixer(int intialStreamPosition) {
    ...
    IWaveData *pWaveData = CreateWaveDataStream(*this, static_cast<IWaveStreamSource *>( this ), m_pSfx->GetFileName(),
                                                0, m_dataSize, m_pSfx, 0);
    if (pWaveData) {
...
CAudioSource *Audio_CreateStreamedMP3(CSfxTable *pSfx) {
    ...
    CAudioSourceCachedInfo *info = audiosourcecache->GetInfo(CAudioSource::AUDIO_SOURCE_MP3, pSfx->IsPrecachedSound(),
                                                             pSfx);
    if (info) {
