...
VoiceRecord_AudioQueue::VoiceRecord_AudioQueue() :
        ...
        m_SampleBufferReadPos(0), m_SampleBufferWritePos(0), m_bRecordingAudio(false), m_hThread(NULL),
        m_bFirstInit(true) {
    ClearInterfaces();
}
...
static OSStatus
recordingCallback(void *inRefCon, AudioUnitRenderActionFlags *ioActionFlags, const AudioTimeStamp *inTimeStamp,
...
bool VoiceRecord_AudioQueue::InitalizeInterfaces() {
    ...
    status = AudioConverterSetProperty(m_Converter, kAudioConverterSampleRateConverterQuality, sizeof(UInt32),
                                       &quality);
    if (status != noErr)
        return false;
