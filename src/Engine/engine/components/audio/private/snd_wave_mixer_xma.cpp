...
public:
    ...
    virtual void
    Mix(IAudioDevice *pDevice, channel_t *pChannel, void *pData, int outputOffset, int inputOffset, fixedint fracRate,
...
CON_COMMAND(snd_xma_info, "Spew XMA Info") {
    ...
    for (int hMixer = g_XMAMixerList.Head();
         hMixer != g_XMAMixerList.InvalidIndex(); hMixer = g_XMAMixerList.Next(hMixer)) {
...
bool CAudioMixerWaveXMA::IsReadyToMix() {
    ...
    if (!m_pXMAPlayback) {
        ...
        if (g_XMAMemoryPool.BytesAllocated() != numBuffers * g_XMAMemoryPool.ChunkSize()) {
            ...
            xmaPlaybackInit.outputBufferSizeInSamples = (m_NumChannels == 2) ? XMA_STEREO_OUTPUT_BUFFER_SAMPLES
