...
static GUID IID_IDirectSound3DBufferDef = {0x279AFA86, 0x4981, 0x11CE,
...
private:
    ...
    bool
    LockDSBuffer(LPDIRECTSOUNDBUFFER pBuffer, DWORD **pdwWriteBuffer, DWORD *pdwSizeBuffer, const char *pBufferName,
                 int lockFlags = 0);
    ...
    void
    S_TransferSurround16(portable_samplepair_t *pfront, portable_samplepair_t *prear, portable_samplepair_t *pcenter,
                         int lpaintedtime, int endtime, int cchan);
    ...
    void
    S_TransferSurround16Interleaved_FullLock(const portable_samplepair_t *pfront, const portable_samplepair_t *prear,
                                             const portable_samplepair_t *pcenter, int lpaintedtime, int endtime);
...
void CAudioDirectSound::ClearBuffer(void) {
    ...
    if (IsUsingBufferPerSpeaker()) {
        ...
        if (m_bSurroundCenter) {
            SURROUNDreps = 0;
            while ((SURROUNDhresult = pDSBufFC->Lock(0, m_bufferSizeBytes, (void **) &pDataFC, &dwSizeFC, NULL, NULL,
                                                     0)) != DS_OK) {
                if (SURROUNDhresult != DSERR_BUFFERLOST) {
...
bool CAudioDirectSound::SNDDMA_InitSurround(LPDIRECTSOUND lpDS, WAVEFORMATEX *lpFormat, DSBCAPS *lpdsbc, int cchan) {
    ...
    if (cchan == 5) {
        reps = 0;
        while ((hresult = pDSBufFC->Lock(0, lpdsbc->dwBufferBytes, (void **) &lpData, &dwSize, NULL, NULL, 0)) !=
               DS_OK) {
            if (hresult != DSERR_BUFFERLOST) {
...
void CAudioDirectSound::S_TransferSurround16Interleaved_FullLock(const portable_samplepair_t *pfront,
    ...
    int linearCount;                             // space in output buffer for linearCount mono samples
    int sampleMonoCount = m_bufferSizeBytes / (DeviceSampleBytes() *
