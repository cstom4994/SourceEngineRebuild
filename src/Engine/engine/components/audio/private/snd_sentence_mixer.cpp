...
public:
    ...
    virtual int
    MixDataToDevice(IAudioDevice *pDevice, channel_t *pChannel, int sampleCount, int outputRate, int outputOffset);
...
int CSentenceMixer::MixDataToDevice(IAudioDevice *pDevice, channel_t *pChannel, int sampleCount, int outputRate,
    ...
    while (sampleCount > 0 && m_pCurrentWordMixer) {
        int outputCount = m_pCurrentWordMixer->MixDataToDevice(pDevice, pChannel, sampleCount, outputRate,
                                                               outputOffset);
