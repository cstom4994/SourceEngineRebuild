...
#include "utldict.h"
#include "mempool.h"
#include "../src/enginetrace.h"            // for traceline
...
#include "../src/ispatialpartitioninternal.h"    // for entity enumerator
#include "../src/debugoverlay.h"
#include "icliententity.h"
#include "../src/cmodel_engine.h"
#include "../src/staticpropmgr.h"
#include "../src/server.h"
#include "edict.h"
#include "../src/pure_server.h"
...
*/
channel_t *
SND_StealDynamicChannel(SoundSource soundsource, int entchannel, const Vector &origin, CSfxTable *sfx, float flDelay,
...
channel_t *
SND_PickDynamicChannel(SoundSource soundsource, int entchannel, const Vector &origin, CSfxTable *sfx, float flDelay,
                       bool bDoNotOverwriteExisting) {
    channel_t *pChannel = SND_StealDynamicChannel(soundsource, entchannel, origin, sfx, flDelay,
                                                  bDoNotOverwriteExisting);
    if (!pChannel)
        return NULL;
...
void
S_SpatializeChannel(int pVolume[CCHANVOLUMES / 2], int master_vol, const Vector *psourceDir, float gain, float mono) {
...
ConVar dsp_dist_min("dsp_dist_min", "0.0",
...
ConVar dsp_db_min("dsp_db_min", "80",
                  FCVAR_DEMO);            // sounds with sndlvl below this get dsp_db_mixdrop % less dsp mix
    ...
    switch (g_ssp.chan) {
        ...
        case 0: // front left: trace max 100' 'cone' to player's left
            if (g_AudioDevice->IsSurround()) {
                ...
                v_dir2 = g_ssp.cycle ? (g_ssp.cycle == 1 ? listener_forward2d * 0.5 : -listener_forward2d * 0.5)
                                     : v_dir;
...
void
RemapPlayerOrMusicVols(channel_t *ch, int volumes[CCHANVOLUMES / 2], bool fplayersound, bool fmusicsound, float mono) {
    ...
    if (fplayersound) // && (ch->bstereowav && ch->wavtype != CHAR_DIRECTIONAL && ch->wavtype != CHAR_DISTVARIANT) )
    {
        ...
        if (ch->flags.bstereowav &&
            (ch->wavtype == CHAR_OMNI || ch->wavtype == CHAR_SPATIALSTEREO || ch->wavtype == 0)) {
    ...
    if (nSndShowStart > 0 && nSndShowStart < 7 && nSndShowStart != 4) {
        ...
        if (nSndShowStart == 2 || nSndShowStart == 5)
            DevMsg("\t dspmix %1.2f : distmix %1.2f : dspface %1.2f : lvol %1.2f : cvol %1.2f : rvol %1.2f : rlvol %1.2f : rrvol %1.2f\n",
                   ...
                   pTargetChan->fvolume[IFRONT_LEFT], pTargetChan->fvolume[IFRONT_CENTER],
                   pTargetChan->fvolume[IFRONT_RIGHT], pTargetChan->fvolume[IREAR_LEFT],
    ...
    if (nSndShowStart > 0 && nSndShowStart < 7 && nSndShowStart != 4) {
        ...
        if (nSndShowStart == 2 || nSndShowStart == 5)
            DevMsg("\t dspmix %1.2f : distmix %1.2f : dspface %1.2f : lvol %1.2f : cvol %1.2f : rvol %1.2f : rlvol %1.2f : rrvol %1.2f\n",
                   ...
                   ch->fvolume[IFRONT_LEFT], ch->fvolume[IFRONT_CENTER], ch->fvolume[IFRONT_RIGHT],
...
int S_StartSound(StartSoundParams_t &params) {
    ...
    if (params.staticsound) { VPROF_("StartStaticSound", 0, VPROF_BUDGETGROUP_OTHER_SOUND, false, BUDGETFLAG_OTHER);
        return S_StartStaticSound(params);
    } else { VPROF_("StartDynamicSound", 0, VPROF_BUDGETGROUP_OTHER_SOUND, false, BUDGETFLAG_OTHER);
    ...
    if (snd_show.GetInt()) {
        ...
        for (int i = 0; i < activeChannels.Count(); i++) {
            ...
            if (snd_surround.GetInt() < 4) {
                Con_NXPrintf(&np,
                             "%02i l(%03d) r(%03d) vol(%03d) ent(%03d) pos(%6d %6d %6d) timeleft(%f) looped(%d) %50s",
                             total + 1,
                ...
                Con_NXPrintf(&np,
                             "%02i l(%03d) c(%03d) r(%03d) rl(%03d) rr(%03d) vol(%03d) ent(%03d) pos(%6d %6d %6d) timeleft(%f) looped(%d) %50s",
                             total + 1,
                             (int) channel->fvolume[IFRONT_LEFT],
...
void GetSoundTime(void) {
    ...
    if (cl_movieinfo.IsRecording() || IsReplayRendering()) {
        ...
        if (IsReplayRendering()) {
            IReplayMovieRenderer *pMovieRenderer = (g_pClientReplayContext != NULL)
                                                   ? g_pClientReplayContext->GetMovieRenderer() : NULL;
...
#endif
        {
            ...
            if (s_lastsoundtime != t) {
                double flSamples =
                        (double) g_pSoundServices->GetHostFrametime() * (double) g_AudioDevice->DeviceDmaSpeed();
