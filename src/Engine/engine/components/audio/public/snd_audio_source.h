...
public:
    ...
    virtual int
    MixDataToDevice(IAudioDevice *pDevice, channel_t *pChannel, int sampleCount, int outputRate, int outputOffset) = 0;
...
public:
    ...
    OpenStreamedLoad(char const *pFileName, int dataSize, int dataStart, int startPos, int loopPos, int bufferSize,
                     int numBuffers, streamFlags_t flags) = 0;
...
struct CAudioSourceCachedInfoHandle_t {
    ...
    inline CAudioSourceCachedInfo *
    Get(int audiosourcetype, bool soundisprecached, CSfxTable *sfx, int *pcacheddatasize) {
...
public:
    ...
    virtual int
    GetOutputData(void **pData, int samplePosition, int sampleCount, char copyBuf[AUDIOSOURCE_COPYBUF_SIZE]) = 0;
