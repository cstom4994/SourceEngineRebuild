...
static VideoSystemInfo_t s_VideoAppSystems[] =
        {
                {VideoSystem::QUICKTIME,
                                    PLATFORM_WIN32 |
                                    PLATFORM_OSX,                                                       "video_quicktime", VIDEO_SUBSYSTEM_INTERFACE_VERSION},
                {VideoSystem::BINK, PLATFORM_WIN32 | PLATFORM_OSX | PLATFORM_XBOX_360 |
                                    PLATFORM_LINUX,                                                     "video_bink",      VIDEO_SUBSYSTEM_INTERFACE_VERSION},
...
void *CValveVideoServices::QueryInterface(const char *pInterfaceName) {
    if (Q_strncmp(pInterfaceName, VIDEO_SERVICES_INTERFACE_VERSION, Q_strlen(VIDEO_SERVICES_INTERFACE_VERSION) + 1) ==
        0) {
...
VideoSystem_t
CValveVideoServices::LocateVideoSystemForPlayingFile(const char *pFileName, VideoSystemFeature_t playMode) {
...
VideoResult_t CValveVideoServices::LocatePlayableVideoFile(const char *pSearchFileName, const char *pPathID,
    ...
    AssertExitV(IS_NOT_EMPTY(pSearchFileName) || pPlaybackSystem == nullptr || pPlaybackSystem == nullptr ||
                fileNameMaxLen <= 0, SetResult(VideoResult::BAD_INPUT_PARAMETERS));

    VideoResult_t Status = ResolveToPlayableVideoFile(pSearchFileName, pPathID,
                                                      VideoSystem::DETERMINE_FROM_FILE_EXTENSION, playMode,
...
// ===========================================================================	
IVideoMaterial *
CValveVideoServices::CreateVideoMaterial(const char *pMaterialName, const char *pVideoFileName, const char *pPathID,
    ...
    VideoResult_t Status = ResolveToPlayableVideoFile(pVideoFileName, pPathID, videoSystem,
    ...
    IVideoMaterial *pMaterial = m_VideoSystems[sysIndex]->CreateVideoMaterial(pMaterialName, ResolvedFilePath,
                                                                              playbackFlags);
...
VideoResult_t CValveVideoServices::DestroyVideoMaterial(IVideoMaterial *pVideoMaterial) {
    ...
    for (int i = 0; i < m_MaterialList.Count(); i++) {
        if (m_MaterialList[i].m_pObject == pVideoMaterial) {
            VideoResult_t Status = m_VideoSystems[m_MaterialList[i].m_VideoSystem]->DestroyVideoMaterial(
                    pVideoMaterial);
...
VideoResult_t CValveVideoServices::DestroyVideoRecorder(IVideoRecorder *pVideoRecorder) {
    ...
    for (int i = 0; i < m_RecorderList.Count(); i++) {
        if (m_RecorderList[i].m_pObject == pVideoRecorder) {
            VideoResult_t Status = m_VideoSystems[m_RecorderList[i].m_VideoSystem]->DestroyVideoRecorder(
                    pVideoRecorder);
...
VideoResult_t CValveVideoServices::PlayVideoFileFullScreen(const char *pFileName, const char *pPathID, void *mainWindow,
    ...
    VideoResult_t Status = ResolveToPlayableVideoFile(pFileName, pPathID, videoSystem,
...
CValveVideoServices::ResolveToPlayableVideoFile(const char *pFileName, const char *pPathID, VideoSystem_t videoSystem,
                                                ...
                                                int resolvedFileNameMaxLen, VideoSystem_t *pResolvedVideoSystem) {
    SetResult(VideoResult::BAD_INPUT_PARAMETERS);
    AssertExitV(IS_NOT_EMPTY(pFileName), VideoResult::BAD_INPUT_PARAMETERS);
    AssertExitV(videoSystem == VideoSystem::DETERMINE_FROM_FILE_EXTENSION ||
                IS_IN_RANGECOUNT(videoSystem, VideoSystem::VIDEO_SYSTEM_FIRST, VideoSystem::VIDEO_SYSTEM_COUNT),
...
VideoSystem_t
CValveVideoServices::LocateSystemAndFeaturesForFileName(const char *pFileName, VideoSystemFeature_t *pFeatures,
...
// ===========================================================================	
bool
CVideoCommonServices::CalculateVideoDimensions(int videoWidth, int videoHeight, int displayWidth, int displayHeight,
                                               VideoPlaybackFlags_t playbackFlags,
...
// ===========================================================================	
VideoResult_t
CVideoCommonServices::InitFullScreenPlaybackInputHandler(VideoPlaybackFlags_t playbackFlags, float forcedMinTime,
                                                         bool windowed) {
    // already initialized?
    if (m_bInputHandlerInitialized) {
    ...
    m_bScanAll = ANY_BITFLAGS_SET(playbackFlags,
                                  VideoPlaybackFlags::PAUSE_ON_ANY_KEY | VideoPlaybackFlags::ABORT_ON_ANY_KEY);
    ...
    m_bPauseEnabled = ANY_BITFLAGS_SET(playbackFlags,
                                       VideoPlaybackFlags::PAUSE_ON_ESC | VideoPlaybackFlags::PAUSE_ON_RETURN |
                                       VideoPlaybackFlags::PAUSE_ON_SPACE | VideoPlaybackFlags::PAUSE_ON_ANY_KEY);
    m_bAbortEnabled = ANY_BITFLAGS_SET(playbackFlags,
