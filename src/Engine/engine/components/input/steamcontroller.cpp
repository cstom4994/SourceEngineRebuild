        ...
        if (m_bSteamControllerActionsInitialized) {
            ...
            for (int i = 0; i != ARRAYSIZE(g_DigitalMenuActions); ++i) {
                ...
                for (uint64 j = 0; j < m_unNumConnected; ++j) {
                    ...
                    ControllerDigitalActionData_t data = steamcontroller->GetDigitalActionData(nControllerHandles[j],
                                                                                               action.handle);
...
CInputSystem::GetSteamControllerActionOrigin(const char *action, ControllerActionSetHandle_t action_set_handle) {
    auto pSC = SteamControllerInterface();
