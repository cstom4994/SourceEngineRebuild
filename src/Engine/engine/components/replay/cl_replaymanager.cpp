...
void CReplayManager::CommitPendingReplayAndBeginDownload() {
    ...
    CClientRecordingSession *pSession = CL_CastSession(
            CL_GetRecordingSessionManager()->FindSession(m_pReplayThisLife->m_hSession));
...
void CReplayManager::AttemptToSetupNewReplay() {
    ...
    if (!g_pReplay_->IsRecording() || g_pEngineClient->IsPlayingReplayDemo()) {
...
float CReplayManager::GetDownloadProgress(const CReplay *pReplay) {
    ...
    FOR_EACH_VEC(vecBlocks, i) {
        ...
        const float flSubProgress = pCurBlock->m_uFileSize == 0 ? 0.0f : clamp(
                (float) pCurBlock->m_uBytesDownloaded / pCurBlock->m_uFileSize, 0.0f, 1.0f);
