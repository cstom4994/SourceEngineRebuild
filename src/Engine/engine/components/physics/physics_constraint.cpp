...
public:
    ...
    void
    WriteToTemplate(vphysics_save_cphysicsconstraint_t &header, vphysics_save_constraint_t &constraintTemplate) const;
...
void CPhysicsConstraint::InitHinge(IVP_Environment *pEnvironment, CPhysicsConstraintGroup *constraint_group,
    ...
    if (breakable) {
        ...
        m_HkConstraint = new hk_Hinge_Constraint(lcs, builder.get_blueprint(),
                                                 (hk_Rigid_Body *) m_pObjReference->GetObject(),
                                                 (hk_Rigid_Body *) m_pObjAttached->GetObject());
    }
...
void CPhysicsConstraint::InitFixed(IVP_Environment *pEnvironment, CPhysicsConstraintGroup *constraint_group,
    ...
    if (breakable) {
        hk_Constraint *pFixed = new hk_Fixed_Constraint(hkEnvironment, &fixed_bp,
                                                        (hk_Rigid_Body *) m_pObjReference->GetObject(),
                                                        (hk_Rigid_Body *) m_pObjAttached->GetObject());
...
void CPhysicsConstraint::InitBallsocket(IVP_Environment *pEnvironment, CPhysicsConstraintGroup *constraint_group,
    ...
    if (breakable) {
        hk_Ball_Socket_Constraint *pConstraint = new hk_Ball_Socket_Constraint(hkEnvironment, &builder,
                                                                               (hk_Rigid_Body *) m_pObjReference->GetObject(),
                                                                               (hk_Rigid_Body *) m_pObjAttached->GetObject());
...
void CPhysicsConstraint::InitSliding(IVP_Environment *pEnvironment, CPhysicsConstraintGroup *constraint_group,
    ...
    if (breakable) {
        hk_Constraint *pFixed = new hk_Prismatic_Constraint(hkEnvironment, &prismatic_bp,
                                                            (hk_Rigid_Body *) m_pObjReference->GetObject(),
                                                            (hk_Rigid_Body *) m_pObjAttached->GetObject());
...
void CPhysicsConstraint::InitPulley(IVP_Environment *pEnvironment, CPhysicsConstraintGroup *constraint_group,
    ...
    if (breakable) {
        hk_Constraint *pPulley = new hk_Pulley_Constraint(hkEnvironment, &pulley_bp,
                                                          (hk_Rigid_Body *) m_pObjReference->GetObject(),
                                                          (hk_Rigid_Body *) m_pObjAttached->GetObject());
...
void CPhysicsConstraint::InitLength(IVP_Environment *pEnvironment, CPhysicsConstraintGroup *constraint_group,
    ...
    if (breakable) {
        hk_Constraint *pLength = new hk_Stiff_Spring_Constraint(hkEnvironment, &stiff_bp,
                                                                (hk_Rigid_Body *) m_pObjReference->GetObject(),
                                                                (hk_Rigid_Body *) m_pObjAttached->GetObject());
...
void CPhysicsConstraint::WriteHinge(constraint_hingeparams_t &hinge) const {
    ...
    hinge.worldPosition = TransformHavanaLocalToHLWorld(hinge_bp.m_axis_os[0].get_origin(),
                                                        m_pObjReference->GetObject(), true);
    hinge.worldAxisDirection = TransformHavanaLocalToHLWorld(hinge_bp.m_axis_os[0].get_direction(),
...
CreatePhysicsConstraintGroup(IVP_Environment *pEnvironment, const constraint_groupparams_t &group) {
    MEM_ALLOC_CREDIT();
...
IPhysicsConstraint *
CreateHingeConstraint(IVP_Environment *pEnvironment, CPhysicsObject *pReferenceObject, CPhysicsObject *pAttachedObject,
                      IPhysicsConstraintGroup *pGroup, const constraint_limitedhingeparams_t &hinge) {
    MEM_ALLOC_CREDIT();
    CPhysicsConstraint *pConstraint = new CPhysicsConstraint(pReferenceObject, pAttachedObject);
...
IPhysicsConstraint *
CreateFixedConstraint(IVP_Environment *pEnvironment, CPhysicsObject *pReferenceObject, CPhysicsObject *pAttachedObject,
...
IPhysicsConstraint *
CreatePulleyConstraint(IVP_Environment *pEnvironment, CPhysicsObject *pReferenceObject, CPhysicsObject *pAttachedObject,
...
IPhysicsConstraint *
CreateLengthConstraint(IVP_Environment *pEnvironment, CPhysicsObject *pReferenceObject, CPhysicsObject *pAttachedObject,
    ...
    if (IsValidConstraint(header)) {
        switch (header.constraintType) {
            ...
            case CONSTRAINT_HINGE: {
                ...
                *ppConstraint = (CPhysicsConstraint *) pEnvironment->CreateHingeConstraint(header.pObjReference,
                                                                                           header.pObjAttached,
                                                                                           header.pGroup, hinge);
            }
                break;
            case CONSTRAINT_FIXED: {
                ...
                CPhysicsEnvironment *pEnvironment = (CPhysicsEnvironment *) params.pEnvironment;
                *ppConstraint = (CPhysicsConstraint *) pEnvironment->CreateFixedConstraint(header.pObjReference,
                                                                                           header.pObjAttached,
                                                                                           header.pGroup, fixed);
            }
                break;
            case CONSTRAINT_SLIDING: {
                ...
                CPhysicsEnvironment *pEnvironment = (CPhysicsEnvironment *) params.pEnvironment;
                *ppConstraint = (CPhysicsConstraint *) pEnvironment->CreateSlidingConstraint(header.pObjReference,
                                                                                             header.pObjAttached,
                                                                                             header.pGroup, sliding);
            }
                break;
            case CONSTRAINT_PULLEY: {
                vphysics_save_constraintpulley_t pulley;
                ...
                CPhysicsEnvironment *pEnvironment = (CPhysicsEnvironment *) params.pEnvironment;
                *ppConstraint = (CPhysicsConstraint *) pEnvironment->CreatePulleyConstraint(header.pObjReference,
                                                                                            header.pObjAttached,
                                                                                            header.pGroup, pulley);
            }
                break;
            case CONSTRAINT_LENGTH: {
                vphysics_save_constraintlength_t length;
                ...
                CPhysicsEnvironment *pEnvironment = (CPhysicsEnvironment *) params.pEnvironment;
                *ppConstraint = (CPhysicsConstraint *) pEnvironment->CreateLengthConstraint(header.pObjReference,
                                                                                            header.pObjAttached,
                                                                                            header.pGroup, length);
            }
                break;
            case CONSTRAINT_BALLSOCKET: {
                vphysics_save_constraintballsocket_t ballsocket;
                ...
                CPhysicsEnvironment *pEnvironment = (CPhysicsEnvironment *) params.pEnvironment;
                *ppConstraint = (CPhysicsConstraint *) pEnvironment->CreateBallsocketConstraint(header.pObjReference,
                                                                                                header.pObjAttached,
                                                                                                header.pGroup,
                                                                                                ballsocket);
            }
            ...
            case CONSTRAINT_RAGDOLL: {
                ...
                CPhysicsEnvironment *pEnvironment = (CPhysicsEnvironment *) params.pEnvironment;
                *ppConstraint = (CPhysicsConstraint *) pEnvironment->CreateRagdollConstraint(header.pObjReference,
                                                                                             header.pObjAttached,
                                                                                             header.pGroup, ragdoll);
            }
                break;
        }
