...
inline void
IVP_VecFPU::fpu_add_multiple_row(IVP_DOUBLE *target_adress, IVP_DOUBLE *source_adress, IVP_DOUBLE factor, int size,
...
inline IVP_DOUBLE
IVP_VecFPU::fpu_large_dot_product(IVP_DOUBLE *base_a, IVP_DOUBLE *base_b, int size, IVP_BOOL adress_aligned) {
    if (adress_aligned == IVP_FALSE) {
        //we have to calculate the block size and shift adresses to lower aligned adresses
...
inline void
IVP_VecFPU::fpu_multiply_row(IVP_DOUBLE *target_adress, IVP_DOUBLE factor, int size, IVP_BOOL adress_aligned) {
    if (adress_aligned == IVP_FALSE) {
        //we have to calculate the block size and shift adresses to lower aligned adresses
...
inline void IVP_VecFPU::fpu_exchange_rows(IVP_DOUBLE *target_adress1, IVP_DOUBLE *target_adress2, int size,
                                          IVP_BOOL adress_aligned) {
    if (adress_aligned == IVP_FALSE) {
        //we have to calculate the block size and shift adresses to lower aligned adresses
        long adress, result_adress;
...
inline void
IVP_VecFPU::fpu_copy_rows(IVP_DOUBLE *target_adress, IVP_DOUBLE *source_adress, int size, IVP_BOOL adress_aligned) {
    if (adress_aligned == IVP_FALSE) {
        //we have to calculate the block size and shift adresses to lower aligned adresses
        long adress, result_adress;
...
#endif
    IVP_VecFPU::fpu_exchange_rows(&matrix_values[b * aligned_row_len], &matrix_values[a * aligned_row_len], columns,
                                  IVP_TRUE);
...
IVP_RETURN_TYPE IVP_Great_Matrix_Many_Zero::solve_lower_null_matrix() {
    ...
    for (i = columns - 1; i >= 0; i--) {
        ...
        if (IVP_Inline_Math::fabsd(left_side) < MATRIX_EPS) {
            ...
            desired_vector[i] = right_side /
                                left_side; // division left_side is not needed, it was done already above (make a vector of inverses)
...
void IVP_Great_Matrix_Many_Zero::fill_from_bigger_matrix(IVP_Great_Matrix_Many_Zero *big_matrix, int *original_pos,
                                                         int n_column) {
...
void
IVP_Great_Matrix_Many_Zero::copy_to_sub_matrix(IVP_DOUBLE *values_big_matrix, IVP_Great_Matrix_Many_Zero *sub_matrix,
                                               int *original_pos) {
    for (int i = 0; i < sub_matrix->columns; i++) {
        for (int j = 0; j < sub_matrix->columns; j++) {
            sub_matrix->matrix_values[i * sub_matrix->columns + j] = values_big_matrix[original_pos[i] * columns +
                                                                                       original_pos[j]];
        }
    }
}
...
IVP_RETURN_TYPE
IVP_Linear_Constraint_Solver::init_and_solve_lc(IVP_DOUBLE *A_in, IVP_DOUBLE *b_in, IVP_DOUBLE *result_vec_out,
                                                int var_num, int actives_at_begin, IVP_U_Memory *my_mem) {
    n_variables = var_num;
    ...
    lu_sub_solver.MATRIX_EPS =
            SOLVER_EPS * 10; // makes incrementel lu more often to fail but prevents from getting numerical inaccurate
...
void IVP_Linear_Constraint_Solver::increment_sub_solver() {
    ...
    if (sub_solver_status == 0) {
        IVP_ASSERT(lu_sub_solver.n_sub == r_actives - 1);
        for (k = 0; k < r_actives; k++) {
            lu_sub_solver.U_matrix[lu_sub_solver.n_sub * lu_sub_solver.aligned_row_len + k] = full_A[
                    new_var_orig * aligned_size + actives_inactives_ignored[k]];
        }
    ...
    while (1) {
		...
		smallest_step = P_DOUBLE_MAX;
            ...
            if (limiting_var < r_actives) {
                ...
                IVP_IF(debug_lcs) {
                    printf("\nactive %d becomes inactive (relp %d)\n\n", actives_inactives_ignored[limiting_var],
                           limiting_var);
                }
                ...
                if (limiting_var < ignored_pos) {
                    ...
                    move_not_necessary_actives_to_inactives();
                    IVP_IF(debug_lcs) {
                        printf("\nignored %d becomes active (relp %d)\n\n", actives_inactives_ignored[ignored_pos],
                               ignored_pos);
                    }
                    int indx = actives_inactives_ignored[limiting_var];
                    IVP_ASSERT(IVP_Inline_Math::fabsd(accel[indx]) < 0.001f);
        ...
        IVP_IF(debug_lcs) {
            ...
            for (i = 0; i < r_actives; i++) {
                ...
                for (j = 0; j < r_actives; j++) {
                    ...
                    sub_solver_mat.matrix_values[i * sub_solver_mat.aligned_row_len + j] = this->full_A[
                            index * aligned_size + index2];
...
#else
    IVP_VecFPU::fpu_exchange_rows(&U_matrix[base_a + pivot_col], &U_matrix[base_b + pivot_col], n_sub - pivot_col,
                                  IVP_FALSE);
...
#else
    IVP_VecFPU::fpu_add_multiple_row(&base_b[row_down + 1], &base_a[row_down + 1], -factor, n_sub - row_down - 1,
                                     IVP_FALSE);
#endif
...
#else
    IVP_VecFPU::fpu_add_multiple_row(&U_matrix[base_b + pivot_row + 1], &U_matrix[base_a + pivot_row + 1], -factor,
...
void IVP_Incr_L_U_Matrix::mult_vec_with_L() {
    int i;
    for (i = n_sub - 1; i >= 0; i--) {
        IVP_DOUBLE sum = IVP_VecFPU::fpu_large_dot_product(&L_matrix[i * aligned_row_len], &mult_vec[0], n_sub,
                                                           IVP_TRUE);
        temp_vec[i] = sum;
    }
}
...
void IVP_Linear_Constraint_Solver::debug_dep_var(int full_var_nr) {
    ...
    for (i = 0; i < sub_var_nr; i++) {
        for (j = 0; j < r_actives; j++) {
            ...
            sub_solver_mat.matrix_values[j * sub_solver_mat.aligned_row_len + col_nr] = full_solver_mat.matrix_values[
                    full_j * aligned_size + full_i];
        }
        col_nr++;
    }
