...
public:
    ...
    static inline IVP_DOUBLE
    get_closing_speed_core_i(const IVP_Impact_Solver_Long_Term *, int core_index, const IVP_U_Float_Point *rot_sped_cs,
                             IVP_U_Float_Point *speed_ws);  // based on current core speed
    static inline IVP_DOUBLE
    get_inv_virtual_mass(const IVP_Impact_Solver_Long_Term *i) { return i->inv_virtual_mass; };  // based on core
    ...
    static inline void
    test_push_core_i(const IVP_Impact_Solver_Long_Term *, int core_index, IVP_U_Float_Point *rot_change,
                     IVP_U_Float_Point *speed_change, IVP_FLOAT factor);
    ...
    static void
    ease_test_two_mindists(IVP_Contact_Point *dist0, IVP_Contact_Point *dist1, IVP_U_Float_Point *world_surf_normal);

    static void ease_two_mindists(IVP_Contact_Point *dist0, IVP_Contact_Point *dist1, IVP_U_Float_Point *diff_vec0,
                                  IVP_U_Float_Point *diff_vec1, IVP_DOUBLE ease_factor);


    // setup
    static inline IVP_DOUBLE
    calc_desired_gap_speed(IVP_DOUBLE closing_speed, IVP_DOUBLE gap_diff, IVP_FLOAT speedup_factor);

    void setup_coords_mindists(IVP_Friction_System *fs);

    inline void calc_distance_matrix_column(int current_contact_point_index, IVP_Core *core_to_test,
    ...
    // gauss
    IVP_RETURN_TYPE test_gauss_solution_suggestion(IVP_DOUBLE *push_results, int *active_is_at_pos, int total_actives,
                                                   IVP_U_Memory *my_mem);

    // penalty method
    IVP_FLOAT do_penalty_step(IVP_FLOAT *impulses, IVP_FLOAT *pretension, IVP_FLOAT force_factor,
                              IVP_FLOAT damp_factor); // returns quad_sum_desired_speed
    void do_penalty_method(IVP_Friction_System *fs);

    // barraf
    void solve_linear_equation_and_push(IVP_Friction_System *fs, int *active_is_at_pos, int total_actives,
                                        IVP_U_Memory *my_mem);
...
IVP_DOUBLE
IVP_Friction_Solver::calc_desired_gap_speed(IVP_DOUBLE closing_speed, IVP_DOUBLE gap_diff, IVP_FLOAT speedup_factor) {
