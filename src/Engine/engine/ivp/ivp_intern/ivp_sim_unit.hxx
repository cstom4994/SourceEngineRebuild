...
public:
    ...
    IVP_BOOL
    sim_unit_calc_movement_state(IVP_Environment *env); //return TRUE when Simulation_Unit was removed from Simulation
    void sim_unit_clear_movement_check_values();

    void
    sim_unit_revive_for_simulation(IVP_Environment *env); //can handle mixture of simulated objs and not simulated objs
    inline void sim_unit_ensure_in_simulation();                      //can not ...
    void sim_unit_ensure_cores_movement();    //reset_freeze_check_values
    IVP_Core *
    sim_unit_union_find_test(); //problem: quadratic search, because every controller gives list of associated cores (but this list depends on core)
...
inline void IVP_Simulation_Unit::prefetch0_init_moving_core_for_psi(IVP_Core *core) {
    IVP_IF_PREFETCH_ENABLED(IVP_TRUE) {
        IVP_PREFETCH_BLOCK((char *) core + sizeof(IVP_Core_Fast_Static),
