...
public:
    ...
    void do_impact_long_term(IVP_Core *pushed_cores[2], IVP_FLOAT rescue_speed_val,
...
class IVP_Contact_Point : public IVP_Contact_Point_Fast {
    ...
    void read_materials_for_contact_situation(
            IVP_Impact_Solver_Long_Term *info); // #+# check calls to this function when material is changed on the fly -> recalc some values

    void
    calc_coll_distance(); // returns the distance reached in one PSI and the rescue speed val and stores result in impact.distance_reached_in_time
    ...
    bool friction_force_local_constraint_2d_wheel(IVP_Core *core_a, IVP_Impact_Solver_Long_Term *info,
    ...
    IVP_BOOL
    get_world_direction_second_friction(IVP_U_Float_Point *ret_second_friction_vec, IVP_FLOAT *sin_second_friction);
    ...
    void p_calc_friction_s_PP(const IVP_U_Point *pp0, const IVP_U_Point *pp1, IVP_Impact_Solver_Long_Term *info,
                              IVP_U_Float_Point *diff_contact_vec);

    void p_calc_friction_qr_PF(const IVP_U_Point *pp, const IVP_Compact_Edge *F, IVP_Cache_Ledge_Point *m_cache_F,
                               IVP_Impact_Solver_Long_Term *info, IVP_U_Float_Point *diff_contact_vec);

    void p_calc_friction_ss_KK(const IVP_Compact_Edge *K, const IVP_Compact_Edge *L, IVP_Cache_Ledge_Point *m_cache_K,
                               IVP_Cache_Ledge_Point *m_cache_L, IVP_Impact_Solver_Long_Term *info,
                               IVP_U_Float_Point *diff_contact_vec);

    void p_calc_friction_s_PK(const IVP_U_Point *pp, const IVP_Compact_Edge *K, IVP_Cache_Ledge_Point *m_cache_K,
                              IVP_Impact_Solver_Long_Term *info, IVP_U_Float_Point *diff_contact_vec);
...
class IVP_Friction_Core_Pair {
    ...
    IVP_DOUBLE destroy_mutual_energy(
            IVP_DOUBLE d_e); //returns amount of energy that was actually reduced, rest is accumulated in integrated_anti_energy
...
public:
    ...
    int check_all_fr_mindists_to_be_valid(
            IVP_Friction_System *fs); //returns number of still valid mindists; warning, maybe this function deletes whole structure 'this'
...
public:
    ...
    IVP_BOOL
    dist_removed_update_pair_info(IVP_Contact_Point *old_dist); //returns IVP_TRUE if union find has to be called
    ...
    void apply_real_friction(
            const IVP_Event_Sim *es); // does real friction according to pressure calculated by 'do_friction_system' in last PSI
...
class IVP_Mutual_Energizer {
    ...
    static inline void get_rot_inertia(IVP_Core *core, IVP_U_Float_Point *rot_vec_obj, IVP_DOUBLE *rot_inertia,
                                       IVP_DOUBLE *inv_rot_inertia);

    IVP_DOUBLE calc_impulse_to_reduce_energy_level(IVP_DOUBLE speed_pot, IVP_DOUBLE inv_mass0, IVP_DOUBLE inv_mass1,
                                                   IVP_DOUBLE delta_e);
...
public:
    ...
    static IVP_DOUBLE
    calc_energy_potential(IVP_DOUBLE speed_pot, IVP_DOUBLE mass0, IVP_DOUBLE mass1, IVP_DOUBLE inv_mass0,
