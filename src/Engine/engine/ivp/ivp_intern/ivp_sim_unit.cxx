...
IVP_Simulation_Unit::~IVP_Simulation_Unit() {
    this->clean_sim_unit();;//printf("delete_simu %lx\n",(long)this&0x0000ffff);
}
...
void IVP_Controller_Manager::announce_controller_to_environment(IVP_Controller_Dependent *cntrl) {
    ...
    if (mtype < IVP_MT_NOT_SIM) {
        reference_unit->sim_unit_revive_for_simulation(
                l_environment); //ensure in simulation is not enough (cannot handle mixture of simulated and not simulated objects)
    }
}
...
void IVP_Simulation_Unit::sim_unit_sort_controllers() {
    ...
    while (1) {
        ...
        IVP_CONTROLLER_PRIORITY second_prio = controller_cores.element_at(
                second)->l_controller->get_controller_priority();
        if (controller_cores.element_at(first)->l_controller->get_controller_priority() > second_prio) {
            ...
            int test_pos = first;
            while ((test_pos > 0) &&
                   (controller_cores.element_at(test_pos - 1)->l_controller->get_controller_priority() >
    ...
    IVP_IF(1) {
        for (int i = 0; i < contr_num - 1; i++) {
            IVP_ASSERT(!(controller_cores.element_at(i)->l_controller->get_controller_priority() >
...
//    IVP_ASSERT ( c_time.get_time() == 0 || IVP_Inline_Math::fabsd( core->time_of_last_psi - c_time + 1.0f / core->i_delta_time ) < 10E-4f);
    ...
    core->m_world_f_core_last_psi.vv.add_multiple(&core->pos_world_f_core_last_psi, &core->delta_world_f_core_psis,
                                                  d_time);
}
...
//	check_movement_state = IVP_FALSE;   // this might be wrong if flag is set to IVP_MT_SLOW
    ...
    for (int j = controller_num - 1; j >= 0; j--) {
        ...
        my_controller->do_simulation_controller(es, &controller_cores.element_at(
                j)->cores_controlled_by); //speed dependent, real speed
        IVP_IF(1) {
            for (int c = sim_unit_cores.len() - 1; c >= 0; c--) {
...
void IVP_Sim_Units_Manager::simulate_sim_units_psi(IVP_Environment *env, IVP_U_Vector<IVP_Core> *touched_cores) {
    ...
    while (n2_su) { prefetch0_simulate_single_sim_unit_psi(n2_su);
