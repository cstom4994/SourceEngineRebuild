...
//				limit.m_ref_position += HK_PI * 2.0f;
        ...
        if (limit.m_limit_is_enabled) { // do the limits
            ...
            hk_real angular_vel = query.get_vmq_storage().get_velocities()[0] +
            ...
            if (next_alpha > limit.m_limit_max) {
                hk_real delta =
                        (next_alpha - limit.m_limit_max) * limit.m_limit_tau * tau_factor * pi.get_inv_delta_time();
                impulse -= virtual_mass * delta;
            } else if (next_alpha < limit.m_limit_min) {
                hk_real delta =
                        (next_alpha - limit.m_limit_min) * limit.m_limit_tau * tau_factor * pi.get_inv_delta_time();
                impulse -= virtual_mass * delta;
            }
        }
        if (impulse) {
    ...
    static inline void do_linear_limit(
        ...
        if (limit.m_limit_is_enabled) { // do the limits
            ...
            if (next_pos > limit.m_limit_max) {
                hk_real delta =
                        (next_pos - limit.m_limit_max) * tau_factor * limit.m_limit_tau * pi.get_inv_delta_time();
                impulse += virtual_mass * delta;
            } else if (next_pos < limit.m_limit_min) {
                hk_real delta =
                        (next_pos - limit.m_limit_min) * tau_factor * limit.m_limit_tau * pi.get_inv_delta_time();
                impulse += virtual_mass * delta;
            }
        }
        if (impulse) {
