...
#else
    hk_VecFPU::fpu_exchange_rows(&m_U_matrix[base_a + pivot_col], &m_U_matrix[base_b + pivot_col], m_n_sub - pivot_col,
                                 HK_FALSE);
...
#else
    hk_VecFPU::fpu_add_multiple_row(&base_b[row_down + 1], &base_a[row_down + 1], -factor, m_n_sub - row_down - 1,
                                    HK_FALSE);
#endif
...
#else
    hk_VecFPU::fpu_add_multiple_row(&m_U_matrix[base_b + pivot_row + 1], &m_U_matrix[base_a + pivot_row + 1], -factor,
...
void hk_Incr_LU_Matrix::mult_vec_with_L() {
    int i;
    for (i = m_n_sub - 1; i >= 0; i--) {
        hk_incrlu_real sum = hk_VecFPU::fpu_large_dot_product(&m_L_matrix[i * m_aligned_row_len], &m_inout_vec[0],
                                                              m_n_sub, HK_TRUE);
        m_temp_vec[i] = sum;
    }
}
...
#else
    hk_VecFPU::fpu_add_multiple_row(&m_L_matrix[base_b], &m_L_matrix[base_a], -factor, m_n_sub,
