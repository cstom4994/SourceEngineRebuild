...
public:
    ...
    virtual void
    change_fixing_point_Ros(const IVP_U_Point *anchor_Ros);            // just set a new fixing point at both objects
    virtual void change_target_fixing_point_Ros(const IVP_U_Point *anchor_Ros);

    virtual void change_translation_axes_Ros(
            const IVP_U_Matrix3 *m_Ros_f_Rfs);   // just changes the translation axes. Normally has no effect
    virtual void
    change_target_translation_axes_Ros(const IVP_U_Matrix3 *m_Ros_f_Rfs); // move attached object to a new postion
