...
public:
    ...
    inline void
    increase_hull_by_x(IVP_Time t_now, IVP_FLOAT delta_time, IVP_FLOAT gradient, IVP_FLOAT center_gradient_);
    ...
    inline IVP_FLOAT insert_synapse(IVP_Listener_Hull *syn, IVP_Time t_now,
                                    IVP_DOUBLE delta_valid_hull_time); // assert delta_valid_hull_time ensures no events till next PSI
    inline IVP_FLOAT insert_lazy_synapse(IVP_Listener_Hull *syn, IVP_Time t_now,
                                         IVP_DOUBLE delta_valid_hull_time); // same, but adds some extra time to avoid assert on delta_valid_hull_time
...
IVP_FLOAT
IVP_Hull_Manager::insert_lazy_synapse(IVP_Listener_Hull *syn, IVP_Time /*t_now*/, IVP_DOUBLE delta_valid_hull_time) {
...
void
IVP_Hull_Manager::update_lazy_synapse(IVP_Listener_Hull *syn, IVP_Time /*t_now*/, IVP_DOUBLE delta_valid_hull_time) {
    sorted_synapses.remove_minlist_elem(syn->minlist_index);
    IVP_FLOAT val = hull_value_next_psi;
    syn->minlist_index = sorted_synapses.add((void *) syn, val + delta_valid_hull_time);
}
