...
class IVP_Environment {        // the environment
    ...
    IVP_Environment(IVP_Environment_Manager *manager, IVP_Application_Environment *appl_env, const char *costumer_name,
    ...
    inline IVP_BOOL must_perform_movement_check() {
        ...
        if (next_movement_check == 0) {
            next_movement_check =
                    IVP_MOVEMENT_CHECK_COUNT * 3 / 2 + (short) (ivp_rand() * (IVP_MOVEMENT_CHECK_COUNT / 2));
            return IVP_TRUE;
        } else {
            return IVP_FALSE;
        }
...
public:
    ...
    void
    set_delta_PSI_time(IVP_DOUBLE new_delta_PSI_time);        // range: IVP_MIN_DELTA_PSI_TIME - IVP_MAX_DELTA_PSI_TIME

    static void set_global_collision_tolerance(IVP_DOUBLE tolerance = 0.01f,
                                               IVP_DOUBLE gravity_length = 0.01f);   // set the collision tolerance (try to go for higher values if possible)
    ...
    IVP_Actuator_Spring *create_spring(
            IVP_Template_Spring *templ);     // creates a damped spring, Note IVP_Controller_Stiff_Spring are better
    ...
    void install_listener_object_global(
            IVP_Listener_Object *); // adds a callback which listens to events of all objects if its not in the list already
    void remove_listener_object_global(IVP_Listener_Object *);

    void add_listener_object_private(IVP_Real_Object *,
                                     IVP_Listener_Object *);        // adds a callback which listens to object events of one object
    void remove_listener_object_private(IVP_Real_Object *,
                                        IVP_Listener_Object *);    // (same as IVP_Real_Object::add_listener_object())
    ...
    void add_listener_collision_private(IVP_Real_Object *,
                                        IVP_Listener_Collision *);    // adds a callback which listens to just one object
    void remove_listener_collision_private(IVP_Real_Object *,
                                           IVP_Listener_Collision *);// (note: same as IVP_Real_Object::add_listener_collision())
    ...
    /////////// simulation
    void simulate_until(
            IVP_Time time);                // the MAIN function: simulates the entire physical environment until 'time'
    void simulate_dtime(
            IVP_DOUBLE deta_time);            // the MAIN function: simulates the entire physical for a given time
    void simulate_time_step(
            IVP_FLOAT sub_psi_pos = 1.99);    // simulate exactly one time step (for synchronous use of the physics engine (e.g. playstation 2).
...
public:
    ...
    IVP_Environment *
    create_environment(IVP_Application_Environment *appl_env, const char *costumer_name, unsigned int auth_code);
