...
public:
    ...
    inline IVP_BOOL
    is_compact() { return (IVP_BOOL) is_compact_flag; }; // returns true if vertex info is included in compact ledge
    ...
    void
    byte_swap(); // just byte swap this data BUT WILL NOT do the point array, as may be external data or shared and so may be byte swapped > 1 times
    void byte_swap_all(
            IVP_U_BigVector<IVP_Compact_Poly_Point> *pre_swapped_points); // byte_swap, and recurse too all related child data including the point and triangle data
};
...
public:
    ...
    void byte_swap(); // just byte swap this data
    void byte_swap_all(
            IVP_U_BigVector<IVP_Compact_Poly_Point> *pre_swapped_points); // byte_swap, and recurse too all related child data
