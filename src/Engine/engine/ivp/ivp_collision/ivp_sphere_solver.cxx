...
static inline void
sphere_against_ledgetree_node(const IVP_Compact_Ledgetree_Node *cln, IVP_U_Point *query_pos, IVP_DOUBLE query_rad,
 ...
 */
static inline IVP_BOOL
sphere_against_object(IVP_Real_Object *obj, IVP_U_Point *query_pos, IVP_DOUBLE query_rad, int max_depth) {
    ...
    if (obj->get_type() == IVP_POLYGON) {
        ...
        return sphere_overlaps_sphere_d(query_pos, query_rad, obj->get_core()->get_position_PSI(),
...
void IVP_Sphere_Solver::recursively_collect_intruding_objects(IVP_OV_Node *node, IVP_U_Float_Point *query_rlb,
    ...
    node_pos.set(node_luf.k[0] + (node_size / 2.0f), node_luf.k[1] + (node_size / 2.0f),
        ...
        if (AABB_overlaps_AABB_d(&node_rlb, &node_pos, query_rlb, query_pos)) {
            for (int i = 0; i < node->elements.len(); i++) {
                ...
                if (sphere_overlaps_sphere_d(&m_center, m_radius, pos, rad)) {
                    ...
                    if (sphere_against_object(query_object, &query_center_object_space, m_radius,
