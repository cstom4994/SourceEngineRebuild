...
IVP_DOUBLE
IVP_OV_Tree_Manager::calc_optimal_box(const IVP_OV_Element *element, IVP_DOUBLE min_radius, IVP_DOUBLE max_radius) {
...
void IVP_OV_Tree_Manager::collect_collision_partners(const IVP_OV_Element *elem, const IVP_OV_Node *masternode,
    ...
    if (new_node != NULL) {
        ...
        for (;;) {
            if (this->root->data.rasterlevel >= new_node->data.rasterlevel) {
                if (!box_contains_box(&this->root->data, new_node,
...
void
IVP_OV_Tree_Manager::get_center_coordinates_ws(const IVP_OV_Node *node, IVP_U_Float_Point *p, IVP_FLOAT *cubesize) {
