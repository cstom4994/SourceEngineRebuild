...
CreateSimpleThread(ThreadFunc_t, void *pParam, ThreadId_t *pID, unsigned stackSize = 0);
...
ThreadWaitForObjects(int nEvents, const HANDLE *pHandles, bool bWaitAll = true, unsigned timeout = TT_INFINITE);
...
inline void const *ThreadInterlockedCompareExchangePointerToConst(void const *volatile *p, void const *value,
    ...
    return ThreadInterlockedCompareExchangePointer(const_cast < void *volatile * > ( p ),
...
typedef CInterlockedIntT<uint> CInterlockedUInt;
...
public:
    ...
    bool
    AssertOwnedByCurrentThread() { if (*pCondition) return BaseClass::AssertOwnedByCurrentThread(); else return true; }
...
public:
    FORCEINLINE CAutoLockT(MUTEX_TYPE &lock, const char *pMutexName, const char *pFilename, int nLineNum,
            ...
            : m_lock(const_cast< typename V_remove_const<MUTEX_TYPE>::type & >( lock )), m_pMutexName(pMutexName),
              m_pFilename(pFilename), m_nLineNum(nLineNum), m_bOwned(true) {
...
inline CAutoLockT<MUTEX_TYPE>
make_auto_lock(MUTEX_TYPE &lock, const char *pMutexname, const char *pFilename, int nLineNum,
               int nMinReportDurationUs = 1) {
...
public:
    ...
    static int
    WaitForMultiple(int nEvents, CStdThreadEvent *const *pEvents, bool bWaitAll = true, unsigned timeout = TT_INFINITE);
...
inline int
ThreadWaitForEvents(int nEvents, CThreadEvent *const *pEvents, bool bWaitAll = true, unsigned timeout = TT_INFINITE) {
#ifdef POSIX
    Assert( nEvents == 1);
...
#else
    HANDLE handles[64];
    for (int i = 0; i < std::min(nEvents, (int) ARRAYSIZE(handles)); i++)
