...
// Such a monstrosity. :(
static void
GenerateLightmapSamplesForMesh(const matrix3x4_t &_matPos, const matrix3x4_t &_matNormal, int _iThread, int _skipProp,
                               int _nFlags, int _lightmapResX, int _lightmapResY,
...
static void
DumpLightmapLinear(const char *_dstFilename, const CUtlVector<colorTexel_t> &_srcTexels, int _width, int _height);
...
private:
    ...
    void
    ApplyLightingToStaticProp(int iStaticProp, CStaticProp &prop, const CComputeStaticPropLightingResults *pResults);
...
public:
    ...
    float
    ComputeCoverageForTriangle(int shadowTextureIndex, const Vector2D &t0, const Vector2D &t1, const Vector2D &t2) {
...
//			flEpsilon = 1.0;
        ...
        GatherSampleLightSSE(sampleOutput, dl, -1, adjusted_pos4, &normal4, 1, iThread,
                             nLFlags | GATHERLFLAGS_FORCE_FAST,
...
void CVradStaticPropMgr::ApplyLightingToStaticProp(int iStaticProp, CStaticProp &prop,
    ...
    for (int bodyID = 0; bodyID < pStudioHdr->numbodyparts; ++bodyID) {
        ...
        for (int modelID = 0; modelID < pBodyPart->nummodels; ++modelID) {
            ...
            const CUtlVector<colorVertex_t> *colorVerts = pResults->m_ColorVertsArrays.Count()
                                                          ? pResults->m_ColorVertsArrays[iCurColorVertsArray++]
                                                          : nullptr;
            const CUtlVector<colorTexel_t> *colorTexels = pResults->m_ColorTexelsArrays.Count()
...
void CVradStaticPropMgr::ComputeLighting(CStaticProp &prop, int iThread, int prop_index,
    ...
    const int skip_prop = (g_bDisablePropSelfShadowing || (prop.m_Flags & STATIC_PROP_NO_SELF_SHADOWING)) ? prop_index
                                                                                                          : -1;
...
void CVradStaticPropMgr::SerializeLighting() {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        unsigned char *pVertexData = (unsigned char *) (sizeof(HardwareVerts::FileHeader_t) +
    ...
    for (int i = 0; i < count; ++i) {
        ...
        unsigned char *pTexelData = (unsigned char *) (sizeof(HardwareTexels::FileHeader_t) +
                                                       m_StaticProps[i].m_MeshData.Count() *
                                ...
                                for (int i = 0; i < pStrip->numIndices; i += 3) {
                                    ...
                                    if (shadowTextureIndex >= 0) {
                                        if (bInitTriangles) {
                                            ...
                                            if (coverage < 1.0f) {
                                                materialIndex = g_ShadowTextureList.AddMaterialEntry(shadowTextureIndex,
                                                                                                     *vertData->Texcoord(
                                                                                                             vertex1),
                                                                                                     *vertData->Texcoord(
                                                                                                             vertex2),
...
static void
GenerateLightmapSamplesForMesh(const matrix3x4_t &_matPos, const matrix3x4_t &_matNormal, int _iThread, int _skipProp,
                               int _flags, int _lightmapResX, int _lightmapResY, studiohdr_t *_pStudioHdr,
                               mstudiomodel_t *_pStudioModel, OptimizedModel::ModelHeader_t *_pVtxModel, int _meshID,
...
// ------------------------------------------------------------------------------------------------
static void
BuildFineMipmap(unsigned int _resX, unsigned int _resY, bool _applyFilter, const CUtlVector<colorTexel_t> &_srcTexels,
                CUtlVector<RGB888_t> *_outTexelsRGB888, CUtlVector<Vector> *_outLinear) {
