...
//-----------------------------------------------------------------------------
ChunkFileResult_t
CManifest::LoadManifestMapKeyCallback(const char *szKey, const char *szValue, CManifestMap *pManifestMap) {
            ...
            for (int nBrushNum = 0; nBrushNum < g_MainMap->entities[i].numbrushes; nBrushNum++) {
                ...
                for (int nCordon = 0; nCordon < m_Cordons.Count(); nCordon++) {
                    ...
                    for (int nBox = 0; nBox < m_Cordons[nCordon].m_Boxes.Count(); nBox++) {
                        if (m_Cordons[nCordon].m_Boxes[nBox].IsIntersectingBox(g_MainMap->mapbrushes[nIndex].mins,
                                                                               g_MainMap->mapbrushes[nIndex].maxs) ==
                            true) {
                            bRemove = false;
                            break;
                        }
                ...
                if (bRemove) {
                   ...
                   m_CordoningMapEnt) {    // for all other entities, even if they include brushes, we look at origin
