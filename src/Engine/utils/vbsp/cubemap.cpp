...
static bool LoadSrcVTFFiles(IVTFTexture *pSrcVTFTextures[6], const char *pSkyboxMaterialBaseName,
    ...
    for (i = 0; i < 6; i++) {
        ...
        IMaterialVar *pSkyTextureVar = pSkyboxMaterial->FindVar("$basetexture",
                                                                NULL); // Since we're setting it to black anyway, just use $basetexture for HDR
...
//-----------------------------------------------------------------------------
static bool
PatchEnvmapForMaterialAndDependents(const char *pMaterialName, const PatchInfo_t &info, const char *pCubemapTexture) {
