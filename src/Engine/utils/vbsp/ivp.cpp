...
public:
    ...
    CPhysCollisionEntryFluid(CPhysCollide *pCollide, const char *pSurfaceProp, float damping, const Vector &normal,
...
CPhysCollisionEntryFluid::CPhysCollisionEntryFluid(CPhysCollide *pCollide, const char *pSurfaceProp, float damping,
...
CPhysConvex *
CPlaneList::BuildConvexForBrush(int brushnumber, float shrink, CPhysCollide *pCollideTest, float shrinkMinimum) {
...
static void Flood_FindConnectedWaterVolumes_r(CUtlVector<node_t *> &list, node_t *pLeaf, waterleaf_t &baseleaf,
...
void EmitWaterVolumesForBSP(dmodel_t *pModel, node_t *node) {
    ...
    for (i = 0; i < list.Count(); i++) {
        ...
        if (waterAreaList.Count()) {
            ...
            tmp.fogVolumeIndex = FindOrCreateLeafWaterData(tmp.waterLeafData.surfaceDist, tmp.waterLeafData.minZ,
    ...
    for (int i = 0; i < g_WaterModels.Count(); i++) {
        ...
        if (pCollide) {
            ...
            if (!waterModel.waterLeafData.hasSurface) {
                ...
                Vector top = physcollision->CollideGetExtent(pCollide, vec3_origin, vec3_angle,
...
static void ConvertWorldBrushesToPhysCollide(CUtlVector<CPhysCollisionEntry *> &collisionList, float shrinkSize,
...
static void
BuildWorldPhysModel(CUtlVector<CPhysCollisionEntry *> &collisionList, float shrinkSize, float mergeTolerance) {
...
static void ConvertModelToPhysCollide(CUtlVector<CPhysCollisionEntry *> &collisionList, int modelIndex, int contents,
                                      float shrinkSize, float mergeTolerance) {
    int i;
    CPlaneList planes(shrinkSize, mergeTolerance);
