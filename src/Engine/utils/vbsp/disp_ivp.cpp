...
MaterialSystemMaterial_t GetMatIDFromDisp(mapdispinfo_t *pMapDisp) {
    ...
    MaterialSystemMaterial_t matID = FindOriginalMaterial(TexDataStringTable_GetString(pTexData->nameStringTableID),
                                                          NULL, true);
    return matID;
}
    ...
    for (int grid = 0; grid < gDispGridList.Count(); grid++) {
        ...
        for (int listIndex = 0; listIndex < gDispGridList[grid].dispList.Count(); listIndex++) {
            ...
            for (int iTri = 0; iTri < nTriCount; ++iTri) {
                ...
                physcollision->PolysoupAddTriangle(pTerrainPhysics, tmpVerts[0], tmpVerts[1], tmpVerts[2],
                                                   nMaterialIndex);
            }
        }
...
public:
    ...
    virtual void
    GetTrianglesInSphere(void *userData, const Vector &center, float radius, virtualmeshtrianglelist_t *pList);
...
void CDispMeshEvent::GetTrianglesInSphere(void *userData, const Vector &center, float radius,
