...
bool
RunProcessFromArgs(CUtlVector<char *> &newArgv, bool bShowAppWindow, bool bCreateSuspended, const char *pWorkingDir,
...
bool CheckDownloaderFinished() {
    ...
    if (FindArg(__argc, __argv, "-TryDLLMode") &&
        ...
        if (g_Waiting_bPatching) {
            ...
            ResumeThread(
                    g_hRunningThread); // We started the installer suspended so we could make sure we'd send out the patch command.
