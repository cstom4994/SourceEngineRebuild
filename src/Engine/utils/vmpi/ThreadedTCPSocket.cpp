...
private:
    ...
    bool Init(SOCKET iSocket, CIPAddr remoteAddr, ITCPSocketHandler *pHandler) {
        ...
        m_hSendThread = CreateThread(NULL, 0, &CThreadedTCPSocket::StaticSendThreadFn, this, CREATE_SUSPENDED,
                                     &dwSendThreadID);
        m_hRecvThread = CreateThread(NULL, 0, &CThreadedTCPSocket::StaticRecvThreadFn, this, CREATE_SUSPENDED,
                                     &dwRecvThreadID);
...
private:
    ...
    bool InternalSend(void const *const *pChunks, const int *pChunkLengths, int nChunks, bool bPrependLength) {
        ...
        if (bPrependLength) {
            *((int *) pOut) = totalLength -
                              4;    // The length we prepend is the size of the data, not data size + integer for length.
            pOut += 4;
        }
        for (int i = 0; i < nChunks; i++) {
        ...
        *pSocket = NULL;
        ...
        if (status > 0) {
            ...
            if (newSock == INVALID_SOCKET) {
                ...
                IThreadedTCPSocket *pRet = CThreadedTCPSocket::Create(newSock, connectedAddr,
                                                                      m_pHandler->CreateNewHandler());
                if (!pRet) {
                    Assert(false);
                    closesocket(m_Socket);
                    m_Socket = INVALID_SOCKET;
    ...
    virtual bool Update(IThreadedTCPSocket **pSocket, unsigned long milliseconds) {
        *pSocket = NULL;
        ...
        if (m_bConnected) {
            ...
            IThreadedTCPSocket *pRet = CThreadedTCPSocket::Create(m_Socket, m_RemoteAddr,
                                                                  m_pHandlerCreator->CreateNewHandler());
            if (pRet) {
                m_Socket = INVALID_SOCKET;
                *pSocket = pRet;
                return true;
