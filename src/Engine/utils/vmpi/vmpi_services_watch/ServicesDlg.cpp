...
int CheckServiceVersion(const char *pPatchDir, char *pServiceVersion, int maxServiceVersionLen) {
    ...
    if (hInst) {
        bool bFound = FindStringResourceEx(hInst, VMPI_SERVICE_IDS_VERSION_STRING,
                                           MAKELANGID(LANG_NEUTRAL, SUBLANG_NEUTRAL), pServiceVersion,
                                           maxServiceVersionLen);
        if (bFound) {
...
void CServicesDlg::OnStopServices() {
    if (MessageBox(
            "Warning: if you stop these services, you won't be able to control them from this application, and must restart them manually. Contine?",
