...
static void
Shader_ParseShaderInfoFromCompileCommands(const CfgProcessor::CfgEntryInfo *pEntry, ShaderInfo_t &shaderInfo);
...
private:
    ...
    static bool
    CompareDynamicComboIDs(const std::unique_ptr<CByteCodeBlock> &pA, const std::unique_ptr<CByteCodeBlock> &pB) {
...
static void FlushCombos(size_t &pnTotalFlushedSize, CUtlBuffer &pDynamicComboBuffer, CUtlBuffer &pBuf) {
    ...
    uint8_t *pCompressedShader = LZMA::OpportunisticCompress(reinterpret_cast<uint8_t *>(pDynamicComboBuffer.Base()),
...
static void
OutputDynamicCombo(size_t &pnTotalFlushedSize, CUtlBuffer &pDynamicComboBuffer, CUtlBuffer &pBuf, uint64_t nComboID,
                   uint32_t nComboSize, const uint8_t *pComboCode) {
...
static void GetVCSFilenames(std::span<char> pszMainOutFileName, const ShaderInfo_t &si) {
    ...
    if (status.exists()) {
        ...
        if ((status.status().permissions() & (fs::perms::owner_read | fs::perms::owner_write)) !=
            (fs::perms::owner_read | fs::perms::owner_write)) {
            // It isn't writable. . we'd better change its permissions (or check it out possibly)
            std::cout << clr::pinkish << "Warning: making " << clr::red << pszMainOutFileName.data() << clr::pinkish
                      << " writable!" << clr::reset << std::endl;
...
static void WriteShaderFiles(const char *pShaderName) {
    ...
    std::cout << "\r" << szShaderFileOperation << " " << (bShaderFailed ? clr::red : clr::green) << pShaderName
              << clr::reset << "...\r";
    ...
    if (bShaderFailed) {
        ...
        std::cout << "\r" << clr::red << pShaderName << clr::reset << " "
                  << FormatTimeShort(std::chrono::duration_cast<std::chrono::seconds>(Clock::now() - lastTime).count())
                  << "                                        \r";
    ...
    if (g_bVerbose) {
        std::cout << "\033[B";
        std::cout << std::showbase << pShaderName << " : " << clr::green << shaderInfo.m_nTotalShaderCombos
                  << clr::reset << " combos, centroid mask: " << clr::green << std::hex << shaderInfo.m_CentroidMask
                  << std::dec << clr::reset << ", numDynamicCombos: " << clr::green << shaderInfo.m_nDynamicCombos
                  << clr::reset << ", flags: " << clr::green << std::hex << shaderInfo.m_Flags << std::dec << clr::reset
        ...
        for (CStaticCombo *pStatic = pByteCodeArray->Chain(nChain).Head(); pStatic; pStatic = pStatic->Next()) {
            ...
            if (code.GetLength()) {
                ...
                for (const size_t i: hash) {
                    ...
                    if (check.m_nCRC32 == hdr.m_nCRC32 && checkCode.GetLength() == code.GetLength() &&
                        memcmp(checkCode.GetData(), code.GetData(), checkCode.GetLength()) == 0) {
                        // this static combo is the same as another one!!
                        duplicateCombos.emplace_back(
                                StaticComboAliasRecord_t{hdr.m_nStaticComboID, check.m_nStaticComboID});
                        bIsDuplicate = true;
                        break;
                    }
                }
    ...
    //
    std::ofstream ShaderFile(szVCSfilename, std::ios::binary |
                                            std::ios::trunc); // Streaming buffer for vcs file (since this can blow memory)

    // ------ Header --------------
    const ShaderHeader_t header{
            SHADER_VCS_VERSION_NUMBER,
            gsl::narrow_cast<int32_t>(
                    shaderInfo.m_nTotalShaderCombos), // this is not actually used in vertexshaderdx8.cpp for combo checking
    ...
    ShaderFile.write(reinterpret_cast<const char *>(StaticComboHeaders.data()),
                     sizeof(StaticComboRecord_t) * StaticComboHeaders.size()); // dummy write, 8 bytes per static combo
    ...
    ShaderFile.write(reinterpret_cast<const char *>(duplicateCombos.data()),
                     sizeof(StaticComboAliasRecord_t) * duplicateCombos.size());
    ...
    std::cout << "\r" << clr::green << pShaderName << clr::reset << " "
              << FormatTimeShort(std::chrono::duration_cast<std::chrono::seconds>(Clock::now() - lastTime).count())
              << "                                        \r";
    //std::cout << ( "\033["s + std::to_string( lastLine ) + "A" );
    lastTime = Clock::now();
...
static size_t
AssembleWorkerReplyPackage(const CfgProcessor::CfgEntryInfo *pEntry, uint64_t nComboOfEntry, CUtlBuffer &pBuf) {
    GLOBAL_DATA_MTX_LOCK();
    ...
    if (std::chrono::duration_cast<std::chrono::seconds>(fCurTime - s_fLastInfoTime).count() != 0) {
        ...
        s_nLastEntry = nComboOfEntry;
        std::cout << "\rCompiling " << (g_ShaderHadError.contains(pEntry->m_szName) ? clr::red : clr::green)
                  << pEntry->m_szName << clr::reset << " [ " << clr::blue << PrettyPrint(nComboOfEntry) << clr::reset
                  << " remaining ("
                  << clr::green2 << s_averageProcess.GetAverage() << clr::reset << " c/m) ] "
                  << FormatTimeShort(std::chrono::duration_cast<std::chrono::seconds>(fCurTime - g_flStartTime).count())
                  << " elapsed         \r";
...
public:
    explicit CWorkerAccumState(TMutexType *pMutex) noexcept
            : m_pMutex(pMutex), m_iFirstCommand(0), m_iNextCommand(0), m_iEndCommand(0), m_iLastFinished(0),
              m_hCombo(nullptr) {}
...
template<Threading::Mutex TMutexType>
void
CWorkerAccumState<TMutexType>::HandleCommandResponse(CfgProcessor::ComboHandle hCombo, CmdSink::IResponse *pResponse) {
    Assert(pResponse);
    ...
    if (pResponse->Succeeded()) {
        ...
        StaticComboFromDictAdd(pEntryInfo->m_szName, nStComboIdx)->AddDynamicCombo(nDyComboIdx,
                                                                                   pResponse->GetResultBuffer(),
    ...
    if (szListing || !pResponse->Succeeded()) {
        ...
        if (!szListing) {
            sprintf_s(chUnreportedListing,
                      "%s(0,0): error 0000: Compiler failed without error description. Command number %llu",
                      pEntryInfo->m_szShaderFileName, iCommandNumber);
...
void CWorkerAccumState<TMutexType>::TryToPackageData(uint64_t iCommandNumber) {
    ...
    for (; pInfoBegin && (pInfoBegin->m_iCommandStart < pInfoEnd->m_iCommandStart || nComboBegin > nComboEnd);) {
        ...
        if (nPackedLength) {
            // Packed buffer
            GLOBAL_DATA_MTX_LOCK();
            uint8_t *pCodeBuffer = StaticComboFromDictAdd(pInfoBegin->m_szName, nComboBegin)->AllocPackedCodeBlock(
                    nPackedLength);
            GLOBAL_DATA_MTX_UNLOCK();
...
static void Shared_ParseListOfCompileCommands() {
    ...
    const std::string name = Parser::ConstructName(fs::path(*cmdLine.lastArgs[0]).filename().string(),
                                                   g_pShaderVersion);
    if (Parser::CheckCrc((fs::path(g_pShaderPath) / *cmdLine.lastArgs[0]).string(), name, g_pShaderCRC) &&
        !cmdLine.isSet("-force")) {
        exit(0);
    }
    ...
    if (!Parser::ParseFile((fs::path(g_pShaderPath) / *cmdLine.lastArgs[0]).string(), g_pShaderVersion, static_c,
                           dynamic_c, skip, centroid_mask, includes)) {
        std::cout << clr::red << "Failed to parse " << *cmdLine.lastArgs[0] << clr::reset << std::endl;
        exit(-1);
    }
    Parser::WriteInclude((fs::path(g_pShaderPath) / "include" / (name + ".inc")).string(), name, static_c, dynamic_c,
                         skip);
    ConfigurationProcessing::SetupConfigurationDirect(name, g_pShaderVersion, centroid_mask, static_c, dynamic_c, skip,
                                                      includes);
    ...
    std::cout << "\rCompiling " << clr::green << PrettyPrint(g_numCompileCommands) << clr::reset
              << " commands, setup took " << clr::green
              << std::chrono::duration_cast<std::chrono::seconds>(tt_end - tt_start).count() << clr::reset
              << " seconds.         \r";
}
...
static LONG WINAPI ExceptionFilter(_EXCEPTION_POINTERS *pExceptionInfo) {
    constexpr const auto iType = static_cast<MINIDUMP_TYPE>(MiniDumpNormal | MiniDumpWithDataSegs |
    ...
    const HANDLE hFile = ::CreateFile(rgchFileName, GENERIC_WRITE, FILE_SHARE_WRITE, nullptr, CREATE_ALWAYS,
    ...
    if (hFile) {
        ...
        bMinidumpResult = MiniDumpWriteDump(::GetCurrentProcess(), ::GetCurrentProcessId(), hFile, iType, &ExInfo,
                                            nullptr, nullptr);
        CloseHandle(hFile);
    }
...
static void PrintCompileErrors() {
    ...
    if (!g_CompilerMsg.empty()) {
        ...
        std::cout << clr::yellow << "WARNINGS" << clr::reset << "/" << clr::red << "ERRORS " << clr::reset
                  << totalWarnings << "/" << totalErrors << std::endl;
        ...
        for (const auto &sMsg: g_CompilerMsg) {
            ...
            const std::string searchPat =
                    *cmdLine.lastArgs[0] + "("; // TODO: rework when readding support for multiple files

            if (const size_t warnings = msg.warning.size())
                std::cout << shaderName << " " << clr::yellow << warnings
                          << " WARNING(S):                                                         " << clr::reset
                          << std::endl;
            ...
            if (const size_t errors = msg.error.size())
                std::cout << shaderName << " " << clr::red << errors
                          << " ERROR(S):                                                               " << clr::reset
                          << std::endl;
            ...
            for (const auto &err: msg.error) {
                ...
                if (size_t find = m.find(searchPat); find != std::string::npos)
                    m = m.replace(find - cwdLen, cwdLen, "");
                std::cout << m << "\nReported " << clr::green << numReported << clr::reset
                          << " time(s), example command: " << std::endl;
...
static void WriteStats() {
    ...
    std::cout << clr::green << FormatTime(std::chrono::duration_cast<std::chrono::seconds>(end - g_flStartTime).count())
              << clr::reset << " elapsed                                           " << std::endl;
}
...
int main(int argc, const char *argv[]) {
    ...
    cmdLine.add("", false, 0, 0, "Directs the compiler to not use flow-control constructs where possible", "/Gfa",
                "-no-flow-control");
    cmdLine.add("", false, 0, 0, "Directs the compiler to use flow-control constructs where possible", "/Gfp",
                "-prefer-flow-control");
    ...
    if (!cmdLine.gotExpected(badOptions)) {
        std::cout << clr::red << clr::bold << "ERROR: Got unexpected number of arguments for option"
                  << (badOptions.size() == 1 ? ": " : "s:\n") << clr::reset;
        for (const auto &option: badOptions)
            std::cout << option << "\n";
        std::cout << clr::reset << std::endl;
    ...
    if (!Parser::ValidateVersion(g_pShaderVersion)) {
        std::cout << clr::red << "Shader uses unknown shader version: " << clr::pinkish << g_pShaderVersion
                  << clr::reset << std::endl;
        return -1;
    }
    ...
    if (cmdLine.isSet("-dynamic")) {
        ...
        const std::string name = Parser::ConstructName(fs::path(*cmdLine.lastArgs[0]).filename().string(),
                                                       g_pShaderVersion);
        Parser::WriteInclude((fs::path(g_pShaderPath) / "include" / (name + ".inc")).string(), name, static_c,
                             dynamic_c, skip);
        return 0;
    }
    ...
    std::cout << "\rCompiling " << clr::green << PrettyPrint(g_numCompileCommands) << clr::reset << " commands in "
              << clr::green << PrettyPrint(g_numStaticCombos) << clr::reset
              << " static combos.                      \r";
    CompileShaders();
