...
void CheckWhichLibrary() {
    Assert(!g_pStudioRender);
    Assert(!studiorender);
    Assert(!g_pMatSystemSurface);
    Assert(!g_pVGui);
    Assert(!g_pVGuiPanel);
    Assert(!g_pVGuiInput);
    Assert(!g_pVGuiSurface);
    Assert(!g_pDataCache);
    Assert(!g_pMDLCache);
    Assert(!mdlcache);
    Assert(!g_pVideo);
    Assert(!g_pDmeMakefileUtils);
    Assert(!g_pPhysicsCollision);
    Assert(!g_pVGuiLocalize);
    Assert(!g_pSoundEmitterSystem);
    Assert(!g_pVGuiSchemeManager);
    Assert(!g_pVGuiSystem);
}
...
void ConnectTier3Libraries(CreateInterfaceFn *pFactoryList, int nFactoryCount) {
    ...
    for (int i = 0; i < nFactoryCount; ++i) {
        ...
        if (!g_pSoundEmitterSystem) {
            g_pSoundEmitterSystem = (ISoundEmitterSystemBase *) pFactoryList[i](SOUNDEMITTERSYSTEM_INTERFACE_VERSION,
                                                                                NULL);
        }
        if (!g_pVTex) {
