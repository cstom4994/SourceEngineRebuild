...
#pragma pack()
    ...
    template<class CDestPixel>
    static inline void
    DecodeAlpha3BitLinear(CDestPixel *pImPos, DXTAlphaBlock3BitLinear *pAlphaBlock, int width, int nChannelSelect = 3) {
...
// Use AMD compressor to convert RGBA input to ATI1N, ATI2N or DXT5_GA format
    ...
    bool ConvertToDXTLegacy(const uint8 *src, ImageFormat srcImageFormat,
        ...
        Assert(!"DXT is no longer supported.");
        return false;
    ...
    void CompressSTB(uint8 *pDstBytes, ImageFormat dstFmt, const uint8 *pSrcBytes, int nWidth, int nHeight) {
        ...
        for (uint32 y = 0; y < cPixY; y += 4) {
            ...
            for (uint x = 0; x < cPixX; x += 4) {
                for (uint i = 0; i < 4; ++i) {
                    uint32 offsetX = std::min(x + i, (uint) cLastX);
...
// HDRFIXME: This assumes that the 16-bit integer values are 4.12 fixed-point.
    void
    ConvertImageFormat_RGBA16161616_To_RGB323232F(unsigned short *pSrcImage, float *pDstImage, int width, int height) {
...
// HDRFIXME: This assumes that the 16-bit integer values are 4.12 fixed-point.
    void
    ConvertImageFormat_RGB323232F_To_RGBA16161616(float *pSrcImage, unsigned short *pDstImage, int width, int height) {
    ...
    bool ConvertImageFormat(const uint8 *src, ImageFormat srcImageFormat,
        ...
        if (srcImageFormat == IMAGE_FORMAT_RGBA16161616) {
            ...
            if (dstImageFormat == IMAGE_FORMAT_RGBA32323232F) {
                Assert(dstStride == 0);
                ConvertImageFormat_RGBA16161616F_To_RGBA323232F((float16 *) src, (float *) dst, width, height,
                                                                srcStride);
                return true;
            }
            if (dstImageFormat == IMAGE_FORMAT_RGBA16161616) {
                Assert(srcStride == 0 && dstStride == 0);
                ConvertImageFormat_RGBA16161616F_To_RGBA16161616((float16 *) src, (unsigned short *) dst, width,
                                                                 height);
                return true;
            }
...
//		Assert( srcStride == 0 && dstStride == 0 );
        ...
        } else if ((srcImageFormat == IMAGE_FORMAT_ARGB8888) &&
                   // RGBA source and
                   (dstImageFormat == IMAGE_FORMAT_ATI1N ||
...
#undef DST
    ...
    bool
    FlipImageHorizontally(void *pSrc, void *pDst, int nWidth, int nHeight, ImageFormat imageFormat, int nDstStride) {
        if (IsCompressed(imageFormat))
            return false;
