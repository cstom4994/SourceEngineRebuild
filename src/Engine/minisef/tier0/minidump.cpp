    ...
    if (pfnMiniDumpWrite) {
        ...
        if (hFile) {
            ...
            bMinidumpResult = (*pfnMiniDumpWrite)(::GetCurrentProcess(), ::GetCurrentProcessId(), hFile,
                                                  (MINIDUMP_TYPE) minidumpType, &ExInfo, &StreamInformationHeader,
                                                  NULL);
            ::CloseHandle(hFile);
...
struct CatchAndWriteContext_t {
    ...
    void
    Set(ECatchAndWriteFunctionType eType, ECatchAndWriteMinidumpAction eAction, void *pfn, int *pargc, tchar **pargv[],
        void **ppv) {
        m_eType = eType;
        m_eAction = eAction;
        m_pfn = pfn;
...
//-----------------------------------------------------------------------------
int
CatchAndWriteMiniDumpExReturnsInt(FnWMainIntRet pfn, int argc, tchar *argv[], ECatchAndWriteMinidumpAction eAction) {
    CatchAndWriteContext_t ctx;
...
void CatchAndWriteMiniDumpForVoidPtrFn(FnVoidPtrFn pfn, void *pv, bool bExitQuietly) {
    return CatchAndWriteMiniDumpExForVoidPtrFn(pfn, pv, bExitQuietly ? k_ECatchAndWriteMiniDumpAbort
...
#define PROCESS_CALLBACK_FILTER_ENABLED     0x1
    ...
    tGetProcessUserModeExceptionPolicy pGetProcessUserModeExceptionPolicy = (tGetProcessUserModeExceptionPolicy) GetProcAddress(
            kernel32, "GetProcessUserModeExceptionPolicy");
    tSetProcessUserModeExceptionPolicy pSetProcessUserModeExceptionPolicy = (tSetProcessUserModeExceptionPolicy) GetProcAddress(
...
const char *MinidumpUserStreamInfoGet(int Index) {
    ...
    Index = ((Index + (ARRAYSIZE(g_UserStreamInfo) - 1)) +
             //subtract 1 in a way that circularly wraps. Since 0 maps to the header, the comment indices are 1 based
