...
//-----------------------------------------------------------------------------
int
ParseVectorFromKeyValueString(const char *pParamName, const char *pScan, const char *pMaterialName, float vecVal[4]) {
...
//-----------------------------------------------------------------------------
bool
CImportVMT::CreateVectorMaterialVarFromKeyValue(CDmElement *pElement, const char *pParamName, const char *pString) {
    Vector4D vecVal;
...
bool CImportVMT::UnserializeProxies(CDmElement *pElement, KeyValues *pKeyValues) {
    ...
    for (KeyValues *pProxy = pKeyValues->GetFirstTrueSubKey(); pProxy != NULL; pProxy = pProxy->GetNextTrueSubKey()) {
        ...
        for (KeyValues *pProxyParam = pProxy->GetFirstValue();
             pProxyParam != NULL; pProxyParam = pProxyParam->GetNextValue()) {
...
//		WriteKeyValuesToFile( "patch.txt", keyValues );
        ...
        if (pIncludeFileName) {
            ...
            bool success = includeKeyValues->LoadFromFile(g_pFullFileSystem, pIncludeFileName,
                                                          IsX360() ? "GAME" : NULL);
            if (success) {
...
CDmElement *CImportVMT::UnserializeFromKeyValues(KeyValues *pKeyValues) {
    ...
    for (KeyValues *pSubKey = pKeyValues->GetFirstTrueSubKey();
         pSubKey != NULL; pSubKey = pSubKey->GetNextTrueSubKey()) {
