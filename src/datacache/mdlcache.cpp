...
public:
    ...
    static void
    QueuedLoaderCallback_MDL(void *pContext, void *pContext2, const void *pData, int nSize, LoaderError_t loaderError);
...
private:
    ...
    FSAsyncStatus_t
    LoadData(const char *pszFilename, const char *pszPathID, void *pDest, int nBytes, int nOffset, bool bAsync,
    ...
    vertexFileHeader_t *
    CreateThinVertexes(vertexFileHeader_t *originalData, const studiohdr_t *pStudioHdr, int *cacheLength);
...
studiohdr_t *CMDLCache::UnserializeMDL(MDLHandle_t handle, void *pData, int nDataSize, bool bDataValid) {
    ...
    if (IsX360()) {
        if (CLZMA::IsCompressed((unsigned char *) pData)) {
            ...
            unsigned int nOutputSize = CLZMA::Uncompress((unsigned char *) pData,
                                                         (unsigned char *) pOriginalData.Get());
            if (nOutputSize != nOriginalSize) {
                // decoder failure
                return NULL;
...
bool CMDLCache::ReadMDLFile(MDLHandle_t handle, const char *pMDLFileName, CUtlBuffer &buf) {
    ...
    if (IsX360()) {
        if (CLZMA::IsCompressed((unsigned char *) buf.PeekGet())) {
            ...
            unsigned int nOutputSize = CLZMA::Uncompress((unsigned char *) buf.PeekGet(),
...
//-----------------------------------------------------------------------------
FSAsyncStatus_t
CMDLCache::LoadData(const char *pszFilename, const char *pszPathID, void *pDest, int nBytes, int nOffset, bool bAsync,
                    FSAsyncControl_t *pControl) {
    if (!*pControl) {
        if (IsX360() && g_pQueuedLoader->IsMapLoading()) {
...
//-----------------------------------------------------------------------------
vertexFileHeader_t *
CMDLCache::CreateThinVertexes(vertexFileHeader_t *originalData, const studiohdr_t *pStudioHdr, int *cacheLength) {
...
//-----------------------------------------------------------------------------
bool
CMDLCache::ProcessDataIntoCache(MDLHandle_t handle, MDLCacheDataType_t type, int iAnimBlock, void *pData, int nDataSize,
                                bool bDataValid) {
            ...
            if (IsX360() && !(pStudioDataCurrent->m_nFlags & STUDIODATA_FLAGS_NO_STUDIOMESH) && bCompressedVVDs) {
                ...
                if (pStudioHdrCurrent->numflexdesc == 0) {
                    ...
                    if (originalVertexData) {
                        ...
                        vertexFileHeader_t *thinVertexData = CreateThinVertexes(originalVertexData, pStudioHdrCurrent,
                                                                                &thinVertexDataSize);
            ...
            if (bDataValid) {
                ...
                if (IsX360()) {
                    if (CLZMA::IsCompressed((unsigned char *) pData)) {
                        ...
                        void *pOriginalData = g_pFullFileSystem->AllocOptimalReadBuffer(FILESYSTEM_INVALID_HANDLE,
                                                                                        nOriginalSize, 0);
                        unsigned int nOutputSize = CLZMA::Uncompress((unsigned char *) pData,
                                                                     (unsigned char *) pOriginalData);
                        if (nOutputSize != nOriginalSize) {
...
vertexFileHeader_t *CMDLCache::BuildAndCacheVertexData(studiohdr_t *pStudioHdr, vertexFileHeader_t *pRawVvdHdr) {
    ...
    if (IsX360()) {
        ...
        if (CLZMA::IsCompressed(pInput)) {
            ...
            unsigned int nOutputSize = CLZMA::Uncompress(pInput, sizeof(vertexFileHeader_t) +
                                                                 (unsigned char *) pOriginalData.Get());
            if (nOutputSize != nOriginalSize) {
                // decoder failure
                return NULL;
