...
//-----------------------------------------------------------------------------
    ...
    public:
        CNilSDO(const KeyType &key, const char *pchMemcachedKeyName) : CBaseSDO<KeyType, eSDOType, ProtoMsg>(key),
...
//-----------------------------------------------------------------------------
    ...
    public:
        ...
        static void
        CreateSimpleMemcachedName(CFmtStr &strDest, const char *pchPrefix, uint32 unPrefixLen, uint32 unSuffix);
...
//-----------------------------------------------------------------------------
    template<typename KeyType, int ESDOType, class ProtoMsg>
    bool CBaseSDO<KeyType, ESDOType, ProtoMsg>::BYldLoadFromSQL(CUtlVector<ISDO *> &vecSDOToLoad,
...
//-----------------------------------------------------------------------------
    template<class T>
    bool CSDOCache::BYldLoadSDO(CSDORef<T> *pPSDORef, const typename T::KeyType_t &key,
...
//-----------------------------------------------------------------------------
    template<class T>
    template<class SCH>
    bool CSDOSQLLoadHelper<T>::BYieldingExecuteSingleTable(int nFieldID, const CColumnSet &csetRead,
...
//-----------------------------------------------------------------------------
    ...
    void CSDOSQLLoadHelper<T>::AddTableToQuery(int nFieldID, const CColumnSet &csetRead) {
        ...
        {
            TSQLCmdStr sSelect;
            const char *pchColumnName = GSchemaFull().GetSchema(SCH::k_iTable).GetRecordInfo()->GetColumnInfo(
                    nFieldID).GetName();

            BuildSelectStatementText(&sSelect, csetRead);
            sCommand.Format("%s WHERE %s IN (%.*s)", sSelect.Access(), pchColumnName,
                            GetInsertArgStringChars(m_vecKeys.Count()), GetInsertArgString());
        }
...
//-----------------------------------------------------------------------------
    ...
    bool CSDOSQLLoadHelper<T>::BGetResults(int nQuery, CResults<SCH> &Results) {
        ...
        Assert(pSQLResults && nQuery >= 0 && (uint32) nQuery < pSQLResults->GetResultSetCount() &&
               pSQLResults->GetResultSetCount() == (uint32) m_vecQueries.Count());
        if (NULL == pSQLResults || nQuery < 0 || (uint32) nQuery >= pSQLResults->GetResultSetCount() ||
            pSQLResults->GetResultSetCount() != (uint32) m_vecQueries.Count())
            return false;
        ...
        if (!CopyResultToSchVector(pSQLResults->GetResultSet(nQuery), m_vecQueries[nQuery].m_ColumnSet,
                                   &Results.m_Results))
            return false;
    ...
    const SCH *CSDOSQLLoadHelper<T>::CResults<SCH>::GetSingleResultForKey(typename T::KeyType_t Key) const {
        ...
        AssertMsg((nIndex == InvalidIndex()) || (GetNextResultIndex(nIndex) == InvalidIndex()),
                  "Requested a result from a SQL load helper assuming it was a singular entry, but found multiple instances of it");
    ...
    int CSDOSQLLoadHelper<T>::CResults<SCH>::GetFirstResultIndex(typename T::KeyType_t Key) const {
        ...
        const SKeyToResult<typename T::KeyType_t> *pMatch = std::lower_bound(m_KeyToResult.begin(), m_KeyToResult.end(),
                                                                             SearchKey);
