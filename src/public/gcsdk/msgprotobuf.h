...
//-----------------------------------------------------------------------------
    ...
    public:
        CProtoBufNetPacket(CNetPacket *pNetPacket, GCProtoBufMsgSrc eReplyType, const CSteamID steamID,
...
//-----------------------------------------------------------------------------
    ...
    public:
        ...
        static bool
        BAsyncSendWithPreSerializedBody(IProtoBufSendHandler &sender, MsgType_t eMsgType, const CMsgProtoBufHeader &hdr,
                                        const byte *pubBody, uint32 cubBody);

        //similar to the above, but sends a protobuf object that will be serialized into the buffer
        static bool BAsyncSendProto(IProtoBufSendHandler &sender, MsgType_t eMsgType, const CMsgProtoBufHeader &hdr,
    ...
    private:
        ...
        static uint8 *AllocateMessageMemory(MsgType_t eMsgType, const CMsgProtoBufHeader &hdr, uint32 cubBodySize,
...
//-----------------------------------------------------------------------------
    ...
    public:
        ...
        CProtoBufMsg(MsgType_t eMsg, const CProtoBufMsgBase &msgReplyingTo)
                : CProtoBufMsgBase(eMsg), m_pProtoBufBody(NULL) {
            VPROF_BUDGET("CProtoBufMsg::CProtoBufMsg( EMsg, CProtoBufMsgMemoryPoolBase )",
                         VPROF_BUDGETGROUP_OTHER_NETWORKING);
            m_pProtoBufBody = AllocProto();
