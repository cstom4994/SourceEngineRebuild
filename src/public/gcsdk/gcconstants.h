...
//-----------------------------------------------------------------------------
    ...
    const uint32 k_cchWebAPIKeyStringMax = (k_cubWebAPIKey * 2) +
                                           1;    // size of string representing web API key (hex encoded so twice the size + NUL)
...
//-----------------------------------------------------------------------------
    ...
    const uint32 k_cchGameServerTokenStringMax = (k_cubGameServerToken * 2) +
                                                 1;    // size of string representing game server identity token (hex encoded so twice the size + NUL)
...
//-----------------------------------------------------------------------------
    ...
    const int k_cubMaxExpectedMsgDataSize =
            5 * k_nMegabyte;// the maximum application data that we EXPECT to be sent in a single message
