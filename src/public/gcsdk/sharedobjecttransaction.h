...
namespace GCSDK {
    ...
    struct SharedObjectContainsAuditEntryType {
        ...
        enum {
            kValue = sizeof(Test<TSharedObject>(NULL)) == sizeof(t_Yes)
        };
    };
...
//-----------------------------------------------------------------------------
    ...
    public:
        ...
        template<typename TSharedObject>
        void ModifyObject(TSharedObject *pObject,
                          const typename CSharedObjectModifyAndAuditFunction<TSharedObject>::ModifyFunctionType &funcModifyAndAudit) {
            COMPILE_TIME_ASSERT((!AreTypesIdentical<TSharedObject, CSharedObject>::kValue));
            ...
            AssertMsg(!m_bTransactionBuildSuccess || pWritableObject != NULL,
                      "Cannot be tracking state for an object but not having a writable version!");

            m_bTransactionBuildSuccess &= funcModifyAndAudit(*m_pSQLAccess, m_SODirtyList,
...
#define ModifyObjectProto(obj_, fieldfunc_, fieldname_, newvalue_) \
    ...
    private:
        ...
        const tCommitInfo *
        InternalFindCommitInfo(const CSharedObject *pObject, const CUtlVector <tCommitInfo> &vec) const {
            FOR_EACH_VEC(vec, i)
            {
        ...
        const CSharedObject *
        InternalFindSharedObject(CGCSharedObjectCache *pSOCache, const CSharedObject &soIndex) const;
    ...
    private:
        ...
        void SetError(const char *pszNewError) {
            AssertMsg(pszNewError && (pszNewError[0] != '\0'),
                      "Invalid NULL/empty error set in CSharedObjectTransaction::SetError()! This will have the effect of clearing the error state, which is unsupported.");
        ...
        struct undoinfo_t {
            ...
            bool operator==(const undoinfo_t &other) const {
                return other.pObject == pObject && other.pSOCache == pSOCache && other.pOriginalCopy == pOriginalCopy;
            }
        };
        ...
        bool
        AssertValidInput(const CGCSharedObjectCache *pSOCache, const CSharedObject *pObject, const char *pszContext);
