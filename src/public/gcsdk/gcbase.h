...
#define SERVER_KEY_HASH 0x5a4f4944u
    ...
    public:
        ...
        virtual bool
        OnMessageFromClient(const CSteamID &senderID, uint32 unMsgType, void *pubData, uint32 cubData) { return false; }
        ...
        CreateGSSession(const CSteamID &steamID, CGCSharedObjectCache *pSOCache, uint32 unServerAddr,
        ...
        CGCSharedObjectCache *FindSOCache(
                const CSteamID &steamID);                    // non-yielding, but may return NULL if the cache exists but is not loaded
        ...
        CGCGSSession *YieldingFindOrCreateGSSession(const CSteamID &steamID, uint32 unServerAddr, uint16 usServerPort,
        ...
        bool BYieldingRecordSupportAction(const CSteamID &actorID, const CSteamID &targetID, const char *pchData,
                                          const char *pchNote);

        void PostAlert(EAlertType eAlertType, bool bIsCritical, const char *pchAlertText,
                       const CUtlVector<CUtlString> *pvecExtendedInfo = NULL, bool bAlsoSpew = true);
        ...
        bool BYieldingAddFreeLicense(const CSteamID &steamID, uint32 unPackageID, uint32 unIPPublic = 0,
                                     const char *pchStoreCountryCode = NULL);

        int YieldingGrantGuestPass(const CSteamID &steamID, uint32 unPackageID, uint32 unPassesToGrant = 1,
        ...
        bool
        BYldSendMessageAndGetReply(const CSteamID &steamIDTarget, CProtoBufMsgBase &msgOut, CProtoBufMsgBase *pMsgIn,
                                   MsgType_t eMsg);
        ...
        bool BYldSendSystemMessageAndGetReply(const ::google::protobuf::Message &msgSend, MsgType_t eSendMsg,
        ...
        bool BSendGCMsgToClientWithPreSerializedBody(const CSteamID &steamIDTarget, MsgType_t eMsgType,
                                                     const CMsgProtoBufHeader &hdr, const byte *pubBody,
                                                     uint32 cubBody) const;

        bool
        BSendGCMsgToSystemWithPreSerializedBody(MsgType_t eMsgType, const CMsgProtoBufHeader &hdr, const byte *pubBody,
                                                uint32 cubBody) const;

        bool
        BReplyToMessageWithPreSerializedBody(MsgType_t eMsgType, const CProtoBufMsgBase &msgIn, const byte *pubBody,
                                             uint32 cubBody) const;
        ...
        void
        QueueStartPlaying(const CSteamID &steamID, const CSteamID &gsSteamID, uint32 unServerAddr, uint16 usServerPort,
                          const uint8 *pubVarData, uint32 cubVarData);
        ...
        bool BIsSOCacheBeingLoaded(const CSteamID &steamID) const {
            return m_rbtreeSOCachesBeingLoaded.Find(steamID) != m_rbtreeSOCachesBeingLoaded.InvalidIndex();
        }

        bool BYieldingLockSteamID(const CSteamID &steamID, const char *pszFilename, int nLineNum);

        bool BYieldingLockSteamIDPair(const CSteamID &steamIDA, const CSteamID &steamIDB, const char *pszFilename,
                                      int nLineNum);
        ...
        bool
        BYieldingMemcachedGet(const CUtlVector<CUtlString> &vecKeys, CUtlVector<GCMemcachedGetResult_t> &vecResults);
    ...
    private:
        ...
        //
        void YieldingStartPlaying(const CSteamID &steamID, const CSteamID &gsSteamID, uint32 unServerAddr,
                                  uint16 usServerPort, CUtlBuffer *pVarData);

        void YieldingStartGameserver(const CSteamID &steamID, uint32 unServerAddr, uint16 usServerPort,
                                     const uint8 *pubVarData, uint32 cubVarData);
...
// update their implementation and then we can delete the scaffolding whenever.
    ...
    struct IsSplitGC {
        ...
        enum {
            kValue = sizeof(Test<tBaseGCType>(NULL)) == sizeof(t_Yes)
        };
    };

    template<bool tTest, typename T>
    struct EnableIf {
        typedef T Type;
    };
    template<typename T>
...
/// Scope object that remembers if a Steam ID is locked, and automatically unlocks it upon destruction
    ...
    public:
        ...
        CScopedSteamIDLock(const CSteamID &steamID) : m_steamID(steamID), m_bLockedSuccesfully(false),
                                                      m_iSavedLockCount(-1) {}
        ...
        void VerifyPreUnlock() const {
            ...
            if (pLock) {
                AssertMsg1(m_iSavedLockCount == pLock->GetReferenceCount(),
                           "Lock imbalance on %s detected by scope lock", pLock->GetName());
            } else {
                Assert(pLock);
            }
        }
...
/// Scope object that remembers if a specific lock is taken, and releases it
        ...
        void Unlock() {
            if (m_bLockedSuccesfully) {
                ...
                if (m_pLock->GetJobLocking() != &GJobCur()) {
                    AssertMsg(false, "CScopedGenericLock::Unlock called when job %s doesn't own the lock",
                              GJobCur().GetName());
                    return;
                }
    ...
    private:
        void VerifyPreUnlock() const {
            ...
            AssertMsg1(m_iSavedLockCount == m_pLock->GetReferenceCount(), "Lock imbalance on %s detected by scope lock",
                       m_pLock->GetName());
        }
