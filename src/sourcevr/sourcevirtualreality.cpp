...
static ConCommand vr_reset_home_pos("vr_reset_home_pos", CC_VR_Reset_Home_Pos,
...
ConVar vr_use_offscreen_render_target("vr_use_offscreen_render_target", "0", 0,
                                      "Experimental: Use larger offscreen render target for pre-distorted scene in VR");
...
InitReturnVal_t CSourceVirtualReality::Init() {
    ...
    m_pDistortionTextureLeft.Init(materials->CreateProceduralTexture("vr_distort_map_left", TEXTURE_GROUP_PIXEL_SHADERS,
                                                                     distortionTextureSize, distortionTextureSize,
                                                                     IMAGE_FORMAT_RGBA16161616,
                                                                     TEXTUREFLAGS_NOMIP | TEXTUREFLAGS_NOLOD |
                                                                     TEXTUREFLAGS_NODEBUGOVERRIDE |
                                                                     TEXTUREFLAGS_SINGLECOPY | TEXTUREFLAGS_CLAMPS |
                                                                     TEXTUREFLAGS_CLAMPT));
    m_pDistortionTextureRight.Init(
            materials->CreateProceduralTexture("vr_distort_map_right", TEXTURE_GROUP_PIXEL_SHADERS,
...
void CSourceVirtualReality::CreateRenderTargets(IMaterialSystem *pMaterialSystem) {
    ...
    if (UsingOffscreenRenderTarget()) {
        ...
        m_pPredistortRTDepth.Init(
                pMaterialSystem->CreateNamedRenderTargetTextureEx2("_rt_vr_predistort_depth", nWidth, nHeight,
                                                                   RT_SIZE_LITERAL, IMAGE_FORMAT_NV_DST24,
...
// --------------------------------------------------------------------
ITexture *CSourceVirtualReality::GetRenderTarget(ISourceVirtualReality::VREye eEye,
...
// ----------------------------------------------------------------------
inline static void
ComposeProjectionTransform(float fLeft, float fRight, float fTop, float fBottom, float zNear, float zFar,
...
// ----------------------------------------------------------------------
bool
CSourceVirtualReality::GetEyeProjectionMatrix(VMatrix *pResult, VREye eEye, float zNear, float zFar, float fovScale) {
    Assert (pResult != NULL);
...
bool CSourceVirtualReality::DoDistortionProcessing(VREye eEye) {
    ...
    pRenderContext->DrawScreenSpaceRectangle(pDistortMaterial,
                                             x, y, w, h,
                                             0, 0, distortionTextureSize - 1, distortionTextureSize - 1,
                                             distortionTextureSize, distortionTextureSize);
...
// --------------------------------------------------------------------
bool CSourceVirtualReality::CompositeHud(VREye eEye, float ndcHudBounds[4], bool bDoUndistort, bool bBlackout,
                                         bool bTranslucent) {
