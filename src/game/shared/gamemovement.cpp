...
void CGameMovement::CheckParameters(void) {
    ...
    if (!IsDead()) {
        ...
        if (player->GetMoveType() != MOVETYPE_ISOMETRIC &&
            player->GetMoveType() != MOVETYPE_NOCLIP) {
            mv->m_vecAngles[ROLL] = CalcRoll(v_angle, mv->m_vecVelocity, sv_rollangle.GetFloat(),
...
//-----------------------------------------------------------------------------
void
CGameMovement::TryTouchGroundInQuadrants(const Vector &start, const Vector &end, unsigned int fMask, int collisionGroup,
                                         trace_t &pm) {
            ...
            if (player->m_Local.m_bAllowAutoMovement || bInAir || player->m_Local.m_bDucking) {
                ...
                if (CanUnduck()) {
                    ...
                    if ((player->m_Local.m_bDucking || player->m_Local.m_bDucked)) {
                        float flDuckMilliseconds = max(0.0f,
                                                       GAMEMOVEMENT_DUCK_TIME - (float) player->m_Local.m_flDucktime);
                        float flDuckSeconds = flDuckMilliseconds * 0.001f;
                        ...
                        if (flDuckSeconds > TIME_TO_UNDUCK && !bInAir) {
                            ...
                            // Reverse our process
                            float flDuckFraction = clamp(SimpleSpline(1.0f - (flDuckSeconds / TIME_TO_UNDUCK)) * 2.0f,
                                                         0, 1.0f);
