...
static ConVar sv_soundemitter_trace("sv_soundemitter_trace", "0", FCVAR_REPLICATED,
...
public:
    ...
    void
    EmitCloseCaption(IRecipientFilter &filter, int entindex, const CSoundParameters &params, const EmitSound_t &ep) {
        // No close captions in multiplayer...
        if (gpGlobals->maxClients > 1 || (gpGlobals->maxClients == 1 && !g_pClosecaption->GetBool())) {
            return;
        }
    ...
    void
    EmitAmbientSound(int entindex, const Vector &origin, const char *soundname, float flVolume, int iFlags, int iPitch,
    ...
    void
    EmitAmbientSound(int entindex, const Vector &origin, const char *pSample, float volume, soundlevel_t soundlevel,
                     int flags, int pitch, float soundtime /*= 0.0f*/, float *duration /*=NULL*/ ) {
#ifdef STAGING_ONLY
