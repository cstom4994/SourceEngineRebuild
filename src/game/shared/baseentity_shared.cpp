...
extern bool
ParseKeyvalue(void *pObject, typedescription_t *pFields, int iNumFields, const char *szKeyName, const char *szValue);

extern bool
ExtractKeyvalue(void *pObject, typedescription_t *pFields, int iNumFields, const char *szKeyName, char *szValue,
                int iMaxLen);
...
float k_flMaxEntityEulerAngle = 360.0 *
...
//-----------------------------------------------------------------------------
IPhysicsObject *
CBaseEntity::VPhysicsInitNormal(SolidType_t solidType, int nSolidFlags, bool createAsleep, solid_t *pSolid) {
    if (!VPhysicsInitSetup())
        return NULL;
...
IPhysicsObject *CBaseEntity::VPhysicsInitShadow(bool allowPhysicsMovement, bool allowPhysicsRotation, solid_t *pSolid) {
    ...
    if (GetSolid() == SOLID_BBOX) {
        ...
        pPhysicsObject = PhysModelCreateOBB(this, CollisionProp()->OBBMins(), CollisionProp()->OBBMaxs(), origin,
                                            angles, false);
    } else {
...
void CBaseEntity::FireBulletProjectiles(const ProjectileBulletsInfo_t &info) {
    ...
    for (int iShot = 0; iShot < safeInfo.m_iShots; iShot++) {
        ...
        UTIL_TraceLine(startPos, endPos, MASK_BULLETSCANTOUCH, safeInfo.m_pAdditionalIgnoreEnt, COLLISION_GROUP_NONE,
                       &tr);
...
void CBaseEntity::FireBullets(const FireBulletsInfo_t &info) {
    ...
    traceFilter.SetPassEntity(
            this); // Standard pass entity for THIS so that it can be easily removed from the list after passing through a portal
