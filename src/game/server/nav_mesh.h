...
public:
    ...
    unsigned int operator()(const NavVisPair_t &item) const {
        ...
        intp key[2] = {(intp) item.pAreas[0] + item.pAreas[1]->GetID(),
                       (intp) item.pAreas[1] + item.pAreas[0]->GetID()};
        if (sizeof(key) >= 16) {
            return Hash16(key);
        } else {
            return Hash8(key);
...
public:
    ...
    IndexType GetIndex(
            Place place) const;                /// return the directory index corresponding to this Place (0 = no entry)
...
public:
    ...
    bool
    IsAnalyzed(void) const { return m_isAnalyzed; }            // return true if a Navigation Mesh has been analyzed
    ...
    const CUtlVector<Place> *GetPlacesFromNavFile(
            bool *hasUnnamedPlaces);    // Reads the used place names from the nav file (can be used to selectively precache before the nav is loaded)

    virtual bool Save(void) const;                                    // store Navigation Mesh to a file
    bool IsOutOfDate(
            void) const { return m_isOutOfDate; }            // return true if the Navigation Mesh is older than the current map version
    ...
    virtual void SaveCustomData(
            CUtlBuffer &fileBuffer) const {}                                // store custom mesh data for derived classes
    virtual void LoadCustomData(CUtlBuffer &fileBuffer,
                                unsigned int subVersion) {}            // load custom mesh data for derived classes
    virtual void SaveCustomDataPreArea(
            CUtlBuffer &fileBuffer) const {}                        // store custom mesh data for derived classes that needs to be loaded before areas are read in
    virtual void LoadCustomDataPreArea(CUtlBuffer &fileBuffer,
                                       unsigned int subVersion) {}    // load custom mesh data for derived classes that needs to be loaded before areas are read in
    ...
    virtual void
    OnAvoidanceObstacleEnteredArea(CNavArea *area);                    // invoked when the area becomes obstructed
    virtual void
    OnAvoidanceObstacleLeftArea(CNavArea *area);                    // invoked when the area becomes un-obstructed
    ...
    CNavArea *GetNavArea(const Vector &pos,
                         float beneathLimt = 120.0f) const;    // given a position, return the nav area that IsOverlapping and is *immediately* beneath it
    ...
    Place NameToPlace(
            const char *name) const;                        // given a place name, return a place ID or zero if no place is defined
    Place PartialNameToPlace(
            const char *name) const;                    // given the first part of a place name, return a place ID or zero if no place is defined, or the partial match is ambiguous
    void PrintAllPlaces(void) const;                                    // output a list of names to the console
    int PlaceNameAutocomplete(char const *partial,
                              char commands[COMMAND_COMPLETION_MAXITEMS][COMMAND_COMPLETION_ITEM_LENGTH]);    // Given a partial place name, fill in possible place names for ConCommand autocomplete

    bool GetGroundHeight(const Vector &pos, float *height,
                         Vector *normal = NULL) const;        // get the Z coordinate of the topmost ground level below the given point
    bool GetSimpleGroundHeight(const Vector &pos, float *height,
                               Vector *normal = NULL) const;// get the Z coordinate of the ground level directly below the given point
    ...
    void DrawFuncNavPrerequisite(
            void) const;                            // draw bot prerequisite areas from func_nav_prerequisite entities
#endif
    //-------------------------------------------------------------------------------------
    // Auto-generation
    //
#define INCREMENTAL_GENERATION true
    ...
    void AddWalkableSeed(const Vector &pos,
    ...
    GetGenerationTraceMask(void) const;            // return the mask used by traces when generating the mesh
    ...
    virtual void
    CommandNavFloodSelect(const CCommand &args);            // select current area and all connected areas, recursively
    ...
    void
    CommandNavSelectHalfSpace(const CCommand &args);                // selects all areas that intersect the half-space
    ...
    void CommandNavCornerPlaceOnGround(
            const CCommand &args);            // position a corner on the current area at ground height
    ...
    void CommandNavMakeSniperSpots(
            void);                                // cuts up the marked area into individual areas suitable for sniper spots
    ...
    void
    CommandNavMergeMesh(const CCommand &args);                    // Merge a saved selected set into the current mesh
    ...
    CNavLadder *
    GetMarkedLadder(void) const { return m_markedLadder; }        // return ladder marked by user in edit mode

    CNavArea *
    GetSelectedArea(void) const { return m_selectedArea; }        // return area user is pointing at in edit mode
    CNavLadder *
    GetSelectedLadder(void) const { return m_selectedLadder; }    // return ladder user is pointing at in edit mode
    ...
    float SnapToGrid(float x,
                     bool forceGrid = false) const;                    // snap given coordinate to generation grid boundary
    Vector SnapToGrid(const Vector &in, bool snapX = true, bool snapY = true,
                      bool forceGrid = false) const;    // snap given vector's X & Y coordinates to generation grid boundary
    ...
    bool IsInSelectedSet(
            const CNavArea *area) const;                    // return true if the given area is in the selected set
    ...
    void CollectAreasOverlappingExtent(const Extent &extent, CUtlVector<NavAreaType *> *outVector) {
        ...
        for (int x = startX; x <= endX; ++x) {
            for (int y = startY; y <= endY; ++y) {
                ...
                if (iGrid >= m_grid.Count()) {
                    ExecuteNTimes(10,
                                  Warning("** Walked off of the CNavMesh::m_grid in CollectAreasOverlappingExtent()\n"));
                    return;
                }
...
protected:
    ...
    bool FindActiveNavArea(
            void);                                // Finds the area or ladder the local player is currently pointing at.  Returns true if a surface was hit by the traceline.
...
private:
    ...
    void
    AllocateGrid(float minX, float maxX, float minY, float maxY);    // clear and reset the grid to the given extents
    ...
    void DestroyNavigationMesh(
            bool incremental = false);        // free all resources of the mesh and reset it to empty state
    ...
    bool FindLadderCorners(Vector *c1, Vector *c2,
    ...
    void GetEditVectors(Vector *pos,
                        Vector *forward);        // Gets the eye position and view direction of the editing player
    ...
    bool UpdateGeneration(
            float maxTime = 0.25f);                // process the auto-generation for 'maxTime' seconds. return false if generation is complete.
    ...
    CNavNode *
    AddNode(const Vector &destPos, const Vector &destNormal, NavDirType dir, CNavNode *source, bool isOnDisplacement,
    ...
    void
    StitchAreaSet(CUtlVector<CNavArea *> *areas);        // Stitches an arbitrary set of areas into the existing mesh
