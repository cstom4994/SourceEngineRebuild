...
void CPhysicsProp::HandleAnyCollisionInteractions(int index, gamevcollisionevent_t *pEvent) {
    ...
    if (HasInteraction(PROPINTER_PHYSGUN_FIRST_IMPALE)) {
        ...
        if (pHitEntity->IsWorld()) {
            ...
            CBaseEntity * pRagdoll = CreateServerRagdollAttached(pNPC, vec3_origin, -1,
                                                                 COLLISION_GROUP_INTERACTIVE_DEBRIS, pObj, this, 0,
...
void
CBreakableProp::StickAtPosition(const Vector &stickPosition, const Vector &savePosition, const QAngle &saveAngles) {
...
static ConCommand prop_debug("prop_debug", CC_Prop_Debug,
...
pEntity->
SetDmgModExplosive( pBreakableOwner
->
...
pEntity->
CopyFadeFrom( pBreakableOwner );
}
pEntity->
SetModelName( AllocPooledString(pModel->modelName)
);
pEntity->
SetModel(STRING(pEntity->GetModelName())

);
pEntity->
SetCollisionGroup( pModel
->collisionGroup );

if ( pModel->fadeMinDist > 0 && pModel->fadeMaxDist >= pModel->fadeMinDist )
{
pEntity->
SetFadeDistance( pModel
->fadeMinDist, pModel->fadeMaxDist );
}
...
false );
pEntity->
IgniteNumHitboxFires( pOwnerFlame
->
...
pEntity->
IgniteHitboxFireScale( pOwnerFlame
->
...
static CBaseAnimating *BreakModelCreate_Ragdoll(CBaseEntity * pOwner, breakmodel_t * pModel,
const Vector &position,
const QAngle &angles
)
{
...
m_nSkin = nSkin;
pEntity->
m_iHealth = pModel->health;
if ( g_ActiveGibCount >= ACTIVE_GIB_FADE

)
{
pModel->
fadeTime = MIN(3, pModel->fadeTime);
}
if ( pModel->fadeTime )
{
pEntity->
SUB_StartFadeOut( pModel
->fadeTime, false );
...
{
pList[i]->
SetVelocity( &velocity, &angVelocity
);
}
}
else
...
void PlayLockSounds(CBaseEntity * pEdict, locksound_t * pls, int
flocked,
int fbutton
);
...
void CBasePropDoor::InputOpenAwayFrom(inputdata_t &inputdata) {
    CBaseEntity * pOpenAwayFrom = gEntList.FindEntityByName(NULL, inputdata.value.String(), NULL, inputdata.pActivator,
                                                            inputdata.pCaller);
...
inline void
TraceHull_Door(const CBasePropDoor *pDoor, const Vector &vecAbsStart, const Vector &vecAbsEnd, const Vector &hullMin,
...
private:
    ...
    void
    InputSetRotationDistance(inputdata_t &inputdata);            // Set the degree difference between open and closed

    void
    CalcOpenAngles(void);        // Subroutine to setup the m_angRotation QAngles based on the m_flDistance variable
...
//-----------------------------------------------------------------------------
void
CPropDoorRotating::CalculateDoorVolume(QAngle closedAngles, QAngle openAngles, Vector *destMins, Vector *destMaxs) {
...
public:
    ...
    bool CreateVPhysics() {
        ...
        IPhysicsObject *pPhysicsObject = physenv->CreateSphereObject(12, 0, GetAbsOrigin(), GetAbsAngles(), &params,
                                                                     false);
        if (pPhysicsObject) {
