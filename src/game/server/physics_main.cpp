...
{
GameRules()->
WeaponTraceEntity( pBaseEntity, vecAbsStart, vecAbsEnd, mask, ptr
);
}
else
{
...
// Output : trace_t
//-----------------------------------------------------------------------------
static void PhysicsCheckSweep(CBaseEntity * pEntity,
const Vector &vecAbsStart,
const Vector &vecAbsDelta, trace_t
*pTrace )
{
...
|| pEntity->
IsSolidFlagSet( FSOLID_VOLUME_CONTENTS)
)
{
if ( pEntity->
...
//-----------------------------------------------------------------------------
void
CPhysicsPushedEntities::ComputeRotationalPushDirection(CBaseEntity *pBlocker, const RotatingPushMove_t &rotPushMove,
...
bool CPhysicsPushedEntities::SpeculativelyCheckPush(PhysicsPushedInfo_t &info, const Vector &vecAbsPush,
    ...
    bool bIsUnblockable = (m_bIsUnblockableByPlayer && (pBlocker->IsPlayer() || pBlocker->MyNPCPointer())) ? true
                                                                                                           : false;
    if (bIsUnblockable) {
...
void UTIL_TraceLineFilterEntity(CBaseEntity * pEntity,
const Vector &vecAbsStart,
const Vector &vecAbsEnd,
unsigned int mask,
const int nCollisionGroup, trace_t
*ptr );
