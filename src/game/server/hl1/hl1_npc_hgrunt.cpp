...
DEFINE_KEYFIELD( m_iWeapons, FIELD_INTEGER,
"weapons" ),
DEFINE_KEYFIELD( m_SquadName, FIELD_STRING,
"netname" ),
...
void CNPC_HGrunt::PrescheduleThink(void) {
    ...
    if (m_pSquad && GetEnemy() != NULL) {
        ...
        CNPC_HGrunt * pSquadLeader = (CNPC_HGrunt * )
        m_pSquad->GetLeader()->MyNPCPointer();
...
int CNPC_HGrunt::SquadRecruit(int searchRadius, int maxMembers) {
    ...
    if (m_SquadName != NULL_STRING) {
        ...
        while (pEntity) {
            CNPC_HGrunt * pRecruit = (CNPC_HGrunt * )
...
void CNPC_HGrunt::HandleAnimEvent(animevent_t *pEvent) {
    ...
    switch (pEvent->event) {
        ...
        case HGRUNT_AE_GREN_TOSS: {
            CHandGrenade *pGrenade = (CHandGrenade *) Create("grenade_hand", GetAbsOrigin() + Vector(0, 0, 60),
                                                             vec3_angle);
            if (pGrenade) {
