...
SendPropInt( SENDINFO(m_bHasLongJump),
1, SPROP_UNSIGNED ),
SendPropInt( SENDINFO(m_nFlashBattery),
8, SPROP_UNSIGNED ),
...
void CHL1_Player::StartPullingObject(CBaseEntity *pObject) {
    ...
    m_pPullConstraint = physenv->CreateBallsocketConstraint(VPhysicsGetObject(), pObject->VPhysicsGetObject(), NULL,
                                                            ballsocket);
...
int CHL1_Player::OnTakeDamage(const CTakeDamageInfo &inputInfo) {
    ...
    // Armor.
    if (ArmorValue() &&
        !(info.GetDamageType() & (DMG_FALL | DMG_DROWN | DMG_POISON)) &&
...
public:
    ...
    void
    AttachEntity(CBasePlayer *pPlayer, CBaseEntity *pEntity, IPhysicsObject *pPhys, bool bIsMegaPhysCannon = false);
...
const
float DEFAULT_MAX_ANGULAR = 360.0f * 10.0f;
