...
DEFINE_KEYFIELD( m_iszReachTarget, FIELD_STRING,
"reachtarget" ),
DEFINE_KEYFIELD( m_iszReachSequence, FIELD_STRING,
"reachsequence" ),
DEFINE_KEYFIELD( m_iszPreSequence, FIELD_STRING,
"presequence" ),
DEFINE_OUTPUT( m_OnAnimationEvent,
"OnAnimationEvent" ),
...
DEFINE_KEYFIELD( m_iszNetName, FIELD_STRING,
"netname" ),
...
//						pHurt->pev->velocity = pHurt->pev->velocity + (vecFwd * 150) + Vector(0,0,250) - (vecRight * 200);
                        pHurt->SetAbsVelocity(
                                pHurt->GetAbsVelocity() + (vecFwd * 150) + Vector(0, 0, 250) - (vecRight * 200));
                        break;
        ...
        case BIG_AE_JUMP_FORWARD:
            ...
            SetAbsOrigin(
                    GetAbsOrigin() + Vector(0, 0, 1));// take him off ground so engine doesn't instantly reset onground
...
int CNPC_BigMomma::RangeAttack1Conditions(float flDot, float flDist) {
    ...
    if (flDist <= BIG_MORTARDIST && m_mortarTime < gpGlobals->curtime) {
        ...
        if (pEnemy) {
            ...
            m_vTossDir = VecCheckSplatToss(this, startPos, pEnemy->BodyTarget(GetAbsOrigin()),
