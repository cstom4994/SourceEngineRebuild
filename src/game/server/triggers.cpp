...
private:
    ...
    static int
    AddTransitionToList(levellist_t *pLevelList, int listCount, const char *pMapName, const char *pLandmarkName,
    ...
    static int
    BuildEntityTransitionList(CBaseEntity *pLandmarkEntity, const char *pLandmarkName, CBaseEntity **ppEntList,
                              int *pEntityFlags, int nMaxList);

    // Adds a single entity to the transition list, if appropriate. Returns the new count
    static int
    AddEntityToTransitionList(CBaseEntity *pEntity, int flags, int nCount, CBaseEntity **ppEntList, int *pEntityFlags);
...
void CChangeLevel::NotifyEntitiesOutOfTransition() {
    ...
    while (pEnt) {
        ...
        if (pEnt->ObjectCaps() & FCAP_NOTIFY_ON_TRANSITION) {
            ...
            if (!(pEnt->ObjectCaps() & (FCAP_ACROSS_TRANSITION | FCAP_FORCE_TRANSITION)) ||
...
////	Msg( "Level touches %d levels\n", ChangeList( levels, 16 ) );
    ...
    if (g_debug_transitions.GetInt() == 0) {
        ...
        if (pSaveData) {
            ...
            pSaveData->levelInfo.connectionCount = BuildChangeList(pSaveData->levelInfo.levelList,
...
int CChangeLevel::ChangeList(levellist_t *pLevelList, int maxList) {
    ...
    for (i = 0; i < count; i++) {
        ...
        int iEntity = BuildEntityTransitionList(pLandmarkEntity, pLevelList[i].landmarkName, pEntList, entityFlags,
                                                MAX_ENTITY);
...
void CTriggerCamera::Spawn(void) {
    ...
    SetRenderColorA(
            0);                                // The engine won't draw this model if this is set to 0 and blending is on
...
CTriggerVPhysicsMotion::Simulate(IPhysicsMotionController *pController, IPhysicsObject *pObject, float deltaTime,
