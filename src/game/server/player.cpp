...
ConVar spec_freeze_traveltime("spec_freeze_traveltime", "0.4", FCVAR_REPLICATED,
...
ConVar sv_noclipduringpause("sv_noclipduringpause", "0", FCVAR_REPLICATED | FCVAR_CHEAT,
...
extern void respawn(CBaseEntity * pEdict, bool
fCopyCorpse);
...
ConVar sv_player_usercommand_timeout("sv_player_usercommand_timeout", "3", FCVAR_CHEAT,
                                     "After this many seconds without a usercommand from a player, the server will RunNullCommand as if client sends an empty command.");
#ifdef _DEBUG
...
#endif // _DEBUG
ConVar sv_player_display_usercommand_errors("sv_player_display_usercommand_errors", "0", FCVAR_CHEAT,
...
int CBasePlayer::OnTakeDamage(const CTakeDamageInfo &inputInfo) {
...
#endif
    {
        ...
        Q_snprintf(outputString, 256, "%f: Player %s at [%0.2f %0.2f %0.2f] took %f damage from %s, type %s\n",
                   ...
                   GetAbsOrigin().x, GetAbsOrigin().y, GetAbsOrigin().z, info.GetDamage(),
...
static ConVar sv_clockcorrection_msecs("sv_clockcorrection_msecs", "30", 0,
                                       "The server tries to keep each player's m_nTickBase withing this many msecs of the server absolute tickcount");
static ConVar sv_playerperfhistorycount("sv_playerperfhistorycount", "20", 0,
...
ConVar sv_usercmd_custom_random_seed("sv_usercmd_custom_random_seed", "1", FCVAR_CHEAT,
...
void CBasePlayer::DumpPerfToRecipient(CBasePlayer *pRecipient, int nMaxRecords) {
    ...
    for (int i = m_vecPlayerSimInfo.Tail();
         i != m_vecPlayerSimInfo.InvalidIndex(); i = m_vecPlayerSimInfo.Previous(i)) {
    ...
    for (int i = m_vecPlayerCmdInfo.Tail();
         i != m_vecPlayerCmdInfo.InvalidIndex(); i = m_vecPlayerCmdInfo.Previous(i)) {
...
void FixPlayerCrouchStuck(CBasePlayer *pPlayer) {
    ...
    for (i = 0; i < 18; i++) {
        UTIL_TraceHull(pPlayer->GetAbsOrigin(), pPlayer->GetAbsOrigin(),
                       VEC_DUCK_HULL_MIN, VEC_DUCK_HULL_MAX, MASK_PLAYERSOLID, pPlayer, COLLISION_GROUP_PLAYER_MOVEMENT,
                       &trace);
        if (trace.startsolid) {
        ...
        if (pSpot) {
            ...
            for (CEntitySphereQuery sphere(pSpot->GetAbsOrigin(), 128);
                ...
                if (ent->IsPlayer() && !(ent->edict() == player))
                    ent->TakeDamage(
                            CTakeDamageInfo(GetContainingEntity(INDEXENT(0)), GetContainingEntity(INDEXENT(0)), 300,
                                            DMG_GENERIC));
            }
            goto ReturnSpot;
        }
...
IMPLEMENT_SERVERCLASS_ST(CBasePlayer, DT_BasePlayer)
                    ...
                    SendPropDataTable("localdata", 0, &REFERENCE_SEND_TABLE(DT_LocalPlayerExclusive),
...
void
CBasePlayer::SetupVPhysicsShadow(const Vector &vecAbsOrigin, const Vector &vecAbsVelocity, CPhysCollide *pStandModel,
    ...
    m_pShadowStand = PhysModelCreateCustom(this, pStandModel, GetLocalOrigin(), GetLocalAngles(), pStandHullName, false,
                                           &solid);
    m_pShadowStand->SetCallbackFlags(CALLBACK_GLOBAL_COLLISION | CALLBACK_SHADOW_COLLISION);

    // create crouchig hull
    m_pShadowCrouch = PhysModelCreateCustom(this, pCrouchModel, GetLocalOrigin(), GetLocalAngles(), pCrouchHullName,
                                            false, &solid);
...
void CBasePlayer::InputSetFogController(inputdata_t &inputdata) {
    ...
    CFogController *pFogController = dynamic_cast<CFogController *>( gEntList.FindEntityByName(NULL,
