...
//		m_fireLast = gpGlobals->curtime - (1/m_fireRate) - 0.01;  // to make sure the gun doesn't fire too many bullets
    ...
    if ((pNPC->CapabilitiesGet() & bits_CAP_NO_HIT_SQUADMATES) && pNPC->IsInSquad()) {
        ...
        if (pNPC->IsSquadmateInSpread(vecBarrelEnd, vecBarrelEnd + vecForward * 2048, gTankSpread[m_spread].x,
                                      8 * 12)) {
            return;
        }
    }
...
public:
    ...
    void
    Fire(int bulletCount, const Vector &barrelEnd, const Vector &forward, CBaseEntity *pAttacker, bool bIgnoreSpread);
};
...
public:
    ...
    static CMortarShell *
    Create(const Vector &vecStart, const Vector &vecTarget, const Vector &vecShotDir, float flImpactDelay,
...
CMortarShell *
CMortarShell::Create(const Vector &vecStart, const Vector &vecTarget, const Vector &vecShotDir, float flImpactDelay,
                     float flWarnDelay, string_t warnSound) {
...
//---------------------------------------------------------
void
CFuncTankCombineCannon::Fire(int bulletCount, const Vector &barrelEnd, const Vector &forward, CBaseEntity *pAttacker,
                             bool bIgnoreSpread) {
