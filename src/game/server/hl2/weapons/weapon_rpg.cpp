...
CMissile *CMissile::Create(const Vector &vecOrigin, const QAngle &vecAngles, edict_t *pentOwner = NULL) {
    ...
    CMissile *pMissile = (CMissile *) CBaseEntity::Create("rpg_missile", vecOrigin, vecAngles,
...
CAPCMissile *
CAPCMissile::Create(const Vector &vecOrigin, const QAngle &vecAngles, const Vector &vecVelocity, CBaseEntity *pOwner) {
    ...
    if (flBlendTime > DOWNWARD_BLEND_TIME_START) {
        if (m_flReachedTargetTime != 0.0f) {
            ...
            *pHomingSpeed = SimpleSplineRemapVal(flDeltaTime, 0.0f, CORRECTION_TIME, 0.2f, *pHomingSpeed);
            flMinHomingDistance = SimpleSplineRemapVal(flDeltaTime, 0.0f, CORRECTION_TIME, MIN_NEAR_HOMING_DISTANCE,
                                                       flMinHomingDistance);
            flMaxHomingDistance = SimpleSplineRemapVal(flDeltaTime, 0.0f, CORRECTION_TIME, MAX_NEAR_HOMING_DISTANCE,
                                                       flMaxHomingDistance);
        } else {
            ...
            if (vecDelta.z > MIN_HEIGHT_DIFFERENCE) {
                ...
                float flHeightAdjustFactor = SimpleSplineRemapVal(flClampedHeight, MIN_HEIGHT_DIFFERENCE,
        ...
        if (flBlendTime > 0.6f) {
            ...
            *pHomingSpeed = SimpleSplineRemapVal(flTargetLength, flMaxHomingDistance, flMinHomingDistance,
                                                 *pHomingSpeed, 0.01f);
        }
    }
...
void EnableLaserDot(CBaseEntity * pLaserDot, bool
bEnable )
{
CLaserDot *pDot = assert_cast< CLaserDot * >(pLaserDot );
if ( bEnable )
