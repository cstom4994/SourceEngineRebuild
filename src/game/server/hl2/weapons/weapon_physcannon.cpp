...
static void
TraceCollideAgainstBBox(const CPhysCollide *pCollide, const Vector &start, const Vector &end, const QAngle &angles,
                        const Vector &boxOrigin, const Vector &mins, const Vector &maxs, trace_t *ptr) {
...
public:
    ...
    Simulate(IPhysicsMotionController *pController, IPhysicsObject *pObject, float deltaTime, Vector &linear,
...
void
CGrabController::AttachEntity(CBasePlayer *pPlayer, CBaseEntity *pEntity, IPhysicsObject *pPhys, bool bIsMegaPhysCannon,
                              const Vector &vGrabPosition, bool bUseGrabPosition) {
...
IMotionEvent::simresult_e
CGrabController::Simulate(IPhysicsMotionController *pController, IPhysicsObject *pObject, float deltaTime,
...
void PlayerPickupObject(CBasePlayer *pPlayer, CBaseEntity *pObject) {
    ...
    CPlayerPickupController *pController = (CPlayerPickupController *) CBaseEntity::Create("player_pickup",
                                                                                           pObject->GetAbsOrigin(),
                                                                                           vec3_angle, pPlayer);
...
protected:
    ...
    CBaseEntity *
    MegaPhysCannonFindObjectInCone(const Vector &vecOrigin, const Vector &vecDir, float flCone, float flCombineBallCone,
    ...
    if (pEntity->GetMoveType() != MOVETYPE_VPHYSICS) {
        ...
        if (IsMegaPhysCannon()) {
            if (pEntity->IsNPC() && !pEntity->IsEFlagSet(EFL_NO_MEGAPHYSCANNON_RAGDOLL) &&
                ...
                CBaseEntity * pRagdoll = CreateServerRagdoll(pEntity->MyNPCPointer(), 0, info,
...
CWeaponPhysCannon::FindObjectResult_t CWeaponPhysCannon::FindObject(void) {
    ...
    if (!IsMegaPhysCannon()) {
        ...
        pConeEntity = MegaPhysCannonFindObjectInCone(start, forward,
                                                     physcannon_cone.GetFloat(), physcannon_ball_cone.GetFloat(),
                                                     bAttach || bPull);
    }
...
CBaseEntity *CWeaponPhysCannon::MegaPhysCannonFindObjectInCone(const Vector &vecOrigin,
                                                               const Vector &vecDir, float flCone,
...
bool CGrabController::UpdateObject(CBasePlayer *pPlayer, float flError) {
    CBaseEntity * pEntity = GetAttached();
    if (!pEntity || ComputeError() > flError || pPlayer->GetGroundEntity() == pEntity ||
    ...
    Vector radial = physcollision->CollideGetExtent(pPhys->GetCollide(), vec3_origin, pEntity->GetAbsAngles(),
                                                    -forward);
...
void CWeaponPhysCannon::DoEffectIdle(void) {
    ...
    if (m_bPhyscannonState != IsMegaPhysCannon()) {
        ...
        if (m_bPhyscannonState == false) {
            ...
            CCitadelEnergyCore *pCore = static_cast<CCitadelEnergyCore *>( CreateEntityByName(
...
CSoundPatch *CWeaponPhysCannon::GetMotorSound(void) {
    if (m_sndMotor == NULL) {
        ...
        if (IsMegaPhysCannon()) {
            m_sndMotor = (CSoundEnvelopeController::GetController()).SoundCreate(filter, entindex(), CHAN_STATIC,
