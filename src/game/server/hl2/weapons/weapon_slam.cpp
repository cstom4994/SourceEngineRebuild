...
void CWeapon_SLAM::SlamTouch(CBaseEntity *pOther) {
    ...
    if (pOther->GetFlags() & FL_CLIENT) {
        ...
        // ------------------------------------------------
        CWeapon_SLAM * oldWeapon = (CWeapon_SLAM * )
        pBCC->Weapon_OwnsThisType(GetClassname());
        if (oldWeapon != this) {
            UTIL_Remove(this);
        } else {
...
void CWeapon_SLAM::SatchelAttach(void) {
    ...
    if (tr.fraction < 1.0) {
        ...
        if (pEntity && !(pEntity->GetFlags() & FL_CONVEYOR)) {
            ...
            CSatchelCharge *pSatchel = (CSatchelCharge *) CBaseEntity::Create("npc_satchel",
                                                                              tr.endpos + tr.plane.normal * 3, angles,
                                                                              NULL);
