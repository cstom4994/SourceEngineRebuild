...
float
PixelVisibility_DrawProxy(IMatRenderContext *pRenderContext, OcclusionQueryObjectHandle_t queryHandle, Vector origin,
                          float scale, float proxyAspect, IMaterial *pMaterial, bool screenspace) {
    Vector point;
...
private:
    ...
    friend void PixelVisibility_ShiftVisibilityViews(int iSourceViewID,
                                                     int iDestViewID); //need direct access to private data to make shifting smooth
};
...
CPixelVisSet *
CPixelVisibilitySystem::FindOrCreatePixelVisSet(const pixelvis_queryparams_t &params, pixelvis_handle_t *queryHandle) {
    if (queryHandle[0]) {
...
public:
    ...
    CTraceFilterGlow(const IHandleEntity *passentity, int collisionGroup) : CTraceFilterSimple(passentity,
                                                                                               collisionGroup) {}
