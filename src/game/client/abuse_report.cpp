...
void CAbuseReportManager::DestroyIncidentData() {
    ...
    if (g_pFullFileSystem->FileExists(
            k_rchScreenShotFilename)) // !KLUDGE! To prevent warning if the file doesn't exist!
    {
...
CON_COMMAND_F(abuse_report_queue,
              "Capture data for abuse report and queue for submission.  Use abose_report_submit to activate UI to submit the report",
              FCVAR_DONTRECORD) {
    if (!g_AbuseReportMgr) {
