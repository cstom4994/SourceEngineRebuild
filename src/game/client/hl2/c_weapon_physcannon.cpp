...
int C_WeaponPhysCannon::DrawModel(int flags) {
    ...
    // Do the core effects
    for (int i = 0; i < 4; i++) {
        sParticle = (SimpleParticle *) m_pLocalEmitter->AddParticle(sizeof(SimpleParticle),
                                                                    m_pLocalEmitter->GetPMaterial(
                                                                            "effects/strider_muzzle"), vec3_origin);
    ...
    if (m_bWasUpgraded && m_bIsCurrentlyUpgrading) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            sParticle = (SimpleParticle *) m_pAttractor->AddParticle(sizeof(SimpleParticle), m_pAttractor->GetPMaterial(
