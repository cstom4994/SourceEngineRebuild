...
void CHudCredits::DrawIntroCreditsName(void) {
    ...
    for (int i = 0; i < m_CreditsList.Count(); i++) {
        ...
        surface()->DrawSetTextColor(m_cColor[0], m_cColor[1], m_cColor[2],
                                    FadeBlend(m_flFadeInTime, m_flFadeOutTime, m_flFadeHoldTime + pCredit->flTimeAdd,
                                              localTime) * m_cColor[3]);
            ...
            if (m_CreditsList.IsValidIndex(i + 3)) {
                ...
                if (pNextCredits && pNextCredits->flTime == 0.0f) {
                    ...
                    if (i < 3) {
                        ...
                        pNextCredits->flTime =
                                gpGlobals->curtime + m_flFadeInTime + m_flFadeOutTime + m_flFadeHoldTime +
