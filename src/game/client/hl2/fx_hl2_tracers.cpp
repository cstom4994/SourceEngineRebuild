...
void FX_PlayerAR2Tracer(const Vector &start, const Vector &end) {
    ...
    //Create the line
    CFXStaticLine *tracerLine = new CFXStaticLine("Tracer", dStart, dEnd, random->RandomFloat(6.0f, 12.0f), 0.01f,
...
void MuzzleFlash_Airboat(ClientEntityHandle_t hEntity, int attachmentIndex) {
    ...
    for (int i = 1; i < 7; i++) {
        ...
        pParticle = (SimpleParticle *) pSimple->AddParticle(sizeof(SimpleParticle), pMuzzle[random->RandomInt(0, 1)],
                                                            offset);
...
void MuzzleFlash_Chopper(ClientEntityHandle_t hEntity, int attachmentIndex) {
    ...
    for (int i = 1; i < 7; i++) {
        ...
        pParticle = (SimpleParticle *) pSimple->AddParticle(sizeof(SimpleParticle), pSimple->GetPMaterial(
                VarArgs("effects/combinemuzzle%d", random->RandomInt(1, 2))), offset);
