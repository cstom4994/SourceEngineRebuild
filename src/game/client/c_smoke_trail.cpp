...
void C_RocketTrail::Update(float fTimeDelta) {
    ...
    for (i = 1; i < 9; i++) {
        ...
        pParticle = (SimpleParticle *) pSimple->AddParticle(sizeof(SimpleParticle), pSimple->GetPMaterial(
                VarArgs("effects/muzzleflash%d", random->RandomInt(1, 4))), offset);
    ...
    if (m_bEmit) {
        ...
        for (i = 1; i < numPuffs + 1; i++) {
            ...
            pParticle = (SimpleParticle *) m_pRocketEmitter->AddParticle(sizeof(SimpleParticle),
                                                                         m_MaterialHandle[random->RandomInt(0, 1)],
                                                                         offset);
...
Vector SporeEffect::UpdateColor(const SimpleParticle *pParticle) {
    Vector color;
    float ramp = ((float) pParticle->m_uchStartAlpha / 255.0f) * sin(M_PI * (pParticle->m_flLifetime /
...
void C_SporeExplosion::AddParticles(void) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        sParticle = (SimpleParticle *) m_pSporeEffect->AddParticle(sizeof(SimpleParticle), m_hMaterial,
                                                                   GetAbsOrigin() + offset);
...
void C_SporeTrail::AddParticles(void) {
    ...
    //Make a new particle
    SimpleParticle *sParticle = (SimpleParticle *) m_ParticleEffect.AddParticle(sizeof(SimpleParticle),
...
#define    MAX_SPEED            64
    ...
    //if ( ShouldEmit() )
    {
        ...
        for (int i = 1; i < numPuffs + 1; i++) {
            ...
            pParticle = (SimpleParticle *) m_pSmokeEmitter->AddParticle(sizeof(SimpleParticle),
                                                                        m_hMaterial[random->RandomInt(FTRAIL_FLAME1,
                                                                                                      FTRAIL_FLAME5)],
                                                                        offset);
        ...
        pParticle = (SimpleParticle *) m_pSmokeEmitter->AddParticle(sizeof(SimpleParticle),
                                                                    m_hMaterial[random->RandomInt(FTRAIL_SMOKE1,
                                                                                                  FTRAIL_SMOKE2)],
                                                                    offset);
        ...
        if (pParticle != NULL) {
            ...
            pParticle->m_flDieTime = (PARTICLE_LIFETIME * 10.0f) +
                                     random->RandomFloat(PARTICLE_LIFETIME * 0.75f, PARTICLE_LIFETIME * 1.25f);
