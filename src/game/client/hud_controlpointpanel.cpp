...
// Purpose: 
//-----------------------------------------------------------------------------
CHudCapturePanelProgressBar::CHudCapturePanelProgressBar(vgui::Panel *parent, const char *name) : vgui::ImagePanel(
        parent, name) {
    m_flPercent = 0.0f;
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CHudCapturePanel::CHudCapturePanel(const char *pElementName) : CHudElement(pElementName),
                                                               BaseClass(NULL, "HudCapturePanel") {
...
void CHudCapturePanel::OnThink() {
    ...
    if (!m_bInitializedFlags) {
        ...
        for (int i = 0; i < GetNumberOfTeams(); i++) {
            ...
            m_pTeamFlags[i] = dynamic_cast< vgui::ImagePanel * >(FindChildByName(
                    VarArgs("CapturePanelTeamFlag_%d", i)));
        }
        InvalidateLayout();
    }
    ...
    if (pPlayer) {
        ...
        if (iCappingTeam == TEAM_UNASSIGNED || iCappingTeam != nPlayerTeam) {
            ...
            if (m_pMessage) {
                ...
                } else if (!TeamplayGameRules()->PlayerMayCapturePoint(pPlayer, m_iCurrentCP, szReason,
                                                                       sizeof(szReason))) {
