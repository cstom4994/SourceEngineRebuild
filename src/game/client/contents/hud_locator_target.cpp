...
const char *CLocatorTarget::UseBindingImage(char *pchIconTextureName, size_t bufSize) {
    ...
    if (Q_strcmp(pchBinding, "MOUSE1") == 0) {
        ...
        return NULL;
               ...
               Q_strcmp(pchBinding, "F10") == 0 || Q_strcmp(pchBinding, "F11") == 0 ||
...
void CLocatorPanel::AnimateIconAlpha(int flags, int *alpha, float fadeStart) {
    ...
    if (flags & LOCATOR_ICON_FX_FADE_OUT) {
        flScale *=
                MAX(0.0f,
                    (locator_fade_time.GetFloat() - (gpGlobals->curtime - fadeStart)) / locator_fade_time.GetFloat());
...
void CLocatorPanel::DrawDynamicIcon(CLocatorTarget *pTarget, bool bDrawCaption, bool bDrawSimpleArrow) {
    ...
    bool bShouldDrawCaption =
            ((pTarget->GetIconEffectsFlags() & LOCATOR_ICON_FX_FORCE_CAPTION) ||
             (!pTarget->m_bOccluded && pTarget->m_distFromPlayer <= ICON_DIST_TOO_FAR) ||
             locator_topdown_style.GetBool());
...
//-----------------------------------------------------------------------------
void
CLocatorPanel::DrawBindingName(CLocatorTarget *pTarget, const char *pchBindingName, int x, int y, bool bController) {
