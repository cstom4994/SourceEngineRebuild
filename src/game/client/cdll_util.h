...
void UTIL_MakeSafeName(const char *oldName, OUT_Z_CAP(newNameBufSize) char *newName,
                       int newNameBufSize);    ///< Cleans up player names for putting in vgui controls (cleaned names can be up to original*2+1 in length)
...
void
UTIL_SetTrace(trace_t &tr, const Ray_t &ray, C_BaseEntity *edict, float fraction, int hitgroup, unsigned int contents,
