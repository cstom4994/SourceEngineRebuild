...
static ConVar mat_postprocessing_combine("mat_postprocessing_combine", "1", FCVAR_NONE,
    ...
    if (mat_tonemap_algorithm.GetInt() == 1) // New algorithm
    {
        ...
        for (int i = N_LUMINANCE_RANGES_NEW - 2; i >= 0; i--) // Start at the bright end
        {
            ...
            if (flThisBinPercentOfTotalPixels >= flPixelPercentNeeded) // We found the bin needed
            {
                ...
                flPercentLocationOfBorder = MAX(CurHistogram[i].m_min_lum, MIN(CurHistogram[i].m_max_lum,
    ...
    if (mat_tonemap_algorithm.GetInt() == 1) // New algorithm
    {
        ...
        if (bGetIdealTargetForDebugMode == true)
            flPercentLocationOfTarget = FindLocationOfPercentBrightPixels(
                    mat_tonemap_percent_bright_pixels.GetFloat()); // Don't pass in the second arg so the scalar doesn't snap to a bin
        else
...
void CLuminanceHistogramSystem::DisplayHistogram(void) {
    ...
    if (bDrawTextThisFrame == true) {
        ...
        engine->Con_NPrintf(21,
                            ...
                            MAX(0.0f, FindLocationOfPercentBrightPixels(50.0f)) * 100.0f,
                            mat_tonemap_min_avglum.GetFloat(),
    ...
    if (mat_tonemap_algorithm.GetInt() == 1) // New algorithm only
    {
        float flYellowTargetPixelStart = (xpStart +
                                          (float(nTotalGraphPixelsWide) * mat_tonemap_percent_target.GetFloat() /
                                           100.0f));
        float flYellowAveragePixelStart = (xpStart +
                                           (float(nTotalGraphPixelsWide) * mat_tonemap_min_avglum.GetFloat() / 100.0f));

        float flTargetPixelStart = (xpStart + (float(nTotalGraphPixelsWide) * FindLocationOfPercentBrightPixels(
                mat_tonemap_percent_bright_pixels.GetFloat(), mat_tonemap_percent_target.GetFloat())));
        float flAveragePixelStart = (xpStart +
                                     (float(nTotalGraphPixelsWide) * FindLocationOfPercentBrightPixels(50.0f)));
    ...
    if (1) {
        ...
        pRenderContext->Viewport(flBarStart + (flBarWidth *
                                               ((pRenderContext->GetToneMappingScaleLinear().x - flAutoExposureMin) /
                                                (flAutoExposureMax - flAutoExposureMin))),
...
static ConVar mat_software_aa_strength("mat_software_aa_strength", "-1.0", FCVAR_ARCHIVE,
...
static ConVar mat_software_aa_edge_threshold("mat_software_aa_edge_threshold", "1.0", FCVAR_ARCHIVE,
...
static ConVar mat_software_aa_blur_one_pixel_lines("mat_software_aa_blur_one_pixel_lines", "0.5", FCVAR_ARCHIVE,
                                                   "How much software AA should blur one-pixel thick lines: (0.0 - none), (1.0 - lots)");
static ConVar mat_software_aa_tap_offset("mat_software_aa_tap_offset", "1.0", FCVAR_ARCHIVE,
...
static ConVar mat_software_aa_debug("mat_software_aa_debug", "0", FCVAR_NONE,
...
static ConVar mat_software_aa_strength_vgui("mat_software_aa_strength_vgui", "-1.0", FCVAR_ARCHIVE,
...
public:
    static IMaterial *SetupEnginePostMaterial(const Vector4D &fullViewportBloomUVs, const Vector4D &fullViewportFBUVs,
                                              ...
                                              bool bPerformSoftwareAA, bool bPerformBloom, bool bPerformColCorrect,
                                              float flAAStrength);

    static void SetupEnginePostMaterialAA(bool bPerformSoftwareAA, float flAAStrength);

    static void
    SetupEnginePostMaterialTextureTransform(const Vector4D &fullViewportBloomUVs, const Vector4D &fullViewportFBUVs,
...
IMaterial *CEnginePostMaterialProxy::SetupEnginePostMaterial(const Vector4D &fullViewportBloomUVs,
                                                             const Vector4D &fullViewportFBUVs,
                                                             const Vector2D &destTexSize,
                                                             bool bPerformSoftwareAA, bool bPerformBloom,
                ...
                if (bDrawTextThisFrame == true) {
                    if (mat_tonemap_algorithm.GetInt() == 0) {
                        ...
                        engine->Con_NPrintf(19,
                                            ...
                                            g_HDR_HistogramSystem.GetTargetTonemapScalar(true), flAutoExposureMin,
                                            flAutoExposureMax, mat_hdr_tonemapscale.GetFloat(),
                                            pRenderContext->GetToneMappingScaleLinear().x);
                    }
                }
            }
                ...
                if (!bFirstFrame || !bPerformColCorrect) {
                    ...
                    if (mat_postprocessing_combine.GetInt()) {
                        ...
                        IMaterial *post_mat = CEnginePostMaterialProxy::SetupEnginePostMaterial(
                                ...
                                bPerformSoftwareAA, bPerformBloom, bPerformColCorrect, flAAStrength);
                    ...
                    if (bVisionOverride && g_pMaterialSystemHardwareConfig->SupportsPixelShaders_2_0() &&
                        ...
                        IMaterial *pPyroVisionPostMaterial = materials->FindMaterial("dev/pyro_post",
                                                                                     TEXTURE_GROUP_OTHER, true);
...
void DoImageSpaceMotionBlur(const CViewSetup &viewBlur, int x, int y, int w, int h) {
    ...
    if (true) {
        ...
        if ((VectorLength(vPositionChange) > 30.0f) && (flTimeElapsed >= 0.5f)) {
            ...
            float flVerticalFov = (viewBlur.m_flAspectRatio <= 0.0f) ? (viewBlur.fov) : (viewBlur.fov /
                                                                                         viewBlur.m_flAspectRatio);
            ...
            g_vMotionBlurValues[0] = flUndampenedYaw * (1.0f - (fabs(flCurrentPitch) /
            ...
            if (flCurrentPitch > 0.0f)
                flPitchDiffAdjusted = flPitchDiffOriginal - ((flViewDotMotion / 2.0f) *
            ...
            g_vMotionBlurValues[2] = clamp((fabs(g_vMotionBlurValues[2]) - flMotionBlurFallingMin) /
                                           (flMotionBlurFallingMax - flMotionBlurFallingMin), 0.0f, 1.0f) *
