...
public:
    ...
    // its ID later.
    virtual int
    DrawString(vgui::HFont pCustomFont, int x, int y, int r, int g, int b, int a, const char *fmt, int messageID,
               ...) = 0;

    virtual int DrawString(vgui::HFont pCustomFont, int x, int y, const char *fmt, int messageID, ...) = 0;

    virtual int
    DrawStringForTime(float flTime, vgui::HFont pCustomFont, int x, int y, int r, int g, int b, int a, const char *fmt,
                      int messageID, ...) = 0;

    virtual int
    DrawStringForTime(float flTime, vgui::HFont pCustomFont, int x, int y, const char *fmt, int messageID, ...) = 0;

    // Remove all messages with the specified ID (passed into DrawStringForTime).
    virtual void RemoveStringsByID(int messageID) = 0;

    virtual void
    GetStringLength(vgui::HFont pCustomFont, int *width, int *height, PRINTF_FORMAT_STRING const char *fmt, ...) = 0;
