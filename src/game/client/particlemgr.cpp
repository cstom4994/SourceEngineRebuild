...
static ConVar particle_simulateoverflow("particle_simulateoverflow", "0", FCVAR_CHEAT,
...
ConVar cl_particle_show_bbox_cost("cl_particle_show_bbox_cost", "0", FCVAR_CHEAT,
...
static ConCommand cl_particle_stats_stop("cl_particle_stats_stop", StatsParticlesStop,
...
inline void CParticleEffectBinding::StartDrawMaterialParticles(
    ...
    if (bWireframe) {
        IMaterial *pMaterialWire = m_pParticleMgr->m_pMaterialSystem->FindMaterial("debug/debugparticlewireframe",
                                                                                   TEXTURE_GROUP_OTHER);
...
void
CParticleEffectBinding::DoBucketSort(CEffectMaterial *pMaterial, float *zCoords, int nZCoords, float minZ, float maxZ) {
...
int CParticleMgr::ComputeParticleDefScreenArea(int nInfoCount, RetireInfo_t *pInfo, float *pTotalArea,
                                               ...
                                               const CViewSetup &viewParticle, const VMatrix &worldToPixels,
                                               float flFocalDist) {
    int nCollection = 0;
    ...
    *pTotalArea = 0.0f;
    ...
    for (; pCollection; pCollection = pCollection->GetNextCollectionUsingSameDef()) {
        ...
        float flProjRadius = (lSqr > flCullRadiusSqr) ? 0.5f * flFocalDist * flCullRadius /
                                                        sqrtf(lSqr - flCullRadiusSqr) : 1.0f;
...
bool CParticleMgr::RetireParticleCollections(CParticleSystemDefinition *pDef,
                                             int nCount, RetireInfo_t *pInfo, float flScreenArea,
...
bool CParticleMgr::EarlyRetireParticleSystems(int nCount, ParticleSimListEntry_t *ppEffects) {
    ...
    for (int i = 0; i < nDefCount; ++i) {
        ...
        int nActualCount = ComputeParticleDefScreenArea(nCount, pInfo, &flScreenArea, pDef, *pViewSetup, worldToScreen,
                                                        flFocalDist);
        bool bShouldRetire;
...
// #endif
    flTimeDelta *= r_particle_timescale.GetFloat();VPROF_BUDGET("CParticleMSG::UpdateNewEffects",
                                                                "Particle Simulation");
    ...
    if ((nParticleStatsTriggerCount > 0 && (nParticleActiveParticlesCount >= nParticleStatsTriggerCount)) ||
        bFrameWarningNeeded) {
        if (m_nStatsFramesSinceLastAlert >= (300 *
                                             3))        // 3 seconds at the clamp of 300 fps (or 15 secs at 60fps). Just stop it spamming too much.
        {
            m_nStatsFramesSinceLastAlert = 0;
            if (m_bStatsRunning) {
...
struct ParticleInfo_t {
    ParticleInfo_t() : m_nCount(0), m_nChildCount(0), m_nTotalActiveParticles(0), m_nTotalDrawnParticles(0),
                       m_nCountMax(0), m_nChildCountMax(0), m_nTotalActiveParticlesMax(0), m_nTotalDrawnParticlesMax(0),
                       pDef(NULL) {}
