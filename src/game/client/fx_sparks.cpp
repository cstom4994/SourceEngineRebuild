...
void
FX_Sparks(const Vector &pos, int nMagnitude, int nTrailLength, const Vector &vecDir, float flWidth, float flMinSpeed,
...
void FX_MicroExplosion(Vector &position, Vector &normal) {
    ...
    SimpleParticle *sParticle = (SimpleParticle *) pSimple->AddParticle(sizeof(SimpleParticle),
                                                                        pSimple->GetPMaterial("sprites/rico1"), offset);
...
void FX_Explosion(Vector &origin, Vector &normal, char materialType) {
    ...
    int i;
    for (i = 0; i < numSparks; i++) {
        TrailParticle *pParticle = (TrailParticle *) pSparkEmitter->AddParticle(sizeof(TrailParticle), g_Material_Spark,
                                                                                offset);
    ...
    if (materialType == 'C' || materialType == 'W') {
        ...
        for (i = 0; i < numFlecks; i++) {
            FleckParticle *pParticle = (FleckParticle *) fleckEmitter->AddParticle(sizeof(FleckParticle),
                                                                                   hMaterialArray[random->RandomInt(0,
                                                                                                                    1)],
                                                                                   offset);
            ...
            pParticle->m_vecVelocity = dir *
                                       (random->RandomFloat(EXPLOSION_FLECK_MIN_SPEED, EXPLOSION_FLECK_MAX_SPEED) *
    ...
    SimpleParticle *pSphereParticle = (SimpleParticle *) pSimpleEmitter->AddParticle(sizeof(SimpleParticle),
                                                                                     hSphereMaterial, vecBurstOrigin);
    if (pSphereParticle) {
    ...
    for (i = 0; i < numBalls; i++) {
        SimpleParticle *pParticle = (SimpleParticle *) pSimpleEmitter->AddParticle(sizeof(SimpleParticle),
                                                                                   hSphereMaterial, vecBurstOrigin);
        if (pParticle) {
            pParticle->m_flLifetime = 0.0f;
    ...
    for (i = 0; i < 2; i++) {
        SimpleParticle *pParticle = (SimpleParticle *) pSmokeEmitter->AddParticle(sizeof(SimpleParticle),
                                                                                  g_Mat_DustPuff[1], offset);
        if (pParticle == NULL)
            break;
...
void FX_ConcussiveExplosion(Vector &origin, Vector &normal) {
    ...
    SimpleParticle *pParticle = (SimpleParticle *) pSimple->AddParticle(sizeof(SimpleParticle),
                                                                        pSimple->GetPMaterial("effects/blueflare1"),
                                                                        offset);
