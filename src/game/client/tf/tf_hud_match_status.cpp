...
CRoundCounterPanel::CRoundCounterPanel(Panel *parent, const char *panelName)
        : BaseClass(parent, panelName), m_pRoundIndicatorKVs(NULL), m_pRoundWinIndicatorRedKV(NULL),
          m_pRoundWinIndicatorBlueKV(NULL), m_bCountDirty(false) {
...
void CRoundCounterPanel::FireGameEvent(IGameEvent *event) {
    if (FStrEq(event->GetName(), "winlimit_changed"))    // Resize if the win limit changes
    {
        m_bCountDirty = true;
               ...
               || FStrEq(event->GetName(),
                         "teamplay_round_start")) // Make sure we're accurate when the round starts in case the hud event didnt happen
    {
...
CTFHudMatchStatus::CTFHudMatchStatus(const char *pElementName)
        : CHudElement(pElementName), BaseClass(NULL, "HudMatchStatus"), m_pTimePanel(NULL), m_bUseMatchHUD(false),
...
void CTFHudMatchStatus::ShowRoundSign(int nRoundNumber) {
    ...
    int nSkin = 0;
    int nBodyGroup = 0;
    if (GetMatchGroupDescription(TFGameRules()->GetCurrentMatchGroup())->BGetRoundStartBannerParameters(nSkin,
                                                                                                        nBodyGroup)) {
...
void CTFHudMatchStatus::InitPlayerList(SectionedListPanel *pPlayerList, int nTeam) {
    ...
    int nExtraSpace = pPlayerList->GetWide() - m_iAvatarWidth - m_iSpacerWidth - m_iNameWidth - (2 *
                                                                                                 SectionedListPanel::COLUMN_DATA_INDENT); // the SectionedListPanel will indent the columns on either end by SectionedListPanel::COLUMN_DATA_INDENT
