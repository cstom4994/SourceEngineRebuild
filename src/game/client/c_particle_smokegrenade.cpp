...
private:
    ...
    inline SmokeParticleInfo *
    GetSmokeParticleInfo(int x, int y, int z) { return &m_SmokeParticleInfos[GetSmokeParticleIndex(x, y, z)]; }
...
private:
    ...
    SmokeParticleInfo m_SmokeParticleInfos[
...
void C_ParticleSmokeGrenade::Update(float fTimeDelta) {
    ...
    Vector vMins = m_SmokeBasePos -
                   Vector(m_SpacingRadius + SMOKEGRENADE_PARTICLERADIUS, m_SpacingRadius + SMOKEGRENADE_PARTICLERADIUS,
    ...
    if (m_CurrentStage == 1) {
        ...
        m_ExpandRadius =
                (m_SpacingRadius * 2) * (float) sin(m_ExpandTimeCounter * M_PI * 0.5 / SMOKESPHERE_EXPAND_TIME);
                    ...
                    */
                    {
                        SmokeGrenadeParticle *pParticle =
                                (SmokeGrenadeParticle *) m_ParticleEffect.AddParticle(sizeof(SmokeGrenadeParticle),
                                                                                      m_MaterialHandles[rand() %
                                                                                                        NUM_MATERIAL_HANDLES]);
