...
public:
    ...
    virtual void
    CreateBeamRingPoint(const Vector &center, float start_radius, float end_radius, int modelIndex, int haloIndex,
                        float haloScale,
...
private:
    ...
    void
    DrawBeamWithHalo(Beam_t *pbeam, int frame, int rendermode, float *color, float *srcColor, const model_t *sprite,
    ...
    void
    DrawLaser(Beam_t *pBeam, int frame, int rendermode, float *color, model_t const *sprite, model_t const *halosprite,
...
bool
ComputeBeamEntPosition(C_BaseEntity *pEnt, int nAttachment, bool bInterpretAttachmentIndexAsHitboxIndex, Vector &pt) {
...
void CViewRenderBeams::CreateBeamRingPoint(const Vector &center, float start_radius, float end_radius,
                                           ...
                                           float endWidth,
                                           float fadeLength, float amplitude, float brightness, float speed,
                                           int startFrame, float framerate,
...
//-----------------------------------------------------------------------------
void
CViewRenderBeams::CreateBeamRing(int startEnt, int endEnt, int modelIndex, int haloIndex, float haloScale, float life,
...
bool CViewRenderBeams::RecomputeBeamEndpoints(Beam_t *pbeam) {
    if (pbeam->flags & FBEAM_STARTENTITY) {
        if (ComputeBeamEntPosition(pbeam->entity[0], pbeam->attachmentIndex[0],
                                   (pbeam->flags & FBEAM_USE_HITBOXES) != 0, pbeam->attachment[0])) {
...
//			return false;
    ...
    if (pbeam->flags & FBEAM_ENDENTITY) {
        if (ComputeBeamEntPosition(pbeam->entity[1], pbeam->attachmentIndex[1],
                                   (pbeam->flags & FBEAM_USE_HITBOXES) != 0, pbeam->attachment[1])) {
