                ...
                if (iSelectedParam >= 0) {
                    ...
                    if (pKeyValues) {
                        ...
                        const char *pReasonString = m_pComboBox->GetActiveItemUserData()
                                                    ? m_pComboBox->GetActiveItemUserData()->GetName() : "other";
...
void CHudVote::MsgFunc_CallVoteFailed(bf_read &msg) {
    ...
    switch (nReason) {
        ...
        case VOTE_FAILED_RATE_EXCEEDED: {
            const char *pszTimeString = (bMinutes) ? ((nTime < 2) ? "#GameUI_vote_failed_vote_spam_min"
        ...
        case VOTE_FAILED_ON_COOLDOWN: {
            const char *pszTimeString = (bMinutes) ? ((nTime < 2) ? "#GameUI_vote_failed_recently_min"
...
void CHudVote::MsgFunc_VoteStart(bf_read &msg) {
    ...
    if (m_bIsYesNoVote) {
        ...
        wchar_t *pszText = g_pVGuiLocalize->Find(::input->IsSteamControllerActive() ? "#GameUI_vote_yes_sc_instruction"
                                                                                    : "#GameUI_vote_yes_pc_instruction");
        if (pszText) {
...
void CHudVote::MsgFunc_VoteSetup(bf_read &msg) {
    ...
    if (pStringTable) {
        int index = bMvM ? pStringTable->FindStringIndex("ServerMapCycleMvM") : pStringTable->FindStringIndex(
                "ServerMapCycle");
