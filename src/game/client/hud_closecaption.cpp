...
static ConVar cc_subtitles("cc_subtitles", "0", FCVAR_ARCHIVE | FCVAR_ARCHIVE_XBOX,
...
class CAsyncCaptionResourceManager
        : public CAutoGameSystem, public CManagedDataCacheClient<AsyncCaptionData_t, asynccaptionparams_t> {
public:
        ...
        if (SplitCommand(&curpos, cmd, args)) {
            if (!wcscmp(cmd, L"delay")) {
                ...
                *out = L'\0';
                ...
                if (wcslen(phrase) > 0) {
                    CCloseCaptionItem *item = new CCloseCaptionItem(phrase, lifespan, addedlife, delay, valid,
                                                                    fromplayer);
...
public:
    ...
    void
    OnDataArrived(CUtlVector<AsyncCaption_t> &directories, int nFileIndex, int nBlockNum, AsyncCaptionData_t *pData) {
        int c = m_Tokens.Count();
        for (int i = 0; i < c; ++i) {
            caption_t *caption = m_Tokens[i];
...
void
CHudCloseCaption::_ProcessSentenceCaptionStream(int wordCount, const char *tokenstream, const wchar_t *caption_full) {
...
static int
EmitCaptionCompletion(const char *partial, char commands[COMMAND_COMPLETION_MAXITEMS][COMMAND_COMPLETION_ITEM_LENGTH]) {
    int current = 0;
        ...
        for (int j = 0; j < dict.Count(); ++j) {
            ...
            if (Q_stristr(streamANSI, pchANSI)) {
                ...
                for (int k = soundemitterbase->First();
                     k != soundemitterbase->InvalidIndex(); k = soundemitterbase->Next(k)) {
                ...
                if (IsPC()) {
                    for (int r = g_pVGuiLocalize->GetFirstStringIndex();
                         r != INVALID_LOCALIZE_STRING_INDEX; r = g_pVGuiLocalize->GetNextStringIndex(r)) {
