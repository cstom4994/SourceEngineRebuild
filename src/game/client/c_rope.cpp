...
void CRopeManager::DrawRenderCache_NonQueued(bool bShadowDepth, RopeRenderData_t *pRenderCache, int nRenderCacheCount,
    ...
    if (bShadowDepth && !m_pDepthWriteMaterial && g_pMaterialSystem) {
        ...
        m_pDepthWriteMaterial = g_pMaterialSystem->FindProceduralMaterial("__DepthWrite01", TEXTURE_GROUP_OTHER,
                                                                          pVMTKeyValues);
...
void CRopeManager::DrawRenderCache(bool bShadowDepth) {
    ...
    if (r_queued_ropes.GetBool() && (pCallQueue = materials->GetRenderContext()->GetCallQueue()) != NULL) {
        ...
        C_RopeKeyframe::BuildRopeQueuedData_t *pBuildRopeQueuedDataStart = (C_RopeKeyframe::BuildRopeQueuedData_t *) (
                pRenderCachesStart + iRenderCacheCount);
...
void C_RopeKeyframe::ConstrainNodesBetweenEndpoints(void) {
    ...
    for (int i = 1; i < m_RopePhysics.NumNodes() - 1; ++i) {
        ...
        m_RopePhysics.GetNode(i)->m_vPos = ConstrainNode(vNormal, m_RopePhysics.GetNode(i)->m_vPos, vMidpiont,
                                                         fNormalLength);

        // Fix the predicted position
        m_RopePhysics.GetNode(i)->m_vPredicted = ConstrainNode(vNormal, m_RopePhysics.GetNode(i)->m_vPredicted,
...
void Catmull_Rom_Spline_Matrix(const Vector &vecP1, const Vector &vecP2, const Vector &vecP3, const Vector &vecP4,
                               catmull_t &output) {
    output.t3 = 0.5f * ((-1 * vecP1) + (3 * vecP2) + (-3 * vecP3) +
                        vecP4);    // 0.5 t^3 * [ (-1*p1) + ( 3*p2) + (-3*p3) + p4 ]
    output.t2 = 0.5f * ((2 * vecP1) + (-5 * vecP2) + (4 * vecP3) -
                        vecP4);        // 0.5 t^2 * [ ( 2*p1) + (-5*p2) + ( 4*p3) - p4 ]
        ...
        for (int iSegment = 0; iSegment < nSegmentCount; ++iSegment) {
            ...
            if (flScreenSpaceWidth < flMinScreenSpaceWidth) {
                ...
                if (flScreenSpaceWidth > flMaxAlphaScreenSpaceWidth) {
                    ...
                    pSegmentData->m_Segments[iSegment].m_flAlpha = RemapVal(flScreenSpaceWidth, flMinScreenSpaceWidth,
                                                                            flMaxAlphaScreenSpaceWidth, flMinAlpha,
                                                                            flMaxAlpha);
                }
                ...
                if (pSegmentData->m_BackWidths[iSegment] < 0.0f) {
                    ...
                    pSegmentData->m_flMaxBackWidth = MAX(pSegmentData->m_flMaxBackWidth,
