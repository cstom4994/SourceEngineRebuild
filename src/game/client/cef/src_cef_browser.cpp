...
// Purpose: 
//-----------------------------------------------------------------------------
CefClientHandler::CefClientHandler(SrcCefBrowser *pSrcBrowser, SrcCefNavigationType navigationbehavior,
    ...
    for (int i = 0; i < messageQueue.Count(); i++) {
        ...
        switch (data.type) {
            ...
            case MT_LOADEND:
                ...
                break;
            case MT_LOADERROR:
                m_pSrcBrowser->OnLoadError(data.frame, data.data->GetInt(0), data.data->GetString(1).c_str(),
                                           data.data->GetString(2).c_str());
                break;
            case MT_LOADINGSTATECHANGE:
                m_pSrcBrowser->OnLoadingStateChange(data.data->GetBool(0), data.data->GetBool(1),
                                                    data.data->GetBool(2));
                break;
...
// Purpose: Cef browser
//-----------------------------------------------------------------------------
SrcCefBrowser::SrcCefBrowser(const char *name, const char *pURL, int renderFrameRate, int wide, int tall,
                             SrcCefNavigationType navigationbehavior) :
        ...
        m_fLastTriedPingTime(-1), m_bInitializePingSuccessful(false), m_bWasHidden(false), m_bIgnoreTabKey(false),
        m_fLastLoadStartTime(0) {
...
void SrcCefBrowser::Destroy(void) {
    ...
    if (m_CefClientHandler) {
        ...
        if (g_debug_cef.GetBool())
            DevMsg("#%d %s: SrcCefBrowser::Destroy\n", GetBrowser() ? GetBrowser()->GetIdentifier() : -1,
                   m_Name.c_str());
        ...
        if (!m_CefClientHandler->IsInitialized() || m_CefClientHandler->GetLastPingTime() == -1) {
            ...
            if (g_debug_cef.GetBool())
                DevMsg("#%d %s: Browser creation time: %f\n", GetBrowser() ? GetBrowser()->GetIdentifier() : -1,
                       m_Name.c_str(), Plat_FloatTime() - m_fBrowserCreateTime);
        }
    }
...
// Purpose: 
//-----------------------------------------------------------------------------
void SrcCefBrowser::OnLoadError(CefRefPtr <CefFrame> frame, int errorCode, const wchar_t *errorText,
                                const wchar_t *failedUrl) {
#ifdef ENABLE_PYTHON
    if( !SrcPySystem()->IsPythonRunning() )
        return;
...
//-----------------------------------------------------------------------------
CefRefPtr <JSObject>
SrcCefBrowser::ExecuteJavaScriptWithResult(const char *code, const char *script_url, int start_line) {
    if (!IsValid())
        return NULL;
...
// Purpose: 
//-----------------------------------------------------------------------------
CefRefPtr <JSObject> SrcCefBrowser::InvokeWithResult(CefRefPtr <JSObject> object, const char *methodname,
                                                     CefRefPtr <CefListValue> methodargs) {
    if (!IsValid())
        return NULL;
