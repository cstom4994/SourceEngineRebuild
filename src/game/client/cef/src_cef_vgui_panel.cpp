...
bool SrcCefVGUIPanel::ResizeTexture(int width, int height) {
    ...
    CefDbgMsg(1, "Cef#%d: Resizing texture from %d %d to %d %d (wish size: %d %d)\n", GetBrowserID(), m_iTexWide,
              m_iTexTall, po2wide, po2tall, m_iWVWide, m_iWVTall);
    ...
    m_iTexFlags = TEXTUREFLAGS_PROCEDURAL | TEXTUREFLAGS_NOLOD | TEXTUREFLAGS_NOMIP | TEXTUREFLAGS_POINTSAMPLE |
                  TEXTUREFLAGS_SINGLECOPY |
                  TEXTUREFLAGS_NODEPTHBUFFER;

    m_RenderBuffer.InitProceduralTexture(m_TextureWebViewName, TEXTURE_GROUP_VGUI, m_iTexWide, m_iTexTall,
                                         m_iTexImageFormat, m_iTexFlags);
    if (!m_RenderBuffer.IsValid()) {
...
void SrcCefVGUIPanel::Paint() {
    ...
    if (renderer) {
        ...
        if (g_pShaderAPI && m_RenderBuffer.IsValid()) { VPROF_BUDGET("Upload", "CefUploadTexture");

            int nFrame = 0;
            int nTextureChannel = 0;
            ShaderAPITextureHandle_t textureHandle = g_pSLShaderSystem->GetShaderAPITextureBindHandle(m_RenderBuffer,
                                                                                                      nFrame,
                                                                                                      nTextureChannel);
            if (g_pShaderAPI->IsTexture(textureHandle)) {
                ...
                if (renderer->GetTextureBuffer() && m_bTextureDirty) {
                    ...
                    g_pShaderAPI->TexImage2D(iMip, iFace, m_iTexImageFormat, z, renderer->GetWidth(),
                                             renderer->GetHeight(), m_iTexImageFormat, false,
                                             renderer->GetTextureBuffer());
#else
                ...
                if (renderer->GetPopupBuffer() && m_bPopupTextureDirty) {
                    ...
                    g_pShaderAPI->ModifyTexture(textureHandle);
                    g_pShaderAPI->TexSubImage2D(iMip, iFace, renderer->GetPopupOffsetX(), renderer->GetPopupOffsetY(),
                                                z, renderer->GetPopupWidth(), renderer->GetPopupHeight(),
...
void SrcCefVGUIPanel::OnMouseReleased(vgui::MouseCode code) {
    ...
    if ((m_pBrowser->GetUseMouseCapture() && IsPressedParent(code)) ||
        (m_pBrowser->GetPassMouseTruIfAlphaZero() && m_pBrowser->IsAlphaZeroAt(m_iMouseX, m_iMouseY))) {
