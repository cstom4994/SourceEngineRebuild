...
CefRefPtr <CefResourceHandler> AvatarSchemeHandlerFactory::Create(CefRefPtr <CefBrowser> browser,
    ...
    if (steamapicontext->SteamFriends() && steamapicontext->SteamUtils()) {
        ...
        CSteamID steamID(
                static_cast<unsigned __int64>( strSteamID.length() > 1 ? _atoi64(strSteamID.c_str() + 1) : 0 ));
                    ...
                    if (steamapicontext->SteamUtils()->GetImageSize(iAvatar, &wide, &tall) && wide > 0 && tall > 0) {
                        ...
                        if (steamapicontext->SteamUtils()->GetImageRGBA(iAvatar, rgbaDest, destBufferSize)) {
                            ...
                            if (stream) {
                                ...
                                header_map.insert(std::pair<CefString, CefString>(CefString("Cache-Control"),
                                                                                  CefString("no-cache")));
                                pResourceHandler = new CefStreamResourceHandler(200, "OK", "image/jpeg", header_map,
                                                                                stream);
                            }
