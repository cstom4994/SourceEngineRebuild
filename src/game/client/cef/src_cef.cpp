...
void ClientApp::OnContextInitialized() {
    CefRegisterSchemeHandlerFactory("avatar", "small",
                                    new AvatarSchemeHandlerFactory(AvatarSchemeHandlerFactory::k_AvatarTypeSmall));
    CefRegisterSchemeHandlerFactory("avatar", "medium",
                                    new AvatarSchemeHandlerFactory(AvatarSchemeHandlerFactory::k_AvatarTypeMedium));
    CefRegisterSchemeHandlerFactory("avatar", "large",
                                    new AvatarSchemeHandlerFactory(AvatarSchemeHandlerFactory::k_AvatarTypeLarge));
...
bool CCefSystem::Init() {
    ...
    const bool bDisableBeginFrameScheduling =
            CommandLine() && CommandLine()->FindParm("-cef_disable_begin_frame_scheduling") != 0;

    const int iRemoteDebuggingPort = (CommandLine() && CommandLine()->FindParm("-cef_remote_dbg_port") != 0)
                                     ? CommandLine()->ParmValue("-cef_remote_dbg_port", 0) : 0;
    ...
    s_pChainedWndProc = reinterpret_cast<WNDPROC>( SetWindowLongPtr(GetMainWindow(), GWL_WNDPROC,
                                                                    reinterpret_cast<LONG_PTR>( CefWndProcHook )));
#endif // WIN32
...
int CCefSystem::KeyInput(int down, ButtonCode_t keynum, const char *pszCurrentBinding) {
    ...
    if (m_bHasKeyFocus &&
        (keynum != KEY_ESCAPE && (!pszCurrentBinding || V_strcmp("toggleconsole", pszCurrentBinding) != 0)))
        return 0;
...
void CCefSystem::ProcessKeyInput(UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    if (message == WM_KEYDOWN || message == WM_SYSKEYDOWN) {
        ...
        if (m_bHasDeadChar) {
            ...
            int deadCharRet =
#endif // _DEBUG
            ToUnicodeEx(m_lastDeadChar_virtualKey, m_lastDeadChar_scancode, (BYTE *) m_lastDeadChar_kbrdState, unicode,
                        2, 0, currentKb);
