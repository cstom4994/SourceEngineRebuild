...
ConVar hud_showemptyweaponslots("hud_showemptyweaponslots", "0", FCVAR_ARCHIVE,
...
void CHudWeaponSelection::Paint() {
    ...
    if (!pPlayer->IsSuitEquipped()) {
        ...
        switch (hud_fastswitch.GetInt()) {
            case HUDTYPE_FASTSWITCH:
            case HUDTYPE_CAROUSEL:
                pSelectedWeapon = pPlayer->GetDeployingWeapon() ? pPlayer->GetDeployingWeapon()
    ...
    if ((hud_fastswitch.GetInt() == HUDTYPE_FASTSWITCH || !pPlayer->IsSuitEquipped()) ||
        ...
        case HUDTYPE_CAROUSEL: {
            ...
            if (m_iSelectedWeaponBox == -1 || m_WeaponBoxes.Count() <= 1) {
                ...
                for (int i = 0; i < m_WeaponBoxes.Count(); i++) {
                    C_BaseCombatWeapon *pWeapon = GetWeaponInSlot(m_WeaponBoxes[i].m_iSlot,
                                                                  m_WeaponBoxes[i].m_iSlotPos);
                    if (!pWeapon)
                        break;
...
//-----------------------------------------------------------------------------
void
CHudWeaponSelection::DrawLargeWeaponBox(C_BaseCombatWeapon *pWeapon, bool bSelected, int xpos, int ypos, int boxWide,
                                        int boxTall, Color selectedColor, float alpha, int number) {
...
void CHudWeaponSelection::DrawBox(int x, int y, int wide, int tall, Color color, float normalizedAlpha, int number) {
    if (m_iPaintBackgroundType == 0) {
        ...
        SetRoundedCorners(PANEL_ROUND_CORNER_TOP_LEFT | PANEL_ROUND_CORNER_TOP_RIGHT | PANEL_ROUND_CORNER_BOTTOM_LEFT |
...
void CHudWeaponSelection::FastWeaponSwitch(int iWeaponSlot) {
    ...
    int iPosition = -1;
    C_BaseCombatWeapon *pActiveWeapon = pPlayer->GetDeployingWeapon() ? pPlayer->GetDeployingWeapon()
                                                                      : pPlayer->GetActiveWeapon();
