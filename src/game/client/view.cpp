...
void CViewRender::Render(vrect_t *rect) {
    ...
    for (StereoEye_t eEye = GetFirstEye(); eEye <= GetLastEye(); eEye = (StereoEye_t) (eEye + 1)) {
        ...
        float aspectRatio = engine->GetScreenAspectRatio(16.0f, 9.0f) * 0.75f;     // / (4/3)
        ...
        if ((sv_restrict_aspect_ratio_fov.GetInt() > 0 && engine->IsWindowedMode() && gpGlobals->maxClients > 1) ||
            ...
            limitedAspectRatio = MIN(aspectRatio, 1.85f *
        ...
        switch (eEye) {
            case STEREO_EYE_MONO: {
                ...
                viewEye.m_flAspectRatio = (engineAspectRatio > 0.0f) ? engineAspectRatio : ((float) viewEye.width /
                                                                                            (float) viewEye.height);
            }
                break;
        ...
        if (UseVR()) {
            ...
            if (!bInFreezeCam && g_ClientVirtualReality.ShouldRenderHUDInWorld()) {
                ...
                g_ClientVirtualReality.OverlayHUDQuadWithUndistort(viewEye, bDoUndistort,
                                                                   g_pClientMode->ShouldBlackoutAroundHUD(),
                                                                   bTranslucent);
            }
        }
    }
