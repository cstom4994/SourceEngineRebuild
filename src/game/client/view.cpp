...
ConVar zoom_sensitivity_ratio("zoom_sensitivity_ratio", "1.0", 0,
    ...
    for (StereoEye_t eEye = GetFirstEye(); eEye <= GetLastEye(); eEye = (StereoEye_t) (eEye + 1)) {
        ...
        switch (eEye) {
            case STEREO_EYE_MONO: {
                ...
                view.m_flAspectRatio = (engineAspectRatio > 0.0f) ? engineAspectRatio : ((float) view.width /
                                                                                         (float) view.height);
            }
                break;
            ...
            case STEREO_EYE_LEFT: {
                g_pSourceVR->GetViewportBounds((ISourceVirtualReality::VREye) (eEye - 1), &view.x, &view.y, &view.width,
                                               &view.height);
        ...
        if (UseVR()) {
            ...
            if (!bInFreezeCam && g_ClientVirtualReality.ShouldRenderHUDInWorld()) {
                ...
                g_ClientVirtualReality.OverlayHUDQuadWithUndistort(view, bDoUndistort,
