...
private:
    ...
    void BuildGeneralWorldToShadowMatrix(VMatrix &matWorldToShadow,
                                         const Vector &origin, const Vector &dir, const Vector &xvec,
                                         const Vector &yvec);
...
public:
    ...
    ShadowHandle_t
    GetShadowHandle(ClientShadowHandle_t clienthandle) { return m_Shadows[clienthandle].m_ShadowHandle; };
...
private:
    ...
    bool
    CullReceiver(ClientShadowHandle_t handle, IClientRenderable *pRenderable, IClientRenderable *pSourceRenderable);
...
void CClientShadowMgr::InitDepthTextureShadows() {
    ...
    if (!m_bDepthTextureActive) {
        ...
        for (int i = 0; i < m_nMaxDepthTextureShadows; i++) {
            ...
            depthTex.InitRenderTarget(m_nDepthTextureResolution, m_nDepthTextureResolution, RT_SIZE_NO_CHANGE,
...
ClientShadowHandle_t CClientShadowMgr::CreateProjectedTexture(ClientEntityHandle_t entity, int flags) {
    ...
    shadow.m_ShadowHandle = shadowmgr->CreateShadowEx(pShadowMaterial, pShadowModelMaterial, pShadowProxyData,
                                                      createShadowFlags);
    return h;
}
...
//-----------------------------------------------------------------------------
void
CClientShadowMgr::UpdateFlashlightState(ClientShadowHandle_t shadowHandle, const FlashlightState_t &flashlightState) {
...
//	float shadowArea = size.x * size.y;
    ...
    shadowmgr->ProjectShadow(m_Shadows[handle].m_ShadowHandle, worldOrigin,
                             vecShadowDir, matWorldToTexture, size, nCount, pLeafList, maxHeight, falloffStart,
...
//-----------------------------------------------------------------------------
void
CClientShadowMgr::DrawRenderToTextureDebugInfo(IClientRenderable *pRenderable, const Vector &mins, const Vector &maxs) {
    // Get the object's basis
    Vector vec[3];
    AngleVectors(pRenderable->GetRenderAngles(), &vec[0], &vec[1], &vec[2]);
    vec[1] *= -1.0f;
...
void CClientShadowMgr::UpdateBrushShadow(IClientRenderable *pRenderable, ClientShadowHandle_t handle) {
    ...
    } else { VPROF_BUDGET("CClientShadowMgr::UpdateBrushShadow", VPROF_BUDGETGROUP_SHADOW_DEPTH_TEXTURING);
...
void CClientShadowMgr::UpdateProjectedTextureInternal(ClientShadowHandle_t handle, bool force) {
    ...
    if (shadow.m_Flags & SHADOW_FLAGS_FLASHLIGHT) { VPROF_BUDGET("CClientShadowMgr::UpdateProjectedTextureInternal",
...
bool CClientShadowMgr::CullReceiver(ClientShadowHandle_t handle, IClientRenderable *pRenderable,
    ...
    if (m_Shadows[handle].m_Flags & SHADOW_FLAGS_FLASHLIGHT) { VPROF_BUDGET("CClientShadowMgr::CullReceiver",
                                                                            VPROF_BUDGETGROUP_SHADOW_DEPTH_TEXTURING);
...
void CClientShadowMgr::AddShadowToReceiver(ClientShadowHandle_t handle,
    ...
    switch (type) {
        case SHADOW_RECEIVER_BRUSH_MODEL:
            ...
            if (shadow.m_Flags & SHADOW_FLAGS_FLASHLIGHT) { VPROF_BUDGET("CClientShadowMgr::AddShadowToReceiver",
                                                                         VPROF_BUDGETGROUP_SHADOW_DEPTH_TEXTURING);
        ...
        case SHADOW_RECEIVER_STATIC_PROP:
            ...
            } else if (shadow.m_Flags & SHADOW_FLAGS_FLASHLIGHT) { VPROF_BUDGET("CClientShadowMgr::AddShadowToReceiver",
                                                                                VPROF_BUDGETGROUP_SHADOW_DEPTH_TEXTURING);
...
//-----------------------------------------------------------------------------
bool
CClientShadowMgr::BuildSetupShadowHierarchy(IClientRenderable *pRenderable, const ClientShadow_t &shadow, bool bChild) {
...
bool CClientShadowMgr::DrawShadowHierarchy(IClientRenderable *pRenderable, const ClientShadow_t &shadow, bool bChild) {
    ...
    if (!pRenderable || shadowType != SHADOWS_NONE) {
        ...
        if (bDrawModelShadow) {
            ...
            matrix3x4_t *pBoneToWorld = modelrender->DrawModelShadowSetup(pRenderable, pRenderable->GetBody(),
                                                                          pRenderable->GetSkin(), &info);
            if (pBoneToWorld) {
...
CompareLights(const ClientShadowHandle_t *light_left, const ClientShadowHandle_t *light_right) {
    const FlashlightState_t &flashlightState1 = shadowmgr->GetFlashlightState(
            g_pClientShadowMgr->GetShadowHandle(*light_left));
    const FlashlightState_t &flashlightState2 = shadowmgr->GetFlashlightState(
            g_pClientShadowMgr->GetShadowHandle(*light_right));
...
void CClientShadowMgr::ComputeShadowDepthTextures(const CViewSetup &viewSetup) {
    ...
    int nActiveDepthShadowCount = BuildActiveShadowDepthList(viewSetup, ARRAYSIZE(pActiveDepthShadows),
                                                             pActiveDepthShadows);
    ...
    for (int j = 0; j < nActiveDepthShadowCount; ++j) {
        ...
        CMatRenderContextPtr pRenderContext(materials);
        pRenderContext->SetShadowDepthBiasFactors(flashlightState.m_flShadowSlopeScaleDepthBias,
        ...
        if (j <= (INT_FLASHLIGHT_DEPTHTEXTURE_FALLBACK_LAST - INT_FLASHLIGHT_DEPTHTEXTURE_FALLBACK_FIRST)) {
            pRenderContext->SetIntRenderingParameter(INT_FLASHLIGHT_DEPTHTEXTURE_FALLBACK_FIRST + j,
                                                     int((ITexture *) shadowDepthTexture));
