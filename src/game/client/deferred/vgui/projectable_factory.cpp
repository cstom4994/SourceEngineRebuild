...
CVGUIProjectable *CProjectableFactory::AllocateProjectableByScript(const char *pszFileName) {
    ...
    bool bSuccess = pKV->LoadFromFile(g_pFullFileSystem,
                                      VarArgs("%s/%s", PATHLOCATION_PROJECTABLE_SCRIPTS, pszFileName));
    if (!bSuccess)
        bSuccess = pKV->LoadFromFile(g_pFullFileSystem,
                                     VarArgs("%s/%s.txt", PATHLOCATION_PROJECTABLE_SCRIPTS, pszFileName));
