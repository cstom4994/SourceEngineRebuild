...
void InitDeferredRTs(bool bInitial) {
    ...
    const ImageFormat fmt_depthColor = IMAGE_FORMAT_R32F;
    ...
    if (fmt_depth == IMAGE_FORMAT_NV_RAWZ)   // IMAGE_FORMAT_D16_SHADOW
        g_flDepthScalar = pow(2.0, 16);
    else if (fmt_depth == IMAGE_FORMAT_RGBA8888)    // IMAGE_FORMAT_D24X8_SHADOW
        g_flDepthScalar = pow(2.0, 24);

    AssertMsg(fmt_depth == IMAGE_FORMAT_NV_RAWZ || fmt_depth == IMAGE_FORMAT_RGBA8888,
              "Unexpected depth format");

    unsigned int gbufferFlags =
            TEXTUREFLAGS_CLAMPS | TEXTUREFLAGS_CLAMPT | TEXTUREFLAGS_RENDERTARGET | TEXTUREFLAGS_POINTSAMPLE;
    unsigned int lightAccumFlags =
            TEXTUREFLAGS_CLAMPS | TEXTUREFLAGS_CLAMPT | TEXTUREFLAGS_RENDERTARGET | TEXTUREFLAGS_POINTSAMPLE;
    ...
    for (int i = 0; i < MAX_SHADOW_ORTHO; i++)
        GetDeferredExt()->CommitTexture_CascadedDepth(i,
                                                      bShadowUseColor ? g_tex_ShadowColor_Ortho[i]
                                                                      : g_tex_ShadowDepth_Ortho[i]
        );

    for (int i = 0; i < MAX_SHADOW_DP; i++)
        GetDeferredExt()->CommitTexture_DualParaboloidDepth(i,
                                                            bShadowUseColor ? g_tex_ShadowColor_DP[i]
                                                                            : g_tex_ShadowDepth_DP[i]
        );

    for (int i = 0; i < MAX_SHADOW_PROJ; i++)
        GetDeferredExt()->CommitTexture_ProjectedDepth(i,
                                                       bShadowUseColor ? g_tex_ShadowColor_Proj[i]
                                                                       : g_tex_ShadowDepth_Proj[i]
        );
