...
void CLightingEditor::ApplyLightsToCurrentVmfFile() {
    ...
    FOR_EACH_VEC(listToRemove, i) m_pKVVmf->RemoveSubKey(listToRemove[i]);

    if (GetKVGlobalLight() != NULL)
        ApplyLightStateToKV(m_EditorGlobalState);

    FOR_EACH_VEC(listKeyValues, i) listKeyValues[i]->deleteThis();
}
...
void CLightingEditor::ApplyKVToGlobalLight(KeyValues *pKVChanges) {
    ...
    m_EditorGlobalState.bEnabled =
            (m_pKVGlobalLight->GetInt(GetLightParamName(LPARAM_SPAWNFLAGS), 1) & DEFLIGHTGLOBAL_ENABLED) != 0;
    m_EditorGlobalState.bShadow =
            (m_pKVGlobalLight->GetInt(GetLightParamName(LPARAM_SPAWNFLAGS), 1) & DEFLIGHTGLOBAL_SHADOW_ENABLED) != 0;
    ...
    m_EditorGlobalState.diff.AsVector3D() = stringColToVec(
            m_pKVGlobalLight->GetString(GetLightParamName(LPARAM_DIFFUSE), "255 255 255 255"));
    m_EditorGlobalState.ambl.AsVector3D() = stringColToVec(
            m_pKVGlobalLight->GetString(GetLightParamName(LPARAM_AMBIENT_LOW), "0 0 0 0"));
    m_EditorGlobalState.ambh.AsVector3D() = stringColToVec(
            m_pKVGlobalLight->GetString(GetLightParamName(LPARAM_AMBIENT_HIGH), "0 0 0 0"));
}
