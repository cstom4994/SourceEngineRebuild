            ...
            if (!Q_stricmp(pField.externalName, pszKeyName)) {
                if (!Q_stricmp(pszKeyName, GetLightParamName(LPARAM_DIFFUSE)) ||
                    ...
                    if (pCookieValidate != NULL) {
                        ...
                        Q_memcpy((void *) ((int) this + (int) pField.fieldOffset), &iAddedCookie,
...
#define __ND1 0.57735f
    ...
    AssertMsgOnce(leaves.m_LeafList.Count() <= DEFLIGHT_MAX_LEAVES,
                  "You sure have got a huge light there, or crappy leaves.");
