...
ConVar r_entityclips("r_entityclips",
                     "1"); //FIXME: Nvidia drivers before 81.94 on cards that support user clip planes will have problems with this, require driver update? Detect and disable?
...
static inline unsigned long BuildEngineDrawWorldListFlags(unsigned nDrawFlags) {
    ...
    if (nDrawFlags & DF_SSAO_DEPTH_PASS) {
        ...
        nEngineFlags &= ~(DRAWWORLDLISTS_DRAW_WATERSURFACE | DRAWWORLDLISTS_DRAW_REFRACTION |
...
void CViewRender::DrawViewModels(const CViewSetup &view, bool drawViewmodel) {
    ...
    if (bShouldDrawPlayerViewModel || bShouldDrawToolViewModels) {
        ...
        DrawRenderablesInList(translucentViewModelList,
                              STUDIO_TRANSPARENCY | (s_bDrawViewmodelShadow ? STUDIO_SHADOWDEPTHTEXTURE : 0));
    }
...
static ConVar r_csm_size("r_csm_size", "128");
...
void CViewRender::ProcessGlobals(const CViewSetup &view) {
    ...
    QUEUE_FIRE(CommitCommonData, view.origin, vFwd, view.zNear, view.zFar,
               (g_pCSMLight != 0) ? g_pCSMLight->CurrentTime()
                                  : 0.0f, // HACKHACK: csmlight probably shouldnt store current time of the whole map
...
void CViewRender::RenderView(const CViewSetup &view, int nClearFlags, int whatToDraw) {
    ...
    if (!m_rbTakeFreezeFrame[view.m_eStereoEye] && m_flFreezeFrameUntil > gpGlobals->curtime) {
        ...
        if (!building_cubemaps.GetBool() &&
            view.m_bDoBloomAndToneMapping) // We probably should use a different view. variable here
        {
    ...
    if (mat_viewportupscale.GetBool() && mat_viewportscale.GetFloat() < 1.0f) {
        ...
        pRenderContext->DrawScreenSpaceRectangle(pCopyMaterial, UpscaleRect.x, UpscaleRect.y, UpscaleRect.width,
                                                 UpscaleRect.height,
                                                 DownscaleRect.x, DownscaleRect.y,
    ...
    if (UseVR()) {
        saveRenderTarget = g_pSourceVR->GetRenderTarget((ISourceVirtualReality::VREye) (view.m_eStereoEye - 1),
                                                        ISourceVirtualReality::RT_Color);
    }
    ...
    if (whatToDraw & RENDERVIEW_DRAWHUD) { VPROF_BUDGET("VGui_DrawHud", VPROF_BUDGETGROUP_OTHER_VGUI);
        ...
        if (UseVR()) {
            g_pMatSystemSurface->SetFullscreenViewportAndRenderTarget(viewFramebufferX, viewFramebufferY,
                                                                      viewFramebufferWidth, viewFramebufferHeight,
                                                                      saveRenderTarget);
        }
...
void CViewRender::ViewDrawScene_Intro(const CViewSetup &view, int nClearFlags, const IntroData_t &introData) {
    ...
    for (passID = 0; passID < introData.m_Passes.Count(); passID++) {
        ...
        pRenderContext->DrawScreenSpaceRectangle(pOverlayMaterial, 0, 0, view.width, view.height,
                                                 ...
                                                 actualRect.y + actualRect.height - 1,
...
ClientWorldListInfo_t *ClientWorldListInfo_t::AllocPooled(const ClientWorldListInfo_t &exemplar) {
    ...
    pResult->m_pActualLeafIndex = (LeafIndex_t *) ((byte *) (pResult->m_pLeafFogVolume) +
...
void CRendering3dView::BuildWorldRenderLists(bool bDrawEntities, int iForceViewLeaf /* = -1 */,
    ...
    VisOverrideData_t *pVisData = (m_pCustomVisibility &&
                                   m_pCustomVisibility->m_VisData.m_fDistToAreaPortalTolerance != FLT_MAX)
                                  ? &m_pCustomVisibility->m_VisData : NULL;
...
void CRendering3dView::BuildRenderableRenderLists(int viewID) {
    ...
    if (m_pMainView->ShouldDrawEntities() && (viewID != VIEW_SHADOW_DEPTH_TEXTURE)) {
        ClientLeafSystem()->ComputeTranslucentRenderLeaf(
                info.m_LeafCount, info.m_pLeafList, info.m_pLeafFogVolume, m_pMainView->BuildRenderablesListsNumber(),
                viewID);
    }
...
void CRendering3dView::DrawWorld(float waterZAdjust) {
    VPROF_INCREMENT_COUNTER("RenderWorld", 1);VPROF_BUDGET("DrawWorld", VPROF_BUDGETGROUP_WORLD_RENDERING);
...
static void
DrawOpaqueRenderables_Parallel(CClientRenderablesList::CEntry *pEntitiesBegin, int nCount, ERenderDepthMode DepthMode,
...
void CRendering3dView::DrawOpaqueRenderables(ERenderDepthMode DepthMode) {
    ...
    for (int bucket = 0; bucket < RENDER_GROUP_CFG_NUM_OPAQUE_ENT_BUCKETS; ++bucket) {
        for (CClientRenderablesList::CEntry
                     *const pEntitiesBegin = m_pRenderablesList->m_RenderGroups[RENDER_GROUP_OPAQUE_ENTITY_HUGE +
                                                                                2 * bucket],
                     *const pEntitiesEnd = pEntitiesBegin +
                                           m_pRenderablesList->m_RenderGroupCounts[RENDER_GROUP_OPAQUE_ENTITY_HUGE +
                                                                                   2 * bucket],
                     *itEntity = pEntitiesBegin; itEntity < pEntitiesEnd; ++itEntity) {
            C_BaseEntity *pEntity = itEntity->m_pRenderable
                                    ? itEntity->m_pRenderable->GetIClientUnknown()->GetBaseEntity() : NULL;
            if (pEntity) {
                if (pEntity->IsNPC()) {
...
//-----------------------------------------------------------------------------
static inline void
DrawTranslucentRenderable(IClientRenderable *pEnt, bool twoPass, bool bShadowDepth, bool bIgnoreDepth) {
...
// 	bool bClipSkybox = (nFlags & DF_CLIP_SKYBOX ) != 0;
    ...
    if (m_pMainView->ShouldDrawEntities() && r_drawtranslucentrenderables.GetBool()) {
        ...
        while (iCurTranslucentEntity >= 0) {
            ...
            bool bDrawDetailProps = ClientLeafSystem()->ShouldDrawDetailObjectsInLeaf(nLeaf,
                                                                                      m_pMainView->BuildWorldListsNumber());
            if (bDrawDetailProps) {
                ...
                for (; pEntities[iCurTranslucentEntity].m_iWorldListInfoLeaf == iThisLeaf &&
                       iCurTranslucentEntity >= 0; --iCurTranslucentEntity) {
                    IClientRenderable *pRenderable = pEntities[iCurTranslucentEntity].m_pRenderable;
                    ...
                    DetailObjectSystem()->RenderTranslucentDetailObjectsInLeaf(CurrentViewOrigin(),
                                                                               CurrentViewForward(), CurrentViewRight(),
                                                                               CurrentViewUp(), nLeaf,
                                                                               &vecRenderOrigin);
                ...
                DetailObjectSystem()->RenderTranslucentDetailObjectsInLeaf(CurrentViewOrigin(), CurrentViewForward(),
                                                                           CurrentViewRight(), CurrentViewUp(), nLeaf,
                                                                           NULL);
            } else {
...
void CShadowDepthView::Draw() {
    ...
    m_pMainView->SetupVis((*this),
                          visFlags);  // @MULTICORE (toml 8/9/2006): Portal problem, not sending custom vis down
...
void CCSMDepthView::Draw() {
    ...
    if (nCascade < 3) { VPROF_BUDGET("BuildRenderableRenderLists", VPROF_BUDGETGROUP_SHADOW_DEPTH_TEXTURING);
        BuildRenderableRenderLists(CurrentViewID());
    }
    ...
    if (nCascade < 3) { VPROF_BUDGET("DrawOpaqueRenderables", VPROF_BUDGETGROUP_SHADOW_DEPTH_TEXTURING);
        DrawOpaqueRenderables(DEPTH_MODE_SHADOW);
    }
...
#if 1 VPROF_BUDGET("CSimpleWorldView::SSAO_DepthPass", VPROF_BUDGETGROUP_SHADOW_DEPTH_TEXTURING);
...
// Draws the scene when there's no water or only cheap water
//-----------------------------------------------------------------------------
void CSimpleWorldView::Setup(const CViewSetup &view, int nClearFlags, bool bDrawSkybox,
...
//-----------------------------------------------------------------------------
void CAboveWaterView::Setup(const CViewSetup &view, bool bDrawSkybox, const VisibleFogVolumeInfo_t &fogInfo,
                            const WaterRenderInfo_t &waterInfo) {
    BaseClass::Setup(view);
...
void CAboveWaterView::Draw() {
    ...
    if (m_waterInfo.m_bRefract) {
        ...
        if (!m_bSoftwareUserClipPlane) {
            bViewIntersectsWater = DoesViewPlaneIntersectWater(m_fogInfo.m_flWaterHeight,
                                                               m_fogInfo.m_nVisibleFogVolume);
        }
...
//-----------------------------------------------------------------------------
void CUnderWaterView::Setup(const CViewSetup &view, bool bDrawSkybox, const VisibleFogVolumeInfo_t &fogInfo,
                            const WaterRenderInfo_t &waterInfo) {
    BaseClass::Setup(view);
