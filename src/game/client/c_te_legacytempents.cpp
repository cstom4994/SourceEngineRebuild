            ...
            if (flags & (FTENT_GRAVITY | FTENT_SLOWGRAVITY)) {
                ...
                if (traceNormal[2] > 0.9)        // Hit floor?
                {
                    if (m_vecTempEntVelocity[2] <= 0 && m_vecTempEntVelocity[2] >= gravity * 3) {
                        damp = 0;        // Stop
                        flags &= ~(FTENT_ROTATE | FTENT_GRAVITY | FTENT_SLOWGRAVITY | FTENT_COLLIDEWORLD |
                                   FTENT_SMOKETRAIL);
...
//-----------------------------------------------------------------------------
void
CTempEnts::BubbleTrail(const Vector &start, const Vector &end, float flWaterZ, int modelIndex, int count, float speed) {
    C_LocalTempEntity *pTemp;
    const model_t *model;
    int i, frameCount;
    float dist, angle;
...
//-----------------------------------------------------------------------------
C_LocalTempEntity *
CTempEnts::ClientProjectile(const Vector &vecOrigin, const Vector &vecVelocity, const Vector &vecAcceleration,
                            int modelIndex, int lifetime, CBaseEntity *pOwner, const char *pszImpactEffect,
...
//-----------------------------------------------------------------------------
C_LocalTempEntity *
CTempEnts::TempSprite(const Vector &pos, const Vector &dir, float scale, int modelIndex, int rendermode, int renderfx,
                      float a, float life, int flags, const Vector &normal) {
    C_LocalTempEntity *pTemp;
    const model_t *model;
    int frameCount;
...
//-----------------------------------------------------------------------------
void
CTempEnts::BloodSprite(const Vector &org, int r, int g, int b, int a, int modelIndex, int modelIndex2, float size) {
    const model_t *model;
...
void CTempEnts::MuzzleFlash_Combine_Player(ClientEntityHandle_t hEntity, int attachmentIndex) {
    ...
    for (int i = 1; i < 6; i++) {
        ...
        pParticle = (SimpleParticle *) pSimple->AddParticle(sizeof(SimpleParticle),
                                                            m_Material_Combine_MuzzleFlash_Player[random->RandomInt(0,
                                                                                                                    1)],
                                                            offset);
...
void CTempEnts::MuzzleFlash_Combine_NPC(ClientEntityHandle_t hEntity, int attachmentIndex) {
    ...
    CSmartPtr<CLocalSpaceEmitter> pSimple = CLocalSpaceEmitter::Create("MuzzleFlash_Combine_NPC", hEntity,
                                                                       attachmentIndex);
...
#define    FRONT_LENGTH 6
    ...
    for (int i = 1; i < FRONT_LENGTH; i++) {
        ...
        pParticle->m_uchStartSize = ((random->RandomFloat(6.0f, 8.0f) * (FRONT_LENGTH * 1.25f - (i)) / (FRONT_LENGTH)) *
                                     flScale);
        pParticle->m_uchEndSize = pParticle->m_uchStartSize;
        pParticle->m_flRoll = random->RandomInt(0, 360);
        pParticle->m_flRollDelta = 0.0f;
...
#define    SIDE_LENGTH    6
    ...
    for (int i = 1; i < SIDE_LENGTH; i++) {
        ...
        pParticle->m_uchStartSize = ((random->RandomFloat(2.0f, 4.0f) * (SIDE_LENGTH - (i)) / (SIDE_LENGTH * 0.5f)) *
                                     flScale);
        pParticle->m_uchEndSize = pParticle->m_uchStartSize;
        pParticle->m_flRoll = random->RandomInt(0, 360);
        pParticle->m_flRollDelta = 0.0f;
    ...
    pParticle = (SimpleParticle *) pSimple->AddParticle(sizeof(SimpleParticle), g_Mat_Combine_Muzzleflash[2],
                                                        vec3_origin);
    if (pParticle == NULL)
        return;
...
void CTempEnts::MuzzleFlash_SMG1_Player(ClientEntityHandle_t hEntity, int attachmentIndex) {
    ...
    for (int i = 1; i < 6; i++) {
        ...
        pParticle = (SimpleParticle *) pSimple->AddParticle(sizeof(SimpleParticle),
                                                            m_Material_MuzzleFlash_Player[random->RandomInt(0, 3)],
                                                            offset);
...
void CTempEnts::MuzzleFlash_Shotgun_NPC(ClientEntityHandle_t hEntity, int attachmentIndex) {
    ...
    if (random->RandomInt(0, 2) == 0) {
        ...
        for (int i = 0; i < numEmbers; i++) {
            pParticle = (SimpleParticle *) pEmbers->AddParticle(sizeof(SimpleParticle), g_Mat_SMG_Muzzleflash[0],
                                                                origin);
    ...
    for (int i = 0; i < numEmbers; i++) {
        pTrailParticle = (TrailParticle *) pTrails->AddParticle(sizeof(TrailParticle), g_Mat_SMG_Muzzleflash[0],
                                                                origin);
...
void
CTempEnts::HL1EjectBrass(const Vector &vecPosition, const QAngle &angAngles, const Vector &vecVelocity, int nType) {
