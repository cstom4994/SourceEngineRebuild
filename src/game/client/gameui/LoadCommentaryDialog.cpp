...
void CLoadCommentaryDialog::ScanCommentaryFiles() {
    ...
    // add to the list
    for (int saveIndex = 0;
         saveIndex < m_CommentaryItems.Count() && saveIndex < MAX_LISTED_COMMENTARY_ITEMS; saveIndex++) {
    ...
    if (!m_CommentaryItems.Count()) {
        vgui::Label *pNoCommentaryItemsLabel = SETUP_PANEL(
                new Label(m_pGameList, "NoCommentaryItemsLabel", "#GameUI_NoCommentaryItemsToDisplay"));
