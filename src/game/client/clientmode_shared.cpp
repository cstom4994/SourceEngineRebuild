    ...
    if (Q_strcmp("player_connect_client", eventname) == 0) {
        ...
        if (!IsInCommentaryMode()) {
            ...
            if (IsPC() && bFoundReason) {
                g_pVGuiLocalize->ConstructString_safe(wszLocalized,
                                                      g_pVGuiLocalize->Find("#game_player_left_game_with_reason"), 2,
        ...
        if (!bSilent) {
            ...
            if (!IsInCommentaryMode()) {
                wchar_t wszLocalized[100];
                if (bAutoTeamed) {
                    g_pVGuiLocalize->ConstructString_safe(wszLocalized, bUsingCustomTeamName ? g_pVGuiLocalize->Find(
                            "#game_player_joined_autoteam_party_leader") : g_pVGuiLocalize->Find(
                            "#game_player_joined_autoteam"), 2, wszPlayerName, wszTeam);
                } else {
                    g_pVGuiLocalize->ConstructString_safe(wszLocalized, bUsingCustomTeamName ? g_pVGuiLocalize->Find(
                            "#game_player_joined_team_party_leader") : g_pVGuiLocalize->Find(
                            "#game_player_joined_team"), 2, wszPlayerName, wszTeam);
                }
        ...
        if (!IsInCommentaryMode()) {
            ...
            if (pAchievement) {
                ...
                if (g_PR) {
                    ...
                    const wchar_t *pchLocalizedAchievement = ACHIEVEMENT_LOCALIZED_NAME_FROM_STR(
