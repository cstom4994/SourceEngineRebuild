...
CON_COMMAND(grasscluster_preset_density_high,
            "") {
    gcluster_grass_type_huge_oddness.

            Revert();

    gcluster_objectsPerHint.

            Revert();

    gcluster_LOD_objects_per_hint.
...
CON_COMMAND(grasscluster_preset_height_high,
    ...
    gcluster_grass_height_small_min.
            SetValue(gcluster_grass_height_small_min
                             .

                                     GetDefault()

    );
    gcluster_grass_height_small_max.
            SetValue(gcluster_grass_height_small_max
                             .

                                     GetDefault()

    );
    gcluster_grass_height_med_min.
            SetValue(gcluster_grass_height_med_min
                             .

                                     GetDefault()

    );
    gcluster_grass_height_med_max.
            SetValue(gcluster_grass_height_med_max
                             .

                                     GetDefault()

    );
    gcluster_grass_height_huge_min.
            SetValue(gcluster_grass_height_huge_min
                             .

                                     GetDefault()

    );
    gcluster_grass_height_huge_max.
            SetValue(gcluster_grass_height_huge_max
                             .
    ...
    gcluster_grass_width_small_min.
            SetValue(gcluster_grass_width_small_min
                             .

                                     GetDefault()

    );
    gcluster_grass_width_small_max.
            SetValue(gcluster_grass_width_small_max
                             .

                                     GetDefault()

    );
    gcluster_grass_width_med_min.
            SetValue(gcluster_grass_width_med_min
                             .

                                     GetDefault()

    );
    gcluster_grass_width_med_max.
            SetValue(gcluster_grass_width_med_max
                             .

                                     GetDefault()

    );
    gcluster_grass_width_huge_min.
            SetValue(gcluster_grass_width_huge_min
                             .

                                     GetDefault()

    );
    gcluster_grass_width_huge_max.
            SetValue(gcluster_grass_width_huge_max
                             .
...
void CGrassClusterManager::PostRender() {
    if (gcluster_debug.GetInt()) {
        ...
        engine->Con_NPrintf(13, "current efficiency: %3.1f%% // average efficiency: %3.1f%%", efficiency_cur,
                            efficiency_avg);
        engine->Con_NPrintf(14, "engine max quads: %i // cluster max quads: %i", m_iDrawnEngineMax,
                            m_iDrawnPerDrawcall);
...
void CGrassClusterManager::RenderClusters(bool bShadowDepth) {
    ...
    for (int i = 0; i < m_hClusterData.Count(); i++) {
        ...
        if (bFullDbg)
            debugoverlay->AddBoxOverlay(m_hClusterData[i].extents_min, vec3_origin,
                                        m_hClusterData[i].extents_max - m_hClusterData[i].extents_min, vec3_angle, 0,
                                        255, 0, 20, -1);
    }
        ...
        for (int m = 0; m < m_hClusterData[i].pPressureInfo->iNumGrassObjects; m++) {
            if (m_hClusterData[i].pPressureInfo->flAmt[m] != 0 &&
                ...
                m_hClusterData[i].pPressureInfo->flAmt[m] = Approach(0,
                                                                     ...
                                                                     gpGlobals->frametime *
                                                                     gcluster_grass_morph_speed.GetFloat());
                m_hClusterData[i].pPressureInfo->bDirty[m] = true;
                m_hClusterData[i].pPressureInfo->flLastUpdateTime[m] =
                        gpGlobals->curtime + gcluster_grass_morph_framelag.GetFloat();
            }
        }
    }

    float flGrassColScale = (gcluster_grass_width_med_min.GetFloat() +
                             (gcluster_grass_width_med_max.GetFloat() - gcluster_grass_width_med_min.GetFloat()) *
                             0.5f) / 50.0f;
    flGrassColScale = clamp(flGrassColScale, 0.3f, 1.6f);

    C_BasePlayer *pLocal = C_BasePlayer::GetLocalPlayer();
    for (C_BaseEntity *pEnt = ClientEntityList().FirstBaseEntity(); pEnt; pEnt = ClientEntityList().NextBaseEntity(
            pEnt)) {
        ...
        for (int i = 0; i < m_hClusterData.Count(); i++) {
            ...
            if (!IsPointInBox(vPos,
                              data.extents_min - Vector(CONSTBOXEXTENT_COL, CONSTBOXEXTENT_COL, CONSTBOXEXTENT_COL),
...
void CGrassClusterManager::GenerateClusterData() {
    ...
    while (hClusterInfoLocal.Count()) {
        ...
        if (gcluster_LOD_enable.GetInt()) {
            ...
            BuildClusterMesh(*dataLOD, hClusterInfoSorted,
                             min(m_iCurObjectsPerHint, gcluster_LOD_objects_per_hint.GetInt()));
            dataLOD->pos = avgPos;
...
void CGrassClusterManager::BuildClusterMesh(_grassClusterData &data, const CUtlVector<_grassClusterInfo> &hints,
    ...
    VertexFormat_t format = VERTEX_POSITION | VERTEX_NORMAL | VERTEX_COLOR |
                            VERTEX_FORMAT_VERTEX_SHADER | VERTEX_TEXCOORD_SIZE(0, 2) | VERTEX_TEXCOORD_SIZE(1, 3) |
                            VERTEX_TEXCOORD_SIZE(2, 3);
    ...
    for (int i = 0; i < hintCount; i++)
        for (int m = 0; m < numObjectsPerHint; m++)
            BuildSingleGrassObject(pMeshBuilder, data, hints[i], flAverageMinDist, i * numObjectsPerHint + m,
                                   pMorphInfo);
...
void CGrassClusterManager::BuildSingleGrassObject(CMeshBuilder &builder, _grassClusterData &clusterData,
                                                  const _grassClusterInfo &hint,
                                                  const float avgDist,
    ...
    const float flSize_up = bHuge ? RandomFloat(gcluster_grass_height_huge_min.GetFloat(),
                                                gcluster_grass_height_huge_max.GetFloat()) :
                            bSmall ? RandomFloat(gcluster_grass_height_small_min.GetFloat(),
                                                 gcluster_grass_height_small_max.GetFloat()) :
                            RandomFloat(gcluster_grass_height_med_min.GetFloat(),
                                        gcluster_grass_height_med_max.GetFloat());

    const float flSize_side = bHuge ? RandomFloat(gcluster_grass_width_huge_min.GetFloat(),
                                                  gcluster_grass_width_huge_max.GetFloat()) :
                              bSmall ? RandomFloat(gcluster_grass_width_small_min.GetFloat(),
                                                   gcluster_grass_width_small_max.GetFloat()) :
                              RandomFloat(gcluster_grass_width_med_min.GetFloat(),
                                          gcluster_grass_width_med_max.GetFloat());
