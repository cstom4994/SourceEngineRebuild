...
bool ShaderEditorHandler::Init() {
    ...
    Q_snprintf(modulePath, sizeof(modulePath), "%s/bin/shadereditor.dll", engine->GetGameDirectory());
    ...
    if (shaderEditorModule) {
        ...
        shaderEdit = shaderEditorDLLFactory ? ((IVShaderEditor *) shaderEditorDLLFactory(SHADEREDIT_INTERFACE_VERSION,
                                                                                         NULL)) : NULL;
...
void ShaderEditorHandler::PrepareCallbackData() {
    ...
    while (pEnt) {
        if (!Q_stricmp(pEnt->GetClassname(), "class C_Sun")) {
            ...
            trace_t tr;
            UTIL_TraceLine(Editor_MainViewOrigin, Editor_MainViewOrigin + dir * MAX_TRACE_LENGTH, MASK_SOLID, NULL,
    ...
    if (s_flSunAmt_Last != flSunAmt_Goal)
        s_flSunAmt_Last = Approach(flSunAmt_Goal, s_flSunAmt_Last,
                                   gpGlobals->frametime * ((!!flSunAmt_Goal) ? 4.0f : 0.75f));
        ...
        //
        {
            ...
            for (int bucket = 0; bucket < RENDER_GROUP_CFG_NUM_OPAQUE_ENT_BUCKETS; ++bucket) {
                ...
                pProps[bucket][1] = pProps[bucket][0] +
                                    m_pRenderablesList->m_RenderGroupCounts[RENDER_GROUP_OPAQUE_STATIC_HUGE +
                                                                            2 * bucket];
            }
    ...
    void Setup(const CViewSetup &view, CSimpleVCallbackView::EditorViewSettings settings,
        ...
        //if ( settings.bDrawWorld )
        {
            if (!info.m_bOpaqueWater) {
                ...
                bool bViewIntersectsWater = DoesViewPlaneIntersectWater(fogInfo.m_flWaterHeight,
                                                                        fogInfo.m_nVisibleFogVolume);
...
void ShaderEditorHandler::RegisterViewRenderCallbacks() {
    ...
    shaderEdit->RegisterViewRenderCallback("General view", VrCallback_General,
                                           boolNames_generalVrc, boolDefaults_generalVrc,
                                           ARRAYSIZE(boolNames_generalVrc),
                                           intNames_generalVrc, NULL, ARRAYSIZE(intNames_generalVrc),
                                           floatNames_generalVrc, floatDefaults_generalVrc,
                                           ARRAYSIZE(floatNames_generalVrc));
