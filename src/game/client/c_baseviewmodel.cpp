...
ConVar tf_viewmodel_alpha("tf_viewmodel_alpha", "255", FCVAR_ARCHIVE, "Controls translucency of viewmodel [1-255].",
                          true, 1.0f, true, 255.0f);
...
void C_BaseViewModel::UpdateAnimationParity(void) {
    ...
    if (m_nOldAnimationParity != m_nAnimationParity && !GetPredictable()) {
        float curtime = (pPlayer && IsIntermediateDataAllocated()) ? pPlayer->GetFinalPredictedTime()
                                                                   : gpGlobals->curtime;
        // FIXME: this is bad
