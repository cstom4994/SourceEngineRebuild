...
ConVar spec_freeze_distance_min("spec_freeze_distance_min", "96", FCVAR_CHEAT,
                                "Minimum random distance from the target to stop when framing them in observer freeze cam.");
ConVar spec_freeze_distance_max("spec_freeze_distance_max", "200", FCVAR_CHEAT,
...
static ConVar cl_clean_textures_on_death("cl_clean_textures_on_death", "1", 0,
...
void C_BasePlayer::PostDataUpdate(DataUpdateType_t updateType) {
    ...
    if (IsLocalPlayer()) {
        ...
        if (!m_bWasFreezeFraming && GetObserverMode() == OBS_MODE_FREEZECAM) {
            ...
            m_flFreezeFrameDistance = RandomFloat(spec_freeze_distance_min.GetFloat(),
...
void C_BasePlayer::CreateWaterEffects(void) {
    ...
    while (m_tWaterParticleTimer.NextEvent(curTime)) {
        ...
        pParticle = (SimpleParticle *) m_pWaterEmitter->AddParticle(sizeof(SimpleParticle),
                                                                    g_Mat_Fleck_Cement[random->RandomInt(0, 1)],
                                                                    offset);
...
bool
C_BasePlayer::GetRagdollInitBoneArrays(matrix3x4_t *pDeltaBones0, matrix3x4_t *pDeltaBones1, matrix3x4_t *pCurrentBones,
                                       float boneDt) {
    if (!IsLocalPlayer())
    ...
    if (m_Local.m_PlayerFog.m_flTransitionTime != -1) {
        ...
        if (flTimeDelta < m_CurrentFog.duration) {
            ...
            m_CurrentFog.end.Set(
                    (m_Local.m_PlayerFog.m_flNewEnd * flScale) + ((m_Local.m_PlayerFog.m_flOldEnd * (1.0f - flScale))));
        } else {
