    ...
    switch (messageType) {
        ...
        case VORTFX_ARMBEAM: {
            ...
            if (pEnt) {
                ...
                CNewParticleEffect *pEffect = pEnt->ParticleProp()->Create("vortigaunt_beam_charge",
                                                                           PATTACH_POINT_FOLLOW, nAttachment);
                if (pEffect) {
...
void CVortEmissiveProxy::OnBind(C_BaseEntity *pEnt) {
    ...
    if (pVort) {
        ...
        //
        // pVort->m_flBlackFade
    } else {    // if you bind this proxy to anything non-vort (eg a ragdoll) it's always green
        flBlendValue = 0.0f;
    }
