...
//-----------------------------------------------------------------------------
CThreadSafeMultiMemoryPool::CThreadSafeMultiMemoryPool(const MemPoolConfig_t *pMemPoolConfig, int cnMemPoolConfig,
    ...
    for (int iMemPoolConfig = 0; iMemPoolConfig < cnMemPoolConfig; iMemPoolConfig++) {
        ...
        Assert(iMemPoolConfig == 0 || (iMemPoolConfig > 0 && pMemPoolConfig[iMemPoolConfig - 1].m_cubBlockSize <
...
void CThreadSafeMultiMemoryPool::Free(void *pvMem) {
    ...
    if (m_MapRawAllocation.InvalidIndex() == iRawAllocation) {
        AssertMsg3(false,
                   "CThreadSafeMultiMemoryPool::Free: raw allocation %p (original alloc: %p, %d bytes) not found in allocation map",
...
void *CThreadSafeMultiMemoryPool::ReAlloc(void *pvMem, uint32 cubAlloc) {
    ...
    if (cubOldAlloc > m_nBlockSizeMax) {
        ...
        if (m_MapRawAllocation.InvalidIndex() == iRawAllocation) {
            AssertMsg3(false,
                       "CThreadSafeMultiMemoryPool::ReAlloc: raw allocation %p (original alloc: %p, %d bytes) not found in allocation map",
