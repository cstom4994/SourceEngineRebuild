...
//-----------------------------------------------------------------------------
    CJob::~CJob() {
        ...
        AssertMsg2(0 == GetDoNotYieldDepth(),
                   "Job ending with %d open Do Not Yields. Are we missing a END_DO_NOT_YIELD()? Innermost delared at %s",
    ...
    void CJob::WaitForThreadFuncWorkItemBlocking() {
        if (m_pWaitingOnWorkItem) {
            switch (GetPauseReason()) {
                ...
                default:
                    AssertMsg1(false, "CJob::WaitForThreadFuncWorkItemBlocking job %s has unexpected work item state",
                               GetName());
            }
        }
    }
        ...
        if (GetPauseReason() < Q_ARRAYSIZE(k_prgchJobPauseReason)) {
            switch (GetPauseReason()) {
                case k_EJobPauseReasonWaitingForLock: {
                    Q_snprintf(srgchPauseReason, k_cSmallBuff, "WOL: 0x%x (%s)", (unsigned int) m_pWaitingOnLock,
...
//-----------------------------------------------------------------------------
    void CJob::Continue() {
        ...
        if (g_VProfCurrentProfile.IsEnabled()) { VPROF_BUDGET(GetName(), VPROF_BUDGETGROUP_JOBS_COROUTINES);
...
//-----------------------------------------------------------------------------
    bool CJob::BYieldingWaitForMsg(CGCMsgBase *pMsg, MsgType_t eMsg) {
        ...
        if (BHasFailedToReceivedMsgType(eMsg)) {
            AssertMsg2(false, "Job %s cannot wait for msg %u, it has already failed to wait for that msg type.",
                       GetName(), eMsg);
            return false;
        }
...
//-----------------------------------------------------------------------------
    ...
    public:
        ...
        CJobThreadFuncWorkItem(CJob *pJob, JobThreadFunc_t jobThreadFunc, CFunctor *pFunctor) : CWorkItem(
                pJob->GetJobID()),
                                                                                                m_pJob(pJob),
                                                                                                m_pJobThreadFunc(
                                                                                                        jobThreadFunc),
            ...
            if (index != m_vecLocks.InvalidIndex() && this == pLock->GetJobLocking()) {
                ...
                if (m_vecLocks.Count() == 0) {
                    EmitWarning(SPEW_JOB, LOG_ALWAYS,
                                ...
                                pLock->GetJobLocking() ? pLock->GetJobLocking()->GetName() : "(null)",
                                pLock->GetJobLocking());
                } else {
                    EmitWarning(SPEW_JOB, LOG_ALWAYS,
                                "m_vecLocks.Count(): %d this: 0x%p, pLock: 0x%p pLock->GetJobLocking(): %s (0x%p)\n",
                                m_vecLocks.Count(), this, pLock,
                                pLock->GetJobLocking() ? pLock->GetJobLocking()->GetName() : "(null)",
                                pLock->GetJobLocking());
                    FOR_EACH_VEC(m_vecLocks, i) {
                        EmitWarning(SPEW_JOB, LOG_ALWAYS, "m_vecLocks[%d]: %s (0x%p) %s(%d)\n",
                                    i, m_vecLocks[i] ? m_vecLocks[i]->GetName() : "(null)", m_vecLocks[i],
                                    m_vecLocks[i]->m_pFilename, m_vecLocks[i]->m_line);
                    }
                }
                return false;
...
//-----------------------------------------------------------------------------
    bool CJob::_BAcquireLockImmediate(CLock *pLock, const char *filename, int line) {
        ...
        AssertMsg5(m_pWaitingOnLock == NULL,
                   "Job (%s) at %s(%d) trying to take a lock while it was already waiting for the first one at %s(%d)",
                   GetName(), filename, line, m_pWaitingOnLockFilename, m_waitingOnLockLine);
...
//-----------------------------------------------------------------------------
    void CJob::AddJobToNotifyOnLockRelease(CJob *pJob) {
        ...
        if (m_pJobToNotifyOnLockRelease) {
            AssertMsg(false,
                      "AddJobToNotifyOnLockRelease attempting to walk the linked list. We've optimized this out.");
    ...
    void CJob::PassLockToJob(CJob *pNewJob, CLock *pLock) {
        ...
        if (pNewJob->GetPauseReason() == k_EJobPauseReasonWaitingForLock && pNewJob->m_pWaitingOnLock == pLock) {
            ...
            EmitError(SPEW_JOB,
                      ...
                      pNewJob->GetName(), pLock->GetName(), pLock->m_pFilename, pLock->m_line,
                      pNewJob->GetPauseReasonDescription(), m_pWaitingOnLock ? m_pWaitingOnLock->GetName() : "none");
        }
    }
    ...
    void CLock::Dump(const char *pszPrefix, int nPrintMax, bool bPrintWaiting) const {
        if (m_pJob != NULL) {
            EmitInfo(SPEW_JOB, SPEW_ALWAYS, LOG_ALWAYS, "%s%s: Lock owner: %s, Type: %d, %d Waiting\n", pszPrefix,
                     GetName(), CFmtStr("%s (%llu), Reason: %s", m_pJob->GetName(), m_pJob->GetJobID(),
                                        m_pJob->GetPauseReasonDescription()).Access(), (int32) m_nsLockType,
                     m_nWaitingCount);
            ...
            if (bPrint && bPrintWaiting) {
                ...
                if (pCurrWaiting->GetPauseReason() == k_EJobPauseReasonWaitingForLock &&
                    ...
                    EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, 3, "%s\tAt: %s:%d\n", pszPrefix,
                             pCurrWaiting->m_pWaitingOnLockFilename, pCurrWaiting->m_waitingOnLockLine);
                }
                nPrinted++;
            }
