...
//-----------------------------------------------------------------------------
    void
    CSDOCache::CreateSimpleMemcachedName(CFmtStr &strDest, const char *pchPrefix, uint32 unPrefixLen, uint32 unSuffix) {
            ...
            FOR_EACH_VEC(m_vecRequests, i) {
                if (bGetSuccess && vecGetResults.IsValidIndex(i) && vecGetResults[i].m_bKeyFound) {
                    ...
                    bool bNilObj = (result.m_bufValue.Count() == sizeof(k_rgchNilObjSerializedValue)
                                    && 0 == V_memcmp(result.m_bufValue.Base(), k_rgchNilObjSerializedValue,
                                                     sizeof(k_rgchNilObjSerializedValue)));

                    // we've loaded OK
                    if (bNilObj || (result.m_bufValue.Count() &&
                                    m_vecRequests[i].m_pSDO->BReadFromBuffer(result.m_bufValue.Base(),
                                                                             result.m_bufValue.Count()))) {
                        GSDOCache().OnSDOLoadSuccess(m_vecRequests[i].m_pSDO->GetType(), m_vecRequests[i].m_iRequestID,
                                                     bNilObj, &m_vecRequests[i].m_pSDO);
                        ...
                        GSDOCache().OnMemcachedSDOLoadFailure(m_vecRequests[i].m_pSDO->GetType(),
...
//-----------------------------------------------------------------------------
    bool CSDOCache::BFrameFuncRunJobsUntilCompleted(CLimitTimer &limitTimer) {
        ...
        while (!limitTimer.BLimitReached() &&
               m_cubLRUItems > (uint32) (s_ConVarSDOCacheLRULimitMB.GetInt() * k_nMegabyte) && m_listLRU.Count()) {
        ...
        if (m_queueMemcachedRequests.Count()) {
            ...
            if (m_vecMemcachedJobs.Count() < s_ConVarSDOCacheMaxMemcachedReadJobs.GetInt()) {
                ...
                while (m_queueMemcachedRequests.Count() &&
                       cItemsInBatch < s_ConVarSDOCacheMaxMemcachedReadBatchSize.GetInt()) {
    ...
    bool CSDOCache::BFrameFuncRunSQLQueriesUntilCompleted(CLimitTimer &limitTimer) {
        ...
        if (m_StatsSDOCache.m_cItemsUnreferenced) {
            ...
            m_StatsSDOCache.m_cBytesInCacheEst =
                    (m_StatsSDOCache.m_cItemsInCache * m_StatsSDOCache.m_cBytesUnreferenced) /
...
//-----------------------------------------------------------------------------
    void CSDOCache::OnMemcachedSDOLoadFailure(int eSDOType, int iRequestID) {
        ...
        AssertMsg1(m_mapQueueSQLRequests.IsValidIndex(iSQLIndex),
                   "Error, could not load SDO from SQL for unregistered type %d", eSDOType);
    ...
    void CSDOCache::Dump() {
        ...
        FOR_EACH_MAP_FAST(m_mapTypeStats, i) {
                ...
                if (m_mapTypeStats[i].m_nRefed < nLoaded && m_mapTypeStats[i].m_cubUnrefed > 0) {
                    pchRefed = V_pretifymem((((int64) nLoaded * (int64) m_mapTypeStats[i].m_cubUnrefed) /
                                             (nLoaded - m_mapTypeStats[i].m_nRefed)), 2, true);
                }

                EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS, "%-35s %11d %14d %15d %14s\n",
                         m_mapTypeStats[i].m_strName.String(), m_mapTypeStats[i].m_nLoaded, m_mapTypeStats[i].m_nRefed,
                         m_mapTypeStats[i].m_nNilObjects, pchRefed);
            }
    }
