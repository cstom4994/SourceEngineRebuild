...
//------------------------------------------------------------------------------------
    CSQLAccess::CSQLAccess(ESchemaCatalog eSchemaCatalog)
            : m_eSchemaCatalog(eSchemaCatalog), m_pCurrentQuery(NULL), m_bInTransaction(false) {
...
//------------------------------------------------------------------------------------
    CSQLAccess::~CSQLAccess() {
        ...
        AssertMsg(!m_bInTransaction,
                  "GCSDK::CSQLAccess object being destroyed with a transaction pending. Use BCommitTransaction or RollbackTransaction to match your BBeginTransaction call.");
    }
...
//------------------------------------------------------------------------------------
    bool CSQLAccess::BYieldingExecute(const char *pchName, const char *pchSQLCommand, uint32 *pcRowsAffected,
                                      bool bSpewOnError) {
        if (NULL == pchName) {
...
//------------------------------------------------------------------------------------
    bool CSQLAccess::BCommitTransaction(bool bAllowEmpty) {
        ...
        AssertMsg1(!m_pCurrentQuery, "Unexecuted query present in BCommitTransaction: %s",
...
//------------------------------------------------------------------------------------
    ...
    CSQLAccess::BYieldingExecuteSingleResultDataInternal(const char *pchName, const char *pchSQLCommand,
                                                         EGCSQLType eType, uint8 **ppubData, uint32 *punSize,
                                                         uint32 *pcRowsAffected, bool bHasDefaultValue) {
        ...
        if (pResultSet->GetColumnType(0) != eType) {
            AssertMsg2(false, "Expected column of type %s, found %s", PchNameFromEGCSQLType(eType),
...
// Purpose: Perform a query that returns a single string
//------------------------------------------------------------------------------------
    bool CSQLAccess::BYieldingExecuteString(const char *pchName, const char *pchSQLCommand, CFmtStr1024 *psResult,
                                            uint32 *pcRowsAffected) {
        uint8 *pubData;
        uint32 cubData;
        if (CSQLAccess::BYieldingExecuteSingleResultDataInternal(pchName, pchSQLCommand, k_EGCSQLType_String, &pubData,
                                                                 &cubData, pcRowsAffected, false) !=
            eReadSingle_ResultFound)
            return false;
...
//------------------------------------------------------------------------------------
    bool CSQLAccess::BYieldingExecuteScalarInt(const char *pchName, const char *pchSQLCommand, int *pnResult,
                                               uint32 *pcRowsAffected) {
        return BYieldingExecuteSingleResult<int32, uint32>(pchName, pchSQLCommand, k_EGCSQLType_int32, pnResult,
                                                           pcRowsAffected);
    }

    bool CSQLAccess::BYieldingExecuteScalarIntWithDefault(const char *pchName, const char *pchSQLCommand, int *pnResult,
                                                          int iDefaultValue, uint32 *pcRowsAffected) {
        return BYieldingExecuteSingleResultWithDefault<int32, uint32>(pchName, pchSQLCommand, k_EGCSQLType_int32,
                                                                      pnResult, iDefaultValue, pcRowsAffected);
    }

//------------------------------------------------------------------------------------
// Purpose: Perform a query that returns a single uint32
//------------------------------------------------------------------------------------
    bool CSQLAccess::BYieldingExecuteScalarUint32(const char *pchName, const char *pchSQLCommand, uint32 *punResult,
                                                  uint32 *pcRowsAffected) {
        return BYieldingExecuteSingleResult<uint32, uint32>(pchName, pchSQLCommand, k_EGCSQLType_int32, punResult,
                                                            pcRowsAffected);
    }

    bool CSQLAccess::BYieldingExecuteScalarUint32WithDefault(const char *pchName, const char *pchSQLCommand,
                                                             uint32 *punResult, uint32 unDefaultValue,
                                                             uint32 *pcRowsAffected) {
        return BYieldingExecuteSingleResultWithDefault<uint32, uint32>(pchName, pchSQLCommand, k_EGCSQLType_int32,
                                                                       punResult, unDefaultValue, pcRowsAffected);
    }
...
//-----------------------------------------------------------------------------
    bool CSQLAccess::BYieldingInsertWithIdentity(CRecordBase *pRecordBase) {
        ...
        AssertMsg(vecOutputFields.Count() > 0,
                  "BYieldingInsertAndReadRecord called for a record type with no non-insertable columns");
...
// Output:	true if successful, false otherwise
//-----------------------------------------------------------------------------
    EResult CSQLAccess::YieldingReadRecordWithWhereColumns(CRecordBase *pRecord, const CColumnSet &readSet,
                                                           const CColumnSet &whereSet, const char *pchOrderClause) {
        ...
        if (pResultSet->GetRowCount() != 1) {
            ...
            AssertMsg1(false,
                       "BYieldingReadRecordWithWhereColumns from %s failing because multiple records match WHERE clause",
        ...
        FOR_EACH_COLUMN_IN_SET(readSet, nColumnIndex)
        {
            ...
            AssertMsg2(eResultType == eRecordType, "Column %d type mismatch in %s", nColumnIndex,
                       readSet.GetRecordInfo()->GetName());
...
// Output:	true if successful, false otherwise
//-----------------------------------------------------------------------------
    bool CSQLAccess::BYieldingUpdateRecord(const CRecordBase &record, const CColumnSet &whereColumns,
...
//--------------------------------------------------------------------------------------------------------------------------------
    ...
    CSQLUpdateOrInsert::CSQLUpdateOrInsert(const char *pszName, int nTable, const CColumnSet &whereColumns,
                                           const CColumnSet &updateColumns, const char *pszWhereClause,
    ...
    bool CSQLUpdateOrInsert::BYieldingExecute(CSQLAccess &sqlAccess, const CRecordBase &record,
                                              uint32 *out_punRowsAffected /* = NULL */ ) const {
        AssertMsg2(record.GetITable() == m_nTable,
                   "Error: Merge was compiled for table %s, but was attempted to be executed against %s",
                   GSchemaFull().GetSchema(m_nTable).GetRecordInfo()->GetName(), record.GetPRecordInfo()->GetName());
