...
//-----------------------------------------------------------------------------
    void EmitAndAppend(CFmtStr1024 &sTarget, const CGCEmitGroup &Group, int iLevel, int iLevelLog, PRINTF_FORMAT_STRING
...
//-----------------------------------------------------------------------------
    ...
    CUtlString GetAlterColumnText(CRecordInfo *pRecordInfo, const CColumnInfo *pColumnInfoDesired) {
        ...
        char rgchTmp[128];
        CUtlString sCmd;
        sCmd.Format("ALTER TABLE App%u.%s ALTER COLUMN %s %s %s", GGCBase()->GetAppID(), pRecordInfo->GetName(),
...
//-----------------------------------------------------------------------------
    SQLRETURN
    CJobUpdateSchema::YieldingEnsureDatabaseSchemaCorrect(ESchemaCatalog eSchemaCatalog, CSchemaUpdate *pSchemaUpdate) {
...
//		( k_EConversionModeConvertIrreversible == eConversionMode ) );	
        ...
        for (int iTable = 0; iTable < vecPRecordInfoActualDifferent.Count(); iTable++) {
                ...
                if (idx == pRecordInfoActual->GetPKIndex()) {
                    ...
                    nRet = YieldingProcessUnsafeConversion(eSchemaCatalog,
                                                           CFmtStr("ALTER TABLE App%u.%s DROP CONSTRAINT %s",
                                                                   ...
                                                                   fs.GetIndexName()).String(),
                                                           CFmtStr("%s: remove old primary key.",
                                                                   pRecordInfoDesired->GetName()).String());
                    EXIT_ON_SQL_FAILURE(nRet);
                } else {
                    EmitAndAppend(sDetail, SPEW_GC, 2, 2,
                                  "\tTable %s index %s exists in database but is not in specification.  (Possible performance problem?).\n",
                                  pRecordInfoDesired->GetName(), fs.GetIndexName());
                    nRet = YieldingProcessUnsafeConversion(eSchemaCatalog,
                                                           CFmtStr("DROP INDEX %s ON App%u.%s", fs.GetIndexName(),
                                                                   GGCBase()->GetAppID(),
                                                                   pRecordInfoActual->GetName()).String(),
            ...
            if (vecPColumnInfoActualDifferent.Count() > 0) {
                EmitAndAppend(sDetail, SPEW_GC, 2, 2,
                              "\tColumns that differ between specification and database in table %s:\n",
                              pRecordInfoActual->GetName());
                for (int iColumn = 0; iColumn < vecPColumnInfoActualDifferent.Count(); iColumn++) {
                    ...
                    if ((pColumnInfoDesired->GetType() != pColumnInfoActual->GetType()) ||
                        ...
                        if (k_EConversionModeConvertIrreversible != m_eConversionMode) {
                            ...
                            if (pColumnInfoDesired->GetType() != pColumnInfoActual->GetType())
                                EmitAndAppend(sDetail, SPEW_GC, 2, 2, "\t(data types differ: desired=%s, actual=%s)",
                            ...
                            if (pColumnInfoDesired->GetFixedSize() != pColumnInfoActual->GetFixedSize())
                                EmitAndAppend(sDetail, SPEW_GC, 2, 2, "\t(column sizes differ: desired=%d, actual=%d)",
                                              pColumnInfoDesired->GetFixedSize(), pColumnInfoActual->GetFixedSize());
                    ...
                    if (pColumnInfoDesired->GetColFlags() != pColumnInfoActual->GetColFlags()) {
                        ...
                        nRet = YieldingChangeColumnProperties(eSchemaCatalog, pRecordInfoActual, pColumnInfoActual,
                                                              pColumnInfoDesired);
                        EXIT_ON_SQL_FAILURE(nRet);
                    }
            ...
            for (int idx = 0; idx < pRecordInfoDesired->GetIndexFieldCount(); ++idx) {
                ...
                if (idx == pRecordInfoDesired->GetPKIndex()) {
                    ...
                    sCreatePK.Format("GO\nALTER TABLE App%u.%s ADD %s\n", GGCBase()->GetAppID(),
                                     pRecordInfoDesired->GetName(), cmd.String());
                    sCommand += sCreatePK;
                } else {
            ...
            if (!bIndexMismatch && !pRecordInfoActual->CompareIndexLists(pRecordInfoDesired)) {
                EmitAndAppend(sDetail, SPEW_GC, 2, 2,
                              "\tIndex sets in table %s differ between specification and database [GC]:\n",
                              pRecordInfoDesired->GetName());
                CFmtStr1024 sTemp;
            ...
            if (!pRecordInfoActual->CompareFKs(pRecordInfoDesired)) {
                EmitAndAppend(sDetail, SPEW_GC, 2, 2,
                              "\tForeign Key constraints in table %s differs between specification and database [GC]:\n",
                              pRecordInfoDesired->GetName());
            ...
            if (vecPColumnInfoActualUnknown.Count() > 0) {
                EmitAndAppend(sDetail, SPEW_GC, 2, 2,
                              "\tColumns in database [GC] table %s that are not in specification (ignored):\n",
                              pRecordInfoActual->GetName());

                // Since this can actually destroy data, let's not ever, ever run it automatically.
                CUtlString sCommand;
                sCommand.Format("ALTER TABLE App%u.%s DROP COLUMN ", GGCBase()->GetAppID(),
                                pRecordInfoActual->GetName());
        ...
        EmitAndAppend(sDetail, SPEW_GC, 2, 2,
                      "[GC]: # of tables that currently exist in database but were unspecified: %d\n",
        ...
        FOR_EACH_VEC(vecTriggerInfoActual, iActual)
        {
            ...
            if (!vecTriggerInfoActual[iActual].m_bMatched) {
                ...
                EmitAndAppend(sDetail, SPEW_GC, 2, 2,
                              "[GC]: Trigger %s on table %s is not in the declared schema ... Drop %s",
        ...
        if (!m_sRecommendedSQL.IsEmpty() || !m_sDataDestroyingCleanupSQL.IsEmpty()) {
            ...
            EmitAndAppend(sDetail, SPEW_GC, 2, 2, "\t** \n");
            EmitAndAppend(sDetail, SPEW_GC, 2, 2,
                          "\t** DISCLAIMER ** This conversion code is not well tested.  Review the SQL and use at your own risk.\n");
            EmitAndAppend(sDetail, SPEW_GC, 2, 2, "\t** \n");
            {
            ...
            if (!m_sDataDestroyingCleanupSQL.IsEmpty()) {
                EmitAndAppend(sDetail, SPEW_GC, 2, 2,
                              "\t\t-- WARNING: The following operations will *destroy* data that is in the database but not in the specification.\n");
                EmitAndAppend(sDetail, SPEW_GC, 2, 2,
                              "\t\t--          If you have manually created extra tables or columns in your database, it will appear here!\n");
                CUtlVectorAutoPurge<char *> vecLines;
...
//-----------------------------------------------------------------------------
    SQLRETURN CJobUpdateSchema::YieldingGetSchemaID(ESchemaCatalog eSchemaCatalog, int *pSchemaID) {
        ...
        if (!sqlAccess.BYieldingExecuteString(FILE_AND_LINE,
        ...
        if (0 != Q_stricmp(sDefaultSchema, sExpectedDefaultSchema)) {
            AssertMsg2(false, "SQL connection has the wrong default schema. Expected: %s. Actual %s",
...
// Output:  SQL return code.
//-----------------------------------------------------------------------------
    SQLRETURN CJobUpdateSchema::YieldingGetRecordInfoForAllTables(ESchemaCatalog eSchemaCatalog, int nSchemaID,
                                                                  CMapPRecordInfo &mapPRecordInfo) {
        CSQLAccess sqlAccess(eSchemaCatalog);

        // create a query that returns all tables in the database
        sqlAccess.AddBindParam(nSchemaID);
        RETURN_SQL_ERROR_ON_FALSE(sqlAccess.BYieldingExecute(FILE_AND_LINE,
                                                             "SELECT object_id, name FROM sys.objects WHERE type_desc = 'USER_TABLE' AND is_ms_shipped = 0 AND schema_id = ?"));
    ...
    SQLRETURN
    CJobUpdateSchema::YieldingGetColumnInfoForTable(ESchemaCatalog eSchemaCatalog, CMapPRecordInfo &mapPRecordInfo,
                                                    int nTableID, const char *pchTableName) {
        CSQLAccess sqlAccess(eSchemaCatalog);

        sqlAccess.AddBindParam(nTableID);
        RETURN_SQL_ERROR_ON_FALSE(sqlAccess.BYieldingExecute(FILE_AND_LINE,
                                                             "SELECT name, column_id, user_type_id, max_length, is_identity FROM sys.columns WHERE object_id = ?"));
        ...
        FOR_EACH_SQL_RESULT(sqlAccess, 0, columnInfo)
        {
            ...
            pRecordInfo->AddColumn(sColumnName, nColumnID, GetEGCSQLTypeForMSSQLType(nType), nMaxLength, nColFlags,
                                   nMaxLength);
        }
...
//-----------------------------------------------------------------------------
    SQLRETURN CJobUpdateSchema::YieldingGetTriggers(ESchemaCatalog eSchemaCatalog, int nSchemaID,
    ...
    SQLRETURN CJobUpdateSchema::YieldingGetTableFKConstraints(ESchemaCatalog eSchemaCatalog, CRecordInfo *pRecordInfo) {
        ...
        FOR_EACH_SQL_RESULT(sqlAccess, 0, sqlRecord)
        {
            ...
            AssertMsg(Q_stricmp(pchTableName, pRecordInfo->GetName()) == 0,
                      "FOREIGN KEY schema conversion found FK for table not matching search!\n");
...
// Output:  SQL return code.
//-----------------------------------------------------------------------------
    SQLRETURN CJobUpdateSchema::YieldingAddIndex(ESchemaCatalog eSchemaCatalog, CRecordInfo *pRecordInfo,
...
//-----------------------------------------------------------------------------
    ...
    SQLRETURN CJobUpdateSchema::YieldingProcessUnsafeConversion(ESchemaCatalog eSchemaCatalog, const char *pszSQL,
...
// Output:  SQL return code.
//-----------------------------------------------------------------------------
    SQLRETURN CJobUpdateSchema::YieldingAlterTableAddColumn(ESchemaCatalog eSchemaCatalog, CRecordInfo *pRecordInfo,
...
// Output:  SQL return code.
//-----------------------------------------------------------------------------
    SQLRETURN CJobUpdateSchema::YieldingAddConstraint(ESchemaCatalog eSchemaCatalog, CRecordInfo *pRecordInfo,
        ...
        AppendConstraint(pRecordInfo->GetName(), pColumnInfo->GetName(), nColFlagConstraint, true,
...
//-----------------------------------------------------------------------------
    SQLRETURN
    CJobUpdateSchema::YieldingChangeColumnTypeOrLength(ESchemaCatalog eSchemaCatalog, CRecordInfo *pRecordInfo,
...
// Output:  SQL return code.
//-----------------------------------------------------------------------------
    SQLRETURN CJobUpdateSchema::YieldingChangeColumnProperties(ESchemaCatalog eSchemaCatalog, CRecordInfo *pRecordInfo,
        ...
        Assert(!(bAddUniqueConstraint &&
                 bAddPrimaryKeyConstraint));    // primary key constraint adds implicit uniqueness constraint; it's a bug if we decide to do both
        Assert(!(bAddUniqueConstraint &&
                 bAddExplicitIndex));    // uniqueness constraint adds implicit index; it's a bug if we decide to do both
        Assert(!(bAddPrimaryKeyConstraint &&
                 bAddExplicitIndex));    // primary key constraint adds implicit index; it's a bug if we decide to do both
