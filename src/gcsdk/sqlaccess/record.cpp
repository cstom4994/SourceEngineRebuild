...
//-----------------------------------------------------------------------------
    void CRecordBase::RenderField(uint32 unColumn, int cchBuffer, char *pchBuffer) const {
        ...
        ConvertFieldToText(GetPRecordInfo()->GetColumnInfo(unColumn).GetType(), pubData, cubData, pchBuffer, cchBuffer,
                           false);
    }
    ...
    bool CRecordVar::SetVarCharField(CVarCharField &field, const char *pchString, bool bTruncate, int32 iField) {
        ...
        if ((cchMaxLength > 0) && (cchLen > cchMaxLength)) {
            if (bTruncate) {
                ...
                AssertMsg4(false, "Overflow in SetVarCharField (%u > %u) for column %s in table %s", cchLen,
                           cchMaxLength, m_pSchema->GetField(iField).m_rgchName, m_pSchema->GetPchName());
                return false;
            }
        }

        bool bRealloced = false;
        bool fSuccess = GetPSchema()->BSetVarField(PubRecordFixed(), &field, pchString, cchLen, &bRealloced,
...
//-----------------------------------------------------------------------------
    void CRecordVar::SetVarDataField(CVarField &field, const void *pvData, uint32 cubData) {
        ...
        bool bRealloced = false;
        GetPSchema()->BSetVarField(PubRecordFixed(), &field, pvData, cubData, &bRealloced,
                                   BFlagSet(k_EAllocatedVarBlock));
