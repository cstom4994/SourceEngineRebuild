...
namespace GCSDK {
    ...
    bool CGCReportPrinter::AddIntColumn(const char *pszColumn, ESummaryType eSummary,
    ...
    bool
    CGCReportPrinter::AddFloatColumn(const char *pszColumn, ESummaryType eSummary, uint32 unNumDecimal /* = 2 */ ) {
        //don't allow adding columns if data is already present
        if (m_Rows.Count() > 0)
            return false;
...
//utility to count the number of digits on the provided integer
    ...
    static const char *GetIntValueDisplay(int64 nValue, CGCReportPrinter::EIntDisplayType eIntDisplay) {
        ...
        switch (eIntDisplay) {
            case CGCReportPrinter::eIntDisplay_Memory_MB:
                // "1234.56 MB"
                s_fmtResult.sprintf("%lld.%02u MB", (nValue / k_nMegabyte),
