...
// Thread pool size convar
    ...
    GCConVar cv_webapi_throttle_job_threshold("webapi_throttle_job_threshold", "2000", 0,
...
// caches into deterministic-between-GC-reboots and not, and resend based on said flag.
    ...
    static GCConVar cv_assert_minidump_window("assert_minidump_window", "28800", 0,
                                              "Size of the minidump window in seconds. Each unique assert will dump at most assert_max_minidumps_in_window times in this many seconds");
    static GCConVar cv_assert_max_minidumps_in_window("assert_max_minidumps_in_window", "5", 0,
                                                      "The amount of times each unique assert will write a dump in assert_minidump_window seconds");

    static GCConVar cv_debug_steam_startplaying("cv_debug_steam_startplaying", "0", 0,
                                                "Turn this ON to debug the stream of startplaying messages we get from Steam");

    static GCConVar temp_list_mismatched_replies("temp_list_mismatched_replies", "0",
                                                 "When set to 1, this report all replies that fail because the incoming message didn't expect a response. Temporary to help track down some failed state");

    static GCConVar writeback_queue_max_accumulate_time("writeback_queue_max_accumulate_time", "10", 0,
                                                        "The maximum amount of time in seconds that the writeback queue will accumulate database writes before performing queries. This is the time *before* the queries are executed, which is unbounded.");
    static GCConVar writeback_queue_max_caches("writeback_queue_max_caches", "0", 0,
                                               "The maximum amount of caches to write back in a single transaction. Set to zero to remove this restriction.");
    static GCConVar geolocation_spewlevel("geolocation_spewlevel", "4", 0,
...
//-----------------------------------------------------------------------------
    ...
    public:
        ...
        virtual bool BYieldingRunGCJob(GCSDK::IMsgNetPacket *pNetPacket) {
            ...
            m_pGC->QueueStartPlaying(msg.Body().m_steamID, msg.Body().m_steamIDGS, msg.Body().m_unServerAddr,
    ...
    public:
        ...
        virtual bool BYieldingRunGCJob(GCSDK::IMsgNetPacket *pNetPacket) {
            ...
            m_pGC->QueueStartPlaying(msg.Body().m_steamID, CSteamID(), msg.Body().m_unServerAddr,
                                     msg.Body().m_usServerPort, msg.PubVarData(), msg.CubVarData());
...
//-----------------------------------------------------------------------------
        ...
        bool BYieldingRunJobFromMsg(IMsgNetPacket *pNetPacket) {
            ...
            if (pAccountDetails && bIsVacBanned != pAccountDetails->BIsVacBanned()) {
                EmitWarning(SPEW_GC, 2,
                            "VAC status didn't update for %s afetr receiving VacStatusChange and the force reloading the account details\n",
                            steamID.Render());
            }
            return true;
        }
    ...
    GC_REG_JOB(CGCBase, CGCJobAccountVacStatusChange, "CGCJobAccountVacStatusChange", k_EGCMsgGCAccountVacStatusChange,
               k_EServerTypeGC);
...
//-----------------------------------------------------------------------------
    ...
    public:
        ...
        bool BYieldingRunJobFromMsg(IMsgNetPacket *pNetPacket) {
            ...
            if (pAccountDetails && (bHasPhoneVerified != pAccountDetails->BIsPhoneVerified() ||
                ...
                EmitWarning(SPEW_GC, 2,
                            "Phone status didn't update for %s afetr receiving PhoneNumberChange and force reloading the account details\n",
                            steamID.Render());
            }
    ...
    GC_REG_JOB(CGCBase, CGCJobAccountPhoneNumberChange, "CGCJobAccountPhoneNumberChange",
               k_EGCMsgAccountPhoneNumberChange, k_EServerTypeGC);
...
//-----------------------------------------------------------------------------
    ...
    GC_REG_JOB(CGCBase, CGCJobAccountTwoFactorChange, "CGCJobAccountTwoFactorChange", k_EGCMsgAccountTwoFactorChange,
               k_EServerTypeGC);
...
//-----------------------------------------------------------------------------
    CGCBase::CGCBase()
            : m_mapSOCache(),
              ...
              m_nLogonSurgeFramesRemaining(k_nMillion * 10 /
                                           k_cMicroSecPerShellFrame), // stay in "logon surge" mode for at least 10 seconds after boot.
...
//-----------------------------------------------------------------------------
    void CGCBase::Uninit() {
        ...
        for (CGCUserSession **ppSession = m_hashUserSessions.PvRecordFirst();
             ppSession != NULL; ppSession = m_hashUserSessions.PvRecordNext(ppSession)) {
            delete (*ppSession);
        }
        m_hashUserSessions.RemoveAll();
        for (CGCGSSession **ppSession = m_hashGSSessions.PvRecordFirst();
             ppSession != NULL; ppSession = m_hashGSSessions.PvRecordNext(ppSession)) {
            delete (*ppSession);
        }
    ...
    GCConVar cv_flush_inventory_cache_jobs("cv_flush_inventory_cache_jobs", "20", 0,
                                           "The maximum number of jobs flushing inventory caches that can be in flight at once, zero to disable flushing");
    GCConVar cv_flush_inventory_cache_contextid("cv_flush_inventory_cache_contextid", "2" /* k_EEconContextBackpack */,
                                                0, "Which context id we flush for Steam web user-facing inventory");
    GCConVar cv_flush_inventory_cache_spew("cv_flush_inventory_cache_spew", "0", 0,
                                           "Controls spew level for jobs flushing inventory cache (0=off; 1=summary; 2=verbose)");

    class CFlushInventoryCacheAccountsJob : public CGCJob, public IYieldingParallelFarmJobHandler {
    public:
        CFlushInventoryCacheAccountsJob(CGCBase *pGC,
                                        CUtlRBTree<AccountID_t, int32, CDefLess<AccountID_t> > &rbAccounts) : CGCJob(
                pGC) {
        ...
        virtual bool BYieldingRunGCJob() OVERRIDE {
            ...
            if (bShouldSpew) {
                EmitInfo(SPEW_GC, SPEW_ALWAYS, LOG_ALWAYS,
                         "IEconService/FlushInventoryCache: Batch for %d accounts completed in %u ms\n",
        ...
        bool BYieldingFlushRequest(AccountID_t unAccountID) {
            ...
            if (!bSucceededQuery) {
                EmitErrorRatelimited(SPEW_GC,
                                     "IEconService/FlushInventoryCache: Web call did not get a response for %s.\n",
                                     steamID.Render());
            ...
            if (bSucceededQuery) {
                ...
                if (!pKVResponse->LoadFromBuffer("webResponse", *apiResponse.GetBodyBuffer())) {
                    EmitErrorRatelimited(SPEW_GC,
                                         "IEconService/FlushInventoryCache: Web call got code %d for %s, but failed to parse response\n",
                                         apiResponse.GetStatusCode(), steamID.Render());
                    bSucceededQuery = false;
                } else if (!pKVResponse->GetBool("success")) {
                    // We got a response, and it's not success
                    EmitErrorRatelimited(SPEW_GC,
                                         "IEconService/FlushInventoryCache: Web call got code %d for %s, but not success\n",
                                         apiResponse.GetStatusCode(), steamID.Render());
                    bSucceededQuery = false;
                }
            }
...
//-----------------------------------------------------------------------------
    bool CGCBase::BMainLoopOncePerFrame(uint64 ulLimitMicroseconds) {
        ...
        if (m_llStartPlaying.Count() > 0) { VPROF_BUDGET("StartStartPlayingJobs", VPROF_BUDGETGROUP_STEAM);

            int nJobsNeeded = min(m_llStartPlaying.Count(),
                                  cv_concurrent_start_playing_limit.GetInt() - m_nStartPlayingJobCount);
            while (nJobsNeeded > 0) {
                nJobsNeeded--;
        ...
        if (m_rbFlushInventoryCacheAccounts.Count() && ((m_jobidFlushInventoryCacheAccounts == k_GIDNil) ||
            ...
            m_jobidFlushInventoryCacheAccounts = StartNewJobDelayed(
                    new CFlushInventoryCacheAccountsJob(this, m_rbFlushInventoryCacheAccounts))->GetJobID();
        }
    ...
    bool CGCBase::BMainLoopUntilFrameCompletion(uint64 ulLimitMicroseconds) {
        ...
        { VPROF_BUDGET("Expire locks", VPROF_BUDGETGROUP_STEAM);
            ...
            for (CLock *pLock = m_hashSteamIDLocks.PvRecordRun();
                 NULL != pLock; pLock = m_hashSteamIDLocks.PvRecordRun()) {
...
//			get a chance.
//-----------------------------------------------------------------------------
    void CGCBase::QueueStartPlaying(const CSteamID &steamID, const CSteamID &gsSteamID, uint32 unServerAddr,
                                    uint16 usServerPort, const uint8 *pubVarData, uint32 cubVarData) {
...
//			}
                ...
                EmitWarning(SPEW_GC, SPEW_ALWAYS,
                            "Start playing queue corruption!  Map entry points to wrong queue entry!\n");
                pWork = NULL;
...
//-----------------------------------------------------------------------------
    bool CGCBase::BRemoveStartPlayingQueueEntry(const CSteamID &steamID) {
        ...
        if (m_llStartPlaying[nQueueIndex].m_steamID == steamID) {
            ...
            EmitWarning(SPEW_GC, SPEW_ALWAYS,
                        "Start playing queue corruption!  Map entry doesn't point to matching queue index (found while removing entry in BRemoveStartPlayingQueueEntry)!\n");
        }
...
//-----------------------------------------------------------------------------
    void CGCBase::YieldingExecuteStartPlayingQueueEntryByIndex(int idxStartPlayingQueue) {
        ...
        if (nMapIndex == m_mapStartPlayingQueueIndexBySteamID.InvalidIndex()) {
            ...
            EmitWarning(SPEW_GC, SPEW_ALWAYS,
                        "Start playing queue corruption!  Map entry doesn't have proper queue index!\n");
        } else {
            m_mapStartPlayingQueueIndexBySteamID.RemoveAt(nMapIndex);
        }
        ...
        if (work.m_steamID.BIndividualAccount()) {
            YieldingStartPlaying(work.m_steamID, work.m_gsSteamID, work.m_unServerAddr, work.m_usServerPort,
                                 work.m_pVarData);
...
//-----------------------------------------------------------------------------
    void CGCBase::YieldingStartPlaying(const CSteamID &steamID, const CSteamID &gsSteamID, uint32 unServerAddr,
                                       uint16 usServerPort, CUtlBuffer *pVarData) {
        ...
        if (gsSteamID.IsValid() && gsSteamID.BGameServerAccount() &&
            ...
            if (YieldingGetLockedGSSession(gsSteamID, __FILE__, __LINE__) != NULL) {
                ...
                if (enable_startplaying_gameserver_creation_spew.GetBool()) {
                    ...
                    EmitInfo(SPEW_GC, 2, LOG_ALWAYS,
                             "Creating gameserver session %s @ %s as a result of user %s StartPlaying.\n",
            ...
            if (cv_debug_steam_startplaying.GetBool())
                EmitInfo(SPEW_GC, 1, 1,
                         "YieldingStartPlaying ( user = %s ) with invalid GS steam ID %s, calling LeaveServer\n",
                         steamID.Render(), gsSteamID.Render());
...
//-----------------------------------------------------------------------------
    void CGCBase::YieldingStartGameserver(const CSteamID &steamID, uint32 unServerAddr, uint16 usServerPort,
...
//-----------------------------------------------------------------------------
    ...
    IMsgNetPacket *
    CreateIMsgNetPacket(GCProtoBufMsgSrc eReplyType, const CSteamID senderID, uint32 nGCDirIndex, uint32 unMsgType,
        ...
        if (0 != (unMsgType & k_EMsgProtoBufFlag)) {
            if (cubData < sizeof(ProtoBufMsgHeader_t)) {
                ...
                AssertMsg3(false, "Received packet %s(%u) from %s less than the minimum protobuf size",
                           PchMsgNameFromEMsg(unMsgTypeNoFlag), unMsgTypeNoFlag, senderID.Render());
                return NULL;
            }
            ...
            CProtoBufNetPacket *pMsgNetPacket = new CProtoBufNetPacket(pGCPacket, eReplyType, senderID, nGCDirIndex,
            ...
            AssertMsg(eReplyType != GCProtoBufMsgSrc_FromGC,
                      "Warning: Encountered a message from GC to GC that was not of protobuff type, will be unable to reply to this message. Message type: %d",
                      unMsgType);

            if (cubData < sizeof(GCMsgHdrEx_t) - sizeof(GCMsgHdr_t)) {
                AssertMsg(false, "Received packet %s(%u) from %s less than the minimum struct size",
...
//-----------------------------------------------------------------------------
    void CGCBase::MessageFromClient(const CSteamID &senderID, uint32 unMsgType, void *pubData, uint32 cubData) {
        ...
        IMsgNetPacket *pMsgNetPacket = CreateIMsgNetPacket(GCProtoBufMsgSrc_FromSteamID, senderID, nGCDirIndex,
        ...
        switch (unMsgType) {
            ...
            default:
                GetJobMgr().BRouteMsgToJob(this, pMsgNetPacket,
...
//-----------------------------------------------------------------------------
    bool CGCBase::BSendGCMsgToClient(const CSteamID &steamIDTarget, const CGCMsgBase &msg) {
        g_theMessageList.TallySendMessage(msg.Hdr().m_eMsg, msg.CubPkt() - sizeof(GCMsgHdr_t));VPROF_BUDGET("GCHost",
                                                                                                            VPROF_BUDGETGROUP_STEAM);
        { VPROF_BUDGET("GCHost - SendMessageToClient", VPROF_BUDGETGROUP_STEAM);
            return m_pHost->BSendMessageToClient(m_unAppID, steamIDTarget, msg.Hdr().m_eMsg,
...
//-----------------------------------------------------------------------------
    ...
    public:
        ...
        virtual bool BAsyncSend(MsgType_t eMsg, const uint8 *pubMsgBytes, uint32 cubSize) OVERRIDE {
            ...
            g_theMessageList.TallySendMessage(eMsg & ~k_EMsgProtoBufFlag, cubSize);VPROF_BUDGET("GCHost",
                                                                                                VPROF_BUDGETGROUP_STEAM);
            { VPROF_BUDGET("GCHost - SendMessageToClient (ProtoBuf)", VPROF_BUDGETGROUP_STEAM);
                return GGCHost()->BSendMessageToClient(GGCBase()->GetAppID(), m_steamIDTarget,
                                                       eMsg | k_EMsgProtoBufFlag, pubMsgBytes, cubSize);
            }
        }
    ...
    public:
        ...
        virtual bool BAsyncSend(MsgType_t eMsg, const uint8 *pubMsgBytes, uint32 cubSize) OVERRIDE {
            ...
            { VPROF_BUDGET("GCHost - SendMessageToSystem (ProtoBuf)", VPROF_BUDGETGROUP_STEAM);
                return GGCHost()->BSendMessageToClient(GGCBase()->GetAppID(), CSteamID(), eMsg | k_EMsgProtoBufFlag,
                                                       pubMsgBytes, cubSize);
            }
        }
...
// Purpose: Sends a message to the given SteamID
//-----------------------------------------------------------------------------
    bool CGCBase::BSendGCMsgToClientWithPreSerializedBody(const CSteamID &steamIDTarget, MsgType_t eMsgType,
                                                          const CMsgProtoBufHeader &hdr, const byte *pubBody,
...
//-----------------------------------------------------------------------------
    bool CGCBase::BSendGCMsgToSystemWithPreSerializedBody(MsgType_t eMsgType, const CMsgProtoBufHeader &hdr,
                                                          const byte *pubBody, uint32 cubBody) const {
        CProtoBufSystemSendHandler sender;
        return CProtoBufMsgBase::BAsyncSendWithPreSerializedBody(sender, eMsgType, hdr, pubBody, cubBody);
    }
...
// Purpose: send msgOut to the place that msgIn came from
//-----------------------------------------------------------------------------
    bool CGCBase::BReplyToMessageWithPreSerializedBody(MsgType_t eMsgType, const CProtoBufMsgBase &msgIn,
                                                       const byte *pubBody, uint32 cubBody) const {
        // Don't reply if the source is not expecting it
        if (!msgIn.GetJobIDSource())
            return true;
...
//-----------------------------------------------------------------------------
    ...
    bool CGCBase::BYldSendMessageAndGetReply(const CSteamID &steamIDTarget, CProtoBufMsgBase &msgOut,
...
//}
    ...
    bool CGCBase::BYldSendSystemMessageAndGetReply(const ::google::protobuf::Message &msgSend, MsgType_t eSendMsg,
        ...
        return curJob.BYieldingWaitForMsg(&protoMsg,
                                          eRespondMsg); // !FIXME! For some reason system replies are coming back with a universe and instance set (but account ID zero).
    }
...
// Purpose: Creates a new session for the steam ID
//-----------------------------------------------------------------------------
    CGCGSSession *CGCBase::CreateGSSession(const CSteamID &steamID, CGCSharedObjectCache *pSOCache, uint32 unServerAddr,
...
//-----------------------------------------------------------------------------
    CGCUserSession *
    CGCBase::YieldingGetLockedUserSession(const CSteamID &steamID, const char *pszFilename, int nLineNum) {
...
//-----------------------------------------------------------------------------
    CGCUserSession *CGCBase::FindUserSession(const CSteamID &steamID) const {
        ...
        if (!steamID.BIndividualAccount()) {
            AssertMsg1(steamID.BIndividualAccount(), "CGCBase::FindUserSession was passed non-individual Steam ID %s",
                       steamID.Render());
            return NULL;
        }
    ...
    CGCSession *CGCBase::YieldingRequestSession(const CSteamID &steamID) {
        ...
        if (steamID.BIndividualAccount()) {
            if (msgReply.Body().m_bOnline) {
                ...
                YieldingStartPlaying(steamID, msgReply.Body().m_ulSteamIDGS, msgReply.Body().m_unServerAddr,
                                     msgReply.Body().m_usServerPort, msgReply.CubVarData() ? &bufVarData : NULL);
            ...
            if (msgReply.Body().m_bOnline) {
                YieldingStartGameserver(steamID, msgReply.Body().m_unServerAddr, msgReply.Body().m_usServerPort,
...
//-----------------------------------------------------------------------------
    bool CGCBase::BYieldingSendHTTPRequest(const CHTTPRequest *pRequest, CHTTPResponse *pResponse) {
        ...
        if (!BYldSendSystemMessageAndGetReply(pRequest->GetProtoObj(), k_EGCMsgSendHTTPRequest, &msgResponse,
                                              k_EGCMsgSendHTTPRequestResponse)) {
            ReportHTTPError(CFmtStr("No response to HTTP system message for %s", pRequest->GetURL()),
                            CGCEmitGroup::kMsg_Error);
            return false;
        }

        if (!msgResponse.has_status_code()) {
            ReportHTTPError(CFmtStr("No status code on HTTP response for %s", pRequest->GetURL()),
                            CGCEmitGroup::kMsg_Error);
            return false;
        }
        ...
        if (msgResponse.status_code() != k_EHTTPStatusCode200OK) {
            ReportHTTPError(CFmtStr("Invalid status code %u for %s", msgResponse.status_code(), pRequest->GetURL()),
...
//-----------------------------------------------------------------------------
    CGCGSSession *
    CGCBase::YieldingFindOrCreateGSSession(const CSteamID &steamID, uint32 unServerAddr, uint16 usServerPort,
                                           const uint8 *pubVarData, uint32 cubVarData) {
        ...
        if (!pSession) {
            ...
            *ppSession = pSession;
            ...
            if (unServerAddr != 0 && usServerPort != 0 &&
                (unServerAddr != pSession->GetAddr() || usServerPort != pSession->GetPort())) {
...
//-----------------------------------------------------------------------------
    CGCGSSession *CGCBase::FindGSSession(const CSteamID &steamID) const {
        ...
        if (!steamID.BGameServerAccount()) {
            AssertMsg1(steamID.BGameServerAccount(), "CGCBase::FindGSSession was passed non-gameserver Steam ID %s",
                       steamID.Render());
            return NULL;
        }
...
//-----------------------------------------------------------------------------
    CGCSession *CGCBase::FindUserOrGSSession(const CSteamID &steamID) const {
        ...
        if (steamID.BGameServerAccount())
            return FindGSSession(steamID);
        AssertMsg1(false, "CGCBase::FindUserOrGSSession, steam ID %s isn't an individual or a gameserver ID",
                   steamID.Render());
        return NULL;
    }
...
//-----------------------------------------------------------------------------
    bool CGCBase::UnloadUnusedCaches(uint32 unMaxCacheCount, CLimitTimer *pLimitTimer) {
        ...
        for (uint32 unCache = m_listCachesToUnload.Head(), unNextCache = m_listCachesToUnload.InvalidIndex();
            ...
            if ((ownerID.BIndividualAccount() && FindUserSession(ownerID))
                ...
                if (pSOCache->GetLRUHandle() != unCache) {
                    EmitError(SPEW_GC,
                              "Cache for %s has a different LRU handle than the one retrieved from the iterator! 0x%08x vs 0x%08x\n",
            ...
            if (iPreRemoveCount != m_listCachesToUnload.Count() + 1 &&
                ...
                EmitError(SPEW_GC,
                          "CGCBase::UnloadUnusedCaches() sanity check failed! List size changed dramatically removing 0x%08x; delta %i\n",
...
//-----------------------------------------------------------------------------
    void CGCBase::VerifySOCacheLRU() {
        ...
        for (uint32 unCache = m_listCachesToUnload.Head(), unNextCache = m_listCachesToUnload.InvalidIndex();
            ...
            if (!pSOCache) {
                EmitError(SPEW_GC,
                          "CGCBase::UnloadUnusedCaches() sanity[0] check failed! Empty cache in list in slot 0x%08x\n",
                          unCache);
                continue;
            }
            ...
            if (pSOCache->GetLRUHandle() != unCache) {
                EmitError(SPEW_GC,
                          "CGCBase::UnloadUnusedCaches() sanity[1] check failed! Cache entry mismatch [ 0x%08x vs 0x%08x ] (owner: %s)\n",
            ...
            if (!rbTreeUsersEncountered.IsValidIndex(rbTreeUsersEncountered.InsertIfNotFound(ownerID))) {
                EmitError(SPEW_GC,
                          "CGCBase::UnloadUnusedCaches() sanity[2] check failed! Duplicate entry in list for 0x%08x (owner: %s)\n",
                                                                                                                                ...
                                                                                                                                if ( m_listCachesToUnload.IsValidIndex( pSOCache->GetLRUHandle() ) == ( pSOCache->GetLRUHandle() == m_listCachesToUnload.InvalidIndex() ) )
	{
        ...
        if (m_listCachesToUnload.IsValidIndex(pSOCache->GetLRUHandle())) {
            if (m_listCachesToUnload[pSOCache->GetLRUHandle()] != pSOCache->GetOwner()) {
                EmitError(SPEW_GC,
                          "CGCBase::RemoveCacheFromLRU() Attempting to remove SOCache LRU index %d for %s, which really holds %s\n",
...
//-----------------------------------------------------------------------------
    ...
    CGCBase::YieldingGetLockedSOCache(const CSteamID &steamID, const char *pszFilename, int nLineNum) {
        if (!BYieldingLockSteamID(steamID, pszFilename, nLineNum))
            return NULL;
...
//-----------------------------------------------------------------------------
    void CGCBase::YieldingReloadCache(CGCSharedObjectCache *pSOCache) {
        ...
        if (!BYieldingLoadSOCache(pNewCache)) {
            EmitError(SPEW_SHAREDOBJ, "Unable to reload cache for %s because of a SQL error",
...
//-----------------------------------------------------------------------------
    bool CGCBase::BYieldingLockSteamIDPair(const CSteamID &steamIDA, const CSteamID &steamIDB, const char *pszFilename,
                                           int nLineNum) {
...
//-----------------------------------------------------------------------------
    void CGCBase::UnlockSteamID(const CSteamID &steamID) {
        ...
        Assert(pLock);
        if (!pLock) {
            AssertMsg2(false, "UnlockSteamID( '%s' ) called by %s but unable to find lock in map", steamID.Render(),
                       GJobCur().GetName());
            return;
        }

        if (pLock->GetJobLocking() != &GJobCur()) {
            AssertMsg2(false, "UnlockSteamID( '%s' ) called when job %s doesn't own the lock", steamID.Render(),
                       GJobCur().GetName());
            return;
        }
    ...
    bool CGCBase::YieldingWritebackDirtyCaches(uint32 unSecondToDelayWrite) {
        ...
        FOR_EACH_VEC(m_vecCacheWritebacks, nCache) {
            ...
            if (GJobCur().GetMicrosecondsRun() >
                (uint64) (writeback_queue_max_accumulate_time.GetInt() * k_nThousand) ||
...
//-----------------------------------------------------------------------------
    void CGCBase::AddCacheToWritebackQueue(CGCSharedObjectCache *pSOCache) {
        ...
        if ((g_pJobCur != NULL) && PJobHoldingLock(pSOCache->GetOwner()) != g_pJobCur &&
            ...
            AssertMsg2(false, "CGCBase::AddCacheToWritebackQueue called by job %s for %s, but job does not own lock",
...
//-----------------------------------------------------------------------------
    bool
    CGCBase::BMemcachedSet(const CUtlVector<CUtlString> &vecKeys, const CUtlVector<GCMemcachedBuffer_t> &vecValues) {
...
//-----------------------------------------------------------------------------
    bool CGCBase::BYieldingMemcachedGet(const CUtlVector<CUtlString> &vecKeys,
        ...
        if (msgRequest.Body().keys_size() != msgResponse.Body().values_size()) {
            EmitWarning(SPEW_GC, LOG_ALWAYS,
                        "Mismatched reply from IS for BYieldingMemcachedGet, asked for %d keys, got %d back\n",
            ...
            if (pSession) {
                if (!pSession->GetIPPublic()) {
                    EmitInfo(SPEW_GC, 4, LOG_ALWAYS,
                             "BYieldingUpdateGeoLocation Session %s IP == 0, unable to retrieve\n",
...
#define iptod(x) ((x)>>24&0xff), ((x)>>16&0xff), ((x)>>8&0xff), ((x)&0xff)
        ...
        FOR_EACH_VEC(vecIPs, i) {
            EmitInfo(SPEW_GC, geolocation_spewlevel.GetInt(), geolocation_loglevel.GetInt(),
                     "BYieldingUpdateGeoLocation GetIPLocation[%d] = (%s,%u.%u.%u.%u)\n", i, vecSteamIds[i].Render(),
                     iptod(vecIPs[i]));
        }
        ...
        if (BYieldingGetIPLocations(vecIPs, infos)) {
            ...
            if (vecSteamIds.Count() == vecIPs.Count() && vecIPs.Count() == infos.Count()) {
                FOR_EACH_VEC(vecSteamIds, i) {
                    ...
                    if (pSession) {
                        EmitInfo(SPEW_GC, geolocation_spewlevel.GetInt(), geolocation_loglevel.GetInt(),
                                 "BYieldingUpdateGeoLocation[MATCHED] SetIPLocation[%s(%u.%u.%u.%u)] = (%6.3f,%6.3f)\n",
                                 pSession->GetSteamID().Render(), iptod(vecIPs[i]), infos[i].latitude(),
                                 infos[i].longitude());
                ...
                FOR_EACH_VEC(vecSteamIds, i) {
                    FOR_EACH_VEC(infos, j) {
                        if (infos[j].ip() == vecIPs[i]) {
                            ...
                            if (pSession) {
                                EmitInfo(SPEW_GC, 4, LOG_ALWAYS,
                                         "BYieldingUpdateGeoLocation[SEARCHED] SetIPLocation[%s(%u.%u.%u.%u)] = (%6.3f,%6.3f)\n",
                                         pSession->GetSteamID().Render(), iptod(vecIPs[i]), infos[j].latitude(),
                                         infos[j].longitude());
...
//-----------------------------------------------------------------------------
    ...
    void CGCBase::DumpSteamIDLocks(bool bFull, int nMax) {
        ...
        for (CLock *pLock = m_hashSteamIDLocks.PvRecordFirst();
             NULL != pLock; pLock = m_hashSteamIDLocks.PvRecordNext(pLock)) {
            if (pLock->BIsLocked()) {
        ...
        EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
                 "Lock                     Holding Job            First Waiting Job      Wait Count  Lock Time\n");
...
//			empty in that case.
//-----------------------------------------------------------------------------
    bool CGCBase::BYieldingLookupAccount(EAccountFindType eFindType, const char *pchInput,
    ...
    CUtlVector<CSteamID> vecIDs;
    if ( GGCBase()->
    BYieldingLookupAccount( k_EFindAccountTypeURL, args[1], &vecIDs
    )) {
    Msg( "Search success.\n" );
...
//-----------------------------------------------------------------------------
bool CGCBase::BYieldingRecordSupportAction(const CSteamID &actorID, const CSteamID &targetID, const char *pchData,
                                           const char *pchNote) {
...
//-----------------------------------------------------------------------------
void CGCBase::PostAlert(EAlertType eAlertType, bool bIsCritical, const char *pchAlertText,
                        const CUtlVector<CUtlString> *pvecExtendedInfo, bool bAlsoSpew) {
...
// Purpose: Fills the vector with all package IDs this account has a license to
//-----------------------------------------------------------------------------
bool CGCBase::BYieldingAddFreeLicense(const CSteamID &steamID, uint32 unPackageID, uint32 unIPPublic,
...
// Purpose: Fills the vector with all package IDs this account has a license to
//-----------------------------------------------------------------------------
int CGCBase::YieldingGrantGuestPass(const CSteamID &steamID, uint32 unPackageID, uint32 unPassesToGrant,
...
void CGCBase::Dump() const {
    ...
    EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS,
             "\tCaches: %d (%d waiting to unload, %d currently loading, %s %d /+ %d)\n", m_mapSOCache.Count(),
             m_listCachesToUnload.Count(), m_rbtreeSOCachesBeingLoaded.Count(),
             (((m_jobidFlushInventoryCacheAccounts == k_GIDNil) ||
               !m_JobMgr.BJobExists(m_jobidFlushInventoryCacheAccounts)) ? "last flushed" : "currently flushing"),
             m_numFlushInventoryCacheAccountsLastScheduled, m_rbFlushInventoryCacheAccounts.Count());
    EmitInfo(SPEW_CONSOLE, SPEW_ALWAYS, LOG_ALWAYS, "\tWriteback Queue: %d (oldest: %s)\n",
             m_vecCacheWritebacks.Count(),
             m_vecCacheWritebacks.Count() > 0 ? CRTime::Render(m_vecCacheWritebacks[0]->GetWritebackTime(), rtimeBuf)
                                              : "none");
...
void CGCBase::AssertCallbackFunc(const char *pchFile, int nLine, const char *pchMessage) {
    ...
    if (!Plat_IsInDebugSession()) {
        ...
        while (nStale < vecTimes.Count() &&
               (CRTime::RTime32TimeCur() - vecTimes[nStale]) > (uint32) cv_assert_minidump_window.GetInt()) {
            nStale++;
        }
...
EResult YieldingSendWebAPIRequest(CSteamAPIRequest &request, KeyValues *pKVResponse, CUtlString &errMsg,
...
GC_CON_COMMAND( ip_geolocation,
"<a.b.c.d> Perform geolocation lookup" )
{
if ( args.
...
{
EmitError( SPEW_GC,
"Pass at least one IP to lookup\n" );
return;
}
...
GGCBase()->
BYieldingGetIPLocations( vecIPs, vecInfos
);
for (
int i = 0;
i<vecInfos.
