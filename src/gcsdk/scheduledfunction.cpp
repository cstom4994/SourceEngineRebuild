...
//-------------------------------------------------------------------------
    void CScheduledFunctionMgr::InitStartingTime() {
        ...
        for (uint32 nCurrBucket = 0; nCurrBucket < ARRAYSIZE(m_Resolutions); nCurrBucket++) {
            m_Resolutions[nCurrBucket].m_nAbsLastScheduleBucket = m_Resolutions[nCurrBucket].GetAbsScheduleBucketIndex(
                    nCurrTime);
        }
    }
...
//-------------------------------------------------------------------------
    ...
    void CScheduledFunctionMgr::InternalSchedule(uint32 nResolution, IGCScheduledFunction *pEvent, uint32 nMSDelay) {
        ...
        //add it to our list
        uint32 nInsertAfter = m_Resolutions[nResolution].m_pBuckets[nAbsBucket %
                                                                    m_Resolutions[nResolution].m_nNumBuckets];
...
//-------------------------------------------------------------------------
    void CScheduledFunctionMgr::Cancel(IGCScheduledFunction *pEvent) {
        ...
        if (m_ScheduleList.IsValidIndex(pEvent->m_nLLIndex)) {
            ...
            AssertMsg(false,
                      "Warning: Ecountered a remove request for a scheduled event but was unable to find it in the bucket that it was supposed to be in");
        }
...
//-------------------------------------------------------------------------
    ...
    void CScheduledFunctionMgr::CScheduleBucket::Init(TScheduleList &MasterList, uint32 nNumBuckets,
    ...
    void CScheduledFunctionMgr::CScheduleBucket::RunFunctions(TScheduleList &MasterList, uint64 nMicroSTime) {
        ...
        for (uint32 nAbsBucket = m_nAbsLastScheduleBucket; nAbsBucket < nCurrFrameBucket; nAbsBucket++) {
            ...
            AssertMsg2(nStartIndex != MasterList.InvalidIndex(),
                       "Error: Detected list corruption when accessing list bucket %d (%d micro s per bucket)",
            ...
            for (uint32 nCurrEvent = MasterList.Next(nStartIndex); nCurrEvent != MasterList.InvalidIndex();) {
                ...
                AssertMsg2(pEvent->m_nAbsScheduleBucket == nAbsBucket,
                           "Warning: Encountered a scheduled event that was intended to be fired on bucket %d but wasn't until bucket %d",
                           pEvent->m_nAbsScheduleBucket, nAbsBucket);
