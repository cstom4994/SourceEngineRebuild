...
namespace GCSDK {
    ...
    GCConVar max_user_messages_per_second("max_user_messages_per_second", "20", 0,
                                          "Maximum number of messages a user can send per second. 0 disables the rate limiting");
    static GCConVar user_message_rate_limit_warning_period("user_message_rate_limit_warning_period", "30", 0,
                                                           "Number of seconds between warning about rate limiting for users");

    static GCConVar msg_rate_limit_report_user_bucket_1("msg_rate_limit_report_user_bucket_1", "10", 0,
                                                        "These values control where various users are bucketed in rate limiting reports to help identify how frequently users are running into rate limiting");
    static GCConVar msg_rate_limit_report_user_bucket_2("msg_rate_limit_report_user_bucket_2", "100", 0,
                                                        "These values control where various users are bucketed in rate limiting reports to help identify how frequently users are running into rate limiting");

    static GCConVar msg_rate_limit_list_user("msg_rate_limit_list_user", "0", 0,
                                             "When set to a user account ID, this will report all the messages that are rate limited for that user to the console");
    ...
    void CMsgRateLimitTracker::ReportUserStats() const {
        ...
        const uint32 nBucketLimit1 = (uint32) max(0, min(msg_rate_limit_report_user_bucket_1.GetInt(),
                                                         msg_rate_limit_report_user_bucket_2.GetInt()));
        const uint32 nBucketLimit2 = (uint32) max(0, max(msg_rate_limit_report_user_bucket_1.GetInt(),
                                                         msg_rate_limit_report_user_bucket_2.GetInt()));
...
"Dumps a list of users that have been rate limited. Optional parameters can specify the number to dump or the minimum number of messages required." )
{
if( args.
...
{
EG_MSG(SPEW_CONSOLE,
       "Proper usage is: %s <min messages> <top users> - Specify 0 for one or both to have it be ignored\n", args[0]);
return;
}
g_RateLimitTracker.
ReportTopUsers(( uint32 )
max( 0,
atoi( args[1] )
), ( uint32 )max( 0,
atoi( args[2] )
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CGCGSSession::CGCGSSession(const CSteamID &steamID, CGCSharedObjectCache *pCache, uint32 unServerAddr,
                           uint16 usServerPort)
...
GC_REG_JOB(CGCBase, CGCCacheSubscriptionRefresh, "CGCCacheSubscriptionRefresh", k_ESOMsg_CacheSubscriptionRefresh,
           k_EServerTypeGC);
