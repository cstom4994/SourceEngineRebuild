...
//----------------------------------------------------------------------------
    ...
    bool CGCClientSharedObjectTypeCache::BParseCacheSubscribedMsg(const CMsgSOCacheSubscribed_SubscribedType &msg,
        ...
        for (uint16 usObject = 0; usObject < msg.object_data_size(); usObject++) {
            ...
            CSharedObject *pObject = BCreateFromMsg(msg.object_data(usObject).data(), msg.object_data(usObject).size(),
                                                    &bUpdatedExisting);
            if (pObject == NULL) {
                Assert(pObject);
                return false;
    ...
    //----------------------------------------------------------------------------
    CSharedObject *
    CGCClientSharedObjectTypeCache::BCreateFromMsg(const void *pvData, uint32 unSize, bool *bUpdatedExisting) {
    ...
    bool CGCClientSharedObjectCache::BParseCacheSubscribedMsg(const CMsgSOCacheSubscribed &msg) {
        ...
        for (uint16 usObject = 0; usObject < msg.objects_size(); usObject++) {
            ...
            Assert(pTypeCache);
            if (!pTypeCache || !pTypeCache->BParseCacheSubscribedMsg(msgType, vecCreatedObjects, vecUpdatedObjects,
                                                                     vecObjectsToDestroy))
                bResult = false;
        }
    ...
    void CGCClientSharedObjectCache::NotifyUnsubscribe() {
        if (m_bSubscribed) {
            ...
            AssertMsg(m_bSubscribed,
                      "GC Sending us Unsubscribed message when we weren't subscribed"); // Might not be a bug, but something worth checking
        }
    }
    ...
    void CGCClientSharedObjectCache::NotifyResubscribedUpToDate() {
        if (!m_bSubscribed) {
            ...
            AssertMsg(m_bSubscribed,
                      "Got NotifyResubscribedUpToDate when we were already subscribed?"); // Might not be a bug, but something worth checking
        }
    }
