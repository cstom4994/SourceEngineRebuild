...
namespace GCSDK {
    ...
    CUtlString GCMsgHdrEx_t::GetHeaderDescription() {
        CUtlString desc;
        desc.Format("gc msg %s, SteamID %llu, version %hd, job source %llu, job target %llu",
                    PchMsgNameFromEMsg(m_eMsg), m_ulSteamID, m_nHdrVersion, m_JobIDSource, m_JobIDTarget);
        return desc;
    }

    CUtlString GCMsgInterHdr_t::GetHeaderDescription() {
        CUtlString desc;
        desc.Format("gc inter msg %s, SourceAppID %d, version %hd, job source %llu, job target %llu",
                    PchMsgNameFromEMsg(m_eMsg), m_unSourceAppId, m_nHdrVersion, m_JobIDSource, m_JobIDTarget);
        return desc;
    }
...
//-----------------------------------------------------------------------------
    IMsgNetPacket *IMsgNetPacketFromCNetPacket(CNetPacket *pNetPacket) {
        ...
        if (cubPkt >= sizeof(ProtoBufMsgHeader_t) &&
            ...
            CProtoBufNetPacket *pMsgNetPacket = new CProtoBufNetPacket(pNetPacket, GCProtoBufMsgSrc_Unspecified,
                                                                       CSteamID(), 0, 0x7FFFFFFF &
                                                                                      ((ProtoBufMsgHeader_t *) pubPkt)->m_EMsgFlagged);
